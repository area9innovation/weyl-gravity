...
def verify() -> None:
    ...
    assert "R_c=0" in payload["tangent_cones"]["BOUNDED_OR_FINITE_QUASIPERIODIC"]["necessary_formula"]
    assert "R_13,18=0" in payload["tangent_cones"]["BOUNDED_OR_FINITE_QUASIPERIODIC"]["necessary_formula"]
    assert "mu_J3=0" in payload["tangent_cones"]["SMOOTH_EXPONENTIAL_POLYNOMIAL"]["formula"]
    flags = payload["classification"]
    assert not flags["complete_candidate13_bounded_tangent_cone_formula_certified"]
    assert flags["candidate13_known_bounded_functional_ledger_certified"]
    assert not flags["complete_candidate13_bounded_functional_ledger_certified"]
    ...
    assert not flags["five_stabilizer_pressure_and_eighteen_resonance_functionals_sufficient_bounded"]
