"""Explore twist times ell=2 extra sources in a non-axisymmetric channel."""
...
def _mixed_source(extra_parity: str, extra_mode: str, twist_profile: sp.Expr) -> tuple[sp.Matrix, sp.Matrix]:
    ...
    perturbation[1, 2] = perturbation[2, 1] = twist_amplitude * twist_profile * twist_axial_z
    perturbation[1, 3] = perturbation[3, 1] = twist_amplitude * twist_profile * twist_axial_phi
    ...
    maxwell_axial_projection = contravariant_axial_projection(maxwell_equations[2], maxwell_equations[3])
    polar = sp.Matrix(
        [
            -scalar_projection(metric_equations[(0, 0)]),
            2 * scalar_projection(metric_equations[(0, 1)]),
            -scalar_projection(metric_equations[(1, 1)]),
            12 * maxwell_axial_projection,
        ]
    ).applyfunc(lambda value: sp.factor(sp.cancel(_canonical(value))))
    return axial, polar


def source(extra_parity: str, extra_mode: str, twist_case: str) -> tuple[sp.Matrix, sp.Matrix]:
    time = sp.symbols("t", real=True)
    profile = {"position": sp.Integer(1), "velocity": time}[twist_case]
    return _mixed_source(extra_parity, extra_mode, profile)
...
def main() -> None:
    ...
    axial, polar = source(arguments.extra_parity, arguments.extra_mode, arguments.twist_case)
