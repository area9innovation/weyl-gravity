...
class EinsteinAtlasFragmentTests(unittest.TestCase):
    ...
    def test_same_sign_candidate18_smooth_current_radicals_are_fail_closed(self) -> None:
        entry = self.entries["einstein.ph.wm.interaction.ell2_same_sign_candidate18_active_restricted_current_degeneracy"]
        second = entry["mode_data"]["second_order"]
        self.assertEqual(entry["mode_data"]["lee_wald"]["status"], "OBSTRUCTED")
        self.assertIn("four-complex-dimensional", entry["mode_data"]["lee_wald"]["statement"])
        self.assertEqual(entry["mode_data"]["taub_maps"]["status"], "CERTIFIED")
        self.assertIn("all five stabilizer", entry["mode_data"]["taub_maps"]["statement"])
        self.assertEqual(second["bounded_or_finite_quasiperiodic"]["status"], "CERTIFIED")
        self.assertIn("R3+s18 R1", second["bounded_or_finite_quasiperiodic"]["statement"])
        self.assertEqual(second["causal_retarded"]["status"], "NO_CERTIFIED_MAP")
        self.assertIn("not a complete degeneracy-divisor", entry["claim_boundary"])

    def test_same_sign_active_presymplectic_divisors_are_smooth_only(self) -> None:
        entry = self.entries["einstein.ph.wm.interaction.ell2_same_sign_active_presymplectic_divisors"]
        second = entry["mode_data"]["second_order"]
        self.assertEqual(entry["mode_data"]["lee_wald"]["status"], "CERTIFIED")
        self.assertIn("det(J H^{-1} J^dagger)=0", entry["mode_data"]["lee_wald"]["statement"])
        self.assertEqual(entry["mode_data"]["taub_maps"]["status"], "CERTIFIED")
    ...
    def test_candidate17_20_singular_components_intersect_without_connectedness_claim(self) -> None:
        entry = self.entries["einstein.ph.wm.interaction.ell2_same_sign_candidate17_20_singular_component_incidence"]
        second = entry["mode_data"]["second_order"]
        self.assertEqual(entry["descriptions"]["symplectic"], "OPEN")
        self.assertEqual(entry["mode_data"]["taub_maps"]["status"], "CERTIFIED")
        self.assertIn("images meet", entry["mode_data"]["taub_maps"]["statement"])
        self.assertIn("S_plus x S_minus", entry["mode_data"]["resonance"]["statement"])
        self.assertIn("Unlike candidate 18", entry["mode_data"]["resonance"]["statement"])
        self.assertEqual(second["bounded_or_finite_quasiperiodic"]["status"], "CERTIFIED")
        self.assertEqual(second["causal_retarded"]["status"], "NO_CERTIFIED_MAP")
    ...
    def test_candidate17_20_component_incidence_classification_is_candidate_specific(self) -> None:
        entry = self.entries[
        ...
        self.assertEqual(entry["descriptions"]["nonlinear"], "CERTIFIED")
        self.assertIn(
            "four exhaustive occupation strata",
            entry["mode_data"]["lee_wald"]["statement"],
        )
        self.assertIn(
            "exactly one path component",
            entry["mode_data"]["taub_maps"]["statement"],
        )
        self.assertIn(
            "no nonincident component",
            entry["mode_data"]["taub_maps"]["statement"],
        )
        self.assertIn("G=0", entry["mode_data"]["resonance"]["statement"])
        self.assertIn("F=0", entry["mode_data"]["resonance"]["statement"])
        self.assertEqual(
            second["bounded_or_finite_quasiperiodic"]["status"], "CERTIFIED"
        )
        self.assertEqual(second["smooth_secular"]["status"], "CERTIFIED")
        self.assertEqual(second["causal_retarded"]["status"], "NO_CERTIFIED_MAP")
        self.assertIn("does not identify candidates", entry["claim_boundary"])
        self.assertIn("glue distinct total occupations", entry["claim_boundary"])
