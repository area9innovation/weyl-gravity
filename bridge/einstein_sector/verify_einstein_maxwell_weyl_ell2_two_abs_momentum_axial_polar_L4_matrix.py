#!/usr/bin/env python3
"""Independent verifier for an ordered cross-parity L=4 matrix."""
...
CERT = ROOT / "bridge/certificates/einstein_maxwell_weyl_ell2_two_abs_momentum_axial_polar_L4_matrix.json"
    ...
    def evaluate(value: sp.Expr) -> Interval:
        ...
        if value.is_Add:
            ...
            for term in value.args:
                item = evaluate(term)
                result = result[0] + item[0], result[1] + item[1]
            return result
        if value.is_Mul:
...
def qminus_axial(momentum: sp.Expr, frequency: sp.Expr) -> sp.Matrix:
    return sp.Matrix(
        [
            2 * momentum,
            -2 * frequency,
            -2 * sp.sqrt(3) * momentum,
            2 * sp.sqrt(3) * frequency,
        ]
    )
...
def verify_slice_calibration() -> None:
    ...
    assert value["parent"]["q2_sha256"] == sha(Q2)
    assert value["parent"]["row_layout_sha256"] == sha(ROW_LAYOUT)
    assert value["parent"]["action_sha256"] == sha(ACTION)
    assert value["pbw_support"]["ordered_term_counts"] == {
        "axial_then_polar": 832,
        "polar_then_axial": 832,
    }
    ...
    local = {
        ...
        "k_2": k_2,
        "omega_2": omega_2,
        **{str(item): item for item in (*axial, *polar)},
    }
    ...
    specialized = source.subs(
        {
            ...
            k_2: -momentum,
            omega_2: frequency,
            **dict(zip(axial, qminus_axial(momentum, frequency), strict=True)),
            **dict(zip(polar, qminus_polar(-momentum, frequency), strict=True)),
        },
        simultaneous=True,
    )
    prior = json.loads(CALIBRATION.read_text())["direct_source_ledger"]
    expected = sp.Matrix(
        [parse(item) for item in prior["axial_plus_polar_minus"]["source_rows"]]
    )
...
def verify_certificate(
    ...
    schema_path: Path,
    *,
    final_axisymmetric_block: bool,
) -> None:
    value = json.loads(certificate.read_text())
    schema = json.loads(schema_path.read_text())
    Draft202012Validator.check_schema(schema)
    Draft202012Validator(schema).validate(value)
    assert value["schema_sha256"] == sha(schema_path)
    ...
    for row in value["candidate_rows"]:
        ...
        for fixture in row["basis_fixtures"]:
            ...
            has_witness = False
            for pairing, stored in zip(
                pairings, fixture["pairing_intervals"], strict=True
            ):
                if pairing == 0:
                    zeros += 1
                ...
                interval = rational_interval(
                    pairing, int(stored["decimal_digits"])
                )
                assert [fraction_string(item) for item in interval] == [
                    stored["lower"],
                    stored["upper"],
                ]
    ...
    assert summary["basis_fixtures_with_nonzero_cokernel_vector"] == 20
    assert not value["classification"][
        "arbitrary_cross_parity_linear_combinations_classified"
    ]
    if final_axisymmetric_block:
        assert value["workload_progress"]["remaining_axisymmetric_L4_coefficients"] == 0
        assert value["workload_progress"]["remaining_nonaxisymmetric_L1_L3_coefficients"] == 56


def independently_verify(exhaustive: bool = False) -> None:
    verify_slice_calibration()
    verify_certificate(CERT, SCHEMA, final_axisymmetric_block=False)
    if exhaustive:
        import subprocess
