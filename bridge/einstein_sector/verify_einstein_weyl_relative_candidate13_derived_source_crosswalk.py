...
def verify() -> None:
    ...
    assert records["candidate13_cone"]["classification"]["candidate13_complete_bounded_cone_is_origin"]
    assert records["scalar_separation"]["classification"]["exact_rational_Farkas_functional_certified"]
    ...
    assert flags["bounded_derived_source_pullback_is_origin"]
    assert not flags["nonzero_mixed_bounded_derived_source_point_exists"]
    assert flags["nonzero_mixed_bounded_derived_source_point_nonexistence_certified"]
    assert "={0}" in payload["derived_source_pullback"]["BOUNDED_OR_FINITE_QUASIPERIODIC"]["domain"]
