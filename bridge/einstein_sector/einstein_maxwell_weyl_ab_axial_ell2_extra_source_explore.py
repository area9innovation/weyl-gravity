"""Explore a,b generalized-zero cross sources against axial ell=2 extra modes."""
...
def source(global_case: str, mode_case: str) -> sp.Matrix:
    ...
    h_time, h_space, q_time, q_space = [
        wave_amplitude * value * wave for value in representatives[mode_case]
    ]
    ...
    field[2, 1] = -field[1, 2]
    data = _curvature({"epsilon": epsilon, "coordinates": coordinates, "metric": metric, "inverse": inverse, "connection": connection, "field": field}, 2)
    ...
    values = []
    for row in rows:
        mixed = sp.diff(sp.diff(sp.diff(row, epsilon, 2) / 2, global_amplitude), wave_amplitude).subs(epsilon, 0) / wave
