...
INPUTS={
 ...
 "exceptional_zero_source":ROOT/"bridge/certificates/einstein_maxwell_weyl_exceptional_axial_ell1_zero_source_fixture.json",
...
def build():
 ...
 assert rec['exceptional_zero_source']['homogeneous_source_rows_E00_E11_E22_Maxwell1']==['-16/9','0','-8/9','0']
 ...
 X=sp.factor(22464*y1sq+12288*y2sq)
 exceptional_deficit=sp.factor(sp.Rational(1,4)*sp.Rational(4,3)*16*rx2*sp.Rational(1,3))
 control_deficit=sp.factor(sp.Rational(1,4)*sp.Rational(16,3)*X*sp.Rational(1,5))
 deficit=sp.factor(exceptional_deficit+control_deficit)
 kappa=sp.factor(sp.Rational(1,4)*(6-2*sp.sqrt(3))*(3*sp.sqrt(3)-1)*sp.Rational(1,5)); occupation=sp.radsimp(deficit/kappa)
 representative_occupation=sp.factor(occupation/48)
 assert X==sp.Rational(1547725,324)*d**2
 assert exceptional_deficit==sp.Rational(115,9)*d**2 and control_deficit==sp.Rational(309545,243)*d**2
 assert deficit==sp.Rational(312650,243)*d**2
 assert sp.simplify(occupation-sp.Rational(120250,729)*(6+5*sp.sqrt(3))*d**2)==0
 assert sp.simplify(representative_occupation-sp.Rational(60125,17496)*(6+5*sp.sqrt(3))*d**2)==0
 ...
 "normalized_balance":{"harmonic_convention":"axisymmetric Legendre carriers P_1 and P_2 with averaged norms W_1=1/3 and W_2=1/5, as used by the direct nonlinear source fixtures","ellipse_endpoint":"r_p=0, r_x^2=(115/16)d^2","ell2_control_occupation_X":"(1547725/324)d^2","exceptional_H_deficit":"(115/9)d^2","ell2_control_H_deficit":"(309545/243)d^2","negative_H_deficit":"(312650/243)d^2","Einstein_minus_positive_weight":"kappa_-=(1/20)*(6-2sqrt(3))*(3sqrt(3)-1), including W_2=1/5","required_source_normalized_Einstein_minus_occupation":"|e_-|^2=(120250/729)*(6+5sqrt(3))*d^2","source_to_direct_representative_map":"|e_-|^2=48*|A_-|^2","required_direct_representative_occupation":"|A_-|^2=(60125/17496)*(6+5sqrt(3))*d^2"},
