...
The formerly reported rank-four identity-square defect came from composing
the curved attachment with the flat-Fourier equation projection.  With the
actual curved cotangent maps `p_E` and `p_I`, the identities
`A_aux=A_core p_E`, `B_aux=B_core p_I` and
...
`9 -> 24 -> 50 -> 49 -> 14`.  Its earlier rank-four final square is retained
only as a scoped diagnostic of a mixed-coordinate extraction: it used the
flat-Fourier equation projection where the curved cotangent projection was
required.  The exact curved core certificate reconstructs the full order
