...
- `prolonged_green_witness` — The canonical coefficientwise W has exact QW+WQ and fourteen Green blocks.  The complete 421-variable two-factor PBW system and its order-two Schur projection are exact; the 214-parameter action-adjoint branch has an exact rank-52/100 order-two Schur gate with 1,050 projected constraints.  Its 179 independent quadratics have no nonzero affine-linear consequence, so nonlinear ideal solving is genuinely required.  The old bare-Box obstruction orbit is repairable, but that support-minimal repair fails a different order-two channel.  The expanded relative incidence identifies three reciprocal two-pair saddles; the pair-(1,6) numerator maps and the rank-three scalar complement are explicit, while the actual curvature-diagonal inverse, full Douglis block, characteristics and symmetrizer remain open.  Neither branch has yet supplied a Green witness.
