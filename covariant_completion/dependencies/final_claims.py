...
class FinalClaimDependencyReport:
    ...
    def build() -> "FinalClaimDependencyReport":
        ...
        atomic(
            ...
            "The canonical coefficientwise W has exact QW+WQ and fourteen Green blocks.  The complete 421-variable two-factor PBW system and its order-two Schur projection are exact; the 214-parameter action-adjoint branch has an exact rank-52/100 order-two Schur gate with 1,050 projected constraints.  Its 179 independent quadratics have no nonzero affine-linear consequence.  The exact degree-one Macaulay matrix has 136,585 rows and 20,585 multiplier columns; its degree-three rational rank is rigorously bounded between 12,861 and 14,136, but the full rational ranks and constant-ideal question remain undecided.  The old bare-Box obstruction orbit is repairable, but that support-minimal repair fails a different order-two channel.  The expanded relative incidence identifies three reciprocal two-pair saddles.  For pair (1,6), the exact first-order adjoint sign gives BC=-Pi_vector and B D^-1 C=+Pi_vector; the field Schur block is Eaux_2+Dscalar-Pi_vector, and the complete 116-dimensional temporal Douglis matrix has exact rank 116 and determinant one.  R6sharp still lacks its three spatial first-order coefficients, the rank-three scalar diagonal has not yet been lifted to a cyclic all-row witness, and the arbitrary-covector symbol, a positive symmetrizer, and lower-order completion remain open.  Neither branch has yet supplied a Green witness.",
        )
        atomic(
