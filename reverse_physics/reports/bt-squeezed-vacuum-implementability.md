...
An independent pair-block calculation gives the same necessary summability
obstruction. The standard Shale--Stinespring theorem is only a comparison:
the displayed BT shear preserves the cross-Krein CCR, not the positive
Hilbert-star CCR obtained after choosing a fundamental symmetry.

This is a carrier theorem, not a refutation of the authors' algebraic map.
At fixed finite volume every fixed particle sector is regular. This report's
primary calculation did not certify convergence of the full exponential; the
successor `REVERSE_PHYSICS_BT_EXTENDED_SQUEEZE_CARRIER_V1` shows that it also
needs a modewise contraction bound and constructs an inequivalent weighted
candidate. Such a representation's full BT map, domains, and generalized
Born trace are not supplied in the public Letter.
...
Thus a one-particle $\Upsilon$ mode has positive norm squared $\rho$.
Momentum-dependent $\rho(\mathbf p)$ is allowed, but equivalence to the
...
For constant $\rho$, division by the box volume followed by the continuum
limit gives
...
For constant $\rho$, its density is
...
It therefore fails a necessary positive-Fock summability condition. This
second calculation is a cross-check; the direct two-particle-sector divergence
above is the primary proof. It is not a verbatim application of the standard
Shale--Stinespring theorem, because the BT map is cross-Krein canonical rather
than Hilbert-star canonical. Extended representations beyond the
Hilbert--Schmidt condition are known as a mathematical possibility, which is
exactly why the certificate does not claim that no representation exists.
...
One can formally choose $\rho(p)\sim p^\alpha$. The radial norm integral then
behaves as
...
which converges for $\alpha>1/2$. But then $\rho^{-1}$ is unbounded at the
...
- convergence of the full exponential at finite volume (not analyzed by this
  predecessor and classified separately by its successor);
