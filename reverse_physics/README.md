...
bt_euclidean_lattice_step_scaling_experiment.py
                                       independent local Metropolis rail and
                                       L=4,6 lowest-mode block observations
...
bt_euclidean_quartic_score_power_obstruction.py
                                       exact quartic score kernel with a
                                       nonzero linearly soft fixture and an
...
bt_euclidean_complete_g4_seven_kernel_reduction.py
...
| `..._BT_COLLINEAR_PROJECTOR_TRANSPORT_V1` | finite coherent hard--collinear carrier | **SUPERSEDED NORMALIZATION.** The projector algebra is sound, but this historical artifact inserted the absolute `1/512` rate coefficient into a dimensionless projector. Its amplitude and diagonal blocks are replaced by the corrected row below; the artifact remains immutable for audit — [report](reports/bt-collinear-projector-transport.md) |
| `..._BT_ASYMPTOTIC_GENERATOR_PREFLIGHT_V1` | normalized finite carrier + cubic collinear preflight | **the corrected dimensionless target is `1/48` per unordered pair.** Dividing the absolute real response by the BT Born rate gives `eta/48`, so the exact projector amplitude is `sqrt(3)/12`, with hard/collinear blocks `−1/16` and `+1/16`; multiplying back by the Born rate cancels the physical `3/512` response. But the ordinary cubic Fock generator is exactly zero in the massless collinear limit: `M3 ~ t²` and one energy denominator supplies only `1/t`. A Jordan/double-secular, order-`lambda` `R_t` distributional generator is required and unconstructed; its charge bookkeeping and incoming sectors remain open — [report](reports/bt-asymptotic-generator-preflight.md) |
| `..._BT_SOFT_CHARGE_RESOLVED_FLOW_V1` | exact soft normalization + fixed-vacuum charge audit | **the unprojected normalization is now derived, and the remaining barrier is charge, not a missing number.** Parent raising, Bose reduction, angular measure, and the outgoing `2!/3!` ratio give `+1/48` per pair and `+1/16` in total; idempotence forces hard `−1/16` and absolute `−3/512`. Sharp and smooth cutoffs share the leading response, and a formal finite-cutoff anti-Krein flow exists. But both log rows pair generator charges `(+1,−1)` or `(−1,+1)`; projecting first to the published one-sided nonpositive image makes the residue exactly zero. A neutral physical `1/48` needs a missing broken-vacuum zero-mode completion of `R_t P_2 R_t^dagger` with both charge shifts. Until that object exists the exact outcomes are `1/48`, zero, or positive-charge leakage, so the physical coefficient and complete probability remain fail-closed — [report](reports/bt-soft-charge-resolved-flow.md) |
| `..._BT_ZERO_MODE_EQ19_TRILEMMA_V1` | broken-vacuum orbit algebra + Eq. (19) obstruction | **the genuine shift-orbit carrier can be constructed, and it exposes a noncommuting choice.** With `phi=phi_0+varphi` and operator `Z=exp(lambda phi_0)`, Eq. (16) factorizes as `Omega=Z Omega_hat`, `Upsilon=Z^-1 Upsilon_hat`. The unique `Z` powers make every quadratic number-lowering generator neutral; the two soft powers cancel in the Gram, so the conditional neutral response remains exactly `1/48` per pair. But covariance also changes the Appendix-C squeeze from apparent charge `−2` at `Z=1` to neutral `Z²(b_Upsilon^dagger)²`. Conversely, the fixed-vacuum ideal `(Z−1)` is not stable under the charge derivation because `delta(Z−1)=Z=1 mod (Z−1)`. Thus the earlier negative-radical theorem remains valid only in the fixed-vacuum oscillator grading and cannot exclude the completed neutral squeeze. The missing full zero-mode module/trace still prevents reproduction of Eq. (19) or promotion of `1/48` to a physical coefficient — [report](reports/bt-zero-mode-eq19-trilemma.md) |
| `..._BT_SQUEEZED_VACUUM_IMPLEMENTABILITY_V1` | Appendix-C positive-topology + pair-block audit | **the displayed squeezed vacuum is Krein-null but is not a vector in the ordinary massless Fock--Krein topology.** In a finite box its creation kernel is `1/(8|p|²)` per ordered momentum. The first two-particle sector has positive norm density `rho²/(64 pi²)(epsilon^-1−Lambda^-1)`, and the six-mode lowest-shell density already grows linearly with box size. Independently, the pair block has twice this norm and is not Hilbert--Schmidt. Every uniformly equivalent positive fundamental symmetry retains the divergence; a weight `rho(p)->0` can soften it only by changing to an inequivalent topology. This obstructs an ordinary Fock implementer, not the local algebraic map or an explicitly extended/non-Fock realization of Eq. (19) — [report](reports/bt-squeezed-vacuum-implementability.md) |
| `..._BT_EXTENDED_SQUEEZE_CARRIER_V1` | full pair exponential + explicit inequivalent carrier | **the vacuum vector can be repaired, but only by changing representation, and the Born trace still does not follow.** The exact unordered-pair amplitude is `z(p)=rho(p)/(4p²)`; the full exponential needs both `sup|z|<1` and square summability. Every topology uniformly equivalent to BT fails modewise at small momentum. The explicit weight `rho_mu=4 gamma mu²p²/(p²+mu²)`, `0<gamma<1`, gives `z_mu=gamma mu²/(p²+mu²)` and finite square-sum density `gamma²mu³/(16pi)`, but `rho_mu^-1` is unbounded and the thermodynamic vacuum leaves the ordinary Fock sector. The raw BT shear also fails the positive-Hilbert Bogoliubov relation, so generic extended positive-boson theorems are architecture guidance, not a direct import. This predecessor did not construct the cross-Krein map or trace; the successor row constructs the squeeze factor and finite-rank trace while exposing their thermodynamic boundary — [report](reports/bt-extended-squeeze-carrier.md) |
| `..._BT_CROSS_KREIN_TRACE_LIMIT_V1` | covariant squeeze core + trace-extension limit | **the finite-regulator cross-Krein squeeze and cyclic finite-rank trace can be constructed, but the normal thermodynamic trace cannot.** The vacuum orbit completes on `ell²(Z)` with `J e_n=e_-n`; the zero-mode-dressed squeeze is closable on paired polynomial/Gaussian cores and preserves the finite-rank trace. A finite normalized cyclic trace positive on the ghost-even cone must nevertheless kill every orbit-localized rank-one projector: translation makes all weights equal and the rank-`2N+1` symmetric sum forces `(2N+1)c<=1`, hence `c=0`. The finite-rank trace instead gives a localized projector weight one but has infinite identity weight. The transported vacuum projector also has trace norm `exp(V ell+o(V))`, with exact positive `ell=mu³[(1+gamma)^(3/2)+(1-gamma)^(3/2)-2]/(12pi)`. The remaining architecture must therefore be explicitly semifinite, relative, or non-normal; the full nonlinear `R_t`, Eq. (19), and physical `1/48` remain open — [report](reports/bt-cross-krein-trace-limit.md) |
| `..._BT_SEMIFINITE_RELATIVE_BORN_WEIGHT_V1` | semifinite orbit trace + finite-input conditioning | **finite-detector Born normalization survives the normal-trace no-go.** The canonical faithful normal semifinite trace has `Tau(E_n)=1` and `Tau(1)=infinity`. Conditioning on a finite-trace incoming projection gives nonnegative weights summing to one for any finite exhaustive output partition whose process operators satisfy the BT weak-ghost decomposition. This does not turn the normalized corner functional into a trace: with `P=E_00`, `X=E_01`, `Y=E_10`, one has `omega_P(XY)=1` and `omega_P(YX)=0`. The cyclic object is `Tau`; the conditional state is nontracial. The construction is exact on the finite detector ideal but does not show that the unbounded squeeze preserves that ideal, construct a non-normal thermodynamic state, reproduce Eq. (19), or establish `1/48` — [report](reports/bt-semifinite-relative-born-weight.md) |
| `..._BT_UV_HARD_SCATTERING_LAW_V1` | physical nonforward hard/window rate | **the first loop-informed physical scattering law.** The independently projected three-channel hard logarithm satisfies the Callan--Symanzik equation exactly with the certified PS beta function and BT Born rate: `−4(5/16)(3/32)+2·3(5/256)=0`. Leading UV logs therefore resum to `dσ_hard^LL/dΩ=24π²/[25s log²(s/Λ²)]`; in a detector window `θ₀≤θ≤π−θ₀`, the universal scaled limit is `96π³cos(θ₀)/25`. Positive and scheme-universal at leading log on the UV branch. This is the physical hard baseline, not a completed inclusive NLO probability: endpoint dressing, the Jordan `R_t` generator, and real--virtual cancellation remain open — [report](reports/bt-uv-hard-scattering-law.md) |
