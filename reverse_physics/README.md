...
| `..._BT_COLLINEAR_PROJECTOR_TRANSPORT_V1` | finite coherent hard--collinear carrier | **the exact coefficient admits a normalized cross-multiplicity projector.** Rotate one hard channel coherently into the three unordered collinear-pair channels with exact amplitude `sqrt(1/512)` per pair. The order-two equation `P²=P` then forces the hard normalization `−3/512` and the collinear Gram trace `+3/512`; their common logarithmic response cancels. A neutral generator passes the independently recomputed finite one-sided BT charge-radical gate. This breaks the finite algebraic normalization barrier, not the dynamical one: the generator has not been derived from the BT asymptotic Hamiltonian, and no continuum dressed projector, complete NLO quotient trace, or physical probability is claimed — [report](reports/bt-collinear-projector-transport.md) |
