...
| `..._BT_BORN_TRACE_V1` | obstructed shell | **the queued BT-faithful step, unblocked by one paper read.** Bateman–Turok's nullity is a **charge selection rule**, and their `B` is ghost-parity **even** — so Paper 05's ghost-parity-**odd** obstruction must sit in `C`, whose nullity needs one-sided charge, which `cprop:embedding` makes exact **iff ε = 0**. The two results sit at different points of one family. On the shell the Born trace `tr(T†T) = ‖T₊‖² − ‖T₋‖²` is **positive by inspection** (positive rational·`√5` + positive rational; κ-even beats κ-odd ≈ 26×), so the obstruction **is** invisible to the Krein rule there. Finite-shell shadow only — the capstone stays open. Also **repairs an `nsimplify` float-leak** in `verify_doubled_theory.py` that had fabricated `T[1,1]` — [report](reports/bt-born-trace.md) |
