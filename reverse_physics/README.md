...
| `..._SCATTERING_C_FACTORISATION_V1` | `G4` (all Hermitian pairs of inertia (1,2)) | **the last open question, reduced to a finite test**: `C₊ ⊕ C_H` preserves `ran(S)` iff `C₊ = T₊C_H T₊⁻¹`, and pulling the Stokes identity back by `T₋` turns factorisation into whether `det(M − λH_out)` has all roots real and positive. The one missing input is explicit `T₊`; two witnesses with identical certified inertia answer oppositely, so nothing weaker decides it — [report](reports/scattering-c-factorisation.md) |
