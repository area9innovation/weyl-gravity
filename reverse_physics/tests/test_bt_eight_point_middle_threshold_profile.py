"""Falsification tests for the BT eight-point middle-threshold profile."""
import json
import os
import subprocess
import sys
...
PRODUCER = os.path.join(
    ROOT, "reverse_physics/bt_eight_point_middle_threshold_profile.py"
)
VERIFIER = os.path.join(
    ROOT, "reverse_physics/verify_bt_eight_point_middle_threshold_profile.py"
)


class EightPointMiddleThresholdProfileTests(unittest.TestCase):
    ...
    def test_tau3_sample_mutation_rejected(self):
        self.assertNotEqual(
            self.mutate(
                lambda value: value["profile_analysis"][
                    "tau3_four_outer_coefficient"
                ].update(numerator=246628)
            ).returncode,
            0,
        )
        ...
        self.assertNotEqual(
            self.mutate(
                lambda value: value["disposition"].update(
                    two_threshold_scalar_universality="RESTORED"
                )
            ).returncode,
            0,
        )
