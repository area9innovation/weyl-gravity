#!/usr/bin/env python3
"""Independent verifier for the panels 0--47 aggregate."""
...
def main() -> None:
    ...
    assert summary["completed_panel_count"] == 48
    assert summary["last_panel"] == 47
    assert summary["two_sided_interface_gates_pass"]
    assert summary["all_completed_deltas_exclude_zero"]
    assert len(recorded["rows"]) == 48
    ...
    assert gate["first_obstruction"]["first_missing_panel"] == 48
    ...
    print(
        "projective Evans/Riccati v5 verifier: PASS "
        "(contiguous typed prefix 0--47; next missing panel 48)"
    )
