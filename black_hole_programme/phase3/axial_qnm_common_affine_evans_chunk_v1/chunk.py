#!/usr/bin/env python3
"""Checkpointed parallel evaluation of a bounded Evans contour chunk."""
...
def _checkpoint(rows: list[dict], terminal: dict | None) -> dict:
    return {
        "schema": "phase3-axial-qnm-common-affine-evans-chunk-checkpoint-v1",
...
def compute(*, write_checkpoint: bool = False) -> dict:
    """Evaluate ordered batches and stop at the first failed panel."""
    ctx.prec = 128
    rows: list[dict] = []
    terminal = None
    ...
    return {
        "schema": "phase3-axial-qnm-common-affine-evans-chunk-run-v1",
        "requested_panels": [PANEL_START, PANEL_STOP - 1],
        ...
        "argument_principle": {
            "status": "NOT_RUN",
            "reason": (
                "the certified chunk is not the complete closed contour"
            ),
        },
    }
