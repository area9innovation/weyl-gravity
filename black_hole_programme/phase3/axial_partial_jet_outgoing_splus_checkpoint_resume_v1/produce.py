#!/usr/bin/env python3
"""Resume the outgoing S checkpoint for sixteen bounded inward panels."""
...
ROOT = HERE.parents[2]
SOURCE = HERE / "splus_checkpoint_resume.forge"
OUTPUT = HERE / "certificate.json"
CHECKPOINT = HERE / "checkpoint.json"
RECEIPT = HERE / "receipt.json"
COMPILE_LOG = HERE / "compile.txt"
RUN_LOG = HERE / "run.txt"
BINARY = Path("/tmp/axial-partial-jet-outgoing-splus-checkpoint-resume-v1")
PREDECESSOR = ROOT / (
    "black_hole_programme/phase3/"
    "axial_partial_jet_outgoing_splus_checkpoint_v1"
)
PREDECESSOR_SOURCE = PREDECESSOR / "splus_checkpoint.forge"
...
def render_model(name: str, model: dict) -> str:
    require(model["schema"] == "ivtaylor-degree4-v1", "checkpoint schema drift")
    require(model["degree"] == 4, "checkpoint degree drift")
    rows, cols = int(model["rows"]), int(model["cols"])
    lines = [
        f"fn {name}()->IvTaylor4Mat{{",
        *[f"  let c{k}:QMat=qm_new({rows},{cols});" for k in range(5)],
    ]
...
SUPPORT = r'''
fn sr_radius(center:borrow Rat)->IvTaylor4Mat{
...
fn sr_emit(tag:string,a:borrow IvTaylor4Mat)->void{
...
pub fn main()->i64{
  ...
  let base:IvTaylor4Mat=checkpoint_base();
  let tangent_n:IvTaylor4Mat=sc_scale(checkpoint_tangent(),big("1/512"));
  let center:Rat=big("16381/512");
  ...
  while(panel<16){
    ...
    let mh:IvMat=match(ivtm4_hull_checked(models.direct)){
      some(x)=>x,none=>{
        println(strfmt(system_allocator(),
          "SPLUS_RESUME status=REFUSED panel={} code=MODEL_HULL",[panel]));
        return 3;}};
    let sh:IvMat=match(ivtm4_hull_checked(sc_stack(tangent_n,base))){
      some(x)=>x,none=>{
        println(strfmt(system_allocator(),
          "SPLUS_RESUME status=REFUSED panel={} code=SEED_HULL",[panel]));
        return 3;}};
    ...
    if(tail<0.0||!f64_is_finite(tail)){
      println(strfmt(system_allocator(),
        "SPLUS_RESUME status=REFUSED panel={} code=TAIL",[panel]));return 3;}
    ...
    if(!coefficients||!overlap){
      println(strfmt(system_allocator(),
        "SPLUS_RESUME status=REFUSED panel={} code=CORRELATION",[panel]));
      return 3;
    }
    ...
    if(!f64_is_finite(nw)||!f64_is_finite(bw)){
      println(strfmt(system_allocator(),
        "SPLUS_RESUME status=REFUSED panel={} code=WIDTH",[panel]));return 3;}
  ...
  println(strfmt(system_allocator(),
    "SPLUS_RESUME status=PASS generator=7315 panels={} final_r=31.93359375 max_tail={} max_base_width={} max_normalized_width={} final_base_width={} final_normalized_width={} final_tangent_width={}",
...
def source_text(payload: dict) -> str:
    return "\n".join(
        (
            strip_predecessor(),
            render_model("checkpoint_base", payload["base"]),
            render_model("checkpoint_tangent", payload["tangent"]),
            SUPPORT,
            MAIN,
        )
    )
...
def parse_model(output: str, tag: str) -> dict:
    ...
    raise ResumeError(f"missing serialized model {tag}")


def parse_summary(output: str) -> dict:
    match = re.search(
        r"SPLUS_RESUME status=(?P<status>\w+) generator=(?P<generator>\d+) "
    ...
    refused = re.search(
        r"SPLUS_RESUME status=REFUSED panel=(?P<panel>\d+) "
        r"code=(?P<code>\w+)",
        output,
    )
    return (
...
def build() -> tuple[dict, float]:
    ...
    require(
        predecessor_cert["claim_flags"]["S_checkpoint_serialized"],
        "predecessor checkpoint is not certified",
    )
    require(
        canonical_sha256(predecessor_checkpoint["payload"])
        == predecessor_checkpoint["payload_sha256"],
        "predecessor payload hash drift",
    )
    payload = predecessor_checkpoint["payload"]
    require(payload["generator"] == 7315, "predecessor generator drift")
            ...
            json.dumps(
                {
                    ...
                    "payload_sha256": canonical_sha256(final_payload),
                },
                indent=2,
                sort_keys=True,
            )
    ...
    result = {
        "schema": "phase3-axial-partial-jet-outgoing-splus-checkpoint-resume-v1",
        "result_id": "PURE_WEYL_PHASE3_AXIAL_OUTGOING_SPLUS_CHECKPOINT_RESUME",
        "lifecycle": "NUMERIC-ENCLOSURE",
        "dependency_tags": ["LOCAL-ALGEBRAIC", "REDUCED-MODE"],
        "status": (
            "SPLUS_CORRELATED_16_PANEL_RESUME_PASS"
            if passed
            else "SPLUS_CORRELATED_RESUME_REFUSED"
        ),
        "imports": {
            "predecessor_source": {
        ...
        "transport": {
            ...
            "generator": 7315,
            "start_radius": "8191/256",
            "target_radius": "8175/256",
            "panel_width": "1/256",
            "planned_panels": 16,
            "completed_panels": int(summary.get("panels", summary.get("panel", 0))),
        ...
        "claim_flags": {
            ...
            "S_16_panel_resume_certified": passed,
        ...
        "next_gate": (
            "resume from the content-addressed r=8175/256 checkpoint in "
            "additional bounded chunks, stopping on the first correlation, "
            "tail, or width refusal"
        ),
    }
...
def main() -> int:
    ...
    if args.check:
        ...
        print("PASS Splus checkpoint resume producer check")
        return 0
    OUTPUT.write_text(encoded)
    receipt = {
        "schema": "phase3-axial-partial-jet-outgoing-splus-checkpoint-resume-receipt-v1",
        "certificate": str(OUTPUT.relative_to(ROOT)),
        "certificate_sha256": sha256(OUTPUT),
        "command": (
            "python3 -m black_hole_programme.phase3."
            "axial_partial_jet_outgoing_splus_checkpoint_resume_v1.produce"
        ),
