#!/usr/bin/env python3
"""Independent verifier for the panels 0--93 aggregate."""
...
def main() -> None:
    ...
    assert summary["completed_panel_count"] == 94
    assert summary["last_panel"] == 93
    assert summary["two_sided_interface_gates_pass"]
    assert summary["all_completed_deltas_exclude_zero"]
    assert len(recorded["rows"]) == 94
    ...
    assert gate["first_obstruction"]["first_missing_panel"] == 94
    ...
    assert certificate["claim_flags"][
        "stable_root_repair_applied_from_panel_77"
    ]
    assert not certificate["claim_flags"]["threshold_lowered"]
    assert not certificate["claim_flags"]["QNM_or_EP2_certified"]
    print(
        "projective Evans/Riccati v9 verifier: PASS "
        "(contiguous typed prefix 0--93; next missing panel 94)"
    )
