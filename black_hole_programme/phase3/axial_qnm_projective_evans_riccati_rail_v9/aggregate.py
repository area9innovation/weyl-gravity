#!/usr/bin/env python3
"""Aggregate the certified projective prefix through panel 93."""
...
V8_RUN = ROOT / (
    "black_hole_programme/phase3/"
    "axial_qnm_projective_evans_riccati_rail_v8/rail-v8-run.json"
)
CHUNK_CERT = ROOT / (
    "black_hole_programme/phase3/"
    "axial_qnm_common_affine_evans_chunk_v6/certificate.json"
)
CHUNK_RUN = ROOT / (
    "black_hole_programme/phase3/"
    "axial_qnm_common_affine_evans_chunk_v6/chunk-run.json"
)
...
def compute() -> dict:
    ctx.prec = 128
    predecessor = checked_run(V8_CERT, V8_RUN)
    chunk = checked_run(CHUNK_CERT, CHUNK_RUN)
    rows = list(predecessor["rows"])
    rows.extend(typed_row(row) for row in chunk["rows"])
    ...
    full = 512
    return {
        "schema": "phase3-axial-qnm-projective-evans-riccati-run-v9",
        "arithmetic": "python-flint acb/arb, 128 bits",
        "status": "PANELS_0_THROUGH_93_AGGREGATED_LOCAL_QNM_GATE_FAIL_CLOSED",
        "rows": rows,
        "summary": {
        ...
        "local_qnm_gate": {
            ...
            "first_obstruction": {
                ...
                "missing_field": (
                    "stable-root shared-generator horizon/outgoing "
