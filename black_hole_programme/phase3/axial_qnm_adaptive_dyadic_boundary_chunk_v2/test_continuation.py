"""Scoped tests for adaptive continuation v2."""
from __future__ import annotations

import json
import unittest
from fractions import Fraction

from .continuation import AGGREGATE_RUN, RAW_RUN, canonical_sha


class AdaptiveContinuationV2Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.raw = json.loads(RAW_RUN.read_text())
        cls.aggregate = json.loads(AGGREGATE_RUN.read_text())

    def test_budget_and_hashes(self) -> None:
        self.assertLess(self.raw["elapsed_compute_seconds"], 60)
        for entry in self.raw["observations"]:
            self.assertEqual(entry["row_sha256"], canonical_sha(entry["row"]))

    def test_parent_order(self) -> None:
        parents = [
            entry["parent_panel"] for entry in self.raw["observations"]
            if entry["kind"] == "parent_observation"
        ]
        self.assertEqual(parents, list(range(100, 100 + len(parents))))

    def test_contiguous_aggregate(self) -> None:
        bounds = [
            (Fraction(item["start"]), Fraction(item["stop"]))
            for item in self.aggregate["segments"]
        ]
        self.assertTrue(all(
            left[1] == right[0]
            for left, right in zip(bounds, bounds[1:])
        ))

    def test_claim_gates_closed(self) -> None:
        self.assertFalse(any(self.aggregate["closed_claim_gates"].values()))


if __name__ == "__main__":
    unittest.main()
