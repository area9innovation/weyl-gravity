#!/usr/bin/env python3
"""Produce the panels 64--79 common-affine certificate."""
...
PREDECESSOR = ROOT / (
    ...
    "axial_qnm_common_affine_evans_chunk_v4/certificate.json"
)
BASE_SOURCE = ROOT / (
    "black_hole_programme/phase3/"
...
def main() -> None:
    ...
    certificate = {
        "schema": "phase3-axial-qnm-common-affine-evans-chunk-v5",
        "status": (
            "PANELS_64_THROUGH_79_BOUNDARY_NONVANISHING_CERTIFIED"
            if complete else "CHUNK_64_THROUGH_79_FAIL_CLOSED"
        ),
    ...
    REPORT.write_text(
        "# Common-affine projective Evans chunk v5\n\n"
        "Dependency tags: `LOCAL-ALGEBRAIC`, `REDUCED-MODE`.\n\n"
        f"Requested panels 64--79; complete=`{complete}`, terminal="
        f"`{run['terminal']}`.  The smallest passing mismatch lower bound "
        f"is `{lower}` on panel `{panel}`.  The ordered producer stops at "
        "the first boundary, pivot, or self-map failure.  No closed-contour "
        "or QNM claim is made.\n"
    )
    commands = [
        ...
        ["python3", "-m", "unittest", "-v",
         "black_hole_programme.phase3."
         "axial_qnm_common_affine_evans_chunk_v5.test_chunk"],
        ["python3", "-m",
         "black_hole_programme.phase3."
         "axial_qnm_common_affine_evans_chunk_v5.verify"],
    ]
    checks = []
    ...
    RECEIPT.write_text(json.dumps({
        "schema": (
            "phase3-axial-qnm-common-affine-evans-chunk-receipt-v5"
        ),
