#!/usr/bin/env python3
"""Compile and run both bounded exterior-norm replays."""
...
def main() -> int:
    ...
    for index, child_metadata in enumerate(metadata):
        ...
        binary = Path(f"/tmp/axial-h4-plucker-exterior-{index}-v1")
        compile_source(
        ...
        terminal = re.findall(
            r"^(?:PLUCKER_REFUSE|EXTERIOR_[A-Z0-9_]+).*$",
            text,
            flags=re.MULTILINE,
        )
        if code not in (3, 42):
            raise RuntimeError(f"unexpected replay exit {code}")
        if code == 3 and not terminal:
    ...
    manifest = {
        "schema": "phase3-axial-h4-plucker-exterior-norm-manifest-v1",
        "status": "EXTERIOR_PASS" if passed else "EXTERIOR_REFUSED",
        "scope": {
            "children": [0, 1],
