#!/usr/bin/env python3
"""Produce the stable-root panels 78--93 common-affine certificate."""
...
PREDECESSOR = ROOT / (
    ...
    "axial_qnm_projective_evans_riccati_rail_v8/certificate.json"
)
REPAIR_CERT = ROOT / (
    "black_hole_programme/phase3/"
...
def main() -> None:
    ...
    imports = {
        "predecessor_aggregate": PREDECESSOR,
        "repair_certificate": REPAIR_CERT,
        "repair_source": REPAIR_SOURCE,
        "transport_source": BASE_SOURCE,
    }
    certificate = {
        "schema": "phase3-axial-qnm-common-affine-evans-chunk-v6",
        "status": (
            "PANELS_78_THROUGH_93_BOUNDARY_NONVANISHING_CERTIFIED"
            if complete else "CHUNK_78_THROUGH_93_FAIL_CLOSED"
        ),
        "lifecycle": "CLASSIFIED",
        "dependency_tags": ["LOCAL-ALGEBRAIC", "REDUCED-MODE"],
        "claim_flags": {
            "requested_chunk_nonzero_certified": complete,
            "stable_root_repair_applied": True,
        ...
        "method": {
            ...
            "horizon_remainder_root": run["horizon_remainder_root"],
            "shared_generator": (
    ...
    REPORT.write_text(
        "# Stable-root common-affine Evans chunk v6\n\n"
        "Dependency tags: `LOCAL-ALGEBRAIC`, `REDUCED-MODE`.\n\n"
        f"Requested panels 78--93; complete=`{complete}`, terminal="
        f"`{run['terminal']}`.  The smallest passing mismatch lower bound "
        f"is `{lower}` on panel `{panel}`.  The panel-77 stable interval "
        "quadratic-root repair is applied with unchanged self-map gates. "
        "No closed-contour or QNM claim is made.\n"
    )
    commands = [
        ...
        ["python3", "-m", "unittest", "-v",
         "black_hole_programme.phase3."
         "axial_qnm_common_affine_evans_chunk_v6.test_chunk"],
        ["python3", "-m",
         "black_hole_programme.phase3."
         "axial_qnm_common_affine_evans_chunk_v6.verify"],
    ]
    checks = []
    ...
    RECEIPT.write_text(json.dumps({
        "schema": (
            "phase3-axial-qnm-common-affine-evans-chunk-receipt-v6"
        ),
