#!/usr/bin/env python3
"""Produce the panels 0--63 projective Evans aggregate."""
...
from .aggregate import CHUNKS, RUN, V5, compute
...
def main() -> None:
    ...
    summary = run["summary"]
    certificate = {
        "schema": "phase3-axial-qnm-projective-evans-riccati-rail-v6",
        "status": run["status"],
        ...
        "result": run,
        "claim_flags": {
            "panels_0_through_63_typed_two_sided_at_r32": (
                summary["contiguous_prefix"]
                and summary["two_sided_interface_gates_pass"]
            ),
            "panels_0_through_63_delta_nonzero": summary[
        ...
        "does_not_establish": [
            "projective mismatch data for boundary panels 64--511",
    ...
    REPORT.write_text(
        "# Projective Evans/Riccati rail v6\n\n"
        "Dependency tags: `LOCAL-ALGEBRAIC`, `REDUCED-MODE`.\n\n"
        "The contiguous typed two-sided prefix now covers panels 0--63 at "
        "`r=32`.  Every completed Delta excludes zero.  The minimum lower "
        f"bound is `{summary['minimum_delta_modulus_lower']}` on panel "
        f"`{summary['minimum_delta_modulus_lower_panel']}`.  Panel 64 is "
    ...
    commands = [
        ...
        ["python3", "-m", "unittest", "-v",
         ...
         "axial_qnm_projective_evans_riccati_rail_v6.test_aggregate"],
        ["python3", "-m",
         "black_hole_programme.phase3."
         "axial_qnm_projective_evans_riccati_rail_v6.verify"],
    ]
    checks = []
    ...
    RECEIPT.write_text(json.dumps({
        "schema": (
            "phase3-axial-qnm-projective-evans-riccati-rail-receipt-v6"
        ),
