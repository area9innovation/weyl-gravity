#!/usr/bin/env python3
"""Independent verifier for the panels 0--63 aggregate."""
...
def main() -> None:
    ...
    assert summary["completed_panel_count"] == 64
    assert summary["last_panel"] == 63
    assert summary["two_sided_interface_gates_pass"]
    assert summary["all_completed_deltas_exclude_zero"]
    assert len(recorded["rows"]) == 64
    ...
    assert gate["first_obstruction"]["first_missing_panel"] == 64
    ...
    print(
        "projective Evans/Riccati v6 verifier: PASS "
        "(contiguous typed prefix 0--63; next missing panel 64)"
    )
