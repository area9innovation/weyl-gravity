...
| einstein_boundary | `PHASE_SPACE_NOT_CLOSED` | The fixed compact U(1) domain and relative Taub descent are certified. The complete standard axial harmonic gauge quotient is reduced to two masters for every S1 momentum and ell>=2. The generic polar ell>=2 coefficient matrix has an exact two-master reduction with the same dispersion and a full-tensor ell=2 plus-branch fixture, including the corrected Maxwell volume density. | promote the polar matrix to an arbitrary-lambda full-tensor identity, classify exceptional polar ell=0,1, match both reduced currents to the covariant symplectic form, and solve extra fourth-order adjoint classes; independently complete the asymptotic Bach phase space and charge audit |
