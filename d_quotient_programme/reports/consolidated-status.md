...
| einstein_boundary | `PHASE_SPACE_NOT_CLOSED` | The fixed compact U(1) domain, complete standard axial/polar harmonic quotients, and action-normalized Einstein--Maxwell covariant symplectic form are certified on every radiative block. Both ell=1 gauge branches are exact presymplectic kernels and the physical quotients are positive. | compute the homogeneous ell=0 and axial ell=1 twist global presymplectic pairs, then compare the Weyl--Maxwell pullback and solve extra fourth-order adjoint classes; independently complete the asymptotic Bach phase space and charge audit |
