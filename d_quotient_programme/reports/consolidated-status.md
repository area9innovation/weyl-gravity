...
| einstein_boundary | `PHASE_SPACE_NOT_CLOSED` | The fixed compact U(1) domain and relative Taub descent are certified. The complete standard axial and polar harmonic quotients are classified for all ell, momenta, and m. Polar ell=0 contains only homogeneous radion/circumference/electric-charge data, while ell=1 contains one physical triplet and an exactly gauge zero branch. | match the complete axial and polar reduced currents, including exceptional global modes, to the covariant symplectic form and solve extra fourth-order adjoint classes; independently complete the asymptotic Bach phase space and charge audit |
