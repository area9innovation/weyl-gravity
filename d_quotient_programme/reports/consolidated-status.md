...
| einstein_boundary | `PHASE_SPACE_NOT_CLOSED` | The fixed compact U(1) domain and relative Taub descent are certified. Both axial and polar ell>=2 harmonic gauge quotients are reduced to two masters for every S1 momentum and m, with identical exact dispersions. The polar theorem includes an arbitrary-lambda full-tensor identity, complete gauge rank, corrected Maxwell volume density, and a full-rank s=0 audit. | classify exceptional polar ell=0,1, match axial and polar reduced currents to the covariant symplectic form, and solve extra fourth-order adjoint classes; independently complete the asymptotic Bach phase space and charge audit |
