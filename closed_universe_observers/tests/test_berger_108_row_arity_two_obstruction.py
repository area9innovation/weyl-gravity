...
def test_typed_base_is_zero_but_complete_degree_zero_is_obstructed():
    ...
    assert audit["complete_defect_summary"]["operator_key_count"] == 4228
    assert audit["complete_defect_summary"]["serialized_term_count"] == 4552
...
def test_first_witness_is_exact_and_survives_background_quotient():
    ...
    assert witness["coefficient"][0]["coefficient"]["rational"] == {"numerator": -2, "denominator": 1}
    ...
    assert set(audit["first_defect_q2_source_isolation"]) == {"base_maxwell_typed", "emitter_physical", "emitter_Diff_BV"}
