...
def test_four_finite_capabilities_close_but_three_complete_execution_capabilities_remain_fail_closed():
    rows = build()["readiness"]["rows"][1:]
    assert len(rows) == 7
    assert [row["status"] for row in rows].count("CERTIFIED") == 4
