...
def dual_and_pairing_sign(row: int) -> tuple[int, int]:
    if 5 <= row <= 14:
        # The canonical gravity rows store the metric equation as the stress
        # tensor T_ab=-2 delta S/d g_ab, not as the raw covariant-metric
        # action derivative.  This weight is already built into the pinned
        # gravity--Maxwell payload and must also be used by appended matter.
        # Rows 15--16 are scalar Euler rows and do not carry this factor.
        return 27 + row - 5, -2
    if 15 <= row <= 16:
        return 27 + row - 5, 1
    if 55 <= row <= 58:
...
def identical_slot_hessian_audit() -> dict[str, Any]:
    ...
    key = 27, 84, (), 84, ()
    expected = constant(-12)
    return {
        "fixture": "3 h_hat_00 K0_01 K0_01",
        "raw_identical_slot_action_Hessian_coefficient": serialize(constant(6)),
        "metric_Hamiltonian_weight": "-2",
...
def metric_stress_hamiltonian_bridge_audit() -> dict[str, Any]:
    """Pin the raw-action-to-stress normalization on a shared-field key."""

    tensor = action_to_q2(physical_cubic_action()[0])
    key = 31, 55, (1,), 84, ()
    expected = product(parameter("g0"), profile("h0"))
    return {
        "fixture": "q2(A_0 with e1,K0_01)->h_hat_11_star",
        "raw_densitized_two_form_metric_jet": str(
            form_bilinear_metric_jet(2, 0, 0, 4)
        ),
        "dA_01_from_e1_A0": "-1",
        "metric_Hamiltonian_weight": "-2",
        "expected_stress_output_coefficient": serialize(expected),
        "actual_stress_output_coefficient": serialize(tensor.get(key, {})),
        "metric_stress_bridge_defect_count": int(tensor.get(key, {}) != expected),
    }
...
def build(*, payload: dict[str, Any] | None = None, payload_sha256: str | None = None) -> dict[str, Any]:
    ...
    stress_bridge = metric_stress_hamiltonian_bridge_audit()
    metric_audit = metric_jet_audit()
    tensor = action_to_q2(physical_cubic_action()[0])
    symmetry = symmetry_defects(tensor)
    if q1_audit["q1_hessian_recovery_defect_count"] or multiplicity_audit["identical_slot_hessian_defect_count"] or stress_bridge["metric_stress_bridge_defect_count"] or metric_audit["metric_bilinear_first_jet_defect_count"] or symmetry:
    ...
    boundary = (
        "This exact LOCAL-ALGEBRAIC certificate exports the complete physical-action contribution of both selected massive two-form emitters to q2 on the canonical 108-row Berger carrier. The component action is fixed by an exact Hessian recovery: its quadratic free and switched Maxwell--emitter terms reproduce every key of the previously certified scalar emitter q1 overlay with zero defect, including Lorentzian form weights and switch Leibniz jets. Exact first variation of the densitized two- and three-form pairings supplies the free kinetic and mass stress, the switched interaction stress, and the reciprocal clock-switch source. Raw covariant-metric action derivatives are raised to the canonical stress rows with T=-2 delta S/dg; the exact q2(e1 A_0,K0_01)->h_hat_11_star fixture pins the resulting coefficient. Raising every other slot of the same cubic action through the signed odd pairing generates all clock-, Maxwell- and emitter-cotangent cyclic mates with noncommuting Berger-frame PBW integration by parts and exact graded input symmetry. The action Hessian retains multiplicity two when the two remaining slots coincide; an independent 3 h_hat_00 K0_01^2 fixture has raw Hessian coefficient six and raised stress-row coefficient minus twelve, detecting both the former half-weight diagonal emission and the Hamiltonian bridge. Independent symbolic fixtures verify the raw metric bilinear first jets, and deleting one payload term changes the canonical hash. This closes the physical emitter stress/switch q2 subblock only. The two-form Diff--BV q2 cotangent orbit is separately certified. Complete arity two remains obstructed on the relational temporal emitter-Diff orbit; every arity-three, K_Berger, observer-morphism, detector-cone, nonlinear-rank, physical-Bridge-3, finite-parameter causal and quantum claim remains unavailable. No compact-product mode is identified with a Berger row."
    )
    return {
        ...
        "action_and_cyclicity_audit": {"q1_hessian_recovery": q1_audit, "identical_slot_hessian": multiplicity_audit, "metric_stress_Hamiltonian_bridge": stress_bridge, "metric_first_jet": metric_audit, "graded_symmetry_defect_count": symmetry, "cyclicity_generation": "all physical emitter q2 rows are Euler derivatives of one component action and are raised with the canonical signed odd pairing, with metric derivatives converted to the canonical stress-tensor rows by T=-2 delta S/dg"},
