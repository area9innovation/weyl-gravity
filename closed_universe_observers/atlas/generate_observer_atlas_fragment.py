...
def entries() -> list[dict]:
    return [
        ...
        },
        {
            ...
            "operational_observable": {"detector_response": "streamed adaptive route selected; the scalar recurrence stream and six external-clock temporal-weight streams are evaluated but no full Green-weighted detector response", "response_rank": "OPEN for the recoil-corrected response", "emitter_preparation": "existing detector-selected massive Cauchy pair; external-clock-and-s^p weighted scalar coefficients for p=0,2,4,6,8,10 are evaluated through two_j=139", "clock_and_rod_dependence": "radius-1/128 rod profile fixes the necessary two_j>=138 capacity rail; the external a(t)=cos(lambda s) factor and normalized joint clock moments are enclosed without assuming independence", "relational_redshift_contribution": "OPEN until the full same-background response is evaluated", "recoil_backreaction_order": "absolute g^3 operator exact; coefficient remains OPEN", "survives_gauge_reduction": "OPEN for the completed Green-weighted mode sum", "profile_green_boundary_dependencies": "each scalar shard reconstructs 9,870 diagonal values through two_j=139 from 4,970 validated intervals; the six temporal rails enclose E[s^p sec(lambda s)^(2k-1)] with the external detector clock factor retained; polarization/Green charge-block composition, the tail beyond two_j=138, and full Maxwell/massive images remain OPEN"},
            ...
            "evidence": _evidence("adaptive_route", "streaming_sectors", "polarization_recurrence", "high_order_moments", "scalar_stream", "scalar_s0", "scalar_s2", "scalar_s4", "scalar_s6", "scalar_s8", "scalar_s10"),
        },
        {
