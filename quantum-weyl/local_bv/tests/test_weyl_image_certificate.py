...
from local_bv.weyl_image_certificate import (
    DETAILED_PATH,
    RESULT_PATH,
    SCHEMA_PATH,
    build_certificate,
...
class SchoutenZeroWeylImageCertificateTests(unittest.TestCase):
    ...
    def test_detailed_certificate_satisfies_enforced_schema(self) -> None:
        ...
        certificate["image"]["target_dimension"] = 2
        self.assertIn(
            "$.image.target_dimension: value differs from const",
    ...
    def test_claim_boundary_remains_fail_closed(self) -> None:
        ...
        self.assertEqual(
            certificate["checks"]["unrestricted_weyl_jet_quotient"],
            "NOT_COMPUTED",
        )
        self.assertEqual(
            certificate["checks"]["parity_odd_single_epsilon_enumeration"],
            "NOT_COMPUTED",
        )
        self.assertEqual(
