#!/usr/bin/env python3
"""Verify schema, determinism and independent replay for the full D action."""
...
SCHEMA = HERE / "schema/strict-386-full-d-action-v1.schema.json"
...
builder = module(HERE / "build_strict_386_full_d_action.py", "full_d_builder")
checker = module(HERE / "check_strict_386_full_d_action.py", "full_d_checker")
...
def main() -> int:
    ...
    for token in (
        "T=Lie_partial_t", "D=iT", "386 rows", "[T,q1]", "4,374", "Berger",
        "Same-carrier `q2`", "Gate A", "QME",
    ):
        if token not in report:
            errors.append("report token " + token)
    print("STRICT_386_FULL_D_ACTION_V1_SCHEMA: " + ("PASS" if not errors else "FAIL"))
    for error in errors:
        print("  - " + error)
    return bool(errors)
