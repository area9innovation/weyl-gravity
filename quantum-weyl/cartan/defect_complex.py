...
class FirstOrderCartanData:
    ...
    def with_homotopy_shift(
        self,
        shift: HomogeneousOperator,
        *,
        compensate_lie: bool,
        admissible_complex: AdmissibleOperatorComplex | None = None,
    ) -> "FirstOrderCartanData":
        """Apply an allowed degree-minus-one finite homotopy redefinition.

        Without a simultaneous Ward-operator change, the defect representative
        shifts by the exact term ``[Q,shift]``.  With
        ``L_D^(1) -> L_D^(1)+[Q,shift]`` it is unchanged exactly.
        """

        if shift.degree != -1:
            raise ValueError("a homotopy scheme shift must have degree minus one")
        self.complex.validate_operator(shift)
        if admissible_complex is not None:
            if admissible_complex.ambient != self.complex:
                raise ValueError("scheme admissibility complex has the wrong ambient complex")
            admissible_complex.validate_operator(shift)
        lie_shift = (
            graded_commutator(self.complex.q, shift, name="[Q,X]")
            if compensate_lie
            else HomogeneousOperator(
                "zero_L_shift", 0, ExactMatrix.zero(self.complex.dimension, self.complex.dimension)
            )
        )
        return FirstOrderCartanData(
            ...
            lie_0=self.lie_0,
            q_1=self.q_1,
            iota_1=add_operators(self.iota_1, shift, name="iota_1_shifted"),
            lie_1=add_operators(self.lie_1, lie_shift, name="L_D_1_shifted"),
        )

    def conjugated_first_order(
        self,
        generator: HomogeneousOperator,
        *,
        admissible_complex: AdmissibleOperatorComplex | None = None,
    ) -> "FirstOrderCartanData":
        """Apply the order-hbar similarity redefinition generated by ``R``.

        For degree-zero ``R`` the first-order rows transform by
        ``O_1 -> O_1 + [O_0,R]``.  The Cartan defect is conjugated from its
        vanishing classical value and is therefore unchanged at first order.
        """

        if generator.degree != 0:
            raise ValueError("a similarity-scheme generator must have degree zero")
        self.complex.validate_operator(generator)
        if admissible_complex is not None:
            if admissible_complex.ambient != self.complex:
                raise ValueError("scheme admissibility complex has the wrong ambient complex")
            admissible_complex.validate_operator(generator)
        return FirstOrderCartanData(
            complex=self.complex,
            iota_0=self.iota_0,
            lie_0=self.lie_0,
            q_1=add_operators(
                self.q_1,
                graded_commutator(self.complex.q, generator, name="[Q,R]"),
                name="Q_1_conjugated",
            ),
            iota_1=add_operators(
                self.iota_1,
                graded_commutator(self.iota_0, generator, name="[iota_D,R]"),
                name="iota_1_conjugated",
            ),
            lie_1=add_operators(
                self.lie_1,
                graded_commutator(self.lie_0, generator, name="[L_D,R]"),
                name="L_D_1_conjugated",
            ),
        )
