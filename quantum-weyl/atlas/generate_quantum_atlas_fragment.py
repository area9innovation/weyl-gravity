...
def _tangent_crosswalk(values: dict[str, dict[str, Any]]) -> dict[str, Any]:
    return _entry(
        ...
        _quantum_data(
            ...
            qme=("CERTIFIED", "strict one-loop local Euclidean QME is obstructed and the tau-adic compensator-extended one-loop local Euclidean QME is restored; the raw BoxR coefficient and scheme conversion are fixed, the FV anomaly action fixes the Ricci-scalar sector, the generic ghost n=3 triangle is projected exactly onto the scalar-flat ten-dimensional five-carrier quotient, the pure-vector n=1+n=2 CPT sum is exact, and the longitudinal Schur correction has exact Wres(K), Wres(K^2), Wres(log S_L), an exact declared order-two weighted-trace pole/scale row, exact round-S4 reference finite K/K2 rows, a rigorously enclosed round-S4 det_3 tail and weighted modified determinant, and an exact round-S4 zeta-to-weighted factorization defect 5/3; a smoothing witness proves the generic finite rows require a full Green kernel or spectral measure, while the generic noncommuting BCH residue, simplex-integrated repository functions and coefficients, finite normalizations and complete Q1 are underdetermined"),
            lifecycle=("NO_CERTIFIED_MAP", "the coefficient-bearing QME disposition, FV anomaly action, Ricci-sector dependence, selected FV carrier completion, algebraic C3 basis, parity-even third-curvature manifest, universal source kernels, exact n=3 scalar-flat five-carrier parametric projection, pure-vector n=1+n=2 CPT slice, longitudinal Schur resummation, all three critical Schur residues, the declared weighted-trace pole/scale row, complete round-S4 modified-determinant benchmark and exact round-S4 zeta factorization defect are complete in scope, but the generic primed Green/spectral carrier, noncommuting BCH residue, the generic physical fourth-order Hessian kernel, simplex-integrated repository form-factor functions and coefficients, parity-odd derivative manifest, finite normalizations, complete Q1, Bridge 2, and an extended same-background classical carrier map are absent"),
...
def _guard_entries(values: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    specs = [
        ...
        ("generic_background_diff_weyl_ghost_cpt_obstruction", "generic-background effective Diff-Weyl ghost determinant: direct minimal-CPT substitution obstructed; exact Endo-Duhamel reduction supplied; n=3 projected onto the scalar-flat ten-dimensional five-carrier quotient; the n=1+n=2 pure-vector CPT sum is exact and all longitudinal D_W towers are resummed into one normalized Schur kernel; its order-minus-two correction is in S_3, with exact Wres(K), Wres(K^2), Wres(log S_L), and the declared order-two weighted-trace pole/scale row; the round-S4 reference finite K/K2 rows, canonical det_3 tail and weighted modified determinant are complete, while a smoothing witness proves that the generic values require a full primed Green/spectral carrier; the possible local zeta multiplicative term and integrated repository functions remain open", ["LOCAL-ALGEBRAIC", "EUCLIDEAN-SPECTRAL"], ("generic_background_ghost_CPT_obstruction", "generic_ghost_Endo_Duhamel_reduction", "generic_ghost_n1_n2_Hodge_resolvent_reduction", "generic_ghost_n1_n2_vector_CPT_projection", "generic_ghost_longitudinal_Schur_resummation", "generic_ghost_Schur_Schatten_split", "generic_ghost_Schur_Wodzicki_residue", "generic_ghost_Schur_weighted_trace_scale", "round_S4_ghost_Schur_finite_weighted_traces", "generic_ghost_n3_adiabatic_carrier", "generic_ghost_n3_triangle_kernel", "generic_ghost_n3_five_carrier_projection", "scalar_flat_K_Ricci_crosswalk")),
