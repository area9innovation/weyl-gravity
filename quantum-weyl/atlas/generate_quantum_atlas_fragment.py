...
def _tangent_crosswalk(values: dict[str, dict[str, Any]]) -> dict[str, Any]:
    return _entry(
        ...
        _quantum_data(
            ...
            qme=("CERTIFIED", "strict one-loop local Euclidean QME is obstructed and the tau-adic compensator-extended one-loop local Euclidean QME is restored; the raw BoxR coefficient and scheme conversion are fixed, the FV anomaly action fixes the Ricci-scalar sector, the algebraic C3 basis and parity-even five-carrier manifest are complete, five exact universal CPT source kernels are imported, and the generic ghost n=3 triangle is projected exactly onto the scalar-flat ten-dimensional five-carrier quotient, while its n=1/n=2 traces, simplex-integrated repository functions and coefficients, finite normalizations and complete Q1 are underdetermined"),
            lifecycle=("NO_CERTIFIED_MAP", "the coefficient-bearing QME disposition, FV anomaly action, Ricci-sector dependence, selected FV carrier completion, algebraic C3 basis, parity-even third-curvature manifest, universal source kernels, generic-ghost Endo-Duhamel reduction and exact n=3 scalar-flat five-carrier parametric projection are complete in scope, but n=1/n=2 curved-Endo traces, generic physical fourth-order Hessian kernel, simplex-integrated repository form-factor functions and coefficients, parity-odd derivative manifest, finite normalizations, complete Q1, Bridge 2, and an extended same-background classical carrier map are absent"),
        ...
        "Classical second-order obstruction does not imply BRST disappearance, a loop interaction, a quantum constraint, BRST exactness, or ghost removal. The coefficient-bearing QME disposition is complete—strict obstructed, tau-adic compensator extension restored locally at one Euclidean loop—and the exact FV anomaly action proves structural dependence of the Ricci-scalar sector. The zero-derivative algebraic C3 basis and parity-even third-curvature carrier manifest are complete, five exact universal CPT source kernels are imported, and the generic ghost n=3 nonzero-momentum triangle is projected exactly onto the scalar-flat ten-dimensional five-carrier quotient as rational simplex integrands. Its n=1/n=2 curved-Endo traces, simplex-integrated repository functions, generic physical fourth-order Hessian kernel, coefficients, parity-odd derivative manifest, finite normalizations, global Green data and complete Q1 remain open. Bridge 2 and a same-background extended classical carrier map are absent, so no interacting-BRST insertion crosswalk is certified.",
    )
...
def _guard_entries(values: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    specs = [
        ...
        ("generic_background_diff_weyl_ghost_cpt_obstruction", "generic-background effective Diff-Weyl ghost determinant: direct minimal-CPT substitution obstructed; exact Endo-Duhamel reduction supplied; n=3 nonzero-momentum triangle projected exactly onto the scalar-flat ten-dimensional five-carrier quotient as a parametric integrand; n=1/n=2 traces and simplex-integrated repository functions remain open", ["LOCAL-ALGEBRAIC", "EUCLIDEAN-SPECTRAL"], ("generic_background_ghost_CPT_obstruction", "generic_ghost_Endo_Duhamel_reduction", "generic_ghost_n3_adiabatic_carrier", "generic_ghost_n3_triangle_kernel", "generic_ghost_n3_five_carrier_projection", "scalar_flat_K_Ricci_crosswalk")),
