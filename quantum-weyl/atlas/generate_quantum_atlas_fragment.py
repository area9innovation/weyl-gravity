...
def _tangent_crosswalk(values: dict[str, dict[str, Any]]) -> dict[str, Any]:
    return _entry(
        ...
        _quantum_data(
            ...
            qme=("CERTIFIED", "strict one-loop local Euclidean QME is obstructed and the tau-adic compensator-extended one-loop local Euclidean QME is restored; the raw BoxR coefficient and scheme conversion are fixed, the FV anomaly action fixes the Ricci-scalar sector, the generic ghost n=3 triangle is projected exactly onto the scalar-flat ten-dimensional five-carrier quotient and integrated at the normalized symmetric point, the pure-vector n=1+n=2 CPT sum is exact, and the longitudinal Schur correction has exact Wres(K), Wres(K^2), Wres(log S_L), an exact declared order-two weighted-trace pole/scale row, exact round-S4 reference finite K/K2 rows, a rigorously enclosed round-S4 det_3 tail and weighted modified determinant, an exact Einstein-ratio defect 5/3, and an exact generic weight-raised local defect -(1/4)Wres(K^2) specializing to -1/3; a smoothing witness proves the generic finite rows require a full Green kernel or spectral measure, while generic repository functions and coefficients, finite normalizations and complete Q1 are underdetermined"),
            lifecycle=("NO_CERTIFIED_MAP", "the coefficient-bearing QME disposition, FV anomaly action, Ricci-sector dependence, selected FV carrier completion, algebraic C3 basis, parity-even third-curvature manifest, universal source kernels, exact n=3 scalar-flat five-carrier parametric projection and symmetric-point integration, pure-vector n=1+n=2 CPT slice, longitudinal Schur resummation, all three critical Schur residues, the declared weighted-trace pole/scale row, complete round-S4 modified-determinant benchmark, exact round-S4 Einstein-ratio factorization defect, and exact selected generic weight-raised local factorization defect are complete in scope, but the generic primed Green/spectral carrier, the generic physical fourth-order Hessian kernel, generic repository form-factor functions and coefficients beyond the symmetric fixture, parity-odd derivative manifest, finite normalizations, complete Q1, Bridge 2, and an extended same-background classical carrier map are absent"),
        ...
        _evidence(values, "general_tangent_cone", "finite_k0_cone", "smooth_secular_cone", "bounded_resonance_divisor", "Slavnov_preflight", "regulated_Slavnov_breaking", "WZ_compensator_preflight", "WZ_cotangent_lift", "WZ_extended_local_BV", "one_loop_Q1_disposition", "anomaly_induced_Gamma1", "flat_TT_log_Gamma1", "curvature_squared_log_Gamma1", "FV_conformized_C2_log_Gamma1", "FV_anomaly_action_Ricci_sector", "algebraic_cubic_Weyl_carriers", "third_curvature_Weyl_manifest", "CPT_universal_third_curvature_kernels", "generic_background_ghost_CPT_obstruction", "generic_ghost_Endo_Duhamel_reduction", "generic_ghost_n1_n2_Hodge_resolvent_reduction", "generic_ghost_n1_n2_vector_CPT_projection", "generic_ghost_longitudinal_Schur_resummation", "generic_ghost_Schur_Schatten_split", "generic_ghost_Schur_Wodzicki_residue", "generic_ghost_Schur_weighted_trace_scale", "round_S4_ghost_Schur_finite_weighted_traces", "round_S4_ghost_Schur_zeta_factorization", "generic_ghost_Schur_weight_raised_zeta_factorization", "generic_ghost_n3_adiabatic_carrier", "generic_ghost_n3_triangle_kernel", "generic_ghost_n3_five_carrier_projection", "generic_ghost_n3_symmetric_point_simplex_integration", "scalar_flat_K_Ricci_crosswalk", "BoxR_scheme_conversion"),
        "Classical second-order obstruction does not imply BRST disappearance, a loop interaction, a quantum constraint, BRST exactness, or ghost removal. The coefficient-bearing QME disposition is complete—strict obstructed, tau-adic compensator extension restored locally at one Euclidean loop—and the exact FV anomaly action proves structural dependence of the Ricci-scalar sector. The generic ghost n=3 triangle is projected exactly onto the scalar-flat ten-dimensional five-carrier quotient and integrated exactly at the normalized symmetric point. For n=1+n=2, the pure-vector physical sum is evaluated exactly from CPT rows 1, 3 and 14, while all longitudinal D_W towers are resummed into one normalized scalar Schur kernel. Its order-minus-two correction is in S_3; Wres(K), Wres(K^2), Wres(log S_L), and the declared order-two weighted-trace pole/scale row are exact. Both finite low-order rows, the canonical det_3 tail, the selected weighted modified determinant and the Einstein-ratio defect 5/3 are complete on the round-S4 spectral fixture. For the distinct generic weight-raised convention, the noncommuting local defect is exactly -(1/4)Wres(K^2) and specializes to -1/3. A smoothing witness proves that the generic finite values require a full primed Green kernel or spectral measure. Generic repository functions beyond the symmetric fixture, the generic physical fourth-order Hessian kernel, coefficients, parity-odd derivative manifest, finite normalizations, global Green data and complete Q1 remain open. Bridge 2 and a same-background extended classical carrier map are absent, so no interacting-BRST insertion crosswalk is certified.",
    )
...
def _guard_entries(values: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    specs = [
        ...
        ("generic_background_diff_weyl_ghost_cpt_obstruction", "generic-background effective Diff-Weyl ghost determinant: direct minimal-CPT substitution obstructed; exact Endo-Duhamel reduction supplied; n=3 projected onto the scalar-flat ten-dimensional five-carrier quotient and integrated exactly at the normalized symmetric point; the n=1+n=2 pure-vector CPT sum is exact and all longitudinal D_W towers are resummed into one normalized Schur kernel; its order-minus-two correction is in S_3, with exact Wres(K), Wres(K^2), Wres(log S_L), and the declared order-two weighted-trace pole/scale row; the round-S4 reference finite K/K2 rows, canonical det_3 tail and weighted modified determinant are complete; the selected generic weight-raised local zeta defect is exact, while a smoothing witness proves that the generic finite values require a full primed Green/spectral carrier; generic repository functions remain open", ["LOCAL-ALGEBRAIC", "EUCLIDEAN-SPECTRAL"], ("generic_background_ghost_CPT_obstruction", "generic_ghost_Endo_Duhamel_reduction", "generic_ghost_n1_n2_Hodge_resolvent_reduction", "generic_ghost_n1_n2_vector_CPT_projection", "generic_ghost_longitudinal_Schur_resummation", "generic_ghost_Schur_Schatten_split", "generic_ghost_Schur_Wodzicki_residue", "generic_ghost_Schur_weighted_trace_scale", "round_S4_ghost_Schur_finite_weighted_traces", "generic_ghost_Schur_weight_raised_zeta_factorization", "generic_ghost_n3_adiabatic_carrier", "generic_ghost_n3_triangle_kernel", "generic_ghost_n3_five_carrier_projection", "generic_ghost_n3_symmetric_point_simplex_integration", "scalar_flat_K_Ricci_crosswalk")),
...
def validate_fragment(value: dict[str, Any]) -> None:
    ...
    if len(ids) != len(set(ids)) or len(ids) != 21:
