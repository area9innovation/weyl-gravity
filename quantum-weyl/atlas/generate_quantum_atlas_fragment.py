...
def _tangent_crosswalk(values: dict[str, dict[str, Any]]) -> dict[str, Any]:
    return _entry(
        ...
        _quantum_data(
            ...
            qme=("CERTIFIED", "strict one-loop local Euclidean QME is obstructed and the tau-adic compensator-extended one-loop local Euclidean QME is restored; the raw BoxR coefficient and scheme conversion are fixed, the FV anomaly action fixes the Ricci-scalar sector, the generic ghost n=3 triangle is projected exactly onto the scalar-flat ten-dimensional five-carrier quotient, the pure-vector n=1+n=2 CPT sum is exact, and the longitudinal D_W towers are resummed into one normalized Schur kernel, while that relative determinant kernel, its possible local zeta multiplicative term, simplex-integrated repository functions and coefficients, finite normalizations and complete Q1 are underdetermined; S_L-I has pseudodifferential order -2, which does not establish ordinary trace class in four dimensions"),
            lifecycle=("NO_CERTIFIED_MAP", "the coefficient-bearing QME disposition, FV anomaly action, Ricci-sector dependence, selected FV carrier completion, algebraic C3 basis, parity-even third-curvature manifest, universal source kernels, exact n=3 scalar-flat five-carrier parametric projection, pure-vector n=1+n=2 CPT slice and longitudinal Schur resummation are complete in scope, but the normalized regularized Schur relative determinant, its local zeta multiplicative term, the generic physical fourth-order Hessian kernel, simplex-integrated repository form-factor functions and coefficients, parity-odd derivative manifest, finite normalizations, complete Q1, Bridge 2, and an extended same-background classical carrier map are absent; no ordinary Fredholm determinant is inferred from the order -2 Schur correction in four dimensions"),
        ...
        "Classical second-order obstruction does not imply BRST disappearance, a loop interaction, a quantum constraint, BRST exactness, or ghost removal. The coefficient-bearing QME disposition is complete—strict obstructed, tau-adic compensator extension restored locally at one Euclidean loop—and the exact FV anomaly action proves structural dependence of the Ricci-scalar sector. The generic ghost n=3 triangle is projected exactly onto the scalar-flat ten-dimensional five-carrier quotient. For n=1+n=2, the pure-vector physical sum is evaluated exactly from CPT rows 1, 3 and 14, while all three longitudinal D_W towers are resummed into one normalized scalar Schur kernel. The generic regularized Schur relative determinant, possible local zeta multiplicative term, simplex-integrated repository functions, generic physical fourth-order Hessian kernel, coefficients, parity-odd derivative manifest, finite normalizations, global Green data and complete Q1 remain open. Since S_L-I has pseudodifferential order -2, ordinary trace class is not established in four dimensions. Bridge 2 and a same-background extended classical carrier map are absent, so no interacting-BRST insertion crosswalk is certified.",
    )
