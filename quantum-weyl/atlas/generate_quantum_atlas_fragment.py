...
def _tangent_crosswalk(values: dict[str, dict[str, Any]]) -> dict[str, Any]:
    return _entry(
        ...
        _quantum_data(
            ...
            qme=("CERTIFIED", "strict one-loop local Euclidean QME is obstructed and the tau-adic compensator-extended one-loop local Euclidean QME is restored; the raw BoxR coefficient and scheme conversion remain fixed; the physical H1 and H2 blocks are operational on one equal-box TT fixture, where all labelled triangle corners and bubble endpoints give nonzero combined raw coefficient 15707/216, refuting universal algebraic H2 cancellation while leaving the covariant subtraction, generic finite normalizations and complete Q1 underdetermined"),
            lifecycle=("NO_CERTIFIED_MAP", "the coefficient-bearing QME disposition and the scoped raw physical H1-H2 fixture are complete, but a covariant subtraction and renormalized distribution extension, the generic primed Green/spectral carrier, repository form-factor functions and coefficients, parity-odd derivative manifest, finite normalizations, complete Q1, Bridge 2, and an extended same-background classical carrier map are absent"),
        ...
        "Classical second-order obstruction does not imply BRST disappearance, a loop interaction, a quantum constraint, BRST exactness, or ghost removal. The coefficient-bearing QME disposition is complete—strict obstructed, tau-adic compensator extension restored locally at one Euclidean loop—and the exact FV anomaly action proves structural dependence of the Ricci-scalar sector. On one rational equal-box TT fixture the physical H1 and polarized algebraic H2 rows give the nonzero combined raw logarithmic coefficient 15707/216 after all labelled triangle corners and bubble endpoints are included. This refutes a universal algebraic H2 cancellation but does not dispose the M14 relative class: a covariant subtraction and renormalized distribution extension remain open. Generic repository functions and coefficients, parity-odd derivative manifest, finite normalizations, global Green data and complete Q1 remain open. Bridge 2 and a same-background extended classical carrier map are absent, so no interacting-BRST insertion crosswalk is certified.",
    )
...
def _guard_entries(values: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    specs = [
        ...
        ("generic_background_physical_hessian_n3_fixture", "same-gauge rank-nine physical H1 with formal-adjoint completion and algebraic H2 blocks; three-H1 five-carrier projection and M14 corner obstruction certified; one rational equal-box TT fixture includes all six H1-cubed orderings and all three mixed bubbles, yielding nonzero raw log coefficient 15707/216 and requiring a covariant subtraction before renormalized form-factor assembly", ["LOCAL-ALGEBRAIC", "EUCLIDEAN-SPECTRAL"], ("generic_physical_hessian_linear_curvature", "generic_physical_hessian_n3_triangle_fixture", "generic_physical_hessian_n3_five_carrier_projection", "generic_physical_hessian_n3_integration_obstruction", "generic_physical_hessian_curvature_squared", "generic_physical_hessian_mixed_H1_H2_corner_fixture")),
