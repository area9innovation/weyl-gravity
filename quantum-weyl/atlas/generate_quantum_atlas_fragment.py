...
def _tangent_crosswalk(values: dict[str, dict[str, Any]]) -> dict[str, Any]:
    return _entry(
        ...
        _quantum_data(
            ...
            qme=("CERTIFIED", "strict one-loop local Euclidean QME is obstructed and the tau-adic compensator-extended one-loop local Euclidean QME is restored; the raw BoxR coefficient and scheme conversion are fixed, the FV anomaly action fixes the Ricci-scalar sector, the generic ghost n=3 triangle is projected exactly onto the scalar-flat ten-dimensional five-carrier quotient, the pure-vector n=1+n=2 CPT sum is exact, and the longitudinal Schur correction has a canonical det_3 tail with exact Wres(K), Wres(K^2), Wres(log S_L), and an exact declared order-two weighted-trace pole/scale row, while the reference-scale finite R(K) and R(K^2) rows, possible local zeta multiplicative term, simplex-integrated repository functions and coefficients, finite normalizations and complete Q1 are underdetermined"),
            lifecycle=("NO_CERTIFIED_MAP", "the coefficient-bearing QME disposition, FV anomaly action, Ricci-sector dependence, selected FV carrier completion, algebraic C3 basis, parity-even third-curvature manifest, universal source kernels, exact n=3 scalar-flat five-carrier parametric projection, pure-vector n=1+n=2 CPT slice, longitudinal Schur resummation, canonical det_3 tail, all three critical Schur residues and the declared weighted-trace pole/scale row are complete in scope, but the reference-scale finite R(K) and R(K^2) rows, local zeta multiplicative term, the generic physical fourth-order Hessian kernel, simplex-integrated repository form-factor functions and coefficients, parity-odd derivative manifest, finite normalizations, complete Q1, Bridge 2, and an extended same-background classical carrier map are absent"),
