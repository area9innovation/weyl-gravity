...
def _tangent_crosswalk(values: dict[str, dict[str, Any]]) -> dict[str, Any]:
    return _entry(
        ...
        _quantum_data(
            ...
            qme=("CERTIFIED", "strict one-loop local Euclidean QME is obstructed and the tau-adic compensator-extended one-loop local Euclidean QME is restored; the raw BoxR coefficient and scheme conversion are fixed, the FV anomaly action fixes the Ricci-scalar sector, the generic ghost n=3 triangle is projected exactly onto the scalar-flat ten-dimensional five-carrier quotient, has ten exact Delta cancellations and one direct I10 edge source, and is integrated at the normalized symmetric point; the pure-vector n=1+n=2 CPT sum is exact, and the longitudinal Schur correction has exact Wres(K), Wres(K^2), Wres(log S_L), an exact declared order-two weighted-trace pole/scale row, exact round-S4 reference finite K/K2 rows, a rigorously enclosed round-S4 det_3 tail and weighted modified determinant, an exact Einstein-ratio defect 5/3, and an exact generic weight-raised local defect -(1/4)Wres(K^2) specializing to -1/3; generic relative-IBP/corner-flux and edge-bubble data, generic repository functions and coefficients, finite normalizations and complete Q1 are underdetermined"),
            lifecycle=("NO_CERTIFIED_MAP", "the coefficient-bearing QME disposition, FV anomaly action, Ricci-sector dependence, selected FV carrier completion, algebraic C3 basis, parity-even third-curvature manifest, universal source kernels, exact n=3 scalar-flat five-carrier parametric projection, generic barycentric factorization and symmetric-point integration, pure-vector n=1+n=2 CPT slice, longitudinal Schur resummation, all three critical Schur residues, the declared weighted-trace pole/scale row, complete round-S4 modified-determinant benchmark, exact round-S4 Einstein-ratio factorization defect, and exact selected generic weight-raised local factorization defect are complete in scope, but relative-IBP primitives and their corner/edge disposition, the generic primed Green/spectral carrier, the generic physical fourth-order Hessian kernel, generic repository form-factor functions and coefficients, parity-odd derivative manifest, finite normalizations, complete Q1, Bridge 2, and an extended same-background classical carrier map are absent"),
        ...
        "Classical second-order obstruction does not imply BRST disappearance, a loop interaction, a quantum constraint, BRST exactness, or ghost removal. The coefficient-bearing QME disposition is complete—strict obstructed, tau-adic compensator extension restored locally at one Euclidean loop—and the exact FV anomaly action proves structural dependence of the Ricci-scalar sector. The generic ghost n=3 triangle is projected exactly onto the scalar-flat ten-dimensional five-carrier quotient; ten numerators cancel one Delta, only I10 has a nonzero direct open-edge restriction, the I28 relation holds pointwise, and all rows are integrated exactly at the normalized symmetric point. Relative-IBP primitives, punctured-corner flux and the I10 edge-bubble disposition remain open. For n=1+n=2, the pure-vector physical sum is evaluated exactly from CPT rows 1, 3 and 14, while all longitudinal D_W towers are resummed into one normalized scalar Schur kernel. Its order-minus-two correction is in S_3; Wres(K), Wres(K^2), Wres(log S_L), and the declared order-two weighted-trace pole/scale row are exact. Both finite low-order rows, the canonical det_3 tail, the selected weighted modified determinant and the Einstein-ratio defect 5/3 are complete on the round-S4 spectral fixture. For the distinct generic weight-raised convention, the noncommuting local defect is exactly -(1/4)Wres(K^2) and specializes to -1/3. A smoothing witness proves that the generic finite values require a full primed Green kernel or spectral measure. Generic repository functions and coefficients beyond the certified barycentric factorization and symmetric fixture, the generic physical fourth-order Hessian kernel, parity-odd derivative manifest, finite normalizations, global Green data and complete Q1 remain open. Bridge 2 and a same-background extended classical carrier map are absent, so no interacting-BRST insertion crosswalk is certified.",
    )
