...
def _tangent_crosswalk(values: dict[str, dict[str, Any]]) -> dict[str, Any]:
    return _entry(
        ...
        _quantum_data(
            ...
            qme=("CERTIFIED", "strict one-loop local Euclidean QME is obstructed and the tau-adic compensator-extended one-loop local Euclidean QME is restored; the raw BoxR coefficient and scheme conversion remain fixed; the physical Hessian, all ghost n=3 rows and the pure-vector ghost n=1+n=2 slice are assembled in five carrier-labelled Mellin-MS functions; the longitudinal towers are one Schur kernel with an exact non-Einstein product spectrum and a rigorous regular-complement det3 enclosure, while its weighted finite rows, remaining BV rows, finite normalizations and complete Q1 remain underdetermined"),
            lifecycle=("NO_CERTIFIED_MAP", "the coefficient-bearing QME disposition, partial-BV five-carrier Mellin-MS representative, product Schur spectral measure and regular-complement det3 are complete, but the product weighted finite rows, arbitrary-background finite Schur rows, remaining BV rows, complete repository form factors, parity-odd derivative manifest, finite normalizations, complete Q1, Bridge 2, and an extended same-background classical carrier map are absent"),
...
def _guard_entries(values: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    specs = [
        ...
        ("product_s2_s2_schur_spectral_carrier", "non-Einstein S2(k1)xS2(k2) scalar-exact ghost spectral carrier with exact anisotropic Schur eigenvalues and matched vector-zero/Schur-pole correction 3^-6; the regular-complement det3 on S2(1)xS2(2) has certified common prefix 0.3263039; weighted finite rows open; special-background non-particle carrier", ["LOCAL-ALGEBRAIC", "EUCLIDEAN-SPECTRAL"], ("product_S2_S2_ghost_Schur_spectral_carrier", "product_S2_S2_ghost_Schur_det3_enclosure")),
