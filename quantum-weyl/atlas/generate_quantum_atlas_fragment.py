...
def _tangent_crosswalk(values: dict[str, dict[str, Any]]) -> dict[str, Any]:
    return _entry(
        ...
        _quantum_data(
            ...
            qme=("CERTIFIED", "strict one-loop local Euclidean QME is obstructed and the tau-adic compensator-extended one-loop local Euclidean QME is restored; the raw BoxR coefficient and scheme conversion are fixed, the FV anomaly action fixes the Ricci-scalar sector, the generic ghost n=3 triangle is projected exactly onto the scalar-flat ten-dimensional five-carrier quotient, the pure-vector n=1+n=2 CPT sum is exact, and the longitudinal Schur correction has a canonical det_3 tail with exact Wres(K), Wres(K^2), Wres(log S_L), an exact declared order-two weighted-trace pole/scale row, and exact round-S4 reference finite K/K2 rows; a smoothing witness proves the generic rows require a full Green kernel or spectral measure, while the possible local zeta multiplicative term, simplex-integrated repository functions and coefficients, finite normalizations and complete Q1 are underdetermined"),
            lifecycle=("NO_CERTIFIED_MAP", "the coefficient-bearing QME disposition, FV anomaly action, Ricci-sector dependence, selected FV carrier completion, algebraic C3 basis, parity-even third-curvature manifest, universal source kernels, exact n=3 scalar-flat five-carrier parametric projection, pure-vector n=1+n=2 CPT slice, longitudinal Schur resummation, canonical det_3 tail, all three critical Schur residues, the declared weighted-trace pole/scale row and round-S4 finite benchmark are complete in scope, but the generic primed Green/spectral carrier, local zeta multiplicative term, the generic physical fourth-order Hessian kernel, simplex-integrated repository form-factor functions and coefficients, parity-odd derivative manifest, finite normalizations, complete Q1, Bridge 2, and an extended same-background classical carrier map are absent"),
        ...
        "Classical second-order obstruction does not imply BRST disappearance, a loop interaction, a quantum constraint, BRST exactness, or ghost removal. The coefficient-bearing QME disposition is complete—strict obstructed, tau-adic compensator extension restored locally at one Euclidean loop—and the exact FV anomaly action proves structural dependence of the Ricci-scalar sector. The generic ghost n=3 triangle is projected exactly onto the scalar-flat ten-dimensional five-carrier quotient. For n=1+n=2, the pure-vector physical sum is evaluated exactly from CPT rows 1, 3 and 14, while all longitudinal D_W towers are resummed into one normalized scalar Schur kernel. Its order-minus-two correction is in S_3, the canonical det_3 tail exists, Wres(K), Wres(K^2), Wres(log S_L), and the declared order-two weighted-trace pole/scale row are exact. Both finite low-order rows are exact on the round-S4 spectral fixture, and a smoothing witness proves that their generic values require a full primed Green kernel or spectral measure. The possible local zeta multiplicative term, simplex-integrated repository functions, generic physical fourth-order Hessian kernel, coefficients, parity-odd derivative manifest, finite normalizations, global Green data and complete Q1 remain open. Bridge 2 and a same-background extended classical carrier map are absent, so no interacting-BRST insertion crosswalk is certified.",
    )
