...
DEPENDENCIES = {
    ...
    "Berger_temporal_cutoff_Green_family": QROOT / "lorentzian/certificates/BERGER_TEMPORAL_CUTOFF_COMPANION_GREEN_FAMILY.json",
    ...
    "Berger_C26_support_nondefinition": QROOT / "lorentzian/certificates/BERGER_C26_BIKERNEL_SUPPORT_PROFILE_NONDEFINITION.json",
    ...
    "Berger_homogeneous_Krein_covariance_classification": QROOT / "lorentzian/certificates/BERGER_HOMOGENEOUS_KREIN_COVARIANCE_CLASSIFICATION.json",
    ...
    "strict_anomaly_zero_charge_restriction_nondefinition": QROOT / "transfer/certificates/STRICT_ANOMALY_ZERO_CHARGE_RESTRICTION_NONDEFINITION.json",
    ...
    "relative_changed_theory_QME_nondefinition": QROOT / "transfer/certificates/RELATIVE_CHANGED_THEORY_QME_NONDEFINITION.json",
    ...
    "generic_physical_hessian_triangle_six_master_coordinates": QROOT / "spectral/euclidean/certificates/GENERIC_BACKGROUND_PHYSICAL_HESSIAN_TRIANGLE_SIX_MASTER_COORDINATES.json",
    "generic_physical_hessian_triangle_relative_IBP_boundary_flux": QROOT / "spectral/euclidean/certificates/GENERIC_BACKGROUND_PHYSICAL_HESSIAN_TRIANGLE_RELATIVE_IBP_BOUNDARY_FLUX.json",
    "generic_physical_hessian_third_curvature_form_factors": QROOT / "spectral/euclidean/certificates/GENERIC_BACKGROUND_PHYSICAL_HESSIAN_THIRD_CURVATURE_FORM_FACTORS.json",
    "generic_physical_plus_ghost_n3_third_curvature_form_factors": QROOT / "spectral/euclidean/certificates/GENERIC_BACKGROUND_PHYSICAL_PLUS_GHOST_N3_THIRD_CURVATURE_FORM_FACTORS.json",
    "generic_ghost_n1_n2_vector_integrated_functions": QROOT / "spectral/euclidean/certificates/GENERIC_BACKGROUND_GHOST_N1_N2_VECTOR_INTEGRATED_FUNCTIONS.json",
    "generic_partial_BV_third_curvature_form_factors": QROOT / "spectral/euclidean/certificates/GENERIC_BACKGROUND_PARTIAL_BV_THIRD_CURVATURE_FORM_FACTORS.json",
    ...
    "product_S2_S2_ghost_Schur_spectral_carrier": QROOT / "spectral/euclidean/certificates/PRODUCT_S2_S2_GHOST_SCHUR_SPECTRAL_CARRIER.json",
    "product_S2_S2_ghost_Schur_det3_enclosure": QROOT / "spectral/euclidean/certificates/PRODUCT_S2_S2_GHOST_SCHUR_DET3_ENCLOSURE.json",
    "product_S2_S2_ghost_Schur_weighted_rows": QROOT / "spectral/euclidean/certificates/PRODUCT_S2_S2_GHOST_SCHUR_WEIGHTED_ROWS.json",
    "product_S2_S2_ghost_minimal_vector_determinant": QROOT / "spectral/euclidean/certificates/PRODUCT_S2_S2_GHOST_MINIMAL_VECTOR_DETERMINANT_PRECERTIFICATE.json",
    "product_S2_S2_full_BV_join_boundary": QROOT / "spectral/euclidean/certificates/PRODUCT_S2_S2_FULL_BV_JOIN_BOUNDARY.json",
...
def _validate_inputs(values: dict[str, dict[str, Any]]) -> None:
    ...
    cutoff_family = values["Berger_temporal_cutoff_Green_family"]
    cutoff_microlocal = values["Berger_cutoff_microlocal_response"]
    ...
    c26_support = values["Berger_C26_support_nondefinition"]
    ...
    berger_covariance_classification = values[
    ...
    zero_charge_restriction = values[
    ...
    relative_changed_qme = values["relative_changed_theory_QME_nondefinition"]
    ...
    if (
        ...
        != "NOT_SUPPLIED"
        or cutoff_family.get("claim_flags", {}).get(
            "BERGER_TEMPORAL_CUTOFF_COMPANION_GREEN_FAMILY"
        ) is not True
        or cutoff_family.get("claim_flags", {}).get(
            "BERGER_CUTOFF_COMPANION_WAVEFRONT_THEOREM"
        ) is not False
        or cutoff_family.get("claim_flags", {}).get(
            "BERGER_COMPANION_HADAMARD_TWO_POINT_FUNCTION"
        ) is not False
        or cutoff_microlocal.get("claim_flags", {}).get(
        ...
        or cutoff_dilation.get("claim_flags", {}).get(
        ...
        or cutoff_orientation_reduction.get("claim_flags", {}).get(
        ...
        or free_dilation_seed.get("claim_flags", {}).get(
        ...
        or full_dilation_covariance.get("claim_flags", {}).get(
        ...
        or regular_graph_endpoint.get("claim_flags", {}).get(
        ...
        or c26_support.get("first_obstruction", {}).get("obstruction_id")
        != "MISSING_NORMALIZED_SERIALIZED_H26_REPRESENTATIVE"
        or c26_support.get("claim_flags", {}).get("C26_SMOOTH") is not True
        or c26_support.get("claim_flags", {}).get("C26_SERIALIZED") is not False
        or c26_support.get("claim_flags", {}).get(
        ...
        is not False
        or c26_support.get("support_profile", {}).get("x_past_compact")
        != "UNDEFINED_NO_NORMALIZED_SERIALIZED_H26_REPRESENTATIVE"
        ...
        != "OBSTRUCTED"
        or berger_covariance_classification.get(
            "stationary_covariance_classification", {}
        ).get("complete_affine_parameter_dimension")
        != 128
        or berger_covariance_classification.get(
            "stationary_covariance_classification", {}
        ).get("positive_symmetric_cone", {}).get(
            "minimum_forced_radical_dimension"
        )
        != 26
        or berger_covariance_classification.get(
            "stationary_covariance_classification", {}
        ).get("positive_normalized_CCR_covariance", {}).get("status")
        != "EMPTY"
        or berger_covariance_classification.get("claim_flags", {}).get(
            "HOMOGENEOUS_CANONICAL_KREIN_CCR_COVARIANCE_EXISTS"
        )
        is not True
        or berger_covariance_classification.get("claim_flags", {}).get(
            "HOMOGENEOUS_NONSTATIONARY_POSITIVE_COVARIANCE_EXISTS"
        )
        is not True
        or berger_covariance_classification.get("claim_flags", {}).get(
            "FULL_BV_HADAMARD_STATE"
        )
        is not False
    ):
    ...
    if (
        relative_cyclic_pushforward.get("verdict", {}).get("classification")
        != "OBSTRUCTED"
        or relative_cyclic_pushforward.get("claim_flags", {}).get(
            "ACTION_COMPATIBLE_CYCLIC_PUSHFORWARD_OBSTRUCTED_GENERICALLY"
        )
        is not True
        or relative_cyclic_pushforward.get("claim_flags", {}).get(
            "CANONICAL_316_UNARY_CYCLIC_CARRIER_RETAINED"
        )
        is not True
        or relative_cyclic_pushforward.get("claim_flags", {}).get(
    ...
    if (
        relative_changed_qme.get("claim_flags", {}).get(
            "REDUCED_CYCLIC_REPAIR_ORBITS_CLASSIFIED"
        )
        is not True
        or relative_changed_qme.get("claim_flags", {}).get(
        ...
        is not False
        or relative_changed_qme.get("orbit_invariance", {}).get(
            "relative_defect_invariant_across_orbits"
        )
        != "NOT_DEFINED"
    ):
        raise ValueError("relative changed-theory QME boundary drifted")
    if (
        matter_lattice.get("claim_flags", {}).get(
            "HEALTHY_NONNEGATIVE_CANCELLATION_EXISTS"
        )
        is not False
        or matter_lattice.get("claim_flags", {}).get(
            "COMPLETE_SIGNED_STANDARD_SPECIES_LATTICE_CLASSIFIED"
        )
        is not True
        or matter_lattice.get("healthy_nonnegative_classification", {}).get(
            "unbounded_nonnegative_integer_lattice"
        )
        != "EMPTY"
        or matter_lattice.get("claim_flags", {}).get(
            "COMPENSATOR_IS_STRICT_CANCELLATION"
        )
        is not False
    ):
        raise ValueError("matter-selection lattice boundary drifted")
    if (
        matter_gauge.get("claim_flags", {}).get(
            "JOINT_HEALTHY_WEYL_GAUGE_SOLUTION_EXISTS"
        )
        is not False
        or matter_gauge.get("projection_theorem", {}).get(
            "joint_solution_set"
        )
        != "EMPTY"
        or matter_gauge.get("claim_flags", {}).get(
            "GUT_STANDARD_MODEL_OR_PARTICLE_SELECTION_CLAIM"
        )
        is not False
    ):
        raise ValueError("matter/gauge projection boundary drifted")
    if (
        paneitz_column.get("claim_flags", {}).get(
            "PANEITZ_FULL_FOUR_COORDINATE_COLUMN_VERIFIED"
        )
        is not True
        or paneitz_column.get("claim_flags", {}).get(
            "PROJECTED_NONNEGATIVE_CANCELLATION_EXISTS"
        )
        is not True
        or paneitz_column.get("claim_flags", {}).get(
            "HEALTHY_CANCELLATION_EXISTS"
        )
        is not False
        or paneitz_column.get("claim_flags", {}).get(
            "HIGHER_DERIVATIVE_GAUGE_COLUMN_VERIFIED"
        )
        is not False
    ):
    ...
    if (
        ...
        is not False
        or tau_all_loop.get("result_state")
        != "CONDITIONAL_FORMAL_ALL_LOOP_LOCAL_QME_RESTORATION_THEOREM"
        or tau_all_loop.get("quantum_action_principle", {}).get("status")
        != "DECLARED_HYPOTHESIS_NOT_CONSTRUCTED_REGULATOR"
        or tau_all_loop.get("lifecycle", {}).get(
            "tau_adic_all_loop_formal_local"
        )
        != "CONDITIONAL_QME_RESTORED_UNDER_DECLARED_QAP"
        or tau_all_loop.get("lifecycle", {}).get(
            "constructed_all_loop_regulator"
        )
        != "OPEN"
        or any(tau_all_loop.get("claim_flags", {}).values())
        ...
        or any(
            row.get("status") != "UNDEFINED_MISSING_CHAIN_MAP"
            or row.get("background_evaluation_used_as_pullback") is not False
            for row in zero_charge_restriction.get(
                "pullback_dispositions", []
            )
        )
        or zero_charge_restriction.get("claim_flags", {}).get(
            "CYLINDER_RESTRICTED_ANOMALY_FREE"
        )
        is not False
        or zero_charge_restriction.get("claim_flags", {}).get(
            "BERGER_RESTRICTED_ANOMALY_FREE"
        )
        is not False
        or cartan_disposition.get("result_state")
        != "COEFFICIENT_SOURCE_DISPOSED_ALL_DECLARED_CARTAN_ROWS_UNDEFINED_ANALYTICALLY_TYPED_REQUESTS_FILED"
        or {
            row.get("classification")
            for row in cartan_disposition.get(
                "theory_setting_dispositions", []
            )
        }
        != {"UNDEFINED_ANALYTICALLY"}
        or cartan_disposition.get("claim_flags", {}).get(
            "RAW_D_IDENTIFIED_WITH_K_BERGER"
        )
        is not False
        or cartan_disposition.get("claim_flags", {}).get(
            "WZ_COMPENSATOR_IDENTIFIED_WITH_BERGER_CLOCK"
        )
        is not False
        or cartan_disposition.get("claim_flags", {}).get(
        ...
        is not False
        or len(physical_hessian_triangle_boundary_flux.get("channel_rows", []))
        != 11
...
def _berger_gap(values: dict[str, dict[str, Any]]) -> dict[str, Any]:
    return _entry(
        ...
        _quantum_data(
            ...
            complex_structure=("OBSTRUCTED", "on the exact positive homogeneous fixture, simple real A104 eigenlines obstruct every real stationary full-carrier complex structure commuting with the evolution; the complete parity-forgotten homogeneous covariance calculation now confirms that the stationary positive normalized CCR class is empty"),
            ...
            state_space=("OBSTRUCTED", "physical positivity is undefined before BRST descent; on the parity-forgotten rank-80 homogeneous metric-antifield diagnostic, the stationary CCR affine class has dimension 128, every invariant positive symmetric part has radical at least 26, and no normalized stationary covariance is positive; W_K has Krein inertia (40,40), while a positive pure covariance exists only with explicit time dependence"),
        ...
        _evidence(values, "Berger_causal_chain", "Berger_Hadamard_gate", "Berger_Hadamard_regular_morphism_boundary", "Berger_temporal_cutoff_Green_family", "Berger_cutoff_microlocal_response", "Berger_cutoff_Hermitian_dilation", "Berger_cutoff_Volterra_orientation_reduction", "Berger_cutoff_Volterra_normal_convergence", "Berger_free_dilation_Hadamard_seed", "Berger_free_dilation_Krein_covariance", "Berger_full_dilation_Krein_covariance", "Berger_dilation_retained26_restriction_audit", "Berger_regular_graph_endpoint_descent", "Berger_ghost_identity_Hadamard_pair", "Berger_retained26_Hadamard_Ward_reduction", "Berger_C26_support_nondefinition", "Berger_physical_cohomology_positivity", "Berger_A104_complete", "Berger_graph_q_obstruction", "Berger_stationary_normalization_obstruction", "Berger_homogeneous_Krein_covariance_classification", "Slavnov_preflight", "regulated_Slavnov_breaking"),
        "All 26 retained endpoint rows have global Hadamard carriers at the existence-theorem level. The cyclic backward witness gives an exact-CCR candidate Omega26=W26 H26, and its Ward defect is the smooth kernel W26[H26,q26]. The endpoint artifacts do not serialize one normalized content-addressed H26 representative or mode table. Therefore C26 x/y support and pairing-null statuses are undefined, not false, and the classical one-sided homotopies cannot yet be applied. Independently, the exact positive homogeneous A104 fixture contains simple real growth eigenlines. On its parity-forgotten rank-80 metric-antifield block the action-derived commutator and Krein forms give a complete 128-parameter stationary CCR affine class: no element is positive, the canonical Krein element has inertia (40,40), and a positive pure alternative exists only with explicit time dependence. This finite matrix result has no wavefront/Hadamard condition and does not show that the forced 26-dimensional stationary radical survives a corrected BRST quotient. The downstream omega54=iota omega26 pi lift remains conditional. No retained-26 BRST covariance, positive physical state, unavoidable physical Krein sector, particle, renormalized Lorentzian product or Lorentzian QME is inferred.",
    )
...
def _guard_entries(values: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    specs = [
        ...
        ("generic_background_physical_hessian_n3_fixture", "same-gauge rank-nine physical H1/H2 blocks with formal-adjoint completion; the eleven integrated triangle channels, finite contacts and combined scale rows, all ghost n=3 rows and the exactly integrated pure-vector ghost n=1+n=2 slice are assembled into a partial-BV five-carrier Mellin-MS representative in the exact ten-dimensional quotient; exactly three longitudinal/mixed D_W carriers, independent finite C2 normalization and remaining BV rows keep the complete repository form factors open", ["LOCAL-ALGEBRAIC", "EUCLIDEAN-SPECTRAL"], ("generic_physical_hessian_linear_curvature", "generic_physical_hessian_n3_triangle_fixture", "generic_physical_hessian_n3_five_carrier_projection", "generic_physical_hessian_n3_integration_obstruction", "generic_physical_hessian_curvature_squared", "generic_physical_hessian_mixed_H1_H2_corner_fixture", "generic_physical_hessian_mellin_subtraction_scale_row", "generic_physical_hessian_covariant_Volterra_carrier", "generic_physical_hessian_H1_H2_contact_residue_projection", "generic_physical_hessian_symmetric_mixed_boundary_incidence", "generic_physical_hessian_triangle_corner_residues", "generic_physical_hessian_full_boundary_incidence", "generic_physical_hessian_H1_H2_contact_finite_rows", "generic_physical_hessian_triangle_master_completeness", "generic_physical_hessian_triangle_renormalized_master_values", "generic_physical_hessian_triangle_six_master_coordinates", "generic_physical_hessian_triangle_relative_IBP_boundary_flux", "generic_physical_hessian_third_curvature_form_factors", "generic_physical_plus_ghost_n3_third_curvature_form_factors", "generic_ghost_n1_n2_vector_integrated_functions", "generic_partial_BV_third_curvature_form_factors")),
        ("generic_background_diff_weyl_ghost_cpt_obstruction", "generic-background effective Diff-Weyl ghost determinant: direct minimal-CPT substitution obstructed; exact Endo-Duhamel reduction supplied; all n=3 rows and the pure-vector n=1+n=2 slice are integrated into the common triangle/two-log/rational basis with no new transcendental master; the longitudinal/mixed D_W towers are resummed into one normalized Schur kernel; its order-minus-two correction is in S_3, with exact Wres(K), Wres(K^2), Wres(log S_L), and the declared order-two weighted-trace pole/scale row; the round-S4 reference finite K/K2 rows, canonical det_3 tail and weighted modified determinant are complete; the selected S2(1)xS2(2) vector-plus-Schur determinant with matched zero-pole correction 3^-6 is coefficient-computed, but the same-background physical BV carrier is absent and its join to the round-S4 full-BV ledger is rejected", ["LOCAL-ALGEBRAIC", "EUCLIDEAN-SPECTRAL"], ("generic_background_ghost_CPT_obstruction", "generic_ghost_Endo_Duhamel_reduction", "generic_ghost_n1_n2_Hodge_resolvent_reduction", "generic_ghost_n1_n2_vector_CPT_projection", "generic_ghost_n1_n2_vector_integrated_functions", "generic_partial_BV_third_curvature_form_factors", "generic_ghost_longitudinal_Schur_resummation", "generic_ghost_Schur_Schatten_split", "generic_ghost_Schur_Wodzicki_residue", "generic_ghost_Schur_weighted_trace_scale", "round_S4_ghost_Schur_finite_weighted_traces", "product_S2_S2_ghost_Schur_spectral_carrier", "product_S2_S2_ghost_Schur_det3_enclosure", "product_S2_S2_ghost_Schur_weighted_rows", "product_S2_S2_ghost_minimal_vector_determinant", "product_S2_S2_full_BV_join_boundary", "generic_ghost_Schur_weight_raised_zeta_factorization", "generic_ghost_n3_adiabatic_carrier", "generic_ghost_n3_triangle_kernel", "generic_ghost_n3_five_carrier_projection", "generic_ghost_n3_barycentric_factorization", "generic_ghost_n3_symmetric_point_simplex_integration", "scalar_flat_K_Ricci_crosswalk")),
        ...
        ("product_s2_s2_schur_spectral_carrier", "non-Einstein S2(k1)xS2(k2) scalar-exact ghost spectral carrier with exact anisotropic Schur eigenvalues and matched vector-zero/Schur-pole correction 3^-6; on S2(1)xS2(2) the selected vector-plus-Schur determinant is coefficient-computed; the same-background metric/BV measure carrier is absent and cross-background round-S4 insertion is rejected; special-background non-particle carrier", ["LOCAL-ALGEBRAIC", "EUCLIDEAN-SPECTRAL"], ("product_S2_S2_ghost_Schur_spectral_carrier", "product_S2_S2_ghost_Schur_det3_enclosure", "product_S2_S2_ghost_Schur_weighted_rows", "product_S2_S2_ghost_minimal_vector_determinant", "product_S2_S2_full_BV_join_boundary")),
