...
Thus `ell_2(ghost,matter)` and the universal `ell_2(ghost,ghost)` are now
computed and exactly normalized.  This block is chirality diagonal and
closes as the strict residual CE differential.  It is not the missing
`ell_2(matter,matter)` generated by the nonlinear Bach/BV rows.
