#!/usr/bin/env python3
"""Emit or check the exact Berger 54-row q2 replay-engine receipt."""
...
try:
    from .berger_54_row_q2_replay import build_replay_engine_payload
except ImportError:
    from berger_54_row_q2_replay import build_replay_engine_payload


ROOT = Path(__file__).resolve().parent
OUTPUT = ROOT / "certificates" / "BERGER_54_ROW_Q2_REPLAY_ENGINE.json"
...
def build_certificate() -> dict[str, Any]:
    result = dict(build_replay_engine_payload())
    paths = (
...
def main() -> int:
    ...
    if args.check and (
        ...
        raise SystemExit(f"stale Berger q2 replay-engine certificate: {OUTPUT}")
    if not args.emit and not args.check:
        print(content, end="")
    else:
        print("BERGER 54-ROW Q2 REPLAY ENGINE: READY; SCIENTIFIC Q2 INPUT BLOCKED")
    return 0
