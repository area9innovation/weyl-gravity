...
def build(*, run_scientific: bool) -> dict:
    ...
    if (
        ...
        or replay.get("classical_commit") != "41c58d2086d77158ab442841d00588d7b560cbd8"
    ...
    result = {
        ...
        "claim_boundary": (
            "This LOCAL-ALGEBRAIC classical-import acceptance pins the retained typed mixed gravity-Maxwell ell3 at classical commit 41c58d2086d77158ab442841d00588d7b560cbd8. Without importing or executing the classical producer, the quantum-side Q(sqrt(10)) PBW backend parses all 59,598 coefficients of the full mixed q3, reconstructs the explicit typed 64-to-36 contraction, and matches all 25,950 retained contact coefficients exactly. It independently reconstructs the 96 gravity and 12 mixed second-inclusion coefficients and verifies their supports. Contrary to the producer receipt's support-only explanation, the gravity-outer/mixed-inner channel has 144 raw outer/inner coefficient pairs, producing 324 signed unshuffle contributions and 342 canonical full-complex PBW coefficients. None reaches a full output row supported by the retained projection, so that channel vanishes after exact retained output projection; the other two exchange channels have no raw coefficient pairs. Thus all three retained q2 S q2 exchange sectors are exactly zero, but not all for the reason stated by the producer. The relative retained arity-three identity closes on all 36 rows, and a one-coefficient retained-ell3 mutation creates two exact defects. The retained interaction has 7,614 gravity-output terms with two Maxwell inputs and 18,336 Maxwell-output terms with one Maxwell input. This proves a nontrivial retained gravity-light interaction but does not project onto Einstein-like, extra-Weyl or topological residual branches. Cyclicity is imported from the typed cyclic transfer theorem rather than independently replayed here. No unary kinetic operator changes, so no negative physical direction is introduced by this bracket; this is not a unitarity theorem. It does not compute a QME correction, restore a QME, construct renormalized Lorentzian products, certify a particle interpretation or make a quantum claim."
        ),
        "consumer_provenance": {
