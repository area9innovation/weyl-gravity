...
BERGER_SUPPORT_LOCAL_Q2_IMPORT_PATH = (
    TRANSFER_ROOT / "certificates" / "BERGER_SUPPORT_LOCAL_Q2_IMPORT.json"
)
...
def _source_manifest() -> dict[str, str]:
    paths = (
        ...
        "../lorentzian/metric_equal_connection_factor_screen.py",
...
def build_certificate() -> dict[str, Any]:
    ...
    berger_support_local_q2_import = json.loads(
        BERGER_SUPPORT_LOCAL_Q2_IMPORT_PATH.read_text(encoding="utf-8")
    )
    ...
    if (
        berger_54_row_q2_arrival.get("schema")
        != "quantum-weyl-berger-54-row-q2-arrival-readiness-v1"
        or berger_54_row_q2_arrival.get("result_state")
        != "PORTABLE_CONSUMER_AND_MUTATION_RAIL_READY_CLASSICAL_Q2_INPUT_BLOCKED"
        or berger_54_row_q2_arrival.get("input_gate", {}).get("status")
        != "INPUT_BLOCKED"
        or berger_54_row_q2_arrival.get("claim_flags", {}).get(
            "ARRIVAL_ADAPTER_READY"
        )
        is not True
        or berger_54_row_q2_arrival.get("claim_flags", {}).get(
        ...
        is not False
        or berger_54_row_q2_arrival.get("claim_flags", {}).get(
            "ARITY_TWO_IDENTITIES_INDEPENDENTLY_REPLAYED"
        )
        is not False
        or berger_54_row_q2_arrival.get("generality_assessment", {}).get(
            "promotion_to_G2_authorized"
        )
        is not False
    ):
    ...
    if (
        berger_support_local_q2_import.get("schema")
        != "quantum-weyl-berger-support-local-q2-import-v1"
        or berger_support_local_q2_import.get("result_state")
        != "COMPLETE_SUPPORT_LOCAL_Q2_IMPORTED_SCIENTIFIC_REPLAY_PENDING"
        or berger_support_local_q2_import.get("claim_flags", {}).get(
            "CLASSICAL_SUPPORT_LOCAL_Q2_IMPORTED"
        )
        is not True
        or berger_support_local_q2_import.get("claim_flags", {}).get(
        ...
        raise ValueError("Berger support-local q2 scientific import drifted")
    if (
        berger_support_local_q2_scientific_replay.get("schema")
        != "quantum-weyl-berger-support-local-q2-scientific-replay-v1"
        or berger_support_local_q2_scientific_replay.get("result_state")
        != "COMPLETE_SUPPORT_LOCAL_Q2_IMPORTED_IDENTITIES_INDEPENDENTLY_REPLAYED_TRANSFER_PENDING"
        or berger_support_local_q2_scientific_replay.get("replay", {}).get(
            "all_identities_pass"
        )
        is not True
        or berger_support_local_q2_scientific_replay.get("claim_flags", {}).get(
            "SCIENTIFIC_ARITY_TWO_IDENTITIES_INDEPENDENTLY_REPLAYED"
        )
        is not True
        or berger_support_local_q2_scientific_replay.get("claim_flags", {}).get(
    ...
    if (
        berger_metric_factor_screen.get("result_state")
        != "LOWER_BY_TWO_AND_METRIC_CONE_NO_GO_IMPORTED_HYBRID_RETAINED_ROUTE_REQUIRED"
        or berger_metric_factor_screen.get("claim_flags", {}).get(
            "BERGER_METRIC_LOWER_BY_TWO_BIWAVE_IMPORTED"
        ) is not True
        or berger_metric_factor_screen.get("claim_flags", {}).get(
            "BERGER_RAW_ENDPOINT_METRIC_CONE_NO_GO_IMPORTED"
        ) is not True
        or berger_metric_factor_screen.get("screen", {})
        .get("normalized_dual_witness", {})
        .get("value")
        != "1"
        or berger_metric_factor_screen.get("claim_flags", {}).get(
        ...
        or berger_metric_factor_screen.get("claim_flags", {}).get("QUANTUM_CLAIM") is not False
    ):
    ...
    if (
        ...
        != "PARITY_PAIR_EXACT_SETTING_DEFECT_NORMALIZATION_GATES_READY_Q2_BLOCKED"
        ...
        != [-2, -2, 2]
        or einstein_mhv_fixture.get("parity_conjugate_fixture", {}).get(
            "stripped_value"
        )
        != "1"
        or einstein_mhv_fixture.get("parity_conjugate_fixture", {}).get(
        ...
        != [2, 2, -2]
        or einstein_mhv_fixture.get("setting_compatibility_contract", {}).get(
            "berger_input_compatible_with_flat_einstein_fixture"
        )
        is not False
        or einstein_mhv_fixture.get("einstein_defect_tangency_contract", {}).get(
        ...
        is not False
        or einstein_mhv_fixture.get("normalization_contract", {}).get(
            "overall_coefficient_match_authorized"
        )
        is not False
    ...
    for export_id in REQUIRED_EXPORTS:
        ...
        if item["status"] != "AVAILABLE":
            ...
            if export_id == "local_D_action_on_bv_generators":
                continue
            if export_id == "support_local_classical_bv_q2":
                continue
            if export_id in {
                "classical_inclusion_iota_cl",
                "classical_projection_pi_cl",
    ...
    return {
        ...
        "provenance": {
            ...
            "berger_54_row_q2_arrival_readiness_certificate": "quantum-weyl/transfer/certificates/BERGER_54_ROW_Q2_ARRIVAL_READINESS.json",
            ...
            "berger_support_local_q2_import_certificate": "quantum-weyl/transfer/certificates/BERGER_SUPPORT_LOCAL_Q2_IMPORT.json",
            "berger_support_local_q2_import_sha256": _sha256(
                BERGER_SUPPORT_LOCAL_Q2_IMPORT_PATH
            ),
            "berger_support_local_q2_scientific_replay_certificate": "quantum-weyl/transfer/certificates/BERGER_SUPPORT_LOCAL_Q2_SCIENTIFIC_REPLAY.json",
            ...
            "berger_metric_equal_connection_factor_screen_certificate": "quantum-weyl/lorentzian/certificates/BERGER_METRIC_EQUAL_CONNECTION_FACTOR_SCREEN.json",
