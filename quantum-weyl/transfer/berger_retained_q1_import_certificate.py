#!/usr/bin/env python3
"""Emit or check the retained Berger minimal-q1 evidence import."""
...
try:
    from .berger_retained_q1_import import build_import
except ImportError:
    from berger_retained_q1_import import build_import


TRANSFER_ROOT = Path(__file__).resolve().parent
OUTPUT_PATH = TRANSFER_ROOT / "certificates" / "BERGER_RETAINED_MINIMAL_Q1_IMPORT.json"
...
def build_certificate() -> dict[str, Any]:
    ...
    certificate["provenance"]["schema"] = (
        "quantum-weyl/transfer/schema/berger-retained-minimal-q1-import-v1.schema.json"
    )
    return certificate
...
def main() -> int:
    ...
    if args.check and (
        ...
        raise SystemExit(f"Berger retained-q1 import is stale: {OUTPUT_PATH}")
    if not args.emit and not args.check:
        print(content, end="")
    else:
        print("BERGER RETAINED Q1: 26 ROWS IMPORTED, ND2 INPUT INCOMPLETE")
    return 0
