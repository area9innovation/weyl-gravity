#!/usr/bin/env python3
"""Emit the pinned all-weight Berger arity-two Cartan quantum verdict."""
...
try:
    from .berger_all_weight_cartan_import import build_import
except ImportError:
    from berger_all_weight_cartan_import import build_import


TRANSFER_ROOT = Path(__file__).resolve().parent
OUTPUT_PATH = TRANSFER_ROOT / "certificates" / "BERGER_ALL_WEIGHT_ARITY_TWO_CARTAN_IMPORT.json"
...
def build_certificate() -> dict[str, Any]:
    ...
    paths = (
        ...
        "../reports/berger-all-weight-arity-two-cartan-import.md",
    )
...
def main() -> int:
    ...
    if args.check and (not OUTPUT_PATH.exists() or OUTPUT_PATH.read_text(encoding="utf-8") != content):
        raise SystemExit(f"Berger all-weight Cartan import is stale: {OUTPUT_PATH}")
    if not args.emit and not args.check:
        print(content, end="")
    else:
        print("BERGER ALL-WEIGHT ARITY-TWO CARTAN IMPORT: NONZERO EXACT PRIMITIVE (REDUCED-MODE)")
    return 0
