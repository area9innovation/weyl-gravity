...
| D | reduced and Euclidean coefficient ledgers | `STANDARD_VECTOR_TT_AUXILIARY_GHOST_MEASURE_S4_ZERO_MODES_AND_CONTOUR_MATCHED_REPOSITORY_TT_DICTIONARY_AUXILIARY_ROW_GLOBAL_LEDGER_REGULATOR_OPEN`; the exact standard vector `(199/30,-87/20,0)` is reduced against the complete gauge-fixed BV quotient, the odd zero is proved by a parity Ward identity, the physical TT auxiliary Schur identity is exact, the longitudinal Diff/Weyl scalar FP matrix reduces to `Delta_0-R/3`, and the nonzero-mode York/Hodge plus nonminimal quartet super-Jacobians reproduce the standard ghost factors. The action normalization, conformally-flat `C1^sharp C1` Hessian, cylinder factorization, and Nariai endpoint have been audited; the exact missing physical carrier is the round-`S4` TT operator dictionary. The four standard factors have a complete zero-mode ledger—no physical TT zeros and `10+5` conformal ghost zeros—and the standard algebraic TT auxiliary has a normalized convergent `+iR` thimble with modewise phase `+1`. Repository matching of the TT dictionary and auxiliary row, the repository-global ledger, regulator, and total row map remain open |
