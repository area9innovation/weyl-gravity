...
| D | reduced and Euclidean coefficient ledgers | `STANDARD_BACKGROUND_VECTOR_AND_TT_AUXILIARY_IDENTITY_BOUND_REPOSITORY_MATCHING_OPEN`; the exact standard vector `(199/30,-87/20,0)` is reduced against the complete gauge-fixed BV quotient, the odd zero is proved by a parity Ward identity, and the physical TT auxiliary Schur/Jacobian identity is exact; the full repository operator/measure/contour match remains uncomputed |
| E | Euclidean elliptic or generalized block-Green/Hadamard certificate | `CLASSICAL_CAUSAL_GREEN_HOMOTOPY_IMPORTED_HADAMARD_OPEN`; the 26/54-row classical causal chain and canonical covariance lift are independently imported, but no BRST-compatible global Hadamard state, renormalized product, or quantum causal construction exists |
| F | one-loop Slavnov breaking and QME status | `ASSEMBLY_PREFLIGHT_READY_ANALYTIC_MATCHING_OPEN`; the exact quotient reduction and strict analytic export receiver are ready, but the repository regulated breaking and QME disposition are `NOT_COMPUTED` |
