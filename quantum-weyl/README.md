...
| B | counterterm/anomaly bases and descent database | `MINIMAL_BV_H14_COMPLETE_NONMINIMAL_OPEN`; the covariant ghost-zero and minimal-BV ghost-one quotients both have even/odd dimensions `2/1` on the regular Bach locus; the exact small-algebra calculation eliminates independent pure-Diff and mixed Diff--Weyl classes, while the general local nonminimal/gauge-fixed contraction remains open |
