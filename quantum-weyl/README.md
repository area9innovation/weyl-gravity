...
| G | residual quantum transfer and pairing correction | `FORBIDDEN`; the frozen strict residual contraction does not include the compensator. The local Wess--Zumino Hamiltonian contribution to `Q1` is fixed, but an exact rank-two finite-counterterm bulk ambiguity and absent finite nonlocal/renormalized-product data leave complete `Q1` uncertified. Transfer reopens only after finite normalization fixes complete `Q1` and an extended classical contraction is certified |
