...
| D | reduced and Euclidean coefficient ledgers | `STANDARD_VECTOR_TT_AUXILIARY_AND_GHOST_MEASURE_MATCHED_ROUND_S4_TT_DICTIONARY_ZERO_MODES_CONTOUR_OPEN`; the exact standard vector `(199/30,-87/20,0)` is reduced against the complete gauge-fixed BV quotient, the odd zero is proved by a parity Ward identity, the physical TT auxiliary Schur identity is exact, the longitudinal Diff/Weyl scalar FP matrix reduces to `Delta_0-R/3`, and the nonzero-mode York/Hodge plus nonminimal quartet super-Jacobians reproduce the standard ghost factors. The action normalization, conformally-flat `C1^sharp C1` Hessian, cylinder factorization, and Nariai endpoint have been audited; the exact missing physical carrier is the round-`S4` TT operator dictionary, while global zero modes, auxiliary contour/phase, regulator, and total row map remain open |
