...
| F | one-loop Slavnov breaking and QME status | `OBSTRUCTED_STRICT_FIELD_CONTENT`; the regulated insertion is `(199/30)[omega C2]-(87/20)[omega E4]`, nonzero in the complete gauge-fixed quotient, with Wess--Zumino consistency and scheme/gauge decomposition certified; an exact dual-cone witness also rules out cancellation by nonnegative standard-sign free scalar, fermion and vector matter |
| G | residual quantum transfer and pairing correction | `FORBIDDEN_STRICT_FIELD_CONTENT_QME_OBSTRUCTED`; only a certified matter-cancelled or compensator-extended restored QME can reopen this gate |
