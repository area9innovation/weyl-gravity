...
class ActiveFrontierTests(unittest.TestCase):
    ...
    def test_g2_local_bv_cohomology_is_complete(self) -> None:
        ...
        self.assertTrue(
            self.payload["claim_flags"]["GENERIC_BOX_TRIANGLE_CORNER_RESIDUES_COMPUTED"]
        )
        self.assertTrue(
            self.payload["claim_flags"]["PHYSICAL_M14_NONZERO_SCALE_ROW_RENORMALIZED"]
        )
        self.assertTrue(
            self.payload["claim_flags"][
    ...
    def test_reduced_bridge4_and_full_bv_hadamard_boundaries_are_authoritative(self) -> None:
        ...
        self.assertFalse(flags["BERGER_TEMPORAL_CUTOFF_COMPANION_GREEN_FAMILY"])
