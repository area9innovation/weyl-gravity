...
angles.  The full local evolution therefore retains an unbounded-by-this-work
angular leakage sector and still has no gravitational interpretation.  On the
one-dimensional fixed-energy scalar family, an order-20 projective Fej\'er
filter with at most 38 derivatives and two finite rational packets makes that
leakage exactly smaller than $10^{-3}$; the residual-retaining rotating-wave
evolution has half-pulse absorption efficiency above $999/1000$.  This remains
reduced scalar apparatus physics and does not transfer to gravity.
\end{abstract}
...
Such a bound is now explicit on the certified fixed-energy meridian.  For
$\zeta=e^{2i\theta}$, the normalized difference of two order-20 projective
Fej\'er kernels has exact values $+1,-1$ at the target pair lines and is the
symbol of a finite local quadratic density with at most 38 transverse
derivatives.  Exact integration in $dc$ over
\[
 B_0=[-7/25,7/25],\qquad B_1=[5/13,20/29]
\]
gives complement fraction
\[
 \eta=0.000760242003438472779\ldots<10^{-3}.
\]
If $B$ is the normalized selected bright packet and $R$ the complement, the
complete coupled direction is
$v=\sqrt{1-\eta}B+\sqrt\eta R$.  Exponentiating
...
\par\noindent
therefore supplies a sub-per-mille reduced-continuum packet detector and a
half-pulse efficiency above $999/1000$, without substituting the exponential
of a compression.  It does not control the full sphere, energy thickness,
number-scattering or counter-rotating sectors, nor transfer the earlier
equal-weight relative-$q_8$ formula to these unequal-norm packets, and has no
metric or Lorentzian-causal implication.
