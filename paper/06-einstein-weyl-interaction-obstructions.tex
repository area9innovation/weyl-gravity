...
same-chart pullback supplies the missing branch.  Combining this fact with
the charge and parity certificates gives a scoped no-go theorem.  Directness
of the Laurent grading and all-order neutrality force $Q_{<0}=0$, hence the
Eq.~(19) ghost-even neutral term must be the complete pushforward.  The
nonzero rank-four $Z^{-1}[K_+,P_0]$ coefficient then contradicts ghost
evenness at order $\lambda$, and its odd norm $-7/288$ excludes nullity.
Certificate
\par\noindent
\texttt{REVERSE\_PHYSICS\_BT\_REGULAR\_COVARIANT\_}
\allowbreak\texttt{EQ19\_NO\_GO\_V1}
\par\noindent
refutes the public regular covariant Laurent--Fock realization on the
$n=1$ resonant corner.  The theorem does not descend through $Z=1$ and does
not rule out localized, doubled, singular, non-Fock or nonperturbative
completions.
