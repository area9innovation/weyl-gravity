...
map, but a physical zero is not established without the squeezed-vacuum and
...
The missing thermodynamic trace domain still forbids a physical-zero claim.
...
For the scalar orthogonal inclusive detector, higher composite amplitudes do
not enter the leading transition weight, so the complete signed kernel fixes
the physical real-collinear coefficient at zero.  This coefficient and its
proof do not transfer to gravity or complete the scalar NLO probability.
...
\texttt{LOCAL-ALGEBRAIC} and \texttt{REDUCED-MODE}.  It establishes neither a
physical zero nor any metric-BV or \texttt{LORENTZIAN-CAUSAL} statement; the
