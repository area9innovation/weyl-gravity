...
\texttt{REVERSE\_PHYSICS\_BT\_COLLINEAR\_}\allowbreak
\texttt{PROJECTOR\_TRANSPORT\_V1}
now gives an exact finite witness for the dressed alternative: projector
idempotence forces a hard normalization $-3/512$ against the three-pair
$+3/512$ Gram response.  Its asymptotic-Hamiltonian derivation and continuum
carrier are explicitly open, and gravity would still require its own
BRST-compatible construction.
...
select an ordinary axis-compatible infrared prescription.  The verified
finite scalar projector transport shows that coherent cross-multiplicity
normalization is algebraically compatible with that coefficient; it does not
show that BT dynamics generates the transport.  The verified
