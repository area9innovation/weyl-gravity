...
\[
 ...
 =2\delta(t_A-t_B)
\]
collapses to the switched tree and preserves
\(\partial_{\log\mu}T_{6,\mathrm{bb},T}
=(5/4\pi^2)T_{4,T}\).  Certificate
\par\noindent
\texttt{REVERSE\_PHYSICS\_BT\_FULLY\_REARRANGED\_BUBBLE\_}
...
and bubble-with-bridge now complete the connected finite-time auxiliary
\(T_6\) loop on this source, but their coherent value and sign, \(y_5\),
dressing, normalization, complete \(q_{10}\), metric BV--BRST transfer and
causality remain open.
On the selected pulled experiment, the auxiliary coupling
$g=\lambda^2$ removes the apparent odd amplitude order altogether.  Fully
rearranged support and the declared normal ordering give
\[
...
\par\noindent
computes this finite packet functional and proves it is common to the public
Krein and positive Hilbert Born prescriptions.  Its scale identity
$\partial_{\log\mu}q_{10}=5q_8/(2\pi^2)$ cancels the running of
$\lambda^8q_8$ through order $\lambda^{10}$.  Its packet-dependent value
and sign, the standard shift-invariant projector, general Eq.~(19), all-time
scattering, metric BV--BRST transfer and Lorentzian causality remain open.
...
sheet-averaged finite \(q_8\)--\(q_{10}\) trace.  As an off-shell
