...
Four results, each an exact symbolic computation with machine-checked
...
The conclusion is a trichotomy.  Exactly one branch survives, and on it the
question ``is \(\gamma\) universal?'' becomes ``is the scalar profile
universal across galaxies?''---a question about the matter sector rather than
about gravity.  We do not adjudicate it.  We locate it.
\end{abstract}
...
(B2) non-constant \(S\) & field-dependent & \(\gamma\) tied to the scalar profile; open \\
\bottomrule
\end{tabularx}
\end{center}
...
\emph{is the scalar profile universal across galaxies?}
\end{corollary}

That is a question about the symmetry-breaking sector rather than about
gravity, and we know of no reason internal to the gravitational theory for it
to answer yes.  This is the assumption a conformal-gravity rotation-curve fit
is standing on.
...
\item \textbf{No verdict on branch (B2).}  A non-constant scalar is not
  analysed.  Theorem~\ref{thm:vev} shows the non-constancy is load-bearing
  rather than incidental; it does not show the branch fails.
\item \textbf{No resolution of the Flanagan--Mannheim dispute}
  ...
  matter coupling.  Both positions choose branches of
  \S\ref{sec:trichotomy}, and on either side the choice is an assumption.
  We locate the disagreement; we do not settle it.
...
Theorems~\ref{thm:complete} and \ref{thm:vev} and \S\ref{sec:gauge} are
computed in \textsc{sympy}; Theorems~\ref{thm:biharm} and \ref{thm:trace} are
