...
keep both terms and succeed.  An earlier draft recorded that as an unresolved
tension; it is not one, and the resolution is already
published~\cite{OBrien2018}.  Their eq.~(7) gives
\(v^{4}=(BM/M_{\odot})(1+N^{*}/D)\) with \(B/M_{\odot}=2c^{2}G\gamma_{0}\) and
\(D=\gamma_{0}/\gamma^{*}\), so the mass-proportional piece enters exactly as
we say---and the fits sit where \(N^{*}\lesssim D\), so the correction factor
lies between \(1\) and \(2\) and the slope is near \(4\) without being
\(4\).  Our conditional and their fit agree; what we add is that the slope is
\emph{exactly} \(4\) only in the limit where the sourced piece vanishes.
\end{abstract}
...
\item \textbf{No novelty in the physics, anywhere.}  The Mannheim--Kazanas
  ...
  that fail when a claim is vacuous---not the statements themselves.
