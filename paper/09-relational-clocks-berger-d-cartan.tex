...
observer calculation adjoins the complete auxiliary Diff--BV scalar orbit
\(\alpha\int\chi^+c^ae_a\chi\).  Its 32 action-derived \(q_2\) keys close the
30-key contractible-pair covariance block uniquely at \(\alpha=1\), but the
full action remains obstructed already at arity two:
\(\tau^*\leftarrow(e_0e_1A_0,K_{0,01})\) has coefficient \(+g_0h_0\),
independent of \(\alpha\) and all twelve quartic parameters.  The scalar
orbit has no old \(A\)--\(K\) Hessian and the 636-key repair lacks the
required \(\chi^+\) output.  Moreover its cross term is zero at the previous
arity-three coordinate
\(c^{*}_{1}\leftarrow(e_0e_0e_2A_0,\chi,K_{0,12})\), so the inherited
\(-4g_0h_0\) coefficient also survives.  A direct temporal \(A\)--\(K\)
Hessian and a quartic Diff-covariance descendant are therefore required
before a nonlinear observer morphism, redshift or detector-rank theorem can
be claimed.
\end{enumerate}
