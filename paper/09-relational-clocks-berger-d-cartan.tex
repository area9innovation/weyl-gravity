...
observer calculation repairs the typed-source \(q_1,q_2\) common-action
gate, but its complete minimal quartic extension has twelve independent
Berger- and Maxwell-invariant \(q_3\) directions with identical \(q_1,q_2\).
The full arity-three replay leaves the parameter-independent exact coefficient
\(-4g_0h_0\) in the emitter Diff--BV output
\(c^{*}_{1}\leftarrow(e_0e_0e_2A_0,\chi,K_{0,12})\).  Thus the admissible
quartic locus is empty in that declared family; the missing
\((\chi,\chi^+)\) Diff--BV scalar orbit must be incorporated before a unique
nonlinear observer morphism, redshift or detector-rank theorem can be claimed.
\end{enumerate}
