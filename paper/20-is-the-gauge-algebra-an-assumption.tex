...
\title{Is the Gauge Algebra of Conformal Gravity an Assumption?}
...
algebra is a different question, and it is usually assumed rather than
computed: one exhibits the symmetries one built the action to have, and stops.

We compute it.  By Noether's second theorem gauge generators and Noether
identities are the same data, so the question becomes whether the Bach
tensor satisfies any local identity beyond \(\nabla^{a}B_{ab}=0\) and
\(g^{ab}B_{ab}=0\).  Working over a metric family carrying symbolic
...
Either way the question is decidable by computation, and it appears not to have
been asked of this action.
...
vanishes identically.  So the question of \S\ref{sec:question} becomes:
...
\item \(k=3\): at curvature degree zero the kernel is exactly the combinations
  that vanish because of the two known identities.  At curvature degree one,
  eight sub-families were enumerated over all placements and no relation
  survives in any.
\end{itemize}
\end{theorem}
...
\begin{corollary}
At derivative order up to three and the curvature degrees stated, the gauge
algebra of \(S=\alpha\int\!\sqrt{-g}\,C^{2}\) is
\(\mathrm{Diff}\ltimes\mathrm{Weyl}\), with nothing hidden.
\end{corollary}
