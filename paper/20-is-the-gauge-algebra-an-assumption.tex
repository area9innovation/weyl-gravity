...
zero or one in the gauge parameter.  Orders two and four are empty for a
reason that needs no computation at all: index parity.  Order three is
\emph{attempted and unresolved}, for a reason we state rather than bury: the
third covariant derivative of the Bach tensor is seventh order in the metric,
and at the jet degree we can afford only the lowest orders are trustworthy ---
too few to resolve the rank that the question turns on.
...
\item \(k=2\) and \(k=4\): \textbf{empty by index parity}, with no computation
  ...
  cannot be built.
\item \(k=3\): \textbf{unresolved}, and the reason is a resolution limit rather
  than a negative result.  \(\nabla^{3}B\) is seventh order in the metric, so
  at jet degree seven only the lowest outer orders carry information; how many
  is \emph{measured} against contractions that must vanish identically, and the
  answer is three (orders zero through two).  Restricted to those, the rank of
  the candidate contractions is not resolved.  What does hold at this order are
  the controls of \S\ref{sec:negatives} and the separate nonvanishing of the
  individual contractions; what does not is any statement about their
  independence, and hence about completeness.  See Remark~\ref{rem:truncation}.
\end{itemize}
\end{theorem}
...
\begin{remark}[Why order three is unresolved, and how we know]
\label{rem:truncation}
A jet of degree \(d\) supports \(d-k\) trustworthy orders after \(k\)
derivatives, and \(\nabla^{3}B\) costs seven.  Rather than trust that
budget we measure it: the contractions whose innermost operation is
\(\nabla\!\cdot\!B\) must vanish identically, and the highest order at which
they still do is a direct read of how far the data is good.  The arithmetic
predicted zero usable orders; the measurement found three.  The measurement is
what the computation uses.

An earlier version of this work concluded that the order-three kernel is
exactly the two known identities.  That conclusion came from a candidate
relation which failed when compared over the \emph{whole} jet---and its
failure lay entirely in the orders above the valid range.  Within the
trustworthy range it vanished.  The conclusion is withdrawn.  We note also that
the converse would not follow either: a kernel element need not be a
generator, since two placements can yield the same adjoint and contribute a
relation that is an artefact of the parametrisation.
\end{remark}

\begin{corollary}
At derivative orders zero and one, at the curvature degrees stated, the gauge
algebra of \(S=\alpha\int\!\sqrt{-g}\,C^{2}\) is
...
\begin{itemize}\itemsep2pt
\item \textbf{Not all orders, and not order three.}  The result covers
  derivative orders zero and one, with two and four empty by parity.  Order
  three is unresolved (Remark~\ref{rem:truncation}) and order five and above
  untouched.  Nothing here forbids a generator above the ceiling we reached.
