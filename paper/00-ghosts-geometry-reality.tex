...
A complete $54$-row
causal BV construction and action-derived $q_2,q_3$ extend that conclusion to
a cyclic causal Cartan contraction through arity three.  This is a scoped
classical Taylor theorem, not an all-orders, boundary, Hadamard, or quantum
...
proves that $D$ is linearly presymplectically null, and constructs a causal
Cartan null-homotopy through arity three on the rational representative.
\end{enumerate}
...
arbitrary-input four-dimensional $q_2$ and $q_3$, and the local $D$ derivation
admits cyclic causal Cartan primitives through arity three.  These results
...
IX & Relational Berger clock and causal $D$-Cartan contraction & Fixed-coupling compact classical sector through arity three\\
\bottomrule
\end{tabular}
...
\node[pap, fill=purple!9, below=47mm of p4] (p9)
  {\textbf{Paper IX}\\positive Berger clock\\
   causal $D$-Cartan through arity three};
\draw[arr] (p123) -- (p4);
\draw[arr] (p4) -- (p5);
\draw[arr] (p5) -- (p6);
\draw[arr] (p4) -- (p7);
...
Paper~VIII proves its covariant transport, and Paper~IX tests the $D$ quotient
with a healthy clock and the first interaction tensors.}
...
time translation is presymplectically null and Cartan-null-homotopic after a
physical clock candidate and low-order interactions are added.}
...
complex and action-derived $q_2,q_3$ extend the corresponding Cartan
contraction through arity three.  This is a scoped classical theorem, not an
...
\bibitem{paperIX}
\emph{Relational clocks and causal Cartan contraction through arity three in
pure-Weyl gravity on a Berger universe}, companion paper (Paper~IX), 2026.
