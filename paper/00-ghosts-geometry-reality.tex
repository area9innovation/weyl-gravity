...
a nine-paper programme around those three levels.  Papers~I--III classify
...
result.  The programme therefore
...
therefore has three branches after the free gravity classification:
...
and Paper~IX tests a relational-clock realization of the $D$ quotient on a
non-conformally-flat compact background.

\paragraph{Series map.}
The nine technical papers have distinct jobs.
\begin{enumerate}
...
branch, Papers~VII--VIII the state/covariant residual branch, and Paper~IX a
relational-clock validation branch.  Paper~VIII does not perform another
Chevalley--Eilenberg calculation, and Paper~IX does not recompute the residual
$H^4$ group.

A cross-programme $D$-quotient validation dossier records the scope of
this last branch.  Paper~IX is the resulting classical theorem on
...
\bottomrule
\end{tabular}
\caption{Nine-paper programme grouped by mathematical role.}
...
\node[pap, fill=green!9, below=9mm of p7] (p8)
  {\textbf{Paper VIII}\\covariant causal transport\\
   $H^4_{\rm cov}\cong H^4_{\rm res}$};
\node[pap, fill=purple!9, below=47mm of p4] (p9)
...
\draw[arr] (p4) -- (p9);
\end{tikzpicture}
\caption{Dependency structure of the nine technical papers.  The interaction,
residual and relational-clock branches share the free gravity spine but
answer different questions.  Paper~VII computes the state-side group,
Paper~VIII proves its covariant transport, and Paper~IX proves fixed-coupling
$D$-charge rigidity while testing the helical stabilizer $K$ with the first
interaction tensors.}
\label{fig:dependency}
\end{figure}

\keybox{The programme has one local free spine and three branches.
...
Cartan-null-homotopic through the certified interaction order.}
...
\keybox{The nine-paper programme classifies the local free completions,
tests their interacting stability, computes the selected pure-Weyl residual
cohomology, proves its covariant causal realization, and tests a healthy
relational clock through arity three.  Its remaining frontier is higher-order,
boundary, distributional, and quantum: alternative boundary problems,
...
Papers~I--IX
\cite{paperI,paperII,paperIII,paperIV,paperV,paperVI,paperVII,paperVIII,paperIX}.
