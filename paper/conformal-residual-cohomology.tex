...
constraint propagation and all-level $E/A/L$ spectrum are certified.  Six
curvature flags are true; the remaining nine prolongation/causal flags keep the
...
The formerly inferred order-zero identity attachment does not complete the
next square in the actual fibre-identified coordinates.  The common-Rees
calculation gives
\begin{equation}
 \rank\bigl(N_0A_0-B_0C_0\bigr)=4,
\end{equation}
with support on the three $a$ rows and the $s$ row.  This is an attachment or
filtration defect, not a Green-hyperbolicity no-go theorem.  The preceding
square $TK_{\rm aux}=0$ is independently exhaustive
on all diffeomorphism and Weyl-scalar jets and all four boost ghosts.  These
partial maps define an abstract sixteen-block cone template, but not yet a
coefficientwise nilpotent prolonged BV operator or support-local all-row SDR.
After the rank-four attachment defect is repaired, the two remaining analytic
exceptions
...
\begin{equation}\label{eq:open-curvature-flags}
 ...
 \end{aligned}
\end{equation}
Six flags in \eqref{eq:open-curvature-flags} are true: the curved equations,
first-order closure, symmetric hyperbolicity, sourced identity, constraint
propagation, and all-level spectrum.  The remaining nine prolongation,
causal, and transport flags are false.  In particular, the paper does not yet
construct the complete support-local prolonged BV rows, $G_\pm$, or degree-$(-1)$ maps
...
Because the current $B$ attachment leaves the rank-four final-square defect,
this formula is presently conditional and does not promote
\texttt{prolonged\_current\_comparison}.  The equivariance defect of the Cauchy
...
identity and all-level $E/A/L$ spectrum are now proved.  The remaining nine
false prolongation/causal/transport flags in
...
proved.  The all-row local prolongation retract and prolonged BV operator
remain open because the current identity attachment leaves a rank-four
associated-graded square defect.  The
...
conditional on repairing that attachment; its later identification with a
...
Six flags in \eqref{eq:open-curvature-flags} are true and the nine
prolongation/causal/transport flags remain false.  They require repair of the
rank-four final-square attachment, the prolonged Green
