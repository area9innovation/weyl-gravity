...
inverse chain maps throughout the centered energy buffer.  Exact
...
claimed universally.  Identification of this algebraic cyclic model with a
chosen complete gauge-fixed field-theoretic BV domain, and any analytic
completion, require additional control.  We claim neither a positive
...
dressed isometry are exact through the centered window.  The remaining
conditional step is its identification with a chosen complete gauge-fixed
field-theoretic BV domain, not another residual rank calculation.
...
quadratic chain}.  It does not yet derive a chosen gauge-fixed nonminimal
...
\subsection{One remaining field-theoretic identification}
...
\begin{conjecture}[Field-theoretic completion]\label{hyp:pure-weyl}
For the specified algebraic cylinder gauge fixing and closed-universe BFV
choice, the following four statements hold.
\begin{enumerate}
\item \emph{Gauge-fixed-domain equivalence.}  Adjoining the stated
nonminimal sector and applying the gauge fermion gives a canonical BV
transformation and explicit contraction
\begin{equation}
 C^{\rm gf,alg}_{\rm BV}\simeq
 C^{\rm min,alg}_{\rm BV}\oplus C_{\rm nonmin,triv},
\end{equation}
including every generalized auxiliary field introduced by gauge fixing.
\item \emph{Dual zero-mode identification.}  The fifteen local
conformal-Killing ghost modes and their genuine dual constraint/antifield
sector are removed and replaced by the residual BFV variables without
duplication.  The finite comparison realizes
\begin{equation}
 \mathcal Z_{\rm local}\longleftrightarrow
 \mathfrak{so}(4,2)\oplus
 \mathfrak{so}(4,2)^*[\text{dual degree}].
\end{equation}
\item \emph{Complete row inventory.}  Every remaining field or antifield
row which can enter or leave the centered filtered term $(p,q)=(4,0)$ is
represented by the certified raw complex, explicitly contractible,
eliminated as auxiliary, excluded by compact degree, or killed by a
displayed differential.
\item \emph{Pairing transfer.}  The field-theoretic cyclic BV/BFV form
induces the raw cross-energy form of
...
calculation and Proposition~\ref{prop:field-BV-dictionary} already contain
and identify the minimal detour ghost and antifield rows,
the noncompact homotopies, the cross-energy cyclic form, and the complete
centered HPL correction test.  The conjecture asks whether a concrete
gauge-fixed field-theoretic domain has further nonminimal, auxiliary,
reducibility-dual, or antifield rows or domain effects absent from that
algebraic model, and whether its cyclic form induces the certified raw one.
...
with the complete local BV complex.  The explicit closed-universe choice
\eqref{eq:closed-BFV-choice} fixes the selected ghost pairing and
normalization.
...
closed-universe residual choice, and integrated centered ranks are exact
inputs.  The conjecture above states the one remaining field-theoretic
identification without folding it into the metric-to-curvature theorem.  A
...
by exact inverse chain maps.  The remaining classical bridge is the
field-theoretic identification in Conjecture~\ref{hyp:pure-weyl}: one must
show that a concrete gauge-fixed pure-Weyl BV domain, including nonminimal
variables and both sides of the residual zero-mode split, has no additional
rows or domain effects absent from the certified algebraic cylinder model,
and that its cyclic pairing transfers to the raw form.
...
to the raw complex by exact inverse maps.  Promotion to the complete
gauge-fixed field-theoretic pure-Weyl BV/BFV domain is controlled by the
remaining identification in Conjecture~\ref{hyp:pure-weyl}.  Analytic completion,
quantum consistency, and interaction descent are separate questions.
...
vectors at energies two through five; gauge fixing and the dual global
zero-mode sector are not included in this statement.\\
...
remaining parts of Field-Theoretic Hypothesis~\ref{hyp:pure-weyl}: a
complete gauge-fixed/nonminimal row and zero-mode identification, full
equivariant cyclic transfer, and the selected residual BFV normalization.\\
