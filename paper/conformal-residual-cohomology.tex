...
ordinary-derivative auxiliary realization supplies an exact four-row
Fourier-symbol witness and $66$-to-$30$ Fourier-complex deformation retract
with support-local formulas.  The curved lower-order witness/retract and
integration-by-parts adjoint
table, its causal Green homotopies, the full covariant/Cauchy pairing
comparison, and a Hadamard theory remain open.  A direct same-bundle
...
route now supplies an exact four-row symbol witness and a support-preserving
all-row retract.  Its curved lower-order witness/adjoint table, causal
homotopies, the complete covariant/Cauchy pairing comparison,
distributional/Hadamard completion, and alternative boundary polarizations
remain separate problems.  A direct same-bundle metric factorization is an
...
\subsection{Exact ghost companion, auxiliary symbol witness, and local retract}
\label{sec:auxiliary-green-witness}

Theorem~\ref{thm:cauchy-sobolev} concerns the reduced physical branches.
The all-degree symbol statement and local BV equivalence are obtained through
an ordinary-derivative realization.  On the complete trace-free metric bundle
define
...
certified.  The local fallback instead uses the ordinary-derivative
...
$g^{\mu\nu}\zeta_\mu\zeta_\nu I_9$.  The gauge-fixed field Hessian has
principal symbol $g^{\mu\nu}\zeta_\mu\zeta_\nu J_{\rm aux}$ with a
nondegenerate $24$-dimensional fibre form.  More precisely, direct
reconstruction of the quadratic ordinary-derivative Hessian and the
modified de Donder companion gives
\begin{equation}
 E_{\rm aux}(-\zeta)^T=E_{\rm aux}(\zeta),\qquad
 E_{\rm aux}K_{\rm aux}=0,\qquad
 Y_{\rm gh}C_{\rm aux}(\zeta)
   =K_{\rm aux}(-\zeta)^TJ_{\rm aux}.
\end{equation}
The gauge-fixing density is
$-C_0\mathbin{\cdot}C_2-C_1^2/2$, and hence the exact witness field block
is
\begin{equation}
 P_{\rm field}=J_{\rm aux}^{-1}E_{\rm aux}
                 +K_{\rm aux}C_{\rm aux},qquad
 \sigma_2(P_{\rm field})
   =g^{\mu\nu}\zeta_\mu\zeta_\nu I_{24}.
\end{equation}
Thus fibre identification gives the scalar metric principal symbol required
for a normally hyperbolic system; the normalization and symbol-level adjoint
relation are fixed by the action rather than inferred from ranks or harmonic
data.  The complete curved first- and zeroth-order coefficient table and its
integration-by-parts adjoint remain a separate certificate.

The unipotent local change
\begin{equation}
 \eta=\xi_0-d\sigma,\qquad
 \widehat f=f+2d_{(a}v_{b)}
\end{equation}
isolates $Qv=-\eta$.  The remaining auxiliary equation is pointwise
invertible,
\begin{equation}
 \widehat f_{ab}=-2G^v_{ab}+\frac23g_{ab}\operatorname{tr}G^v,
\end{equation}
and substitution gives $\operatorname{Ric}^2-R^2/3$ in the $v=0$ gauge.
The changes, their inverses, and this elimination preserve support.
...
conformal cylinder has an exact $66$-to-$30$ contraction at the four-row
Fourier-complex level.  In that realization the identities
$Q^2=0$, $QW_{\rm aux}+W_{\rm aux}Q=P_{\rm aux}$ and
$W_{\rm aux}^{\sharp}=W_{\rm aux}$ are exact matrix identities, and every
diagonal block has scalar metric principal symbol.  A finite differential
change of variables and pointwise auxiliary inverse split off the complete
Stueckelberg, auxiliary, and cotangent sector, with explicit maps satisfying
$ip-1=Qk+kQ$.  Their formulas are finite differential or pointwise maps and
therefore preserve compact, spacelike-compact, and unrestricted smooth
support.  The complete curved lower-order chain-map identities remain part
of the global witness certificate.
The trace/Weyl and nonminimal doublets attach as pointwise
support-preserving contractible summands.  No local conformal-Killing
projector is used.
\end{proposition}

Once the remaining curved lower-order witness certificate is supplied,
normal hyperbolicity gives unique $G_\pm$ and the general recognition
identity gives
\begin{equation}
 \Lambda_\pm=W_{\rm aux}G_\pm,\qquad
 Q\Lambda_\pm+\Lambda_\pm Q=1.
\end{equation}
This consequence is not asserted as completed by the symbol proposition.

This implements the auxiliary option rather than claiming a direct
factorization of $H$.  The known obstruction to factorizing the
gauge-invariant spin-two conformal operator away from Einstein backgrounds
is relevant context \cite{NutmaTaronna}, but is not used as a no-go theorem
for the gauge-completed $H$.
...
Proposition~\ref{thm:auxiliary-green-witness} supplies the all-degree symbol
witness and Fourier-complex BV retract.  The curved lower-order
witness/retract/adjoint identities, their causal homotopies, the full covariant/Cauchy
pairing comparison, Hadamard completion, and
uniqueness among boundary or deparametrized polarizations remain separate
analytic questions.  The exact ranks in
...
unchanged.  The auxiliary all-degree symbol witness and support-local retract
are now exact, while the curved lower-order witness/retract and causal homotopies,
complete covariant/Cauchy pairing comparison, alternative boundary
polarizations, quantum consistency, and interaction descent are separate
questions.  At the reduced one-particle level, however,
...
vector propagation blocks.  The remaining covariant steps are the curved
auxiliary Green theorem, pairing comparison, and distributional completion.
A direct same-bundle metric factorization remains optional.
...
hyperbolic.\\
Auxiliary symbol witness and $66$-to-$30$ Fourier SDR & Exact cylinder
symbol theorem & The exact ghost biwave, scalar metric auxiliary
symbols, support-local Stueckelberg/auxiliary reduction, and symbol-level
formal adjoints are certified.\\
Curved auxiliary witness/covariant-pairing completion & Open & No complete
curved lower-order witness/adjoint table, causal Green homotopies,
covariant/Cauchy pairing identification, Hadamard theorem, group
exponentiation, or uniqueness among
boundary, clock, asymptotic, or deparametrized polarizations is established.\\
