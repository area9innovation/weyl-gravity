...
\[
 ...
 \qquad G_{\rm rep}=I_2.
\]
Here $G_{\rm rep}=I_2$ is the normalized Hermitian Gram matrix of the two residual
representatives.  The survivors are ghost-dressed vertex or deformation
...
deformation space is one-dimensional and has representative Gram matrix
$I_1$.
...
or boundary charge, the contraction is not a physical quotient.  We
separately formulate the hypotheses under which the result transfers to the
complete free strict-pure-Weyl BV complex.  Formal detour identities and
exact homogeneous-jet calculations through degrees two to six support that
bridge, but the all-level Lorentzian cylinder-kernel theorem and the full
zero-mode transfer remain open.  We claim neither a positive graviton Fock
space nor quantum anomaly freedom.
\end{abstract}
...
imposed.  The exact answer below concerns the residual reduction; the bridge
from that calculation to the complete local BV theory remains conditional.
...
chiral Weyl-square classes, and their normalized residual representative
Gram matrix is positive.  Their parity combinations are the dynamical
...
in \cite{GoverSombergSoucek}.  Cylinder harmonics, oscillator charges, and
...
normalized Hermitian representative Gram matrix is $I_2$.  The standard
...
gauging.
\item The identification of that algebraic answer with the complete free
strict-pure-Weyl local-plus-residual BV cohomology is conditional.  It
requires an all-level Lorentzian local-kernel theorem, a conformal-Killing
zero-mode split, and a compact-degree-equivariant cyclic transfer.  The
formal identities and finite exact tests are given here; their remaining
field-theoretic hypotheses are displayed explicitly rather than hidden.
...
\cite{MetsaevBRSTBV}.  These works do not supply the cylinder zero-mode
transfer used here, but they delimit what must be proved before the
conditional bridge can be promoted to a strict pure-Weyl theorem.
...
\subsection{What the local identities do not yet prove}

The middle local cohomology relevant to the free field is
\begin{equation}\label{eq:local-middle}
 \frac{\ker B_{\rm lin}}{\im K}.
\end{equation}
Equations \eqref{eq:C1K}--\eqref{eq:detour-identities} prove that this
quotient is defined, but not that it is globally isomorphic to a prescribed
normalizable cylinder module.  In particular, the implication
\begin{equation}\label{eq:local-target}
 \frac{\ker B_{\rm lin}}{\im K}
 \stackrel{?}{\cong}\W_+\oplus\W_-
\end{equation}
requires a Lorentzian domain analysis, an explicit curvature intertwiner,
and removal of all local ghost and nonminimal doublets.

An exact finite-jet calculation gives strong evidence without promoting it
to an all-level theorem.  On homogeneous Euclidean polynomial jets of
degree $n=2,\ldots,6$, the matrices for $K$, $C_1$, and $B_1$ obey
\begin{equation}
 C_1K=0,
 \qquad B_1K=0,
 \qquad \ker C_1=\im K.
...
\bottomrule
\end{tabular}
\caption{Exact homogeneous-polynomial detour ranks.  The calculation is a
finite Euclidean jet certificate, not the missing all-level Lorentzian
cylinder intertwiner.}
...
special conformal transformations.  These are exact finite results.  The
all-level version of \eqref{eq:local-target} remains the principal local
bridge stated in Section~\ref{sec:transfer}.

\subsection{A prospective Lorentzian proof route}

The missing implication is global and hyperbolic, not another formal detour
identity.  A proof must first choose its cylinder domain---for example,
smooth finite-energy solutions with a specified frequency completion---
because spacelike-compact, distributional, and unrestricted smooth solution
spaces need not have the same quotient.  The natural route is then:
\begin{enumerate}
\item gauge-fix the linear Bach complex so that its fourth-order wave
operator factors into normally hyperbolic second-order factors, while
isolating the conformal-Killing zero modes;
\item construct advanced/retarded homotopies on the nonzero-mode complex,
using the framework of Green-hyperbolic complexes
\cite{GreenHyperbolic};
\item prove that $C_1$ is injective on the resulting metric quotient and
surjective onto the chosen on-shell curvature module, with the Bianchi and
Bach identities imposed;
\item separate the fifteen global reducibility parameters rather than
burying them in local exactness, as required by the general treatment of
global reducibility modules \cite{BarnichBekaertGrigoriev}; and
\item use the character \eqref{eq:weyl-character}, BGG structure
\cite{CapBGG}, and the finite ranks above only as completeness checks.
\end{enumerate}
Ordinary-derivative BRST--BV formulations of conformal fields provide a
concrete field/antifield language for the gauge-fixed step
\cite{MetsaevBRSTBV}.  None of these references by itself proves the
Lorentzian cylinder quotient \eqref{eq:local-target}; the list records a
proof programme, not an additional premise of the exact residual theorem.
...
\end{equation}
This is the character of the on-shell Weyl-curvature module $\W_+$.
Character equality is not, by itself, a proof of the local metric detour
isomorphism \eqref{eq:local-target}.
...
The terms \emph{moment map} and \emph{Taub charge} refer to related but not
identical constructions.  Equation \eqref{eq:moment-map} is the Hamiltonian
moment map of the residual linear action on oscillator phase space.  A Taub
charge is the pairing of a quadratic field-equation source with an adjoint
zero mode; on a compact Cauchy surface it is a second-order integrability
condition for extending a linearized perturbation to a nonlinear solution
\cite{AltasTekinLinearization,AltasTekinTaub}.  The local zero-mode transfer
and the chosen boundary problem are what identify the two.  The finite
calculation leading to \eqref{eq:taub-normalization} verifies that
identification for the tested proper-conformal kernels.  It does not prove
that every residual constraint in the full strict pure-Weyl BV problem is a
Taub constraint, nor that declaring all residual charges gauge is forced.
...
\cite{HamadaBRST}.  Here it is an algebraic input.  Its derivation from the
strict pure-Weyl local BV zero-mode split is one of the hypotheses in
...
\subsection{The normalized residual representative Gram matrix}

The absolute ghost exterior algebra carries a complementary-degree pairing.
The centered ghost representative $v$ in \eqref{eq:ghost-vacuum-degree} is
normalized, using the standard residual insertion, so that
\begin{equation}\label{eq:ghost-norm}
 (v,v)_{\rm gh}=1.
\end{equation}
This fixed insertion is not a nondegenerate inner product on each individual
ghost-number block.  It is sufficient here because the incoming
...
\eqref{eq:relative-I2} gives their exact normalized residual representative
Gram matrix.  It is Hermitian and sesquilinear on the complex oscillator
module; the parity-compatible real structure gives its real restriction.
We reserve the phrase ``transferred BV cohomological pairing'' for the
cyclic-transfer statement in Section~\ref{sec:transfer}.
...
In finite compact-degree blocks, a cyclic retract exists algebraically when
$q^\sharp=-q$ and the induced cohomology pairing is nondegenerate.  One
chooses a nondegenerate representative space $\cH\subset\ker q$, an
isotropic complement to $\im q$, and defines $s$ as the inverse of $q$ on
that complement.  The remaining field-theory obligation is to make this
choice simultaneously compatible with compact degree, the cylinder domain,
and the zero-mode split.
...
\subsection{Pure-Weyl field-theoretic hypotheses}

\begin{conjecture}[Strict pure-Weyl zero-mode transfer]\label{hyp:pure-weyl}
The gauge-fixed free strict-pure-Weyl BV complex on the stated cylinder
domain has the following properties.
\begin{enumerate}
\item Its local Diff$\times$Weyl ghosts split exactly into the fifteen
conformal-Killing zero modes and a nonzero-mode complement, without double
counting either sector.
\item After nonminimal doublets are contracted,
\begin{equation}
 H^0(q_{\rm local})\cong\F(\W_+\oplus\W_-),
\end{equation}
which includes the all-level Lorentzian cylinder isomorphism
\eqref{eq:local-target}.
\item The local ghost and antifield rows which could enter or leave
$E_r^{4,0}$ in \eqref{eq:spectral-bidegree} vanish, and the filtration is
complete and convergent on the chosen function space.
\item A compact-degree-equivariant cyclic strong deformation retract exists
and satisfies \eqref{eq:D-equivariance} and \eqref{eq:cyclic-SDR}.
\item The transferred differential is the strict residual
$\mathfrak{so}(4,2)$ Chevalley--Eilenberg differential
\eqref{eq:CE-differential}; the higher operations $Q_{[k]}$ vanish or are
removed by a filtered cyclic equivalence in the centered window.
\item The zero-mode measure induces the Hamada ghost polarization with the
degree and normalization \eqref{eq:ghost-vacuum-degree} and
\eqref{eq:ghost-norm}, and the parity-compatible real structure transfers.
\end{enumerate}
\end{conjecture}

These are field-theoretic hypotheses, not consequences of the exact
residual matrices.  In particular, Items 3 and 5 contain the two mechanisms
by which the residual answer could fail to equal the full BV answer.

\begin{corollary}[Conditional strict-pure-Weyl result]\label{cor:conditional}
If Field-Theoretic Hypothesis~\ref{hyp:pure-weyl} holds, the complete free
classical local-plus-residual BV cohomology in the selected degree is
\eqref{eq:main-result}.  Its transferred cohomological Hermitian form is
$I_2$, and its local Euler--Lagrange quotient is
\eqref{eq:dynamic-quotient} with form $I_1$.
\end{corollary}

\begin{proof}
Items 1--3 identify the filtered complex and coefficient module required by
Proposition~\ref{prop:formal-transfer}.  Items 4--5 transfer the Cartan
homotopy, strict residual differential, and pairing.  Item 6 identifies the
selected degree and normalization.  The exact residual theorem then gives
\eqref{eq:main-result}, while \eqref{eq:HPL-isometry} promotes its
representative Gram matrix to the transferred cohomological form.
\end{proof}

The formal detour theorem, factorization \eqref{eq:Bfactor},
fifteen-dimensional zero-mode count, and finite jet table are exact inputs.
Separate finite-dimensional fixtures verify
\eqref{eq:transferred-Cartan} and \eqref{eq:HPL-isometry} with a genuinely
dressed inclusion.  They do not instantiate the full pure-Weyl field
complex.  The unproved core is the Lorentzian local-kernel theorem and the
row/strictness assertions made explicit above.
...
\paragraph{The remaining classical theorem.}
The highest-priority mathematical task is the all-level Lorentzian cylinder
identification
\begin{equation}
 \ker B_{\rm lin}/\im K\cong\W_+\oplus\W_-
\end{equation}
inside the complete local BV complex, followed by a compact-degree
equivariant cyclic zero-mode transfer.  The exact ranks in
Table~\ref{tab:detour-ranks} are a finite certificate and a regression rail,
...
calculation.  It is exhaustive for the stated minimal residual complex, but
it concerns vertices rather than propagating particles.  Its promotion to
the full free pure-Weyl BV theory is controlled by the explicit hypotheses
of Corollary~\ref{cor:conditional}.  Quantum consistency and interaction
...
The scientific input was frozen at repository commit
\begin{center}
\texttt{e928f257c25099eb534eb34109dfc1dc6a3127a1}.
\end{center}
The companion snapshot records the environment, hashes, expected-failure
guards, and command lines.  The complete paper verification is run with
\begin{verbatim}
...
zero modes; no all-level or Lorentzian identification is inferred.\\
On-shell Weyl module and stable generator coefficients & Exact module
construction & Character, tower inventory, invariant form, and cutoff-stable
conformal brackets.\\
...
one particle.\\
Centered two-particle sector and $G_{\rm rep}=I_2$ & Exact residual theorem &
...
Full local-plus-residual pure-Weyl BV result & Conditional & Requires
Field-Theoretic Hypothesis~\ref{hyp:pure-weyl}; the all-level Lorentzian
local-kernel theorem, row inventory, and strict transfer remain open.\\
