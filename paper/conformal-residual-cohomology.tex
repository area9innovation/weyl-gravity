...
that geometric system with the algebraic positive-energy module.  Two
further steps remain conditional: the full equivariant cyclic BV transfer
with its ghost and antifield rows, and derivation of the centered BFV
polarization from the chosen closed-universe reduction.  Any analytic
...
global PDE-surjectivity and finite-mode preimage gaps.  They do not by
themselves compute the complete BV complex.  The remaining bridge is
concentrated in two statements.
...
These are not consequences of the residual rank matrices.  The first
requires the complete local ghost/antifield row inventory and a genuinely
conformal cyclic transfer, rather than merely the
blockwise $D\times SO(4)$ splitting of
Proposition~\ref{prop:cyclic-retract}.  The second is stronger than verifying
...
Proposition~\ref{prop:Fock-lift} gives its free Fock lift.  Item 1 supplies
the full equivariant cyclic hypotheses under
which Propositions~\ref{prop:cyclic-retract} and \ref{prop:single-row} give
strict residual transfer and spectral-sequence collapse.  Item 2 fixes the
...
complex.  The two hypotheses above state the remaining obligations without
folding full conformal equivariance, antifield-row control, or BFV
normalization into the metric-to-curvature theorem.  A Hilbert, Krein,
distributional, or completed Fock statement
...
the algebraic $E/A/L$ curvature intertwiner are exact.  Two sharper
obligations remain: construct the full $SO(4,2)$-equivariant cyclic BV
zero-mode transfer with its antifield-row inventory, and derive the chosen
four-ghost polarization and normalized pairing from the residual BFV
boundary problem.  The direct Taub argument
