...
The retained $30$-row metric endpoint is coefficient-complete.  Its canonical
upper curvature lift is exact, the canonical middle graph lift is obstructed,
and the minimal cyclic relative saddle has zero Schur correction.  Thus the
metric endpoint Green witness remains open.  Nine curvature/current flags are
...
$D\Lambda_\pm+\Lambda_\pm D=1$ causally.  The endpoint witness and Green
operators are not constructed, so no causal flag is promoted.
...
the still-missing object is the endpoint-diagonal witness $W_0$ itself.  The
...
completion are not claimed.  A direct same-bundle metric factorization
remains optional.
...
coefficient-complete and cyclic.  Its upper curvature graph lift is exact;
...
remaining work is the endpoint witness $W_0$ and actual BV Green homotopy,
