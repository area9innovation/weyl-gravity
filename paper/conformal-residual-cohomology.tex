...
obstruction.  The curvature-corrected quadratic and lower equations, which
also contain nonlinear products of the two factor first-order matrices,
remain unassembled and unsolved.  Restoring the two independent factor
splittings and all algebraic coefficients gives
...
show that the naive table adjoint is invalid.  A pairing-aware general
formal-adjoint backend and the $421$-variable coefficient solve remain
prerequisites for a trusted quadratic/lower certificate.
...
order-two equations and remains to be solved.
...
witnesses or an additional support-local first-order prolongation.  No flag
is promoted.
...
hence no cubic obstruction, while the curvature-corrected nonlinear
quadratic and lower solve has $421$ post-cubic variables and remains open.
...
composition backend.  The $48$-entry naive sorted-table defect instead leaves
the pairing-aware general adjoint and the coefficient solve open.  The four
bare-$\Box$ orientations are exactly inconsistent with rank $159/160$ and a
common one-row $f_{01}\to f_{00}$ obstruction; the two-nontrivial-factor
branch remains open.
...
relative witnesses or a support-local first-order prolongation.\\
