...
the latter include the certified trivialization
$\omega\Box R=-\frac1{12}s(R^2)$ modulo a total derivative.  This is only an
antifield-independent candidate catalogue: full Diff--Weyl descent,
antifield completion, cohomological nontriviality and coefficients remain
open.  The receipt is
\texttt{LOCAL\_DIMENSION\_FOUR\_CANDIDATE\_CATALOGUE\_CERTIFICATE}.
The strict-density part of that descent has now been constructed as an exact
horizontal bicomplex.  For a strict Weyl-invariant top density $L$, and
...
$1,-1,\frac12,-\frac16,\frac1{24}$ and a BRST-closed bottom.  This supplies
the nonzero Diff descents for $C^2$, $C\widetilde C$ and their Weyl-ghost
lifts.  It does not identify a nonzero descent with a nontrivial cohomology
class.  The Euler density and $\omega E_4$ are deliberately excluded because
their Weyl variation requires a separate transgression-current descent;
antifield completion remains blocked on the portable classical export.  The
receipt is
\texttt{LOCAL\_STRICT\_DENSITY\_DESCENT\_CERTIFICATE}.
