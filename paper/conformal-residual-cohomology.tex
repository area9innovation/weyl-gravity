...
$E/A/L$ matrices.  Exact cross-energy recursion then fixes the raw
...
polynomial metric-BV model.

\begin{conjecture}[Field-theoretic BV identification]\label{hyp:pure-weyl}
In the selected cylinder domain, the gauge-fixed free strict-pure-Weyl BV
complex is represented completely by the raw algebraic model in the
centered window.
...
field-theoretic cyclic pairing restricts to the raw form of
Proposition~\ref{prop:raw-cross-energy}.
\end{conjecture}

This is not a consequence of the residual rank matrices.  The raw
calculation already contains the minimal detour ghost and antifield rows,
the noncompact homotopies, the cross-energy cyclic form, and the complete
centered HPL correction test.  The conjecture asks only whether the chosen
field-theoretic gauge-fixed domain has further rows or domain effects absent
from that algebraic model.
...
and end-to-end centered ranks.  The remaining classical bridge is the
field-theoretic identification in Conjecture~\ref{hyp:pure-weyl}: one must
show that the chosen gauge-fixed pure-Weyl BV domain has no additional
...
all-energy Taub normalization are now explicit.  Promotion to the complete
field-theoretic pure-Weyl BV/BFV domain is controlled by the single
identification in Conjecture~\ref{hyp:pure-weyl}.  Analytic completion,
