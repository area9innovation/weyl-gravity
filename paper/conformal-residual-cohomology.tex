...
two-dimensional quotient is the helicity-$\pm2$ module, and the reduced
  ...
  exact.  Curvature-aware PBW compression of the flat adjoint-tractor detour
  gives an exact cyclic curved BGG retract and transfers its causal homotopy
  to the trace-free metric endpoint.  An explicit trace/Weyl shear and the
  $356/30$ hybrid retract then give a causal homotopy on all $386$ prolonged
  rows.  A canonical endpoint Green inverse and prolonged Green witness are
  not constructed and are not needed for this direct transfer.  The
  resulting causal map is a compact-to-spacelike-compact quasi-isomorphism;
  on the compact-cylinder Cauchy surface it reaches all smooth solutions.
  Temporal-cutoff representatives recover the fifteen conformal-Killing
  ghosts and their dual endpoint classes without duplication, and the cutoff
  commutator gives an explicit homotopy-equivariant $SO(4,2)$ transfer.  The
  cyclic causal difference agrees on cohomology with the prolonged,
  auxiliary, metric, Cauchy and $E/A/L$ currents.  Consequently the final
  covariant theorem is
  $H^4_{\rm cov}=\operatorname{span}\{[W_+^2],[W_-^2]\}$ with
  $G_{\rm cov}=I_2$.  A Hadamard theory remains open.  A direct invariant
  two-wave factorization of the gauge-completed Bach operator is ruled out
  in the stated complete family.
...
The
  ...
  BV differential are exact as well.  The exact curved adjoint-tractor BGG
...
compression extends to an exact curved cyclic BGG retract: ten invariant
...
\texttt{prolonged\_current\_comparison} is true.  Moreover
...
\begin{equation}\label{eq:final-covariant-H4}
 ...
 \qquad G_{\rm cov}=I_2.}
\end{equation}
The result is transported from the already-certified residual calculation;
no auxiliary-space Chevalley--Eilenberg recomputation is used.
\end{theorem}

\begin{proof}
The support exact sequence and the same-sided homotopies
\eqref{eq:causal-green-homotopy} give the quasi-isomorphism
\eqref{eq:causal-quasi-map}.  Compactness of $S^3$ identifies its target with
...
At the one-loop derivative bound, a separate target-native calculation now
exhausts all 105 quadratic curvature contractions rather than inheriting
relations from that order-six image.  The parity-even Riemann ansatz has
dimension three, and its integrated Weyl-variation map has rank one; the
resulting two-dimensional kernel is exactly spanned by $C^2$ and $E_4$.
The quadratic tracefree-Weyl quotient has dimension one in each parity
sector.  The odd sector is enumerated with a compressed dual-Weyl carrier
whose epsilon-over-two realization and Euclidean/Lorentzian Hodge squares are
checked explicitly.  Adjoining the explicit boundary density $\Box R$ gives
four ghost-number-zero curvature candidates and their four Weyl-ghost lifts.
The type-D lift has the certified local trivialization
\begin{equation}
 \omega\Box R=-\frac1{12}Q_W(R^2)
 -d_h\bigl(R\nabla\omega-\omega\nabla R\bigr).
\end{equation}
This is only an antifield-independent candidate catalogue: the complete
relative coboundary quotient, antifield completion, cohomological
nontriviality and coefficients remain open.  The receipt is
\texttt{LOCAL\_DIMENSION\_FOUR\_CANDIDATE\_CATALOGUE\_CERTIFICATE}.
The universal diffeomorphism part of that descent has now been constructed
as an exact horizontal bicomplex.  For a covariant top density $L$, and
separately for its Weyl-ghost lift $\omega L$, the machine generates
$I_k=\iota_\xi^kI_0$ and solves
$QI_k=(k+1)^{-1}d_hI_{k+1}$.  Hence the verified form-degree $4$--$0$
tower has coefficients
$1,-1,\frac12,-\frac16,\frac1{24}$ and a BRST-closed bottom.  The ledger
therefore separates \path{diff_descent_status} from
\path{intrinsic_weyl_descent_status}: a nonzero universal Diff tower is
not called a nontrivial descent and says nothing about the BV class quotient.
The receipt is \path{HORIZONTAL_BICOMPLEX_CERTIFICATE}.

The Euler current is treated independently in Chern--Weil form.  Starting
from $E_4=\epsilon_{abcd}R^{ab}\wedge R^{cd}$, the machine derives the
Levi--Civita connection variation from $Qg_{\mu\nu}$, verifies
$Q R=D(Q\omega)$ and $D R=0$, and constructs
\begin{equation}
 \Theta_E(Q)=2\epsilon_{abcd}
 Q\omega^{ab}\wedge R^{cd},
 \qquad Q E_4-d_h\Theta_E(Q)=0.
\end{equation}
Equivalently the first descent representative is $a^3=-\Theta_E(Q)$, so
$Q E_4+d_ha^3=0$.  For the anomaly lift, however, the first attempted step
leaves $d_h\omega\wedge\Theta_E$; the complete intrinsic type-A continuation
is therefore still recorded as \path{NOT_COMPUTED}.  This distinction is
certified by \path{EULER_TRANSGRESSION_CERTIFICATE}.  Antifield completion
remains blocked on the portable classical export.  The quantum repository now
provides an executable \path{ANTIFIELD_EXPORT_CONTRACT} requiring the three
minimal antifields, all retained nonminimal rows, the exact split
$Q=\delta+\gamma+Q_{>0}$, and proof artifacts for $\delta^2=0$,
$\delta\gamma+\gamma\delta=0$, and $Q^2=0$.  This is a fail-closed handoff
schema, not a substitute for the missing rows or their independent import
verification.  Independently, an exact sparse mapping-cone engine now
totalizes finite spaces $V^p_g$ with
$D=Q+(-1)^g d_h$, verifies $D^2=0$, and emits ansatz hashes, cocycle and
coboundary ranks, quotient dimensions, representative coordinates and proof
hashes.  Its current receipt, \path{RELATIVE_COHOMOLOGY_ENGINE_CERTIFICATE},
is validated only on a finite fixture; the production $H^{0,4}(s\mid d)$ and
$H^{1,4}(s\mid d)$ matrices remain uncomputed.
...
by the reduced Weyl symbol.  The successful analytic route is the curved
electric/magnetic Weyl prolongation.  Its exact equations,
...
\eqref{eq:endpoint-factor-nullstellensatz}.  The curvature-aware
adjoint-tractor PBW calculation now resolves the former $48$-entry defect in
ten invariant channels.  Its support-local curved cyclic HPL maps are exact,
and the compressed parent middle operator agrees with the complete Bach
block at all derivative orders with normalization $-2$.  The transferred
trace-free homotopy, explicit trace/Weyl shear, and hybrid $356/30$ retract
give the full $386$-row causal identity
\eqref{eq:full-prolonged-causal-homotopy}.  This direct construction neither
provides nor needs a canonical Green inverse for $D_{\rm TF}$ or a prolonged
degreewise Green witness.  The
off-shell prolonged/auxiliary current comparison is exact, and cyclicity of
the transferred homotopy identifies its causal pairing with the metric,
Cauchy and all-energy $E/A/L$ currents.  The prolonged Green witness and
canonical causal Green operators remain open as scoped legacy implementations,
while the BV causal homotopy is exact.  Its causal difference is a
compact-to-global quasi-isomorphism; all fifteen endpoints and their duals
are recovered without duplication, and $SO(4,2)$ transfers up to the explicit
cutoff chain homotopy.
...
Theorem~\ref{thm:cauchy-sobolev} identifies the field Cauchy space with
the energy-mode Krein module and proves Green hyperbolicity of its TT and
vector propagation blocks.  The exact dependency DAG now certifies
\texttt{final\_covariant\_H4}, and
Theorem~\ref{thm:covariant-causal-H4} gives
$H^4_{\rm cov}=\operatorname{span}\{[W_+^2],[W_-^2]\}$ with
$G_{\rm cov}=I_2$.  Distributional and Hadamard
...
is the claim/evidence index for the terminal covariant theorem; it binds each
required claim to its authoritative certificate digest and reproduction
command.
