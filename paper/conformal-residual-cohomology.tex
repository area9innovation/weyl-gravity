...
two-dimensional quotient is the helicity-$\pm2$ module, and the reduced
  ...
  differential.  The first eight curvature flags are therefore true.  The
  ...
  $SO(4,2)$-equivariant transport, prolonged current comparison, and a
...
variables rather than hide it in a $24$-component scalar-wave block.  The
  ...
  transport, prolonged current comparison, distributional/Hadamard
...
differential are certified.  The first eight curvature flags are true; the
remaining seven causal-transport flags keep the final covariant $H^4$ flag
false.
...
remain unassembled and unsolved.  Thus this gate proves neither a
factorization nor a Green theorem.
...
first-order symmetric-hyperbolic theorem does not apply to it.  Explicit
coefficient tables for $R,S$, a support-local first-order reduction of every
$E_{\rm aux}$ occurrence, and a positive temporal symmetrizer for the full
coupled principal matrix remain open.  Neither diagnostic promotes a flag.
...
\begin{equation}\label{eq:open-curvature-flags}
 ...
 \end{aligned}
\end{equation}
The first eight flags in \eqref{eq:open-curvature-flags} are true; the remaining
seven remain false.  In particular, the paper now constructs the complete
...
certified chain homotopy and compare the prolonged current with the auxiliary
current by a $d+Q$ improvement.  The known
...
are now proved.  The remaining seven false flags in
...
and prolonged current comparison remain open.
...
quadratic and lower solve remains open.
The smallest reciprocal relative saddle is the odd-adjoint pair $4+5$.
Its Schur complement is nonlocal, while its unreduced order-two form still
lacks a support-local first-order reduction and full positive symmetrizer.\\
Curvature causal realization and final covariant $H^4$ & Open, fail-closed &
The first eight flags in \eqref{eq:open-curvature-flags} are true and the other
seven remain false.  They require the prolonged Green witness and actual BV
Green homotopy, residual endpoint recovery, equivariance, and
prolonged current comparison.  Consequently the causal quasi-isomorphism
