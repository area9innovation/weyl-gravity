...
two-dimensional quotient is the helicity-$\pm2$ module, and the reduced
  ...
  module.  The proposed degree-resolved odd BV mapping cylinder currently
  fails its final fibre-identified associated-graded square by exact rank
  four.  Thus the support-local all-row prolongation retract, nilpotence of
  the prolonged differential, and prolonged current comparison remain open.
  The
...
Eight curvature flags are true; the remaining seven current/causal flags keep the
final covariant $H^4$ flag false.
...
flag is deliberately left false until its certificate explicitly binds the
corrected curved core-chain provenance.  The two remaining analytic exceptions
...
The principal rank-$14$ quotient itself now has a projector-free local
presentation with rank-$10$ light and rank-$4$ temporal subquotients, exact
same-sided Green algebra and causal temporal source lifting modulo the
certified vector-gauge image.  The raw
$T=(C_1,\operatorname{div}C_1)$ symbol has rank $5$ and kernel rank $9$ on
this cokernel, whereas the weighted Weyl--Cotton compatible-source kernel
has rank $12$.  More importantly, $\operatorname{im}T$ is not contained in
that kernel off shell: the generic compatibility defect has rank $3$ (and
rank $1$ at the aligned null covector).  Hence the tempting rank-seven
quotient $K_{12}/I_5$ is not defined.  This does not contradict the exact
null-cone split of the Weyl image into a rank-three gauge-preimage module and
the rank-two helicity module, on which the reduced Weyl map is
$\frac14 I_2$; that quotient is different from the full constraint kernel.
...
\end{equation}
which is the constraint component of the certified chain square for
$E_{\rm curv}=(L_{\rm WC},K_{\rm state})$.  Thus a direct state retraction is
the wrong target; the full bridge must be an equation-level deformation
retract using the auxiliary $f$ equation and all forty curvature rows.

The first full graded cone audit keeps the incoming gauge row and has degree
ranks
\[
 9\longrightarrow24\longrightarrow50\longrightarrow49\longrightarrow14.
\]
Its original mixed-coordinate extraction separated an ordinary identity
layer from the curved fibre-identified companion and reported internal defect
ranks $11,4$ (or $7,4$ at a null covector).  The curved core calculation shows
that the rank-four part used the wrong equation projection.  Substitution of
the actual $p_E,p_I$ proves the operator chain squares and the support-local
graph mapping-cylinder SDR exactly.  This does not compute the cohomology of
the relative rank-$14$ equation cone and does not construct its deformation
retract or Green operators.  The integer Douglis weight constraints remain a
useful representative for that later causal/PBW problem, but the stale
rank-four matrix is no longer a nilpotence obstruction.
...
\begin{equation}\label{eq:open-curvature-flags}
 ...
 \end{aligned}
\end{equation}
Eight flags in \eqref{eq:open-curvature-flags} are true: the curved equations,
...
prolonged BV operator identity.  The remaining seven current, causal, and
transport flags are false.  In particular, the paper does not yet construct
...
The corrected mapping cylinder makes this a well-typed candidate, but its
current certificate does not yet bind the corrected core-chain digest;
therefore \texttt{prolonged\_current\_comparison} remains false until that
provenance and the associated pullback are checked explicitly.  The equivariance defect of the Cauchy
...
\cite{DecaniniFolacciFKWC}.  This is invariant-basis infrastructure only: the
four-dimensional Schouten and tracefree-Weyl specializations, BRST closure,
antifield descent, anomaly coefficients, and the quantum master equation all
remain open.
...
prolongation and BV differential are exact.  The remaining seven false
current/causal/transport flags in
...
prolonged Green witness and causal Green operators, causal BV Green homotopy
and quasi-isomorphism, endpoint recovery, $SO(4,2)$-equivariant transport,
remain open.  The all-row prolonged/auxiliary current comparison is likewise
pending an explicit corrected-core provenance link; its later identification with a
causal Green pairing is a separate downstream step.
...
Eight flags in \eqref{eq:open-curvature-flags} are true and the seven
current/causal/transport flags remain false.  The local all-row prolongation
and BV differential are complete.  The remaining work is the prolonged Green
witness and actual BV Green homotopy, residual endpoint recovery, and
equivariance.  The prolonged current comparison is not yet promoted;
Green/current equality is further downstream.  Consequently the causal quasi-isomorphism
