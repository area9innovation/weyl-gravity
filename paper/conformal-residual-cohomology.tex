...
\author{Anonymous manuscript for external review}
...
Author names and contribution statements are withheld in this external-review
version and must be supplied before publication.  Repository scripts generate
the identified tables, certificates, and selected displayed summaries; those
artifacts are computational inputs rather than authors.  Responsibility for
the mathematical statements and proofs rests with the human authors named in
the final version.
...
\begin{equation}\label{eq:derived-residual-BFV}
 ...
 Qb_a\big|_{c=0}=\mu_a.
\end{equation}
The restriction $c=0$ in the last formula isolates the Koszul term; the full
$Qb_a$ also contains the standard coadjoint ghost contribution, with signs
fixed by the residual BFV charge used below.
Thus the Koszul part imposes the Taub zero locus $\mu^{-1}(0)$ and the
Chevalley--Eilenberg part takes the derived quotient by the residual
stabilizer.  This formulation retains the nonregular stabilizer directions;
it does not replace them by an ordinary manifold quotient
\cite{CattaneoMnevReshetikhin}.  The statement is a theorem about the formal
BV--BFV model and its certified algebraic domain, not a construction of a
global singular derived stack in a larger analytic category.

After the BV--BFV time-slice map and the selected positive-frequency
polarization, $b_a$ acts as $\iota_a$ and
\eqref{eq:derived-residual-BFV} becomes the absolute complex
\eqref{eq:CE-differential} on the free symmetric-algebra coefficient module.
This is a polarized free-state realization of the linearized classical
theory, not a
claim that the unpolarized classical BV tangent complex is a graviton Fock
space.
\end{theorem}
...
checks, the universal diffeomorphism descent, the first Euler transgression,
and fail-closed interfaces for importing antifields and forming relative
cohomology.  These results do not enter the Lorentzian causal theorem proved
here.  In particular, they do not yet establish the production
