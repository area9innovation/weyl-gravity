...
$E/A/L$ matrices.  The tangent differential derived directly from the
quadratic minimal master action is related to those raw rows by explicit
inverse chain maps throughout the centered energy buffer.  A stationary
$D\times SO(4)$ conformal-Landau gauge fermion, together with the complete
Diff and Weyl nonminimal fields and their antifield duals, gives an exact
canonical conjugation and strong deformation retract back to that minimal
chain without changing the fifteen conformal-Killing modes.  Exact
cross-energy recursion then fixes the raw
cohomology form, makes the raw retract cyclic, and verifies the dressed
isometry and the absence of higher HPL corrections in the centered window.
An all-energy equivariant Taub reconstruction is normalized by a direct
$D$-charge calculation and two independent quadratic Bach-source
integrals.  The closed-universe BFV choice is stated explicitly rather than
claimed universally.  Perfect endpoint pairings now give an exact
fifteen-dimensional isomorphism
$\operatorname{coker}K^\sharp\cong(\ker K)^*$, and the quadratic endpoint
obstruction is the certified Taub/moment map.  This bulk obstruction space is
not itself the BFV ghost-momentum space.  For the selected algebraic
closed-cylinder BV--BFV data, however, compatibility of the normalized bulk
and BFV cohomological vector fields fixes the degree-suspension scalar to
$+1$.  The resulting positive-frequency Lagrangian polarization has state
complex
$\operatorname{Sym}(\W_+\oplus\W_-)\otimes\Lambda^\bullet\mathfrak{so}(4,2)^*$:
negative frequencies supply the symplectic dual, local and nonminimal
doublets contribute only their vacuum, and BFV ghost momenta act by
contraction rather than generating another exterior algebra.  One
action-normalized energy-two comparison and the oriented cotangent ghost
measure then transfer the field pairing and give $G_{\rm field}=I_2$.
The normalized $E/A/L$ basis also defines a canonical energy-mode Hilbert
majorant and an infinite-index one-particle Krein completion.  Its bosonic
second quantization, tensored with the finite residual ghost space, carries
a closed maximal residual BRST operator.  Every total compact-degree block
is finite-dimensional, and the bounded off-center Cartan homotopy proves
closed range and localizes completed cohomology to the same finite centered
block.  Consequently the completed cohomology and field-induced Gram matrix
remain exactly the displayed $\C^2$ and $I_2$.  This is an energy-mode
Krein--Fock theorem.  At the reduced one-particle field level, tensor-curl
factorization now gives a Lorentzian Cauchy--Sobolev realization
Krein-unitarily equivalent to that completion.  An exact third-order gauge
companion factors the full ghost row, and a support-local
ordinary-derivative auxiliary realization supplies the exact curved
four-row differential, a BV-canonical support-local deformation retract,
and the covariant current comparison.  An exact null-symbol rank argument
also proves that no pointwise fibre pairing and first-order gauge companion
can turn its $24$-field Hessian into a scalar-wave block: at a generic null
covector the Hessian and gauge symbols have ranks $11$ and $9$.  The
two-dimensional quotient is the helicity-$\pm2$ module, and the reduced
  linearized-Weyl symbol is the isomorphism $\frac14 I_2$.  Exact decomposition
  of the curved Bianchi--Bach equations gives a $26$-state Weyl--Cotton
  system.  Its constraint-adjusted form is symmetric hyperbolic, its complete
  sourced subsidiary identity propagates all fourteen constraints, and an
  all-level symbolic audit recovers precisely the parity-complete $E/A/L$
  module.  The corrected fibre-identified curved projections close the
  degree-resolved odd BV mapping cylinder.  Its all-row support-local retract,
  nilpotent differential, and off-shell prolonged current comparison are
  exact.  The
  prolonged Green witness, causal Green operators and BV Green homotopies,
  causal quasi-isomorphism, residual endpoint recovery,
  $SO(4,2)$-equivariant transport, Green/current identification, and a
  Hadamard theory remain open.  The
residual $H^4$ calculation itself is complete: the missing theorem is the
causal bridge identifying the covariant complex with that already-certified
module.  A direct same-bundle
factorization of the gauge-completed Bach operator is optional.
We claim neither an integrated $SO(4,2)$ group
representation, a positive graviton Fock space, nor quantum anomaly freedom.
\end{abstract}
...
route now supplies the exact curved four-row operator identities, a
...
$\pm2$, and the reduced Weyl symbol acts on it by $\frac14 I_2$.  Thus the
remaining causal theorem must propagate the physical sector in curvature
variables rather than hide it in a $24$-component scalar-wave block.  The
  exact Weyl--Cotton equations, their constraint-adjusted symmetric-hyperbolic
  ...
  BV differential are exact as well.  What remains is the prolonged Green
  witness, causal operators and BV Green homotopies, the causal
  quasi-isomorphism, residual endpoint recovery, equivariant pairing
  transport, Green/current identification, distributional/Hadamard
  completion, and alternative boundary polarizations.  A direct same-bundle metric factorization is an
...
Seven statements must remain separate.
\begin{enumerate}
...
\item The exact null-symbol quotient is the two-dimensional helicity-$\pm2$
module and the reduced Weyl symbol is $\frac14 I_2$.  The exact $26$-state
...
support-local all-row prolongation and BV operator identity are also exact.
...
complete invariant two-wave-factor family is ruled out by an exact
order-two polynomial certificate.  The canonical upper curvature lift is
exact, the canonical middle graph lift is obstructed, and the minimal cyclic
relative saddle has zero Schur correction.  A flat adjoint-tractor HPL
compression and its principal Bach match are exact, but the corresponding
curved PBW chain map still has a $48$-entry defect.  Thus the endpoint Green
homotopy remains open.  Nine curvature/current flags are
true; the remaining six causal flags keep the final covariant $H^4$ flag
false.
...
\begin{equation}\label{eq:curl-squares}
 \boxed{
 \mathcal C_2^\dagger=\mathcal C_2,quad
 \mathcal C_2^2=-D^2+3;
 \qquad
 \mathcal C_1^\dagger=\mathcal C_1,quad
 \mathcal C_1^2=-D^2+2.}
\end{equation}
...
sources preserve all fourteen constraints.  This proves
\texttt{curved\_EB\_symmetric\_hyperbolicity},
\texttt{curved\_sourced\_constraint\_identity}, and
\texttt{curved\_constraint\_propagation}.  The unadjusted raw subsidiary
...
second solution copy.  Thus \texttt{EAL\_curvature\_spectrum\_match} is true;
the levels $2$ through $6$ are retained only as regressions, not as the proof.
...
\texttt{prolonged\_current\_comparison} is true; Green/current equality
remains downstream.  The two remaining analytic exceptions
...
conjugation does not prove the prolonged Green theorem.  The next witness must
couple the auxiliary and curvature sectors in both directions to form a
nontriangular saddle operator, unless the auxiliary diagonal witness is
independently replaced by a Green-hyperbolic one.
...
Green hyperbolicity.  In particular the causal and final covariant
$H^4$ flags remain fail-closed.
...
There is, however, a positive parent-detour route.  For the flat adjoint
tractor connection, the Yang--Mills detour complex of
...
it to ranks $4\to9\to9\to4$; every flat splitting, retraction, chain-map and
cyclic dual-row defect vanishes.  The compressed middle principal operator
matches the endpoint Bach symbol coefficientwise with normalization $-2$.
For a completed curved cyclic SDR $(i,p)$, the causal transfer would be
\begin{equation}\label{eq:conditional-tractor-green-transfer}
 \Lambda_{{\rm TF},\pm}=p\Lambda_{{\rm parent},\pm}i,
 \qquad
 q\Lambda_{{\rm TF},\pm}+\Lambda_{{\rm TF},\pm}q=1,
\end{equation}
with causal support preserved by finite-order pre- and postcomposition.
The present cylinder substitution is not yet that SDR: treating the
Levi--Civita derivatives as commuting leaves an exact $48$-entry chain defect.
This is the scoped PBW boundary requiring the missing tensor-slot curvature
commutators, not a no-go for curved BGG compression.  Accordingly
\eqref{eq:conditional-tractor-green-transfer} remains conditional and no
causal flag is promoted.

What remains is
$L_{\rm end}=DW_{\rm end}+W_{\rm end}D$ and
$\Gamma_\pm=W_{\rm end}G_{\rm end,\pm}P_{\rm end}$.
Then $\Lambda_\pm=H_{\rm alg}+\Gamma_\pm$ should satisfy
$D\Lambda_\pm+\Lambda_\pm D=1$ causally.  The algebraic endpoint witness
$W_0$ is constructed, but neither a Green inverse for $D_{\rm TF}$ nor the
curved tractor/BGG transfer is complete, so no causal flag is promoted.
...
\begin{equation}\label{eq:open-curvature-flags}
 ...
 \end{aligned}
\end{equation}
Nine flags in \eqref{eq:open-curvature-flags} are true: the curved equations,
first-order closure, symmetric hyperbolicity, sourced identity, constraint
propagation, all-level spectrum, support-local prolongation retract, and
prolonged BV operator identity, together with the off-shell prolonged current
comparison.  The remaining six causal and transport flags are false.  In
particular, the paper does not yet construct
$G_\pm$ or degree-$(-1)$ maps
$\Lambda_\pm$ satisfying
\begin{equation}\label{eq:open-green-homotopy}
 Q_{\rm prol}\Lambda_\pm+\Lambda_\pm Q_{\rm prol}=1,
 \qquad
 \operatorname{supp}\Lambda_\pm f\subseteq J^\pm(\operatorname{supp}f).
\end{equation}
Homogeneous constraint propagation and symmetric hyperbolicity by themselves
would not prove \eqref{eq:open-green-homotopy}: compatible inhomogeneous
sources must be shown to generate constrained retarded and advanced
solutions.  Nor would a nonequivariant vector-space identification suffice;
the causal/Cauchy bridge must preserve the residual $SO(4,2)$ action up to a
certified chain homotopy.  A valid odd cyclic mapping cylinder would give the
quadratic parent
\begin{equation}
...
this pullback explicitly.  Hence
\texttt{prolonged\_current\_comparison} is true; equality with the causal
Green pairing remains downstream.  The equivariance defect of the Cauchy
cutoff is nevertheless now controlled conditionally and exactly.  If
$\rho$ is any of the fifteen local conformal chain generators and
$\kappa=[Q,\chi]$ is the standard temporal-cutoff inverse to the causal map,
then
\begin{equation}\label{eq:causal-cutoff-equivariance}
 [\kappa,\rho]=[Q,[\chi,\rho]].
\end{equation}
The displayed homotopy has compact support on a spacelike-compact section,
because $d\chi$ lies in a compact time slab and $S^3$ is compact.  Together
with the natural curvature mapping cylinder, smooth global BGG
equivariance, and the all-level $E/A/L$ exhaustion, this proves that
full $SO(4,2)$ transport will follow automatically once the actual causal
quasi-isomorphism and endpoint recovery exist.  It does not make the chosen
Cauchy split strictly equivariant, and the corresponding repository flag
remains false while the Green homotopy is absent.  The known
...
input.  Proposition~\ref{prop:Fock-lift} is algebraic.  Its selected
...
\subsection{The state complex is obtained after polarization}
...
positive-energy coefficient states.  The final coefficient complex is
\begin{equation}
...
and positive representative form in this algebraic model.
...
\item the polarized state complex is exactly
...
\item the induced field pairing on the two centered residual classes is
...
\end{equation}
\end{enumerate}
No single-row assertion is made about the unpolarized bulk BV tangent
complex.
\end{theorem}
...
For the selected algebraic closed-cylinder BV--BFV data, the complete free
classical polarized residual state cohomology in the selected degree is
\eqref{eq:main-result}.  Its field-induced Hermitian form is $I_2$, and its
...
Diff$\times$Weyl anomaly can obstruct quantum nilpotency even though
Corollary~\ref{cor:algebraic-pure-weyl} is established in the free classical
algebraic category and Theorem~\ref{thm:energy-mode-completion} extends its
residual cohomology to the selected energy-mode Krein completion.

\paragraph{Separate quantum local-algebra status.}
A parallel \texttt{LOCAL-ALGEBRAIC} computation now exhausts the parity-even,
dimension-independent scalar curvature contractions at order six.  Thirteen
nonzero cubic symmetry orbits reduce by five generated algebraic-Bianchi
relations to eight cubic classes.  After adjoining the generated
$R\nabla^2R$ bridge, integration-by-parts rows, and covariant-commutator
rows, the integrated curvature quotient has dimension ten; two derivative
directions lie outside the eight-dimensional cubic span.  Restoring the
omitted degree-one total divergence gives the standard seventeen-element
local order-six normal form, independently matching the FKWC ledger
\cite{DecaniniFolacciFKWC}.  An exact specialization tower now records named
relation families, quotient projections and kernels, parity blocks, a
five-index antisymmetrization primitive, tracefree-Weyl reduction, and
signature-aware epsilon-pair elimination.  Exhausting 3,328 five-index
endpoint selections produces 72 unique Schouten rows whose induced rank on
the universal quotient is two.  Thus the four-dimensional integrated
Riemann-invariant quotient has dimension eight, with the cubic rank reduced
from eight to six and no new derivative-sector loss.  This independently
matches the dimension losses in the Invar ledger
\cite{MartinGarciaYllanesPortugalInvar}.  This remains invariant-basis
infrastructure only: the tracefree-Weyl and parity-odd specializations, BRST
closure, antifield descent, anomaly coefficients, and the quantum master
equation all remain open.
Before constructing that image, the specialization layer has been hardened:
the Schouten and Cotton tensors, the Ricci decomposition and its differentiated
Weyl--Cotton identity are now exact abstract-index objects.  Expanding Cotton
back into Schouten derivatives reproduces the cyclic differentiated
decomposition term by term.  Consequently the earlier algebraic
Riemann-to-Weyl restriction now fails closed on differentiated curvature, and
the full-Weyl Hodge witness verifies the Euclidean/Lorentzian signs
$\star^2=+1/-1$ while deriving the parity flip.  This is still infrastructure,
not the tracefree-Weyl invariant quotient or a BRST-cohomology theorem; its
machine receipt is
\texttt{LOCAL\_WEYL\_DECOMPOSITION\_FOUNDATIONS\_CERTIFICATE}.
The resulting Schouten-zero restriction has now also been computed exactly.
The eight-dimensional four-dimensional Riemann quotient maps surjectively to
a one-dimensional Weyl target with seven-dimensional kernel.  Starting from
17 nonzero target monomials, the 106 mapped Bianchi, commutator,
integration-by-parts and Schouten rows have rank 16; the cubic and both
derivative sectors reach the same surviving class.  This sets the Cotton
completion to zero together with the Schouten jets, so it is a
Schouten-flat image theorem, not the unrestricted Weyl-jet quotient.  Its
odd Hodge companion is explicit, but exhaustive parity-odd enumeration and
BRST cohomology remain open.  The machine receipt is
\texttt{LOCAL\_SCHOUTEN\_ZERO\_WEYL\_IMAGE\_CERTIFICATE}.
...
The remaining six false causal/transport flags in
\eqref{eq:open-curvature-flags}, the resulting causal
quasi-isomorphism, Hadamard completion, and uniqueness among boundary or
deparametrized polarizations remain separate analytic questions.  The final
covariant $H^4$ transport is therefore not yet a theorem, even though the
residual $H^4$ module and its Gram matrix are already completely computed.
The remaining work is transport through \eqref{eq:covariant-causal-bridge},
not another residual CE calculation.  The exact ranks in
...
particles.  In the finite algebraic cylinder model the cross-energy cyclic
normalization, residual BFV boundary choice, full centered HPL transfer, and
all-energy Taub normalization are now explicit.  The minimal four-row chain
has additionally been derived from the quadratic master action and matched
to the raw complex by exact inverse maps.  Its specified gauge-fixed
nonminimal extension is canonically conjugate and contracts back to the
minimal chain while preserving all fifteen CKVs.  In the selected algebraic
closed-cylinder BV--BFV polarization, endpoint suspension, state-row
completeness, and pairing transfer are now exact, so the same $I_2$ is the
field-induced form on the two residual vertex classes.  This remains a
choice-dependent classical state theorem, not a positive graviton Hilbert
space.  The energy-mode Krein--Fock completion and closed residual operator
of Theorem~\ref{thm:energy-mode-completion} leave the finite centered result
unchanged.  The curved auxiliary operator identities, support-local retract,
and covariant current comparison are now exact.  The scalar-wave witness is
not merely unfinished: the rank-$11$ versus rank-$9$ null-symbol obstruction
proves it impossible for the present $24$-field, $9$-gauge realization.  Its
rank-two complement is exactly helicity $\pm2$ and is carried isomorphically
by the reduced Weyl symbol.  The remaining analytic route is therefore the
curved electric/magnetic Weyl prolongation.  Its exact equations,
first-order closure, constraint-adjusted symmetric hyperbolicity, sourced
constraint identity and propagation and all-level $E/A/L$ spectrum are
proved.  The corrected curved cotangent projections also prove the all-row
local prolongation retract and prolonged BV operator identity; the formerly
reported rank-four associated-graded defect used mixed coordinates.  The
support-local cyclic hybrid projector now contracts $356$ components and
retains the $30$-component endpoint graph.  The
complete curved endpoint is the exact $5\to10\to10\to5$ complex
\eqref{eq:exact-thirty-row-endpoint}.  The canonical upper curvature lift is
local and exact, while the canonical middle graph lift has the scoped
rank-five obstruction described above.  Passing to the full relative saddle
does restore a cyclic two-way incidence, but its minimal Schur correction
vanishes identically by \eqref{eq:endpoint-minimal-saddle-nilpotence}; hence
the exact endpoint diagonal remains intrinsic.  Its corrected canonical
$W_0$ is \eqref{eq:endpoint-canonical-backward-witness}, and its trace-free
principal symbol is scalar by \eqref{eq:endpoint-scalar-tracefree-symbol}.
The complete invariant two-wave-factor route is impossible by
\eqref{eq:endpoint-factor-nullstellensatz}.  The flat adjoint-tractor HPL
compression and principal normalization are exact, while its curved
Levi--Civita PBW lift retains the explicit $48$-entry defect described above.
Thus the missing object is a Green realization of the rank-nine trace-free
endpoint, most plausibly through the completed curved tractor/BGG or
equation-cone triangular route.  The
off-shell prolonged/auxiliary current comparison is exact; its identification
with a causal Green pairing remains downstream.  The prolonged Green witness
and causal Green operators, causal BV Green homotopy
and quasi-isomorphism, endpoint recovery, $SO(4,2)$-equivariant transport,
remain open.
Alternative boundary polarizations,
quantum consistency, and interaction descent are also separate questions.
At the reduced one-particle level, however,
Theorem~\ref{thm:cauchy-sobolev} now identifies the field Cauchy space with
the energy-mode Krein module and proves Green hyperbolicity of its TT and
vector propagation blocks.  Since the curvature Green realization and
causal quasi-isomorphism are not yet constructed,
\texttt{final\_covariant\_H4} remains false.  Distributional and Hadamard
completion are not claimed.  A direct invariant two-wave-factor metric
factorization is now ruled out in the stated complete family rather than left
optional.
...
prolonged.\\
Curvature causal realization and final covariant $H^4$ & Open, fail-closed &
Nine flags in \eqref{eq:open-curvature-flags} are true and the six
causal/transport flags remain false.  The local all-row prolongation, BV
...
flat adjoint-tractor HPL compression and principal $-2$ Bach match are exact,
but its curved PBW lift has a $48$-entry chain defect.  The remaining work is
a Green inverse for the rank-nine trace-free endpoint and the actual BV Green homotopy,
residual endpoint recovery, and
equivariance; Green/current equality is further downstream.  Consequently the causal quasi-isomorphism
and \texttt{final\_covariant\_H4} remain false; the already-complete residual
$H^4=\C^2$ and $G=I_2$ are not being recomputed;
