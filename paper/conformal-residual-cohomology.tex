...
not itself the BFV ghost-momentum space.  The remaining field-theoretic
identification is the one-scalar time-slice transgression between those dual
modules, completeness of the polarized BFV state rows, and transfer of the
BV--BFV--phase-space pairing; analytic completion also requires separate control.  We
claim neither a positive
...
nonminimal extension contracts back to them exactly.  The remaining
conditional steps are the bulk-to-BFV transgression, polarized-state row
completeness, and pairing transfer---not another residual rank calculation.
...
The other remaining issue is BV--BFV rather than local geometry.  The fifteen
conformal-Killing zero modes and the dual endpoint quotient must be
transgressed to a time-slice BFV cotangent sector without double counting.
Dneprov and Grigoriev's minimal
...
model.  Its identification with the pairing of the complete gauge-fixed
field-theory BV domain remains part of Conjecture~\ref{hyp:pure-weyl}; the
broader Riegert sector of \cite{HamadaBRST} is not included.
...
those are the remaining steps below.  The bulk dual endpoint itself is
...
$\tau=\lambda\Theta$, after the generator bases are fixed.  Thus the
remaining task is one $D$-component normalization and one orientation sign,
not fifteen unrelated mode matches.  The exact role audit verifies one
residual ghost copy, one endpoint quotient, one $15+15$ BFV cotangent
sector, and refuses to identify $[u]$ with $b_a$ before
\eqref{eq:zero-mode-transgression}; see
\path{symbolic/verify_conformal_residual_bfv_roles.py}.

Schematically, the no-duplication theorem still to be proved is a transfer,
not a deletion:
...
\end{equation}
up to the fixed real-structure convention.  This is distinct from
...
\subsection{The remaining field-theoretic identifications}
...
quadratic endpoint obstruction with the certified Taub/moment map.  Three
sharply delimited steps remain.

\begin{conjecture}[BV--BFV and polarized-state completion]\label{hyp:pure-weyl}
For the minimal algebraic cylinder BV theory, the closed-universe BFV choice,
and the positive-energy state polarization, the following three statements
hold.
\begin{enumerate}
\item \emph{One-scalar zero-mode transgression.}  The time-slice boundary
form induces the equivariant map
$\tau=\lambda\Theta$ in \eqref{eq:zero-mode-transgression}, with nonzero
$\lambda$ and a fixed orientation.  Under this map the local pair is
transferred, not copied, into exactly one residual BFV cotangent pair
$(c^a,b_a)$, and the residual charge is
\begin{equation}
 \Omega_{\rm res}=c^a\mu_a-\frac12f^a{}_{bc}c^bc^cb_a.
\end{equation}
\item \emph{Polarized-state row completeness.}  After time-slice reduction,
canonical phase-space reduction, positive-frequency polarization, and
nonminimal quartet contraction, every row capable of entering or leaving
the centered term $(p,q)=(4,0)$ is represented by the certified state
complex, contractible, or excluded by compact degree and representation
content.  No single-row claim is imposed on the unpolarized bulk BV complex.
\item \emph{Pairing transfer.}  The sequence
\eqref{eq:pairing-pipeline} induces the raw cross-energy form of
Proposition~\ref{prop:raw-cross-energy} and the normalized residual
complementary-degree ghost pairing, with the same orientation and scalar.
\end{enumerate}
\end{conjecture}

This is not a consequence of the residual rank matrices.  The raw
calculation and Proposition~\ref{prop:field-BV-dictionary} already identify
the minimal detour rows; Proposition~\ref{prop:gauge-fixed-equivalence} is a
separate gauge-fixed realization which contracts every added antighost,
multiplier, and antifield dual.  The endpoint quotient and its Taub map are
now exact.  The conjecture asks for the time-slice degree shift and scalar,
the completeness of the resulting \emph{polarized} state domain, and the
induced matter and ghost pairings.
...
classical polarized residual BFV state cohomology induced from the algebraic
bulk BV theory, in the selected degree, is \eqref{eq:main-result}.  Its transferred
...
Proposition~\ref{prop:gauge-fixed-equivalence} shows independently that the
specified gauge-fixed nonminimal realization contracts to the same minimal
chain; it is not a premise of the gauge-invariant cohomology.
Propositions~\ref{prop:raw-BV-transfer} and
\ref{prop:metric-residual-integration} give the strict induced residual
action and centered cohomology in the algebraic polynomial model.
Conjecture~\ref{hyp:pure-weyl} identifies this model and its cyclic form
with the polarized BFV state complex obtained from the minimal bulk theory.
The explicit closed-universe choice \eqref{eq:closed-BFV-choice} fixes which
generators are constrained; the transgression and pairing clauses fix the
remaining scalar and orientation.
...
\eqref{eq:Bfactor}, raw metric-BV transfer, cross-energy cyclic form,
closed-universe residual choice, canonical endpoint quotient, Taub map, and
integrated centered ranks are exact
inputs.  The conjecture above states the three remaining field-theoretic
identifications without folding them into the metric-to-curvature theorem.
A
Hilbert, Krein, distributional, or completed Fock statement
...
preserved.  The remaining classical bridge is Conjecture~\ref{hyp:pure-weyl}:
one must compute the one-scalar time-slice transgression to BFV ghost
momentum, prove row completeness after positive-frequency polarization
rather than on the unpolarized bulk BV complex, and transfer the pairing
through
$\Omega_{\rm BV}\to\Omega_{\rm BFV}\to\Omega_\Sigma\to J_+$.
...
valuable.  Extension to a chosen Hilbert or Krein completion is a separate
analytic question.  The exact ranks in Table~\ref{tab:detour-ranks} remain
independent convention and regression checks.
...
minimal chain while preserving all fifteen CKVs.  Promotion to the complete
field-theoretic polarized BV--BFV state domain is controlled by the three
remaining identifications in Conjecture~\ref{hyp:pure-weyl}; the gauge-fixed
extension is an independently proved corollary.  Analytic
completion, quantum consistency, and interaction descent are separate
questions.
...
transgression to BFV momentum is not included in this theorem.\\
...
Bulk-BV-induced polarized residual BFV result & Conditional & Requires the
remaining parts of Field-Theoretic Hypothesis~\ref{hyp:pure-weyl}: the
one-scalar time-slice transgression, a row ledger after phase-space reduction
and positive-frequency polarization, and transfer of the
BV--BFV--phase-space pairing and residual orientation.\\
