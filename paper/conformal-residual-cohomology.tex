...
two-dimensional quotient is the helicity-$\pm2$ module, and the reduced
  ...
  module.  A degree-resolved odd BV mapping cylinder also proves the
  support-local all-row prolongation retract and nilpotence of the prolonged
  differential.  The complete odd cyclic mapping cylinder also supplies a
  local quadratic BV parent on all sixteen blocks.  Its current pulls back
  to the auxiliary current off shell up to an explicit finite Lagrange
  concomitant, so the prolonged current-comparison flag is true.  The
...
constraint propagation, all-level $E/A/L$ spectrum, and all-row prolonged BV
differential and all-row prolonged current comparison are certified.  Nine
curvature flags are true; the remaining six causal-transport flags keep the
...
The required mapping-cylinder chain maps have now been constructed.  They
are explicit local operators $A_{\rm eq}$ and $B_{\rm id}$ satisfying
\begin{equation}
 (L,K)T=A_{\rm eq}E_{\rm aux},\qquad
 (-R,S)A_{\rm eq}=B_{\rm id}C_{\rm aux},\qquad
 T=(C_1,\operatorname{div}C_1),
\end{equation}
The first square is verified on all $10\binom{8}{4}=700$ metric four-jets.
In trace-free Bach coordinates the second square is especially sparse:
$A_{\rm eq}$ has only $f_A=-b_{\rm STF}/2$, $a=-b_{0i}/2$, and
$s=-3b_s/2$, while $B_{\rm id}$ sends the four diffeomorphism identities to
the $a$ and $s$ identity rows with coefficient $-1/4$ and kills the Weyl
trace identity.  Thus the six differential secondary rows are retained
exactly.  The preceding square $TK_{\rm aux}=0$ is independently exhaustive
on all diffeomorphism and Weyl-scalar jets and all four boost ghosts.  These
maps and their forced Koszul adjoints have now been inserted into a
degree-resolved sixteen-block odd BV mapping cylinder.  Its oriented
incidence form, cyclic differential and homotopy, nilpotence, canonical
shears, and support-local all-row SDR are exact.  Conjugating the canonical
direct-sum degree-$(-1)$ witness gives an exact identity
$P_{\rm prol}=Q_{\rm prol}W_{\rm prol}+W_{\rm prol}Q_{\rm prol}$, and fourteen
of the sixteen split diagonal blocks are Green hyperbolic.  The two exceptions
...
\]
It also separates the ordinary BV identity layer
$K_{\rm ordinary}(-\zeta)^T J_{\rm aux}$ from both the exact curved identity
and the Green-witness companion.  With the presently mixed leading tables,
the two internal square defects have ranks $11,4$ at generic, timelike,
spacelike and temporal covectors, and ranks $7,4$ at a null covector.
Consequently these tables do not yet form one associated-graded complex and
no cone-cohomology rank is assigned.  The integer Douglis weight constraints
are feasible and an explicit representative is certified; the missing step
is the complete associated-graded extraction, especially the lower $A$
terms and weighted zeroth-order Weyl--Cotton blocks.  Neither the
equation-level SDR nor its Green operators has been constructed.
...
\begin{equation}\label{eq:open-curvature-flags}
 ...
 \end{aligned}
\end{equation}
Nine flags in \eqref{eq:open-curvature-flags} are true: the first eight and
\texttt{prolonged\_current\_comparison}.  The six causal/transport flags remain
false.  In particular, the paper now constructs the complete
support-local prolonged BV rows, but not yet $G_\pm$ or degree-$(-1)$ maps
...
certified chain homotopy.  The prolonged/auxiliary current comparison is now
exact independently: odd cyclicity of the coefficientwise complete mapping
cylinder gives the quadratic parent
\begin{equation}
...
(the $B$ shear is order zero).  A compatible potential representative makes
$\beta=0$.  This does not infer equality with a Green pairing before the
causal homotopy exists.  The equivariance defect of the Cauchy
...
identity, all-level $E/A/L$ spectrum, and complete all-row BV prolongation
are now proved.  The remaining six false causal/transport flags in
...
constraint identity and propagation, all-level $E/A/L$ spectrum, all-row
local prolongation retract, and prolonged BV operator are proved.  The
...
remain open.  The all-row prolonged/auxiliary current comparison is exact;
only its later identification with the causal Green pairing is pending.
...
causal/transport flags remain false.  They require the prolonged Green
witness and actual BV Green homotopy, residual endpoint recovery, and
equivariance.  The prolonged current comparison is already exact off shell;
Green/current equality is downstream.  Consequently the causal quasi-isomorphism
