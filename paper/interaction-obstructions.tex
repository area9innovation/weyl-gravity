...
exactly ghost-parity pseudo-Hermitian.  The Krein/ghost-parity
structure thus survives precisely where pointed positive completions
fail: the two completions are inequivalent in the interacting theory.
...
(Proposition~\ref{cprop:fiveone}) --- the hierarchy is thereby
verified through order four at three qualitatively distinct
resonances; the deformation diverges toward the
...
on-shell branch-changing interactions while the former survives them
exactly (Proposition~\ref{cprop:krein}).
...
(confirmed at order 4, Proposition~\ref{cprop:fiveone}); a full-field
...
accompanying the series (release tag \texttt{paper5-v1.1}; SymPy
...
Proposition~\ref{cprop:fiveone}.  Predicted: $5{:}3$ at order 6.  The
mechanism excluding even mode-2 transfers is open.)  The
...
The Krein/ghost-parity structure of \cite{BT2026} is exactly
preserved by the interaction ($GTG=T^{\dg}$ on shell, finite-time
pseudo-unitarity), while every analytic positive pseudo-Hermitian
completion of the pointed sector is obstructed on an open shell
subset.  In particular the Bateman--Turok probability construction
cannot be secretly equivalent to an ordinary positive pseudo-Hermitian
completion.  The sharpest open form of the reconciliation is a
transport question: under their asymptotic embedding, physical
operators decompose into a ghost-symmetric part and a null orthogonal
...
\begin{equation}
\frac{S_{UU}}{S_{VV}}=\Big(\frac{\delta}{2g}\Big)^{2}g^{2}
=\epsilon\,g ,
\end{equation}
...
$\epsilon=0$: the unregulated massless perfect-square point.  There
the confluent limit gives $S_{VV}\to-1/(4w^2)$, precisely the
coefficient structure of the Bateman--Turok Bogoliubov data
(their Eqs.~(C5)--(C6)).  At split masses no admissible charge frame
removes the $+2$ component: it is invariant under the reference
dispersion and, under the residual charge-preserving Bogoliubov
freedom, the zero locus is reached only in a degenerate
(confluent-type) frame.  (Checks ON1--ON4,
...
null negative-charge component is therefore exact precisely at the
massless boundary, and holds at split masses up to charge
contamination of relative size $\epsilon/g$.  The remaining,
precisely-posed step --- the obstruction-to-null theorem --- is the
boundary Born-trace evaluation: the neutral component's blindness to
the obstruction coefficient and its $\epsilon\to0$ limit.
...
(Proposition~\ref{cprop:fiveone}); the next open case is $5{:}3$ at
order 6, and the mechanism excluding even mode-2 transfers is open.
The gravitational extension --- the cubic Einstein--Weyl vertices
against the covariant real forms of \cite{paperIV} --- is the natural
further target: the machinery here makes branch-changing gravitational
scattering the decisive calculation.  No CPT claim is made: the
...
\texttt{verify\_hardening.py} \quad
\texttt{verify\_doubled\_theory.py} \quad
\texttt{verify\_51\_order4.py} \quad
\texttt{verify\_obstruction\_null.py}
