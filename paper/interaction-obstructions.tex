...
energy pair at second order --- certified by an exact Sturm count of
the effective characteristic polynomial and reproduced by direct
diagonalization of the truncated Hamiltonian --- so that no positive
...
Let $\mathcal A=\Pi\circ K$, where $K$ is complex conjugation in the
$(x,y)$-Schr\"odinger representation ($x,y\mapsto x,y$,
$p,q\mapsto-p,-q$, $i\mapsto-i$) and $\Pi$ the total parity
$(x,y,p,q)\mapsto-(x,y,p,q)$.  Then $\mathcal A$ is antiunitary,
$\mathcal A^2=1$, and
\begin{equation}
...
\ \ (\zeta\in\R):
\end{equation}
$h_0$ is even and $K$-real; $v$ is odd under both $\Pi$ and $K$
separately ($KvK=-v$, $\Pi v\Pi=-v$), hence invariant under their
composition (machine-verified).  ``PT breaking'' below means the
appearance of eigenvalues not fixed by this antilinear symmetry, i.e.\
...
not.  (The statement concerns the second-order effective multiplet and
the truncated operator; extending it to the untruncated cubic operator
requires an operator-theoretic analytic-family argument, which we do
not supply.)
\end{corollary}
...
\end{equation}
with the quartic contact piece exactly zero.  The computation is
