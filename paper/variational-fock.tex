...
$\mathcal F(e^{\xi})=4\|\xi\|_F^2$.  With these conventions
...
and the closure is $\mathfrak{so}(2,\C)$ itself.  (The Lie-closure
dimensions $6$ and $2$ are verified computationally in the repository.)
\end{proof}

\begin{remark}[Symmetry enhancement at alignment]\label{rem:enhancement}
The stratification of Proposition~\ref{prop:hull} is mirrored by a
discontinuous jump of the unitary loci.  For the hull,
$C_j\cap K\cong SU(2)$ on the generic stratum and $U(1)$ at $\w_j=1$; for
the physical stabilizer, $H_j\cap K$ is the discrete $\Z_2$ generically and
jumps to the continuous $U(1)$ at alignment.  The equality locus of
Theorem~\ref{thm:mindist} enlarges accordingly.  This enhancement is a
property of the physical embedding (the twist), not of the abstract groups,
and marks the parameter values at which the twisted and compatible pictures
coincide.
\end{remark}
