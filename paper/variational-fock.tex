...
orthogonal Cartan-norm projection principle, which we prove in
if-and-only-if form: the stabilizer, itself not $\theta$-stable, has
canonical compatible hull exactly the mode-preserving $SL(2,\C)^2$, whose
positive part is a totally geodesic $\mathbb H^3\times\mathbb H^3$ to
which the Bender--Mannheim direction is orthogonal; the closed form is the
resulting mixed hyperbolic/flat Pythagorean law, and the principle extends
verbatim to $n$ PT mode pairs in $\Sp(2n,\C)$.  We then quantize the free fourth-order field modewise and
...
(Section~\ref{ssec:cartan}): an orthogonal Cartan-norm projection principle
along the positive part of a $\dg$-closed subgroup, proved in
if-and-only-if form (Theorem~\ref{thm:projection}); the physical stabilizer
itself fails $\theta$-stability --- which is why the classical
Mostow--Kempf--Ness theory does not apply to it directly --- but its
canonical compatible hull is exactly the mode-preserving $SL(2,\C)^2$
(Proposition~\ref{prop:hull}), and the whole mechanism extends to $n$ mode
pairs (Theorem~\ref{thm:nmode}).
...
log-singular-value vector), and equip the Hermitian directions $\mathfrak p$
with the trace form $\langle\xi,\eta\rangle=\tr(\xi\eta)$.  With these
conventions $\mathcal F(S)=2\|\mu(S)\|^2$ and $\mu(S_+)=(r,r)$, which lies
\emph{on the chamber wall} $x_1=x_2$ of type $C_2$.
...
essentially standard symmetric-space geometry in the tradition of Mostow's
self-adjoint groups \cite{Mostow1955,KempfNess1979,BridsonHaefliger}; it
...
and $(cc^{\dg})^{-1}$ ranges over $\mathbb P_{\mathcal C}$
...
\begin{proposition}[Canonical compatible hull]\label{prop:hull}
Let $\theta(g)=(g^{\dg})^{-1}$ and let
$C_\theta(H)$ be the smallest closed subgroup containing
$H=\Stab(J,G_0')$ and $\theta(H)$.  For $\w_1,\w_2\neq1$ (in the canonical
coordinates),
\begin{equation}
C_\theta(H)=SL(2,\C)\times SL(2,\C),
\end{equation}
the mode-preserving symplectic subgroup: the compatible hull is not merely a
convenient supergroup but the minimal $\theta$-stable subgroup forced by the
physical stabilizer.
\end{proposition}
...
generated group is then the connected $SL(2,\C)^2$.
(The Lie-closure dimensions $6$ and, at the exceptional value $\w_j=1$,
$2$, are verified computationally in the repository.)
\end{proof}
...
blocks).  This yields $\mathcal F(S_+c)\ge\mathcal F(S_+)$ with
$\operatorname{arg\,min}=\mathcal C\cap K$, for every $c$ in the hull ---
a fortiori on $H$, where the minimizers are $H\cap K$.
\end{corollary}
...
\begin{proof}
The mode-diagonal Hermitian Hamiltonian directions are the per-mode
traceless Hermitian $2\times2$ blocks ($3n$ real dimensions), and the trace
form pairs block-diagonal with block-off-diagonal matrices to zero, so the
normal space is exactly the inter-mode couplings.  Everything else is
Theorem~\ref{thm:projection} applied to $\mathcal C_n$, plus the polar-factor
argument of Theorem~\ref{thm:mindist} for the metric statement.
\end{proof}
