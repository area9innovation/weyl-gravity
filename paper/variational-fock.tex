...
stabilizer factor.  Geometrically, the theorem is an instance of an
orthogonal-projection principle for Cartan projections: the stabilizer,
itself not $\theta$-compatible, embeds in the mode-preserving
$SL(2,\C)^2$, whose positive part is a totally geodesic
$\mathbb H^3\times\mathbb H^3$ to which the Bender--Mannheim direction is
orthogonal, and the closed form is the resulting mixed hyperbolic/flat
Pythagorean law.  We then quantize the free fourth-order field modewise and
...
normal form.  We then identify the geometry behind the theorem
(Section~\ref{ssec:cartan}): it is an orthogonal-projection statement for
Cartan projections along the positive part of a $\dg$-closed subgroup
(Theorem~\ref{thm:projection}), applied to the mode-preserving
$SL(2,\C)^2\supset\Stab(J,G_0')$; the stabilizer itself fails
$\theta$-compatibility, which is why the classical Mostow--Kempf--Ness
theory does not apply to it directly.
...
$U(1)^2$ of $SO(2,\C)^2$.
(iii) The theorem is an instance of a general orthogonal-projection principle
for Cartan projections, proved in the next subsection; the elementary proof
above remains valuable because it yields the exact value
\eqref{eq:xformula}, which the geometric argument does not.
\end{remark}

\subsection{Geometric recognition: an orthogonal-projection principle for
Cartan projections}\label{ssec:cartan}

Write $\mu(S)$ for the Cartan projection of $S\in\Sp(4,\C)$: the vector
$(x_1,x_2)$, $x_1\ge x_2\ge0$, of log-singular values in the closed Weyl
chamber of type $C_2$.  Then $\mathcal F(S)=2\|\mu(S)\|^2$, and
$\mu(S_+)=(r,r)$ lies \emph{on the chamber wall} $x_1=x_2$.
Theorem~\ref{thm:mindist} is thus a minimization of the Cartan-projection
norm along a coset of the stabilizer.  It is \emph{not} an application of
the classical Mostow--Kempf--Ness theory \cite{Mostow1955,KempfNess1979} to
$H=\Stab(J,G_0')$, because $H$ is not $\theta$-compatible: it is not closed
under the adjoint, and its unitary part is generically the finite group
$\Z_2\times\Z_2$ rather than a maximal compact subgroup \cite{companion}.
The correct recognition passes through a compatible supergroup.
...
\cite{Bhatia2007,BridsonHaefliger}.
...
positive elements.  Then $\mathbb P_{\mathcal C}$ is a geodesically closed
(hence convex) subset of $\mathbb P$, and
...
\cite{Bhatia2007}.  Since $\mathcal C$ is a closed group closed under $\dg$, it contains,
with each positive $A$, the one-parameter group $A^{t}=e^{t\log A}$
(the closure of $\{A^{n}\}_{n\in\Z}$ in $GL(n,\C)$ contains a
one-parameter subgroup through $A$; equivalently, $\mathcal C$ is a real
algebraic-by-closure group and $\log A\in\mathrm{Lie}\,\mathcal C$),
in particular $A^{\pm1/2}$, hence contains the positive matrix
$M=A^{-1/2}BA^{-1/2}$ and its powers $M^{t}$, hence the whole geodesic,
which consists of positive elements of $\mathcal C$.  For the last claim:
$cc^{\dg}\in\mathbb P_{\mathcal C}$ for $c\in\mathcal C$, and conversely
$A=(A^{1/2})(A^{1/2})^{\dg}$ with $A^{1/2}\in\mathcal C$.
\end{proof}

\begin{theorem}[Projection principle]\label{thm:projection}
Let $\mathcal C\subset GL(n,\C)$ be a closed subgroup, closed under
conjugate transpose, and let $\xi=\xi^{\dg}$ satisfy the orthogonality
\begin{equation}\label{eq:orth}
\tr(\xi\,\eta)=0
\qquad\text{for every Hermitian }\eta\text{ with }e^{t\eta}\in\mathcal C .
\end{equation}
...
\|2\xi\|_F ,
\end{equation}
with equality if and only if $c$ is unitary.
\end{theorem}

\begin{proof}
By congruence invariance of the affine metric,
$d\big(I,(e^{\xi}c)^{\dg}e^{\xi}c\big)
=d\big(I,c^{\dg}e^{2\xi}c\big)
=d\big((cc^{\dg})^{-1},e^{2\xi}\big)$,
and $(cc^{\dg})^{-1}\in\mathbb P_{\mathcal C}$ by Lemma~\ref{lem:totgeo}.
It therefore suffices to show that $I$ is the unique minimizer of
$h(M)=d(M,e^{2\xi})^2$ on $\mathbb P_{\mathcal C}$.  Every point of
$\mathbb P_{\mathcal C}$ is joined to $I$ by a geodesic
$t\mapsto e^{2t\eta}$ inside $\mathbb P_{\mathcal C}$
(Lemma~\ref{lem:totgeo} with $A=I$), along which $h$ is strictly convex
unless constant; and the first variation at $t=0$ is
$-2\langle\gamma'(0),\exp_I^{-1}(e^{2\xi})\rangle\propto-\tr(\eta\,\xi)=0$
by \eqref{eq:orth}.  Hence $t=0$ is the unique minimum along every such
geodesic, so $I$ is the unique minimizer on $\mathbb P_{\mathcal C}$, with
value $d(I,e^{2\xi})^2=\|2\xi\|_F^2$.  Equality therefore forces
$(cc^{\dg})^{-1}=I$, i.e.\ $c$ unitary; the converse is immediate.
\end{proof}

\begin{corollary}[Second proof of Theorem~\ref{thm:mindist}]
\label{cor:secondproof}
Take $\mathcal C=SL(2,\C)\times SL(2,\C)$, the mode-preserving
(block-diagonal) symplectic subgroup, which is closed under $\dg$ and
contains $\Stab(J,G_0')$; its positive part
$\mathbb P_{\mathcal C}\cong\mathbb H^3\times\mathbb H^3$ is totally
geodesic, and the Gram data $(\tau_j,n_j)$ of
Lemma~\ref{lem:invariants} are polar coordinates on the two hyperbolic
factors.  The Bender--Mannheim direction $\xi=\tfrac r2 B$ is
block-off-diagonal, hence trace-orthogonal to every mode-preserving
Hermitian direction, so \eqref{eq:orth} holds and
Theorem~\ref{thm:projection} gives
$\mathcal F(S_+C)\ge\mathcal F(S_+)$ with equality iff $C$ is unitary ---
for every $C$ in the larger group $\mathcal C$, a fortiori on the
stabilizer.
\end{corollary}
...
$\arccosh(\cosh r\cosh b)$ is the hyperbolic hypotenuse law (legs $r$ and
$b$ meeting at a right angle in $\mathbb H^2$) while $a$ adds by flat
Pythagoras in the $\R$ factor.
(ii) The hypothesis \eqref{eq:orth} is sharp, not decorative: in numerical
tests, random Hermitian Hamiltonian directions $\xi$ orthogonal to the
mode-preserving block satisfy the conclusion exactly, while generic $\xi$
with nonzero block-diagonal components violate it.
(iii) The twist that makes the recognition nontrivial is that the physically
natural group --- the stabilizer of the diagonalization problem --- fails
$\theta$-compatibility, so the classical theory does not see it directly;
minimality is inherited from its compatible hull $\mathcal C$.  We expect
the same mechanism to drive the general quadratic case: whenever the
stabilizer of a positive normal form sits inside a mode-preserving
$\dg$-closed subgroup to which the diagonalizer direction is
trace-orthogonal, Theorem~\ref{thm:projection} selects the positive
diagonalizer variationally.
\end{remark}
...
instance of a projection principle for Cartan projections along
$\dg$-closed subgroups --- close in spirit to Kempf--Ness/Mostow
\cite{KempfNess1979,Mostow1955}, but applied through a compatible hull
because the stabilizer itself is not $\theta$-compatible --- while the
elementary proof supplies the minimizing value in closed form, which the
...
general semisimple quadratic PT Hamiltonians via
Theorem~\ref{thm:projection}: the stabilizer is a product of $SO(2,\C)$
factors inside a mode-preserving $\dg$-closed subgroup, and the only
model-dependent input is the trace-orthogonality of the diagonalizer
direction to that subgroup; (ii) determine whether the disjointness
...
\bibitem{Mostow1955}
G.~D.~Mostow,
\emph{Some new decomposition theorems for semi-simple groups},
Mem.\ Amer.\ Math.\ Soc.\ \textbf{14} (1955) 31.
