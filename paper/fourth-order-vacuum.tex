...
of vacuum states, and the geometry of the dynamics.  Modewise, the
...
physical vacuum ray, so the vacuum state is metric-independent; it carries
...
selection and the spectral condition determine the same quasifree state,
the two quantizations differing only in the completion.  Finally the
selected state is Hadamard in the natural fourth-order sense: its
...
$G/P_\Omega$.  The dynamical geometry is the spectral structure of the
Hamiltonian, which degenerates (Jordan blocks) exactly at the resonant
point $m_1=m_2$ \cite{companion1}.  Our results, in brief:

\begin{enumerate}
\item \emph{Parabolic decoupling} (Section~\ref{sec:decoupling}).  In
...
fixes the physical vacuum ray.  The vacuum state is therefore
...
the same quasifree state; the quantizations differ only in the
...
divergent Cartan data into bounded physical squeezing.  It does not:
numerically, $\mu(T_\w^{-1}S_+T_\w)\approx(r,r)$ as well --- the Dyson map
is genuinely non-unitary, with unbounded distortion, in every frame.  What
is bounded is the \emph{state}: the selected vacuum, computed exactly in
\cite{companion2} as the Gaussian ground state of the PT mode Hamiltonian,
has occupation \eqref{eq:N} relative to the two-field vacuum, saturating
at $1/3$.  The two statements are compatible because particle content
depends on the transformation only through the vacuum ray it produces ---
a $G/P_\Omega$ datum --- while $\mu$ is a $G/K$ datum, and the projection
$G/K\to G/P_\Omega$ collapses the noncompact Levi directions.

\begin{theorem}[Parabolic decoupling]\label{thm:decoupling}
For $G=\Sp(2n,\C)$ acting metaplectically on Fock space:
(i) positive-metric geometry is governed by $G/K$ and its Cartan
projection; (ii) Gaussian vacuum rays are governed by $G/P_\Omega$;
(iii) the Cartan norm is unbounded on $P_\Omega$ (already on its Levi
component); consequently (iv) no bound of the form
$\langle N\rangle\ge f(\|\mu\|)$ or $\le f(\|\mu\|)$ holds in general.
The PU family realizes the extreme case: divergent $\mu=(r,r)$ with
identically vanishing canonical-frame excitation, and bounded physical
excitation \eqref{eq:N} with divergent physical-frame $\mu$.
\end{theorem}

The proof of (iii) is the identity \eqref{eq:beamsplitter} together with
$\mu(e^{(r/2)B})=(r,r)$; (iv) follows from the two exhibited extremes.
This corrects a natural but false expectation --- used, e.g., to estimate
...
The selected physical vacuum state is the same for every admissible
positive metric: (i) on the Gaussian orbit $\eta_C$,
...
Two limits deserve emphasis.  First, the resonant limit is
\emph{regular} at the vacuum level: the selected Gaussian is analytic in
$\Delta$ across $\Delta=0$ ($1-f\sim\varepsilon^4$ for
$m_{1,2}=m\pm\varepsilon$), and the occupation limits commute
($\langle N\rangle\equiv\tfrac13$ at $\Delta=0$ for every $k$).  Second,
what does degenerate as $\Delta\to0$ is the metric operator
($\alpha,\beta\to\infty$), the similarity transformation
($S_+\sim\Delta^{-1/2}$), and the diagonalizability of the dynamics
(Jordan blocks; no positive invariant form \cite{companion1}).  The
resonant transition is a failure of \emph{dynamical} and \emph{metric}
geometry, not of vacuum geometry --- the third strand of
\eqref{eq:threegeom}.
...
For every $\Delta\ge0$ the Bender--Mannheim selected vacuum is the
spectral quasifree state of the fourth-order field: its two-point function
is \eqref{eq:Wspectral}.  In particular its $\Delta=0$ limit coincides,
up to the overall action-sign convention, with the Krein-space vacuum of
Bateman and Turok \cite{BT2026} (their
$\widetilde W(p)=\theta(p^0)\delta_1'(p^2)$, realized by the Jordan mode
doublet $e^{-ipx}$, $(1+2i|p|t)e^{-ipx}$ with null pairing
$[a_1,a_2^{\dg}]=[a_2,a_1^{\dg}]\ne0$).  Metric selection and the spectral
condition therefore determine the same state; the two quantizations differ
only in the completion of the field algebra representation: a positive
$\eta$-inner product for $\Delta>0$, a Krein space with ghost parity at
$\Delta=0$.
\end{theorem}
...
the sign of their perfect-square action).  Quasifree states are determined
by their two-point functions, so equality of \eqref{eq:Wmode} with the
Bateman--Turok mode Wightman function --- both equal to
...
W^+_{12}(x)=\frac{1}{8\pi^2}\log\rho+\text{(smooth)},
\end{equation}
the coefficient being universal (mass-independent) and nonzero; the
confluent case has the identical coefficient;
...
The ghost branch carries a \emph{negative} Klein--Gordon--Hadamard
two-point function: the Krein signature is intrinsic to the selected
state, while its local singularity structure is standard on both
branches.  This makes the summary statement precise:
...
spectral, fourth-order Hadamard, metric-independent, and not a state in
the naive two-field (graviton plus ghost) Fock space; PT-based unitarity
