...
There is a stronger categorical statement, but its Bach-module application
has one additional algebraic gate.
...
The repository certifies the generic nonsplit Bach self-extension, but it
does not yet contain a generic simplicity or full endomorphism-ring
certificate for \(M_{\rm RW}\).  Therefore
Lemma~\ref{lem:simple-extension-involution} is exact, while the stronger
conclusion that the Bach block admits only \(\pm I\) remains conditional on
that gate.  Proposition~\ref{prop:no-local-c}, which excludes the
branch-resolving involution directly from the cocycle equation, is
unconditional.  This distinction also separates endpoint Krein
diagonalization, which is algebraic, from invariant bulk diagonalization,
which would require differential-module projectors.
...
Proposition~\ref{prop:no-local-c}, this shows that any successful dynamical
construction cannot be a local rational branch-resolving involution.  A
...
Simple-self-extension involution lemma &
Exact categorical theorem; Bach application conditional &
Characteristic zero and a simple composition factor; generic
Regge--Wheeler simplicity is not certified\\
...
Jordan block;
\item generic simplicity or the complete differential endomorphism ring of
the Regge--Wheeler module, and hence the unconditional claim that every
rational involution of the Bach spin-two block is \(\pm I\);
