...
\end{equation}
The dynamical operator data in Equation~\eqref{eq:mixing} remain over
$\Q(\sqrt{10})$.  The normalization in Equation~\eqref{eq:eo} requires the
exact scalar extension $\Q(\sqrt2,\sqrt{10})$, since
$1/\sqrt2\notin\Q(\sqrt{10})$.  The versioned V2 input contract records these
fields separately rather than weakening the normalization or rewriting V1.
