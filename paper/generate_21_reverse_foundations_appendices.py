...
def build(data: dict, assemblies: dict) -> str:
    ...
    lines.extend(
        [
            ...
            "The completion atlas keeps eleven lifecycle stages separate on seven candidate architectures.  None of the branches is a completed Lorentzian quantum theory.  The strict 386-row branch now has an exact graph-coordinate unary differential, SDR and transported suspension, but its authoritative classical-import stage remains fail closed until represented Green actions and the composed causal homotopy are bound into one accepted object.",
            "",
            r"\begingroup",
            r"\scriptsize",
            r"\setlength{\tabcolsep}{3pt}",
            r"\begin{longtable}{@{}rp{0.25\textwidth}p{0.12\textwidth}p{0.12\textwidth}p{0.39\textwidth}@{}}",
            r"\caption{Ranked next certificates in Lorentzian Weyl BV completion atlas V14.}\label{tab:lorentzian-weyl-completion-routes}\\",
            r"\toprule",
    ...
    lines.extend(
        [
            ...
            r"\endgroup",
            "",
            rf"The graph-coordinate unary object contains {completion['strict_graph_q1_sdr_component_jets']['operator_tables']} operator tables and {completion['strict_graph_q1_sdr_component_jets']['nonzero_rational_coefficients']:,} exact nonzero coefficients.  Its SDR contains {completion['strict_graph_q1_sdr_component_jets']['H_alg_nonzero_entries']} homotopy entries and replays all {completion['strict_graph_q1_sdr_component_jets']['combined_derivative_multiindices']} unary multiindices with zero direct retract defects.  The transported suspension has {completion['strict_graph_q1_sdr_component_jets']['transported_suspension_entries']} entries; the obsolete diagonal form has {completion['strict_graph_q1_sdr_component_jets']['old_diagonal_suspension_cyclicity_defects']} graph-cyclicity defects.  Its first-ranked successor is \cert{{{completion['route_selection'][0]['route']}}}; represented Green actions, Hadamard and QME work remain downstream of the classical import gate.",
            "",
