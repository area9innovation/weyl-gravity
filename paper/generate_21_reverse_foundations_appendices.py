...
def build(data: dict, assemblies: dict) -> str:
    ...
    lines.extend(
        [
            ...
            rf"The completion atlas keeps eleven lifecycle stages separate on seven candidate architectures.  None of the branches is a completed Lorentzian quantum theory.  The strict 386-row branch now has an exact graph-coordinate unary differential, a common-bound local endpoint SDR, transported suspension, represented advanced/retarded Green names, the local cylinder-flow action, and linked common source q2/q3 snapshots.  The M3L manifest pins {len(endpoint_sdr_binding['common_manifest']['artifact_pins'])} artifacts and {len(endpoint_sdr_binding['common_manifest']['object_hashes'])} canonical object hashes; all {endpoint_sdr_binding['exact_replay']['compatibility_links_checked']} compatibility links agree and every projected defect is zero.  The arity-two layer has zero q1/q2, cyclicity, and stationary D/q2 defects.  The arity-three layer contains {completion['strict_source_q3_common_assembly']['source_q3_families']} exhaustive source families and {completion['strict_source_q3_common_assembly']['auxiliary_ordered_q3_coefficients']:,} auxiliary coefficients; its arity-three, cyclicity modulo horizontal boundary, and stationary D/q3 defects are all zero.  Gate A and q2/q3 Green compatibility remain fail closed.",
            "",
            r"\begingroup",
            r"\scriptsize",
            r"\setlength{\tabcolsep}{3pt}",
