...
def build(data: dict, assemblies: dict) -> str:
    ...
    lines.extend(
        [
            ...
            r"The completion atlas keeps eleven lifecycle stages separate on seven candidate architectures.  None of the branches is a completed Lorentzian quantum theory.  The strict 386-row branch now has an exact graph-coordinate unary differential, SDR, transported suspension, represented advanced/retarded Green names, the local cylinder-flow action, exact cyclic q2 and q3 stabilizations, both first nonlinear causal-response names, honest support domains for every finite polarized q2 tree, unique Catalan-weighted q2 formal fixed-point coefficients, and the correctly typed field-equation inverse on Noether-compatible sources and fields modulo gauge.  The q3 candidate transports the complete 72-channel/212-path arity-three identity, S4 cyclicity modulo boundary, and D/q3 derivation with zero defects.  A full ungauge-fixed inverse is exactly obstructed.  The local source-to-split shift supplies the complete curved hh/hv/vv quadratic field map and its exact 386-row cotangent lift, while the three source-forced auxiliary Diff representations have exact cotangent and momentum-map rows.  The primary-source nonlinear Weyl/boost manifest proves that the seven serialized auxiliary cubic families are exhaustive in the declared scope: the internal algebra is Abelian and the shifted auxiliary tensor is invariant.  This census is not an assembled source operator.  The common 386-row source q2/q3 payload and its q1/q2, cyclicity, and D-equivariance replays remain open.  Gate A and general lambda-squared causal source closure remain fail closed.",
            "",
            r"\begingroup",
            r"\scriptsize",
            r"\setlength{\tabcolsep}{3pt}",
            r"\begin{longtable}{@{}rp{0.25\textwidth}p{0.12\textwidth}p{0.12\textwidth}p{0.39\textwidth}@{}}",
            r"\caption{Ranked next certificates in Lorentzian Weyl BV completion atlas V31.}\label{tab:lorentzian-weyl-completion-routes}\\",
            r"\toprule",
    ...
    lines.extend(
        [
            ...
            "",
            rf"The represented Green result uses {completion['strict_graph_green_action_name']['spatial_spectral_branches']} canonical Hodge spectral branches and a rank-{completion['strict_graph_green_action_name']['tractor_rank']} parent action, with the zero mode explicit.  Its distinct retarded and advanced names act on the endpoint and full graph, but are neither effective numerical solvers nor serialized distribution kernels.  The original unary-causal snapshot binds {completion['strict_unary_causal_common_snapshot']['accepted_hashes']} hashes on {completion['strict_unary_causal_common_snapshot']['carrier_rows']} rows; the exact real cylinder flow \(T=\partial_t\) extends this to {completion['strict_full_d_action']['scoped_snapshot_hashes']} hashes.  Its {completion['strict_full_d_action']['D_coefficients']} local entries commute with all {completion['strict_full_d_action']['q1_coefficients_checked']:,} checked q1 coefficients and are formally skew on {completion['strict_full_d_action']['pairing_entries_checked']} pairing entries, with zero defects in both replays.  The stabilized q2 action DAG has {completion['strict_stabilized_q2_lift_preflight']['expanded_component_channels']} ordered-component channels, {completion['strict_stabilized_q2_lift_preflight']['unique_block_triples']} block triples, and {completion['strict_stabilized_q2_lift_preflight']['input_row_envelope']}-row input/output envelopes; q1/q2, Koszul, cyclicity and D/q2 defects are all zero for the candidate.  Both first causal-response orientations compose on the same {completion['strict_q2_green_composition_preflight']['carrier_rows']} rows, with response and causal-difference defect counts {completion['strict_q2_green_composition_preflight']['response_identity_defects']} and {completion['strict_q2_green_composition_preflight']['causal_difference_identity_defects']}.  The PC/FC extension closes every finite polarized retarded or advanced candidate tree.  At four leaves, {completion['strict_recursive_causal_tree_domains']['four_leaf_admissible']} of {completion['strict_recursive_causal_tree_domains']['four_leaf_all_sign_decorations']} sign decorations are uniformly defined; the remaining {completion['strict_recursive_causal_tree_domains']['four_leaf_not_uniformly_defined']} expose the opposite-polarity Green-domain boundary.  The polarized trees now assemble into two unique lambda-adic coefficient families, checked through {completion['strict_polarized_formal_coefficients']['checked_through_leaves']} leaves with {completion['strict_polarized_formal_coefficients']['largest_checked_tree_count']:,} trees at the largest checked order.  This is formal stabilization, not analytic convergence.  The degree-one-to-zero Green component maps {completion['strict_field_equation_green_quotient_inverse']['equation_rows']} equation rows to {completion['strict_field_equation_green_quotient_inverse']['field_rows']} field rows.  It is a right inverse on Noether-compatible sources and a left inverse modulo gauge; the {completion['strict_field_equation_green_quotient_inverse']['gauge_nonzero_coefficients']} nonzero gauge coefficients and {completion['strict_field_equation_green_quotient_inverse']['noether_nonzero_coefficients']} nonzero Noether coefficients exactly obstruct a full ungauge-fixed inverse.  On the exact q1-closed fixture \cert{{{completion['strict_pure_weyl_q3_witness']['fixture_id']}}}, direct cubic Bach differentiation gives {completion['strict_pure_weyl_q3_witness']['metric_q3_term_count']} exact rational terms on {completion['strict_pure_weyl_q3_witness']['metric_q3_nonzero_rows']} metric rows and q1(q3) value \({completion['strict_pure_weyl_q3_witness']['computed_q1_q3_weyl_identity_value']}\).  The arity-three and full lambda-squared witness-source defects are \({completion['strict_pure_weyl_q3_witness']['arity_three_witness_defect']}\) and \({completion['strict_pure_weyl_q3_witness']['lambda2_witness_source_q1_defect']}\).  This is a receiver-derived diagonal metric-sector witness, not an arbitrary-input full-BV q3.  The complete Berger q3 has disposition \cert{{{completion['strict_pure_weyl_q3_witness']['Berger_disposition']}}} because it belongs to a different theory, background and carrier; no nonexistence claim is made.  Gate V13 accepts {completion['strict_gate_v13_reconciliation']['accepted_top_level_hashes']} authoritative hashes after the exhaustive scoped family census.  The first three successors are \cert{{{completion['route_selection'][0]['route']}}}, \cert{{{completion['route_selection'][1]['route']}}}, and \cert{{{completion['route_selection'][2]['route']}}}; general nonlinear source closure, residual data, Hadamard and QME work remain open.",
            "",
            rf"Atlas V31 records {completion['strict_diff_auxiliary_bv_representation']['known_required_cubic_families']} auxiliary cubic families: {completion['strict_diff_auxiliary_bv_representation']['component_complete_families']} are component-complete and {completion['strict_diff_auxiliary_bv_representation']['component_open_families']} remain open.  The exact curved field map contains {completion['strict_hh_hv_auxiliary_cotangent_lift']['hh_field_coefficients']:,} hh, {completion['strict_hh_hv_auxiliary_cotangent_lift']['hv_field_coefficients']} hv and {completion['strict_hh_hv_auxiliary_cotangent_lift']['vv_field_coefficients']} vv coefficients, while formal adjoint collection produces {completion['strict_hh_hv_auxiliary_cotangent_lift']['combined_cotangent_coefficients']:,} cotangent coefficients.  The three Diff families add {completion['strict_diff_auxiliary_bv_representation']['master_density_coefficients']} master-density, {completion['strict_diff_auxiliary_bv_representation']['field_output_coefficients']} field, {completion['strict_diff_auxiliary_bv_representation']['antifield_output_coefficients']} antifield and {completion['strict_diff_auxiliary_bv_representation']['c_star_output_coefficients']} momentum-map coefficients, with zero variational and Koszul defects.  The primary-source manifest contains {completion['strict_nonlinear_weyl_boost_ghost_manifest']['nonzero_ghost_antifield_families']} nonzero ghost-antifield families, certifies {completion['strict_nonlinear_weyl_boost_ghost_manifest']['certified_zero_candidate_families']} candidate families as zero, and requires {completion['strict_nonlinear_weyl_boost_ghost_manifest']['additional_nonlinear_Weyl_boost_ghost_antifield_families']} additional internal Weyl/boost families.  Gate V13 accepts {completion['strict_gate_v13_reconciliation']['accepted_top_level_hashes']} authoritative hashes.  The family census is exhaustive in its declared source scope, but the complete common source q2/q3 assembly and identity replay remain open.",
            "",
