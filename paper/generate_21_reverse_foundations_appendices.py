...
def build(data: dict, assemblies: dict) -> str:
    ...
    lines.extend(
        [
            ...
            "The completion atlas keeps eleven lifecycle stages separate on seven candidate architectures.  None of the branches is a completed Lorentzian quantum theory.  The strict 386-row branch now has an exact graph-coordinate unary differential, SDR, transported suspension and represented advanced/retarded Green names bound into one accepted unary-causal snapshot.  That scoped snapshot is deliberately smaller than the authoritative classical-import freeze, which remains fail closed for q2, D and the residual data.",
            "",
            r"\begingroup",
            r"\scriptsize",
            r"\setlength{\tabcolsep}{3pt}",
            r"\begin{longtable}{@{}rp{0.25\textwidth}p{0.12\textwidth}p{0.12\textwidth}p{0.39\textwidth}@{}}",
            r"\caption{Ranked next certificates in Lorentzian Weyl BV completion atlas V15.}\label{tab:lorentzian-weyl-completion-routes}\\",
            r"\toprule",
    ...
    lines.extend(
        [
            ...
            "",
            rf"The represented Green result uses {completion['strict_graph_green_action_name']['spatial_spectral_branches']} canonical Hodge spectral branches and a rank-{completion['strict_graph_green_action_name']['tractor_rank']} parent action, with the zero mode explicit.  Its distinct retarded and advanced names act on the endpoint and full graph, but are neither effective numerical solvers nor serialized distribution kernels.  The accepted unary-causal snapshot binds {completion['strict_unary_causal_common_snapshot']['accepted_hashes']} hashes on {completion['strict_unary_causal_common_snapshot']['carrier_rows']} rows.  It accepts {completion['strict_unary_causal_common_snapshot']['gate_a_hashes_accepted_by_scoped_result']} of the authoritative Gate A's {completion['strict_unary_causal_common_snapshot']['gate_a_hashes_required']} top-level hashes.  The first-ranked successor is \cert{{{completion['route_selection'][0]['route']}}}; the full D action, same-carrier q2, residual data, Hadamard and QME work remain open.",
            "",
