...
def build(data: dict, assemblies: dict) -> str:
    ...
    lines.extend(
        [
            ...
            "The completion atlas keeps eleven lifecycle stages separate on seven candidate architectures.  None of the branches is a completed Lorentzian quantum theory.  The strict 386-row branch now has an exact graph-coordinate unary differential, SDR, transported suspension, represented advanced/retarded Green names, the local cylinder-flow action, an exact cyclic q2 stabilization, and both first nonlinear causal-response names.  The stabilization and responses are receiver constructions, not an authoritative classical import or recursive interacting theory; Gate A remains fail closed for source-certified q2 theory identity and the residual data.",
            "",
            r"\begingroup",
            r"\scriptsize",
            r"\setlength{\tabcolsep}{3pt}",
            r"\begin{longtable}{@{}rp{0.25\textwidth}p{0.12\textwidth}p{0.12\textwidth}p{0.39\textwidth}@{}}",
            r"\caption{Ranked next certificates in Lorentzian Weyl BV completion atlas V18.}\label{tab:lorentzian-weyl-completion-routes}\\",
            r"\toprule",
    ...
    lines.extend(
        [
            ...
            "",
            rf"The represented Green result uses {completion['strict_graph_green_action_name']['spatial_spectral_branches']} canonical Hodge spectral branches and a rank-{completion['strict_graph_green_action_name']['tractor_rank']} parent action, with the zero mode explicit.  Its distinct retarded and advanced names act on the endpoint and full graph, but are neither effective numerical solvers nor serialized distribution kernels.  The original unary-causal snapshot binds {completion['strict_unary_causal_common_snapshot']['accepted_hashes']} hashes on {completion['strict_unary_causal_common_snapshot']['carrier_rows']} rows; the exact real cylinder flow \(T=\partial_t\) extends this to {completion['strict_full_d_action']['scoped_snapshot_hashes']} hashes.  Its {completion['strict_full_d_action']['D_coefficients']} local entries commute with all {completion['strict_full_d_action']['q1_coefficients_checked']:,} checked q1 coefficients and are formally skew on {completion['strict_full_d_action']['pairing_entries_checked']} pairing entries, with zero defects in both replays.  The stabilized q2 action DAG has {completion['strict_stabilized_q2_lift_preflight']['expanded_component_channels']} ordered-component channels, {completion['strict_stabilized_q2_lift_preflight']['unique_block_triples']} block triples, and {completion['strict_stabilized_q2_lift_preflight']['input_row_envelope']}-row input/output envelopes; q1/q2, Koszul, cyclicity and D/q2 defects are all zero for the candidate.  Both first causal-response orientations now compose on the same {completion['strict_q2_green_composition_preflight']['carrier_rows']} rows, with response and causal-difference defect counts {completion['strict_q2_green_composition_preflight']['response_identity_defects']} and {completion['strict_q2_green_composition_preflight']['causal_difference_identity_defects']}.  This uses completed infinite spaces even though the local q2 layer is finite exact, and recursive causal trees remain uncertified.  Gate V7 nevertheless accepts {completion['strict_gate_v7_reconciliation']['accepted_top_level_hashes']} of seven authoritative hashes because the source theory-identity bridge is missing.  The first-ranked successor is \cert{{{completion['route_selection'][0]['route']}}} and the second is \cert{{{completion['route_selection'][1]['route']}}}; authoritative q2 identity, recursive domains, residual data, Hadamard and QME work remain open.",
            "",
