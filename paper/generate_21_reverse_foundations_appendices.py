...
def build(data: dict, assemblies: dict) -> str:
    ...
    lines.extend(
        [
            ...
            "The completion atlas keeps eleven lifecycle stages separate on seven candidate architectures.  None of the branches is a completed Lorentzian quantum theory.  The strict 386-row branch now has a complete unary snapshot, an exact split local SDR, and an exact fixed-basis canonical shear, but its authoritative classical-import stage remains fail closed until graph-coordinate replay and represented Green actions are bound into one accepted object.",
            "",
            r"\begingroup",
            r"\scriptsize",
            r"\setlength{\tabcolsep}{3pt}",
            r"\begin{longtable}{@{}rp{0.25\textwidth}p{0.12\textwidth}p{0.12\textwidth}p{0.39\textwidth}@{}}",
            r"\caption{Ranked next certificates in Lorentzian Weyl BV completion atlas V13.}\label{tab:lorentzian-weyl-completion-routes}\\",
            r"\toprule",
    ...
    lines.extend(
        [
            ...
            r"\endgroup",
            "",
            rf"The current unary object contains {completion['strict_full_q1_component_jet_table']['operator_tables']} operator tables and {completion['strict_full_q1_component_jet_table']['nonzero_rational_coefficients']:,} exact nonzero coefficients.  Its split local SDR contains {completion['strict_local_sdr_component_maps']['H_alg_nonzero_entries']} homotopy entries and replays all {completion['strict_local_sdr_component_maps']['derivative_multiindices_checked']} unary multiindices with zero defects.  The separate canonical shear and inverse each contain {completion['strict_canonical_shear_component_jets']['forward_table_count']} off-diagonal tables and {completion['strict_canonical_shear_component_jets']['forward_nonzero_off_diagonal_coefficients']:,} exact coefficients.  Its first-ranked successor is \cert{{{completion['route_selection'][0]['route']}}}; graph-coordinate replay, represented Green actions, Hadamard and QME work remain downstream of the classical import gate.",
            "",
