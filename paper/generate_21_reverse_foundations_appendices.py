...
def build(data: dict, assemblies: dict) -> str:
    ...
    m3rc_action_support_dual = data["completion_m3rc_action_support_dual_identification"]
    typed_residual_cyclicity = data["completion_typed_residual_cyclicity"]
    m1_preflight = completion["strict_m1_common_snapshot_preflight"]
    m1a_local = completion["strict_m1a_local_semantic_extension"]
    ...
    nonlinear_green = completion["strict_m2_q2_q3_typed_green_compatibility"]
    hadamard = completion["strict_386_brst_hadamard_two_point"]
    ...
    lines.extend(
        [
            ...
            rf"The completion atlas keeps eleven lifecycle stages separate on seven candidate architectures.  None of the branches is a completed Lorentzian quantum theory.  The strict 386-row branch now has an exact graph-coordinate unary differential, a common-bound local endpoint SDR, transported suspension, represented advanced/retarded Green names, the local cylinder-flow action, linked common source q2/q3 snapshots, and a complete local cyclic pairing.  The M3L manifest pins {len(endpoint_sdr_binding['common_manifest']['artifact_pins'])} artifacts and {len(endpoint_sdr_binding['common_manifest']['object_hashes'])} canonical object hashes; all {endpoint_sdr_binding['exact_replay']['compatibility_links_checked']} compatibility links agree and every projected defect is zero.  M4L has exact pairing rank {local_cyclic_pairing['pairing_replay']['exact_rational_rank']} on {local_cyclic_pairing['pairing_replay']['carrier_rows']} local rows with {local_cyclic_pairing['pairing_replay']['nonzero_ordered_pairing_entries']} ordered entries, and all declared q1/SDR/D/q2/q3 local cyclicity defects vanish.  M3R compares {residual_comparison['comparison']['source']['total_dimension']:,} represented endpoint-complex coefficients with {residual_comparison['comparison']['target']['dimension']} W+/W- residual coordinates at energies two through six; its E/A/L magnetic crosswalk, retraction, and q0 chain identities have zero defects.  Direct pullback to that one-sided degree-zero target is obstructed: the induced degree-minus-one odd form has exact rank {residual_cyclic_obstruction['obstruction_replay']['pulled_back_odd_pairing_rank']} and nullity {residual_cyclic_obstruction['obstruction_replay']['pulled_back_odd_pairing_nullity']}.  M3RC-A repairs the carrier formally: the unchanged {dfinite_cotangent_dual['same_source_impossibility']['original_source_full_dimension']:,}-coordinate source has H0={dfinite_cotangent_dual['same_source_impossibility']['original_source_degree_zero_cohomology_dimension']} and H1={dfinite_cotangent_dual['same_source_impossibility']['original_source_degree_one_cohomology_dimension']}, so it cannot retract onto the doubled residual carrier; the declared {dfinite_cotangent_dual['formal_cotangent_completion']['full_dimension']:,}-coordinate shifted cotangent complex instead retracts exactly onto {dfinite_cotangent_dual['formal_cotangent_completion']['residual_dimension']} coordinates with full/residual pairing ranks {dfinite_cotangent_dual['formal_cotangent_completion']['full_pairing_rank']:,}/{dfinite_cotangent_dual['formal_cotangent_completion']['residual_pairing_rank']} and zero declared defects.  M3RC-B gives all {m3rc_action_support_dual['support_dual_identification']['compact_source_representatives']} formal residual duals explicit compact-source causal representatives and identifies the action/Green and canonical cotangent pairings at exact rank {m3rc_action_support_dual['action_pairing_identification']['phase_pairing_rank']}.  M4R then replays five shifted-cotangent blocks and verifies q-res cyclicity, projection-adjointness, homotopy skewness, inclusion isometry, contraction and normalized side conditions with {typed_residual_cyclicity['exact_cyclic_replay']['all_identity_defects']} defects.  These are finite represented theorems, not an all-energy completion or an identification of the formal comparison source with the authoritative full BV source.  M1A is now complete.  M1A2 typed all {m1a_local['local_386_rows_fully_namespaced']} local rows and its {m1a_local['cotton_component_checks']:,}-component Cotton replay has {m1a_local['cotton_component_defects']} defects.  M1A3 types {m1a_represented['represented_endpoint_rows']:,} represented endpoint rows, excludes {m1a_represented['excluded_test_rows']} comparison-only test rows arranged in {m1a_represented['excluded_test_doublets']} doublets, and types {m1a_represented['action_residual_primal_rows']} primal plus {m1a_represented['action_residual_dual_rows']} action-dual residual rows with zero partition, degree and crosswalk defects.  M1A4 freezes {m1a_ledger['authoritative_rows_total']:,} authoritative rows across {m1a_ledger['authoritative_carrier_objects']} distinct content-addressed objects, with {m1a_ledger['untyped_authoritative_rows']} untyped rows and {m1a_ledger['category_identification_defects']} category-identification defects.  The formal 8,980-row cotangent fixture remains non-authoritative.  M1B, M1C, Gate A and q2/q3 Green compatibility remain fail closed.",
            "",
            r"\begingroup",
            r"\scriptsize",
            r"\setlength{\tabcolsep}{3pt}",
    ...
    lines.extend(
        [
            ...
            "",
            rf"The represented Green result uses {completion['strict_graph_green_action_name']['spatial_spectral_branches']} canonical Hodge spectral branches and a rank-{completion['strict_graph_green_action_name']['tractor_rank']} parent action, with the zero mode explicit.  Its retarded and advanced names are neither effective numerical solvers nor serialized distribution kernels.  The unary-causal snapshot binds {completion['strict_unary_causal_common_snapshot']['accepted_hashes']} hashes on {completion['strict_unary_causal_common_snapshot']['carrier_rows']} rows, and the exact cylinder flow extends this to {completion['strict_full_d_action']['scoped_snapshot_hashes']} hashes.  The accepted source q2 has {completion['strict_source_q2_common_assembly']['source_q2_families']} families and {completion['strict_source_q2_common_assembly']['auxiliary_ordered_component_coefficients']:,} auxiliary coefficients; q1/q2, cyclicity and D/q2 have zero defects.  The accepted source q3 has {completion['strict_source_q3_common_assembly']['source_q3_families']} exhaustive families and {completion['strict_source_q3_common_assembly']['auxiliary_ordered_q3_coefficients']:,} auxiliary coefficients, closing {completion['strict_source_q3_common_assembly']['cyclic_equalities_checked']:,} pointwise cyclic equalities and {completion['strict_source_q3_common_assembly']['Weyl_Ward_checks']} conformal Ward checks.  Gate V30 verifies the immutable snapshot.  Post-freeze M2 checks both Green orientations and arities {joined([str(value) for value in nonlinear_green['nonlinear_arities']])} on all {nonlinear_green['carrier_rows']} rows with {nonlinear_green['exact_or_structural_defects']} exact or structural defects.  It covers every finite same-orientation q2/q3 tree and the general second nonlinear source cocycle, but neither arbitrary mixed-orientation trees nor convergence of the infinite series.",
            "",
            rf"The full-complex free quantum successor constructs a {tex(hadamard['object_type'])} on all {hadamard['carrier_rows']} rows.  It passes {hadamard['proof_obligations']} bisolution, graded-CCR, microlocal Hadamard, BRST-Ward, adjoint, zero-mode and coverage obligations with {hadamard['proof_defects']} defects.  The scalar Hodge zero mode is retained as a smooth scale-free split.  This is deliberately not called a state: positivity and physical-cohomology positivity are both false claim flags.  The first three remaining routes are \cert{{{completion['route_selection'][0]['route']}}}, \cert{{{completion['route_selection'][1]['route']}}}, and \cert{{{completion['route_selection'][2]['route']}}}; renormalized Lorentzian products, QME restoration and residual quantum transfer remain open.",
            "",
            rf"Atlas V47 records the complete local and represented M1A typed diagram and the M1B primal composite in addition to the arity-two and arity-three source assemblies, residual zero-mode coefficients, centered cohomology payload, typed M3L/M3R/M3RC/M4L/M4R chain, and its support-locality boundaries.  The primal composite contracts {m1b_primal['represented_endpoint_rows']:,} represented endpoint coordinates onto {m1b_primal['primal_residual_rows']} modes, with {m1b_primal['q0_nonzero_entries']:,} differential and {m1b_primal['homotopy_nonzero_entries']:,} homotopy entries and zero exact or formal-composition defects.  Its graph factor is support-local but its harmonic restriction is not.  The residual package contains {len(residual_zero_modes['zero_mode_basis']['canonical_generator_order'])} primal and {len(residual_zero_modes['zero_mode_basis']['canonical_dual_order'])} dual modes, {residual_zero_modes['so42_structure_constants']['nonzero_entries']} nonzero structure coefficients, and zero defects in every declared replay.  The centered C3/C4/C5 bases have dimensions {centered['ordered_centered_cochain_basis']['degrees']['3']['dimension']:,}, {centered['ordered_centered_cochain_basis']['degrees']['4']['dimension']:,}, and {centered['ordered_centered_cochain_basis']['degrees']['5']['dimension']:,}; the receiver proves \(\dim H^4={centered['centered_differential_summary']['cohomology_dimension_H4']}\).  Gate V29 remains fail closed until M1B supplies the action-dual lift and typed cyclic replay and M1C replays all ten checks on the common bytes.",
            "",
    ...
    rendered = rendered.replace(
        ...
        "The typed nonlinear Green and full-complex BRST Hadamard pseudo-state layers are now certified; positivity, renormalized products and QME work remain open.",
    )
    rendered = rendered.replace(
        "Atlas V47 records the complete local and represented M1A typed diagram and the M1B primal composite",
        "Atlas V49 records the immutable M1A/M1B/M1C classical snapshot together with its post-freeze nonlinear causal and free Hadamard layers",
    )
    rendered = rendered.replace(
    ...
    rendered = rendered.replace(
        ...
        f"Gate V30 verifies Gate A on snapshot {tex(m1c['snapshot_id'])}.  The separately certified M2 and Hadamard successors preserve that immutable source; neither promotes physical positivity, interacting products or a Lorentzian QME.",
    )
    return rendered
