...
an order-$\lambda$ Jordan/distributional asymptotic map, not yet constructed.
...
\texttt{NOT\_CONSTRUCTED}.
The loop-informed hard scattering result is pinned by
\texttt{REVERSE\_PHYSICS\_BT\_UV\_HARD\_}\allowbreak
\texttt{SCATTERING\_LAW\_V1.json}.  It imports the content-addressed Born,
...
A double-secular/Jordan denominator can remain finite, but the corresponding
order-$\lambda$ part of the BT map $R_t$, its background charge bookkeeping,
incoming sectors, continuum domain, and physical NLO probability are not
constructed.  The bare broken-vacuum cubic has charge $-1$, so neutrality also
cannot be inferred before that missing bookkeeping is supplied.
\end{remark}
