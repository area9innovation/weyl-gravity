...
An exact finite-carrier successor changes the measured object: coherent
hard--collinear projector transport with amplitude $\sqrt{1/512}$ per pair
forces a hard normalization $-3/512$ against the three-pair Gram response
$+3/512$.  This removes the finite algebraic normalization barrier, while the
dynamical asymptotic-Hamiltonian and continuum constructions remain open.
\end{abstract}
...
The finite coherent successor is pinned by
\texttt{REVERSE\_PHYSICS\_BT\_COLLINEAR\_}
\allowbreak\texttt{PROJECTOR\_TRANSPORT\_V1.json}.  Its producer works over
$\mathbb Q(\sqrt2)$, and an independent verifier solves the order-two
projector equation without importing that producer.  The certificate labels
the asymptotic-Hamiltonian derivation and continuum projector
...
Suppress the common factor $\lambda^6\log c/(\pi^4s)$ and place one hard
channel and the three unordered collinear-pair channels in the carrier
$(h,r_{12},r_{13},r_{23})$.  Let
\[
 P_0=\operatorname{diag}(1,0,0,0),\qquad
 K=\begin{pmatrix}
 0&-a&-a&-a\\ a&0&0&0\\ a&0&0&0\\ a&0&0&0
 \end{pmatrix},\qquad a=\frac{\sqrt2}{32}.
\]
...
\[
 (P_2)_{hh}=-3a^2=-\frac3{512},\qquad
 \sum_{r=1}^3(P_2)_{rr}=3a^2=\frac3{512}.
\]
...
not an adjustable weight.  A neutral generator passes the finite one-sided BT
relative-radical criterion, recomputed in the new independent rail.  This is an exact finite
existence witness at the imported reduced-mode coefficient.  It neither
derives $K$ from the BT/PS asymptotic Hamiltonian nor constructs a continuum
dressed projector or physical NLO probability.
\end{remark}
...
The finite coherent-projector successor is reproduced by
\begin{verbatim}
ulimit -v 500000; python3 \
  reverse_physics/bt_collinear_projector_transport.py --check
ulimit -v 500000; python3 \
  reverse_physics/verify_bt_collinear_projector_transport.py
ulimit -v 500000; python3 -m unittest -v \
  reverse_physics.tests.test_bt_collinear_projector_transport
\end{verbatim}
