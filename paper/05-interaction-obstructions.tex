...
quadratic map.  This does not prove a physical zero: the squeezed-vacuum and
dynamical-zero-mode projector trace, the continuum domain, and higher orders
remain open.
...
limit prevents promotion of this finite-regulator zero to a physical zero.
...
unproved.
For an orthogonal inclusive detector, however, the leading probability is
fixed without higher map data: if $U=1+\lambda K+\lambda^2L+\cdots$, its
order-$\lambda^2$ transition weight contains only
$(P_{\rm out}KP_{\rm in})^\dagger(P_{\rm out}KP_{\rm in})$.  The complete
signed kernel makes this integrand pointwise zero.  Squeeze similarity,
neutral zero-mode normalization, and every detector characteristic-function
limit preserve zero.  Thus the physical leading real-collinear generalized-
Born coefficient is zero rather than $1/48$; the full virtual/hard NLO sum and
all-order Eq.~(19) are separate open problems.
...
not survive the complete public quadratic map.  No conclusion about the
physical coefficient follows without the squeezed-vacuum/dynamical-zero-mode
projector trace, its continuum domain, and higher composite orders.
...
\texttt{LOCAL-ALGEBRAIC}, \texttt{REDUCED-MODE} zero is finite-regulator only:
the non-trace-class thermodynamic limit, dynamical zero mode, and higher
orders still block a physical zero.
...
Orthogonality nevertheless suffices for the leading real transition.  The
order-$\lambda^2$ Born weight is determined only by the complete order-
$\lambda$ kernel, independently of unknown higher amplitudes.  Its signed
parent trace vanishes before endpoint removal, and therefore for every
inclusive detector characteristic function.  Certificate
\texttt{REVERSE\_PHYSICS\_BT\_INCLUSIVE\_PHYSICAL\_COEFFICIENT\_V1}
records the physical real-collinear coefficient as zero, while leaving the
complete virtual/hard NLO probability and all-order Eq.~(19) open.
\end{remark}
