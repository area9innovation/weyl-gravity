...
Last substantive update: 19 July 2026.
...
| Is there a clock-defined redshift observable? | An actual retarded Maxwell signal with compact spacetime source support gives a Diff-, Weyl-, Maxwell-gauge-, and raw-\(D\)-invariant clock-slice field-strength observable; its exact spatially global fixture has \(1+z=2\). Two localized emitters produce a leading rank-two detector matrix. Response-specific shell streaming, exact finite kernels, and partition-refined Cauchy covectors certify leading rank two uniformly for \(m_0^2,m_1^2\in[1,2]\); the first retarded emitter channel is executable. | **Global relational observable and finite mass-uniform leading response certified** | The complete all-shell massive image, common emitter comparison, absolute \(q_3\) feedback bounds, recoil, backreaction, phenomenology, and quantum interpretation remain open. |
...
| Is the Einstein sector nonlinearly closed? | Every nonzero pure-extra generic Weyl--Maxwell tangent is Taub-obstructed on the tested fixed compact bundle. On the complete certified finite-support inventory—including generic Einstein/extra modes, standard/extra exceptional dipoles, the twist Jordan block and homogeneous data—a smooth exponential-polynomial second-order correction exists exactly when the five stabilizer moment maps vanish. The standard generalized-zero bounded cone is now exact: \(a=b=Q_e=0\), \(B=0\), with \(c,d,W_x,A\) free; moreover \(b=B=0\) is necessary on the full finite-support bounded cone. | **Complete finite-support smooth cone and standard-global bounded subcone certified; full bounded cone partial** | Smooth continuation may grow and is not stable repeating motion. The remaining oscillator polynomial ideal and shell resonances, infinite inputs, causal/retarded corrections, all-background structure, and all-orders closure remain open. |
...
| Is the quantum theory anomaly-free and unitary? | The strict one-loop insertion has coefficients \((199/30,-87/20)\), so strict field content is obstructed. The tau-adic compensator restores the local Euclidean QME at one loop. The anomaly-induced representative, universal \(-199/60\) \(C^2\log\) coefficient, and exact \(\Box R\) scheme conversion are certified. The generic longitudinal ghost tower is one normalized scalar Schur factor with exact residues and scale row; both reference finite rows, the canonical \(\det_3\) tail and selected weighted modified determinant are complete on round \(S^4\). All eleven generic third-curvature ghost functions, including the pole-four \(I_{29}\) row, are now integrated exactly. | **Strict theory obstructed; extended local Euclidean QME restored; all generic ghost \(n=3\) functions certified** | Assembly into the full generic ghost determinant, the generic physical fourth-order Hessian, complete \(\Gamma_1,Q_1\), Lorentzian QME, residual transfer, positive state, particles, and unitarity remain open. |
| Are black-hole solutions, thermodynamics, and linear gravitational branches present? | The static spherical family has exact energy, Wald entropy, and \(dH=T\,dS\). On Schwarzschild, the axial \(\ell=2\) Bach system splits into Regge--Wheeler and extra Lichnerowicz branches. The pure Einstein flux block is null, but exact real-frequency fixtures give nonzero Einstein--extra and extra--extra horizon flux. The extra branch is horizon-regular, luminal, bounded at infinity, and has the same leading falloff as Einstein waves, so ordinary causal boundary conditions do not remove it in this fixture. The polar equations now have an exact Ricci-flat Einstein/extra split. | **Static theorem and scoped axial causal disposition certified; polar programme opened** | The causal no-go is only for Schwarzschild \(m=1,\ell=2\), real frequency. Polar flux, complex frequencies, a full exterior IBVP, ringdown, stability, and Hawking states remain open. |
...
the pure Einstein/Regge--Wheeler flux block is symplectically null.  Exact
real-frequency fixtures nevertheless give nonzero Einstein--extra and
extra--extra horizon flux.  The extra branch is regular at the horizon and
bounded, luminal, and asymptotically indistinguishable by leading falloff from
the Einstein branch.  Thus, in the declared axial fixture, no local causal
condition at either end selects Einstein waves alone; doing so would require
an additional projection on scattering data.  This is a useful no-go, not a
full black-hole instability theorem.  Polar flux, complex frequencies, a
complete exterior evolution problem, ringdown and stability remain open.  Galaxy and cosmology
...
| [Pure-extra obstruction and balanced extension](91-charge-fibre-taub-bridge.md) | **Theorem frozen** on the generic fixed-bundle domain; the complete finite-support smooth tangent cone and standard-global bounded subcone are certified | Solve the remaining oscillator polynomial ideal and shell-resonance zero locus, add infinite inputs, and extract the structural definite/indefinite Taub theorem. |
...
| Relational clock and light | **Spatially global retarded redshift, localized causal emitters, response-specific shell kernels, first retarded emitter channel, and finite mass-uniform leading rank-two response certified** | Complete the all-shell massive image and absolute \(q_3\) feedback bounds, unite the preparations into one clock-frequency comparison, and finish recoil, backreaction, and the apparatus Dirac bracket. |
| Black holes and axial radiation | **Static first law and scoped axial causal disposition certified: the extra branch has nonzero mixed/self horizon flux and cannot be removed by ordinary local causal boundary conditions; the polar split is exact** | Compute polar current and flux, complex-frequency modes, and a full exterior IBVP before ringdown or stability claims. |
...
| Quantum anomaly and state bridge | **Strict one-loop obstruction, tau-adic local Euclidean restoration, anomaly-fixed terms, reduced Hadamard/Krein carrier, normalized longitudinal Schur/modified-Fredholm reduction, complete round-\(S^4\) weighted determinant, generic local factorization defect, and all eleven generic ghost \(n=3\) functions certified** | Assemble the generic ghost determinant with its primed spectral carrier and physical fourth-order Hessian, then complete \(Q_1\) and test the Lorentzian/full-BV state and quantum \(D\) defect. |
...
- In the generic compact axial block, the extra fourth-order module survives
  ...
  remain undecided.
- In the scoped Schwarzschild axial fixture, the extra branch also survives
  the first physical boundary test: it is horizon-regular, carries nonzero
  mixed/self flux, and has bounded luminal asymptotics. An Einstein-only
  sector therefore cannot be selected there by ordinary local causal
  regularity or decay conditions alone.
...
- [Partition-refined mass-uniform leading detector response](../closed_universe_observers/reports/berger-recoil-partitioned-leading-response-rank-two.md)
- [First retarded Maxwell emitter channel](../closed_universe_observers/reports/berger-recoil-free-emitter-first-retarded-maxwell-channel.md)
...
- [BH-2A nonzero mixed and extra horizon-flux fixtures](../black_hole_programme/reports/bh2a-cross-flux.md)
- [BH-2A axial causal disposition](../black_hole_programme/reports/bh2a-causal-disposition.md)
- [BH-2B exact polar Einstein/extra split](../black_hole_programme/reports/bh2b-polar-split.md)
...
- **19 July 2026, black-hole causal and finite-observer update:** added the
  nonzero axial Einstein--extra and extra--extra horizon-flux fixtures, the
  scoped result that local causal conditions do not remove the Schwarzschild
  axial extra branch, and the exact polar branch split. Updated the observer
  status to the response-specific shell architecture, first retarded emitter
  channel, and mass-uniform finite leading rank-two theorem while retaining
  the open all-shell \(q_3\) feedback and recoil gates. Added the complete
  complete finite-support smooth second-order tangent-cone theorem while
  keeping bounded, infinite and causal continuation separate; the standard
  generalized-zero bounded subcone and universal \(b=B=0\) elimination have
  since narrowed the remaining bounded gate.
  Recorded exact integration of the final pole-four \(I_{29}\) row, completing
  all eleven generic third-curvature ghost functions without promoting them
  to a complete determinant or Lorentzian quantum theory.
