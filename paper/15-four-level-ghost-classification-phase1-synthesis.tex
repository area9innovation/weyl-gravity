...
\date{Phase-1 synthesis with scoped Phase-2 update, 22 July 2026}
...
A Nariai factorization identifies a related mechanism at the operator level:
the two spin-two factors have opposite Lee--Wald residues, although no
absolute energy sign or identification with the compact-product family is
thereby established.  A dyonic-flat-family preflight, by contrast, finds no
fixed-coupling stationary sign laboratory because the required global
Hamiltonian lift and fixed-Chern tangent conditions fail.
...
solutions.  The Phase-2 generic-angular calculation overturns the former
fixture-level Einstein-selection reading: an axial non-Einstein branch has a
finite formal radial pairing for every integer $\ell\ge2$ and real
$\omega\ne0$, while the polar calculation retains a one-dimensional mixed
finite line and an exact exceptional wall.  These are formal-infinity,
fixed-current-representative results, not a completed asymptotically flat
phase space.  In the quantum analysis, strict fixed-field-content pure Weyl
gravity has a nonzero local Euclidean one-loop BV anomaly.  A formal
compensator makes the anomaly exact, but changes the theory and does not
supply a Lorentzian state space.

An exact PT/CPT pilot sharpens, but does not reverse, this classification.
Selected compact reduced blocks admit structured positive pseudo-Hermitian
metrics, yet the available data do not define an independent parity/time
involution and hence do not construct a genuine Mannheim $C$ operator.  On
the cylinder, the natural involution fails the BRST chain-map test.  The
counterflow Hamiltonian--Hopf quartet has complex spectrum and therefore
admits no positive pseudo-Hermitian metric.  For one complete causal Berger
BV complex the local Hadamard singular structure and causal commutator are
available, but the full covariance remains unconstructed because an explicit
104-dimensional stationary carrier is missing.
...
The Phase-2 finite-block pilot makes this last qualification exact.  On the
selected compact physical blocks, the intertwining equation
\begin{equation}
 H^\dagger\eta=\eta H,
 \qquad \eta=\eta^\dagger>0,
 \label{eq:pseudo-hermitian-metric}
\end{equation}
has a nonempty symmetry-compatible structured cone.  Its distinguished
fundamental symmetry is
$C_0=-I_2\oplus I_2\oplus I_4$.  This is a positive
pseudo-Hermitian-metric result, not yet a Mannheim construction: the reduced
data do not supply independent operators $P$ and $T$ from which a physical
$C$ can be defined, and quotient descent has not been established.  On the
selected conformal-cylinder carrier the natural candidate $C_0$ is not a
chain map for the BRST differential, so the declared invariant commutant
does not yield a BRST-descended positive metric.  These conclusions carry
the dependency tags \texttt{LOCAL-ALGEBRAIC} and \texttt{REDUCED-MODE}; they
do not establish a Lorentzian state.
...
The attempted dyonic-flat-family extension fails earlier.  For nonzero
electric parameter the stationary symmetry has no continuous global
Hamiltonian lift on the circle, since $\mathcal L_HA=E\,\dd x$ changes the
holonomy.  Electromagnetic duality also fails to define a tangent direction
in a fixed-Chern sector.  Consequently that preflight constructs neither a
fixed-coupling open stationary family nor a sign, current, inertia, or
signature-wall theorem.  Recording this obstruction prevents a formally
attractive but ill-typed background scan from being read as evidence for
Result card~A.
...
The outer boundary instead gives a filtration-sensitive Phase-2 result.  In
ingoing Eddington--Finkelstein coordinates $(v,r,\vartheta,\varphi)$ let
...
$r^p(\log r)^q$ is nonintegrable for $p\ge-1$ unless its invariant
...
\textbf{Domain.} Integer $\ell\ge2$, real $\omega\ne0$, formal
...
\textbf{Statement.} In the axial zero-rate sector, both Einstein directions
have finite $r^{-2}$ current and a reconstructed non-Einstein all-orders
...
Einstein/additional line and has a generically nonzero first finite $p=-2$
induced pairing.  Its exact exceptional wall is
\[
 Q_{21}\bigl(\ell(\ell+1),\omega^2\bigr)=0,
\]
where $Q_{21}$ is the certified bidegree-$(21,21)$ polynomial.  Exact Sturm
counts give respectively $0,6,2,6,2$ real nonzero zeros of this induced
$p=-2$ coefficient for
...
the deeper filtration on the $Q_{21}$ wall, all-order polar solutions, and
...
The old $\ell=2$, $M\omega=3/5$ divergence table omitted differentiated
$c'$ forcing in the axial reconstruction and is superseded.  That point is
not on the polar $Q_{21}$ wall, but this does not restore its earlier
...
Separately, Phase 2 audits the Hadamard gate on one complete causal 54-row
Berger BV complex.  The local Hadamard singular structure, causal commutator,
and a conditional lifting statement are available.  A full covariance is
not constructed because the explicit 104-dimensional stationary carrier
$A_{104}$, its Cauchy pairing $q^{\rm Cauchy}_{104}$, Green carrier
$G^{\rm Cauchy}_{104}$, and real structure have not been supplied.  The
current ledger leaves 288 coordinates undetermined; the two tested compatible
completion branches have zero-eigenspace dimensions 24 and 0.  This is a
missing-object obstruction, not a proof that no BRST-compatible Hadamard
covariance exists.  The reduced positive $\eta$ result above does not define
the adjoint on this full BV carrier and cannot be used to fill the gap.  Thus
...
mixed line with exact $Q_{21}$ wall & Universal asymptotic phase space,
...
state theorem;
\item supply the missing 104-dimensional stationary carrier and decide the
Hadamard covariance problem for the complete Berger BV complex;
