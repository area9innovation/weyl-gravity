...
\]
The class in brackets is the scalar spin-two graded projection of the
first signed mass-squared jet of Einstein--Weyl gravity.  Its rational
primitive has the same linear asymptotics as the derivative of the moving
massive phase; the Coulomb exponent has zero first mass derivative.  We do
not identify this scalar tangent with the complete coupled massive axial
system without an additional intertwining certificate.
...
and massive spin-two directions at the level of its graded scalar tangent.
At a
...
\item Its projective cocycle has the elementary normal form
  \[
    ...
    =\frac{i\omega}{2}\left[1-\frac2r\right],
  \]
  equal to the scalar spin-two graded projection of the formal critical
  mass-squared jet.
...
Graded mass-direction comparison
& Exact cocycle primitive and formal moving massive phases
& \texttt{LOCAL-ALGEBRAIC}\\
...
& Not established\\
Full coupled massive-system crosswalk
& Parameter-dependent axial intertwiner and matched endpoint maps
& Not established\\
\bottomrule
\end{tabular}
\end{center}
...
Bach response.  We do \emph{not} replace the complete massive axial system
by a scalar equation; the scalar mass equation used below is only its
spin-two graded tangent target.
...
\section{Graded mass-squared tangent and crosswalk boundary}
\label{sec:mass}

\subsection{The scalar graded target}

The covariant parent family contains the second-order tensor pencil
\[
 (\mathcal E+m\mathcal A)\phi=0,
\]
where \(m\) is the signed squared-mass coefficient.  On the
transverse-traceless spin-two graded quotient, the scalar target is
...
system of Refs.~\cite{britocardosopani2013,antoniou2025}.  It is the
spin-two graded projection used to compare first parameter jets.

\begin{proposition}[Exact graded mass direction]\label{thm:mass-jet}
Differentiating \eqref{eq:massive-rw} at \(m=0\) gives
\[
 L_2\,\partial_my=f\,y,\qquad
...
\[
 ...
 \;}
\]
Equivalently, at each fixed \(\omega\ne0\), the two tangent cocycles are
related by \(m=(i\omega/2)\tau\), modulo rational triangular gauge.
Because this factor depends on the spectral variable, it is not a global
reparametrization of two operator families.
\end{proposition}
...
\subsection{Formal endpoint comparison}

Because Lemma~\ref{lem:triangular-gauge} contains a factor \(1/2\), the
field redefinition that removes \(\mathcal K_{U_2}q\) is
\[
 Q_q=2qD-D(q),\qquad
 [L_2,Q_q]|_{\ker L_2}=-\mathcal K_{U_2}(q).
\]
Thus \(L_2x=\mathcal I_{\rm Bach}y\) implies
\[
 L_2(x+Q_qy)=\frac{i\omega}{2}fy.
\]
...
\begin{proposition}[Formal differentiated endpoint asymptotics]
\label{prop:formal-jost-comparison}
...
\[
 ...
 =c(\omega)
 \frac{\partial_my_\sigma^{\rm gr}(0,r)}
 {y_\sigma^{\rm gr}(0,r)}+O(1).
\]
At the horizon, \(f=O(r-2)\), so the graded mass term does not change the
ingoing Frobenius exponent.
\end{proposition}

\begin{proof}
The infinity equation has
\[
...
\]
The Coulomb logarithm therefore cancels at first order, while the moving
plane-wave phase supplies the linear term.  The formula for \(Q_q\) follows
from \(Dy_\sigma/y_\sigma=\sigma i\omega+O(r^{-1})\).
\end{proof}

The proposition is deliberately formal.  It verifies the endpoint
asymptotic target but does not exclude an opposite-Jost admixture in an
exact massive solution.  Promotion to a physical mass-velocity theorem
requires (i) a displayed parameter-dependent map from the complete
coupled massive axial variables to the certified filtration, and
(ii) a Volterra or equivalent uniqueness construction for the
mass-differentiated horizon and infinity maps.  Neither object is imported
as a certificate here.

\begin{remark}[Conditional physical crosswalk]\label{rem:mass-crosswalk}
If those two objects are supplied, scalar endpoint normalizations can only
add an analytic multiple of the unperturbed Evans coefficient.  One would
then obtain
\[
 b_{\rm B}(\omega)
 =\frac{i\omega}{2}\partial_ma(\omega,0)+h(\omega)a(\omega,0),
\]
and, at a simple QNM,
\[
 \omega_n'(0)=\frac{2i}{\omega_n}\kappa_n,\qquad
 \kappa_n=\frac{b_{\rm B}(\omega_n)}{a_\omega(\omega_n,0)}.
\]
These formulas are the precise remaining crosswalk target, not a theorem
of the present paper.
\end{remark}

The graded construction is the asymptotically flat analogue of the
mass-derivative mechanism behind logarithmic partners in critical gravity
\cite{lupope2011,bergshoeff2011}.  Here the formal endpoint tangent is
polynomial rather than an independent radial \(\log r\) mode.
...
\(-i\omega_n\omega_n'(0)/2\), where the derivative is with respect to the
...
physical massive-QNM velocity; that is the crosswalk left open in
Remark~\ref{rem:mass-crosswalk}.
...
is nonzero.  If the additional physical mass crosswalk of
...
& Exact\\
Critical parent jet
& Covariant mass derivative; physical scalar crosswalk remains open
& Exact with gate\\
...
fixed-frequency graded tangent normalization, the Smith and Green formulas, the
...
crosswalk or a physical massive-QNM slope.  The polynomially weighted term
...
\]
That direction equals the scalar spin-two graded projection of the formal
critical mass-squared jet.  Its linearly growing rational primitive matches
the differentiated massive phase, and the Coulomb exponent has vanishing
first derivative.  A full coupled-system and exact-Jost crosswalk remains
an explicit gate.
...
problems: the complete massive axial/Jost crosswalk, and a global causal
domain supporting the differentiated endpoint state and a justified
...
G.~Antoniou, E.~Gualtieri, and P.~Pani,
