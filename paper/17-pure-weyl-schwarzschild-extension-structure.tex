...
& Not established\\
All-order physical massive Jost crosswalk
& Differentiated matrix-Volterra construction and matched endpoint maps
& Not established\\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{Differentiated endpoint asymptotics}
...
Theorem~\ref{thm:mass-normal}.  At the horizon, the mass term is
\(O(r-2)\), so the ingoing indicial exponent is unchanged.

The complete coupled-system map and its leading differentiated endpoint
class are therefore exact.  What remains is analytic rather than
algebraic: a matrix-Volterra, or equivalent, construction must prove
all-order parameter dependence of the selected massive horizon and
infinity Jost bases and exclude an opposite-Jost admixture.

\begin{remark}[Conditional physical QNM velocity]\label{rem:mass-crosswalk}
Once that differentiated-Jost construction is supplied, endpoint
normalizations can only add an analytic multiple of the unperturbed Evans
coefficient.  Equation~\eqref{eq:complete-mass-crosswalk} then gives
\[
 b_{\rm B}(\omega)
 =\frac{3i\omega}{2}\partial_ma(\omega,0)+h(\omega)a(\omega,0),
\]
and, at a simple QNM,
\[
 \omega_n'(0)=\frac{2i}{3\omega_n}\kappa_n,\qquad
 \kappa_n=\frac{b_{\rm B}(\omega_n)}{a_\omega(\omega_n,0)}.
\]
These are the remaining analytic promotion targets, not theorems of this
paper.
\end{remark}
...
crosswalk is not used.  A triangular change
...
Remark~\ref{rem:mass-crosswalk}, the last member of
\eqref{eq:unified-invariant} may also be written
\(-3i\omega_n\omega_n'(0)/2\), where the derivative is with respect to the
physical signed squared-mass coefficient.  The certified enclosure
\eqref{eq:kappa-enclosure} is intrinsic; it is not advertised here as a
massive-QNM slope.
...
measures motion of the residue projector.  This exact parent identity does
not, by itself, identify the certified scalar \(\kappa_n\) with the
physical massive-QNM velocity; that requires the differentiated-Jost
promotion left open in Remark~\ref{rem:mass-crosswalk}.
...
is nonzero.  If the additional differentiated-Jost promotion of
Remark~\ref{rem:mass-crosswalk} is imposed, its parent-normalized form may
be written
\[
...
& Exact\\
Differentiated massive Jost map
& Leading phase matches; all-order matrix-Volterra uniqueness remains open
& Exact with gate\\
...
theorem.  It also does not establish the full coupled massive-system
Jost crosswalk or a physical massive-QNM slope.  The complete local
coupled-system first jet is exact; the missing part is the all-order
analytic endpoint identification.  The polynomially weighted term is
proved only for the isolated local resonance contour.
...
phase, and the Coulomb exponent has vanishing first derivative.  Only the
all-order analytic Jost identification remains an explicit mass-velocity
gate.
...
filtered Bach resonance.  The remaining work separates cleanly into two
problems: the differentiated massive Jost/Fredholm crosswalk, and a global
causal domain supporting the endpoint tangent and a justified retarded
contour deformation.
