(() => {
  ...
  function renderCompletionAtlas() {
    ...
    const recursiveTreeProgress = recursiveTrees.result_id ? `<article class="completion-intro gate-progress"><div><p class="eyebrow">Finite polarized recursion certified</p><h3>Every retarded tree and every advanced tree closes on its natural support space</h3><p>Past-compact and future-compact Green extensions let noncompact causal outputs re-enter the local candidate q2. The fixed-support induction has ${esc(recursiveTrees.support_domain_defects)} support-domain defects. At four leaves, ${esc(recursiveTrees.four_leaf_admissible)} of ${esc(recursiveTrees.four_leaf_all_sign_decorations)} sign decorations are uniformly defined.</p><p><a href="${esc(DATA.source_links.completion_recursive_trees_report)}">Read the recursive-domain theorem</a> · <a href="${esc(DATA.source_links.completion_recursive_trees)}">Inspect the support grammar and census</a></p></div><aside><b>Two four-leaf terms expose the boundary</b><p>A retarded parent fed by two advanced children, and its time reverse, require the Green operator on the wrong support class. This is a domain nondefinition—not a universal no-go. Formal coefficients, higher brackets, authoritative q2, Hadamard data, and QME restoration remain open.</p></aside></article>` : "";
    root.innerHTML = `
      <article class="completion-intro"><div><p class="eyebrow">Long-running programme objective</p><h3>Complete one causal route—or certify the first impossible gate</h3><p>${esc(ATLAS.answer)}</p></div><aside><b>Current front</b><p>Every finite polarized candidate tree now has an honest support domain. The two leading tasks are source-certifying q2 identity and assembling coefficientwise retarded/advanced formal Møller maps; higher classical brackets remain a separate import gate. Berger remains an independent analytic control.</p><a href="${esc(DATA.source_links.completion_atlas_report)}">Read the audited V19 report</a></aside></article>
      ${gateProgress}
      ${causalProgress}
      ${endpointProgress}
      ${suspensionProgress}
