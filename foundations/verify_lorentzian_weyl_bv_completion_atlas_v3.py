#!/usr/bin/env python3
"""Fail-closed schema, generator and independence verifier for atlas V3."""
...
RESULT = DIRECTORY / "results/FOUNDATIONAL_LORENTZIAN_WEYL_BV_COMPLETION_ATLAS_V3.json"
REPORT = DIRECTORY / "reports/lorentzian-weyl-bv-completion-atlas-v3.md"
SCHEMA = DIRECTORY / "schema/foundational-lorentzian-weyl-bv-completion-atlas-v3.schema.json"
CHECKER = DIRECTORY / "check_lorentzian_weyl_bv_completion_atlas_v3.py"
...
builder = module(DIRECTORY / "build_lorentzian_weyl_bv_completion_atlas_v3.py", "completion_atlas_v3_builder")
checker = module(CHECKER, "completion_atlas_v3_checker")
...
def verify(value: dict | None = None, report: str | None = None) -> tuple[list[str], list[str]]:
    ...
    for token in ("Strict Gate-A progress", "4490 full coordinates", "470 residual coordinates", "Updated route selection", "Why the ranking changed", "M2", "eleven-step Berger", "does not establish"):
        if token not in report:
            errors.append("report token " + token)
    return errors, [
        "Draft 2020-12 schema", "independent 77-cell audit", "V2 preservation",
        "finite-versus-full-carrier scope firewall", "unchanged eleven-step Berger chain",
        "updated five-route ranking", "34 content-pinned inputs", "deterministic result and report",
    ]
