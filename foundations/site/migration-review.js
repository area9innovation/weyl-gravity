(() => {
  ...
  function renderCompletionAtlas() {
    ...
    root.innerHTML = `
      <article class="completion-intro"><div><p class="eyebrow">Long-running programme objective</p><h3>Complete one causal route—or certify the first impossible gate</h3><p>${esc(ATLAS.answer)}</p></div><aside><b>Current front</b><p>The algebraic q2 lift now exists. The leading Gate-A task is no longer inventing q2; it is source-certifying which nonlinear classical extension the programme actually means. Berger remains an independent analytic control. Neither route is Lorentzian-quantum complete.</p><a href="${esc(DATA.source_links.completion_atlas_report)}">Read the audited V17 report</a></aside></article>
      ${gateProgress}
      ${causalProgress}
      ${endpointProgress}
      ${suspensionProgress}
