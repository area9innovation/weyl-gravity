(() => {
  ...
  function renderCompletionAtlas() {
    ...
    const q1SignProgress = q1SignGate.result_id ? `<article class="completion-intro gate-progress"><div><p class="eyebrow">Exact classical sign gate</p><h3>One four-row text/matrix conflict blocks the full q1 table</h3><p>The executable ${esc(q1SignGate.executable_sign)} dual arrow has ${esc(q1SignGate.executable_cyclicity_defects)} odd-pairing defects; the declared ${esc(q1SignGate.declared_sign)} arrow has ${esc(q1SignGate.declared_cyclicity_defects)}—eight cyclicity defects in the certified case. Both remain nilpotent and contractible, so pairing cyclicity is the independent discriminator.</p><p><a href="${esc(DATA.source_links.completion_q1_sign_gate_report)}">Read the sign-gate report</a> · <a href="${esc(DATA.source_links.completion_q1_sign_gate)}">Inspect the certificate</a></p></div><aside><b>Repair not yet applied</b><p>The preferred repair changes the factorized dual arrow and matching ledgers to +I₄, then regenerates the affected classical chain. The causal Green theorem remains preserved.</p></aside></article>` : "";
    root.innerHTML = `
      <article class="completion-intro"><div><p class="eyebrow">Long-running programme objective</p><h3>Complete one causal route—or certify the first impossible gate</h3><p>${esc(ATLAS.answer)}</p></div><aside><b>Current fronts</b><p>Strict pure Weyl preserves the target theory. Berger is analytically furthest along. Neither is Lorentzian-quantum complete.</p><a href="${esc(DATA.source_links.completion_atlas_report)}">Read the audited V9 report</a></aside></article>
      ${gateProgress}
      ${causalProgress}
      ${endpointProgress}
      ${suspensionProgress}
