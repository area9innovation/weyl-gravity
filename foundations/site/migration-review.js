(() => {
  ...
  function renderCompletionAtlas() {
    ...
    const gateProgress = finite.full_coordinates ? `<article class="completion-intro gate-progress"><div><p class="eyebrow">A real repair, with a hard boundary</p><h3>Finite residual contraction: ${Number(finite.full_coordinates).toLocaleString()} full → ${Number(finite.residual_coordinates).toLocaleString()} residual coordinates</h3><p>The three previously absent maps are now portable and ${esc(finite.all_sdr_identities)} exact SDR identities replay independently at energies ${esc((finite.energies || []).join(", "))}. This finite exact result uses no Choice inside the declared cutoff.</p><p><a href="${esc(DATA.source_links.completion_sdr_report)}">Read the finite SDR certificate</a> · <a href="${esc(DATA.source_links.completion_sdr)}">Inspect its exact matrices</a></p></div><aside><b>Gate A still closed</b><p>${esc(gate.receiver_verified_scoped_exports)} of 20 exports and ${esc(gate.receiver_verified_scoped_checks)} of 10 checks are receiver-verified in a same-theory scope, but there are still ${esc(gate.accepted_common_snapshot_hashes)} accepted common-snapshot hashes. The missing work is now a full support-local carrier extension, not missing serialization.</p><a href="${esc(DATA.source_links.completion_gate_report)}">Read Gate V3 reconciliation</a></aside></article>` : "";
    root.innerHTML = `
      <article class="completion-intro"><div><p class="eyebrow">Long-running programme objective</p><h3>Complete one causal route—or certify the first impossible gate</h3><p>${esc(ATLAS.answer)}</p></div><aside><b>Current fronts</b><p>Strict pure Weyl preserves the target theory. Berger is analytically furthest along. Neither is Lorentzian-quantum complete.</p><a href="${esc(DATA.source_links.completion_atlas_report)}">Read the audited V3 report</a></aside></article>
      ${gateProgress}
