(() => {
  ...
  function renderCompletionAtlas() {
    ...
    const unaryCausalProgress = unaryCausal.receiver_status === "ACCEPTED_SCOPED" ? `<article class="completion-intro gate-progress"><div><p class="eyebrow">Scoped common snapshot accepted</p><h3>${esc(unaryCausal.accepted_hashes)} hashes bind one unary-causal carrier</h3><p>The component basis, pairing, graph q1, local retract, suspension, represented spaces, transport contract, and both Green orientations now share one content-addressed ${esc(unaryCausal.carrier_rows)}-row snapshot. This closes the ambiguity about whether the unary causal ingredients refer to the same mathematical object.</p><p><a href="${esc(DATA.source_links.completion_unary_causal_snapshot_report)}">Read the snapshot report</a> · <a href="${esc(DATA.source_links.completion_unary_causal_snapshot)}">Inspect all accepted hashes</a></p></div><aside><b>Why Gate A is still closed</b><p>This scoped snapshot accepts ${esc(unaryCausal.gate_a_hashes_accepted_by_scoped_result)} of the Gate's ${esc(unaryCausal.gate_a_hashes_required)} top-level hashes. Gate A requires ${esc(unaryCausal.gate_a_exports_required)} exports and ${esc(unaryCausal.gate_a_freeze_checks_required)} checks, including six still-missing bundles. The next target is a full 386-row cylinder generator D, followed by q2 on the same carrier.</p></aside></article>` : "";
    root.innerHTML = `
      <article class="completion-intro"><div><p class="eyebrow">Long-running programme objective</p><h3>Complete one causal route—or certify the first impossible gate</h3><p>${esc(ATLAS.answer)}</p></div><aside><b>Current fronts</b><p>Strict pure Weyl preserves the target theory and now has one accepted unary-causal snapshot. Berger remains an analytically mature independent control. Neither is Lorentzian-quantum complete.</p><a href="${esc(DATA.source_links.completion_atlas_report)}">Read the audited V15 report</a></aside></article>
      ${gateProgress}
      ${causalProgress}
      ${endpointProgress}
      ${suspensionProgress}
