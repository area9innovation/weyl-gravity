#!/usr/bin/env python3
"""Independently check Atlas V30 projections, routes, and firewalls."""
...
GATE = ROOT / "quantum-weyl/classical_import/certificates/CLASSICAL_IMPORT_GATE_V12_RECONCILIATION.json"
...
def check(value: dict[str, Any] | None = None) -> list[str]:
    ...
    previous, receiver, gate = (json.loads(path.read_text()) for path in (PREDECESSOR, DIFF, GATE))
    errors: list[str] = []
    if value.get("result_id") != "FOUNDATIONAL_LORENTZIAN_WEYL_BV_COMPLETION_ATLAS_V30" or value.get("predecessor", {}).get("result_id") != previous["result_id"] or value.get("predecessor", {}).get("sha256") != sha(PREDECESSOR):
    ...
    p = value.get("strict_diff_auxiliary_bv_representation", {})
    expected = {
        "result_id": receiver["result_id"], "carrier_rows": 386, "completed_families": 3,
        ...
        "known_required_cubic_families": 7, "component_complete_families": 7,
        "component_open_families": 0, "diffeomorphism_BV_representation_component_complete": True,
    ...
    if p != expected:
        errors.append("Diff auxiliary projection")
    g = value.get("strict_gate_v12_reconciliation", {})
    if g.get("result_id") != gate["result_id"] or g.get("gate_a_status") != "FAIL_CLOSED" or g.get("accepted_top_level_hashes") != 0 or g.get("candidate_q2_hash_accepted") is not False:
        errors.append("Gate V12 projection")
    routes = [item.get("route") for item in value.get("route_selection", [])]
    expected_front = ["STRICT_NONLINEAR_WEYL_BOOST_GHOST_MANIFEST", "STRICT_SOURCE_Q2_Q3_PULLBACK_IDENTITY", "STRICT_LAMBDA2_GENERAL_SOURCE_COCYCLE_CLOSURE", "STRICT_CANDIDATE_Q2_Q3_GREEN_LAMBDA2_RESPONSE"]
    if len(routes) != 11 or routes[:4] != expected_front or "STRICT_DIFF_AUXILIARY_BV_REPRESENTATION_COMPONENTS" in routes:
        errors.append("route contraction/order")
    if [item.get("priority") for item in value.get("research_queue", [])] != list(range(1, 12)):
    ...
    for path in (PREDECESSOR, DIFF, GATE):
...
def main() -> int:
    errors = check()
    print("FOUNDATIONAL_LORENTZIAN_WEYL_BV_COMPLETION_ATLAS_V30: " + ("PASS" if not errors else "FAIL"))
    for error in errors:
        print("  - " + error)
    return bool(errors)
