...
COMPLETION_ATLAS = ROOT / "foundations/results/FOUNDATIONAL_LORENTZIAN_WEYL_BV_COMPLETION_ATLAS_V38.json"
...
COMPLETION_GATE_V20 = ROOT / "quantum-weyl/classical_import/certificates/CLASSICAL_IMPORT_GATE_V20_RECONCILIATION.json"
...
def check(data: dict[str, Any] | None = None) -> tuple[list[str], dict[str, Any]]:
    ...
    if completion.get("result_id") != "FOUNDATIONAL_LORENTZIAN_WEYL_BV_COMPLETION_ATLAS_V38":
    ...
    if route_names[:3] != ["STRICT_ENDPOINT_TO_RESIDUAL_SPECTRAL_COMPARISON", "STRICT_TYPED_RESIDUAL_CYCLICITY", "STRICT_COMMON_FREEZE_SNAPSHOT_AND_FINAL_CYCLIC_CONTRACTION"]:
        errors.append("strict V38 residual-comparison/cyclicity/freeze route frontier")
    ...
    gate_v20 = completion.get("strict_gate_v20_reconciliation", {})
    if (
        gate_v20.get("result_id") != gate_v20_source.get("result_id")
        or gate_v20.get("gate_a_status") != "FAIL_CLOSED"
        or gate_v20.get("accepted_top_level_hashes") != 1
        or gate_v20.get("remaining_top_level_hashes") != 6
        or gate_v20.get("M3L_common_endpoint_sdr_bound") is not True
        or gate_v20.get("M3R_typed_residual_comparison_constructed") is not False
        or gate_v20.get("M4L_local_graph_cyclic_pairing_complete") is not True
        or gate_v20.get("M4R_typed_residual_cyclicity_complete") is not False
        or gate_v20.get("minimal_missing_bundle") != ["M1_COMMON_STRICT_SNAPSHOT", "M3R_TYPED_RESIDUAL_COMPARISON", "M4R_TYPED_RESIDUAL_CYCLICITY"]
    ):
        errors.append("strict Gate V20 projection")
    if [item.get("route") for item in completion.get("route_selection", [])[:3]] != ["STRICT_ENDPOINT_TO_RESIDUAL_SPECTRAL_COMPARISON", "STRICT_TYPED_RESIDUAL_CYCLICITY", "STRICT_COMMON_FREEZE_SNAPSHOT_AND_FINAL_CYCLIC_CONTRACTION"]:
        errors.append("strict V38 route frontier")
    ...
    if result_flags.get("strict_m3l_common_endpoint_sdr_binding_exposed") is not True or result_flags.get("strict_m3r_typed_residual_comparison_exposed") is not False:
