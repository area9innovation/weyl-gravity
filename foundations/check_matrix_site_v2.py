...
COMPLETION_ATLAS = ROOT / "foundations/results/FOUNDATIONAL_LORENTZIAN_WEYL_BV_COMPLETION_ATLAS_V37.json"
...
COMPLETION_GATE_V19 = ROOT / "quantum-weyl/classical_import/certificates/CLASSICAL_IMPORT_GATE_V19_RECONCILIATION.json"
...
def digest(data: dict[str, Any]) -> str:
    projection = {key: data[key] for key in ("axes", "cells", "evidence", "ladder", "graph", "completion_atlas", "completion_common_endpoint_sdr_binding", "completion_residual_zero_modes", "completion_centered_cohomology", "completion_residual_sdr_type_audit", "cross_cell_interfaces", "carrier_interfaces", "numerical_reproducibility_records")}
...
def check(data: dict[str, Any] | None = None) -> tuple[list[str], dict[str, Any]]:
    ...
    if completion.get("result_id") != "FOUNDATIONAL_LORENTZIAN_WEYL_BV_COMPLETION_ATLAS_V37":
    ...
    if route_names[:3] != ["STRICT_FULL_CYCLIC_PAIRING", "STRICT_ENDPOINT_TO_RESIDUAL_SPECTRAL_COMPARISON", "STRICT_COMMON_FREEZE_SNAPSHOT_AND_FINAL_CYCLIC_CONTRACTION"]:
        errors.append("strict V37 pairing/residual-comparison/freeze route frontier")
    if "STRICT_COMMON_ENDPOINT_SDR_BINDING" in route_names:
        errors.append("strict V37 completed M3L route retained")
    ...
    gate_v19 = completion.get("strict_gate_v19_reconciliation", {})
    if (
        gate_v19.get("result_id") != gate_v19_source.get("result_id")
        or gate_v19.get("gate_a_status") != "FAIL_CLOSED"
        or gate_v19.get("accepted_top_level_hashes") != 1
        or gate_v19.get("remaining_top_level_hashes") != 6
        or gate_v19.get("M3L_common_endpoint_sdr_bound") is not True
        or gate_v19.get("M3R_typed_residual_comparison_constructed") is not False
        or gate_v19.get("minimal_missing_bundle") != ["M1_COMMON_STRICT_SNAPSHOT", "M3R_TYPED_RESIDUAL_COMPARISON", "M4_FULL_CYCLIC_PAIRING"]
    ):
        errors.append("strict Gate V19 projection")
    if [item.get("route") for item in completion.get("route_selection", [])[:3]] != ["STRICT_FULL_CYCLIC_PAIRING", "STRICT_ENDPOINT_TO_RESIDUAL_SPECTRAL_COMPARISON", "STRICT_COMMON_FREEZE_SNAPSHOT_AND_FINAL_CYCLIC_CONTRACTION"]:
        errors.append("strict V37 route frontier")
