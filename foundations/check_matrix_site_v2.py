...
COMPLETION_M3RC_ACTION_SUPPORT_DUAL = ROOT / "quantum-weyl/classical_import/certificates/STRICT_M3RC_ACTION_SUPPORT_DUAL_IDENTIFICATION_V1.json"
COMPLETION_TYPED_RESIDUAL_CYCLICITY = ROOT / "quantum-weyl/classical_import/certificates/STRICT_TYPED_RESIDUAL_CYCLICITY_V1.json"
...
COMPLETION_GATE_V27 = ROOT / "quantum-weyl/classical_import/certificates/CLASSICAL_IMPORT_GATE_V27_RECONCILIATION.json"
COMPLETION_M1A_LOCAL = ROOT / "quantum-weyl/classical_import/certificates/STRICT_M1A_LOCAL_SEMANTIC_EXTENSION_V1.json"
COMPLETION_GATE_V28 = ROOT / "quantum-weyl/classical_import/certificates/CLASSICAL_IMPORT_GATE_V28_RECONCILIATION.json"
...
COMPLETION_GATE_V29 = ROOT / "quantum-weyl/classical_import/certificates/CLASSICAL_IMPORT_GATE_V29_RECONCILIATION.json"
COMPLETION_M1B_PRIMAL = ROOT / "quantum-weyl/classical_import/certificates/STRICT_M1B_PRIMAL_COMPOSITE_CONTRACTION_V1.json"
COMPLETION_GATE_V30 = ROOT / "quantum-weyl/classical_import/certificates/CLASSICAL_IMPORT_GATE_V30_RECONCILIATION.json"
...
def check(data: dict[str, Any] | None = None) -> tuple[list[str], dict[str, Any]]:
    ...
    if len(completion.get("route_selection", [])) != 7 or len(completion.get("berger_h26_c26_decision_chain", [])) != 11:
    ...
    if full_q1.get("full_sdr_tables_serialized") is not False or full_q1.get("classical_import_gate_passed") is not False or completion_flags.get("strict_pure_weyl_classical_gate_passed") is not True:
    ...
    if "STRICT_ENDPOINT_TO_RESIDUAL_SPECTRAL_COMPARISON" in route_names:
        errors.append("strict V40 completed M3R route retained")
    if "STRICT_COMMON_ENDPOINT_SDR_BINDING" in route_names:
        errors.append("strict V38 completed M3L route retained")
    if "STRICT_FULL_CYCLIC_PAIRING" in route_names:
        errors.append("strict V38 completed untyped M4 route retained")
    if "STRICT_RESIDUAL_SDR_COMMON_CARRIER" in route_names:
        errors.append("strict V37 retired type-conflated residual route")
    ...
    endpoint_sdr_binding = data.get("completion_common_endpoint_sdr_binding", {})
    if (
        endpoint_sdr_binding != endpoint_sdr_binding_source
        or endpoint_sdr_binding.get("result_id") != "STRICT_386_COMMON_ENDPOINT_SDR_BINDING_V1"
        or endpoint_sdr_binding.get("common_manifest", {}).get("carrier_rows") != 386
        or endpoint_sdr_binding.get("common_manifest", {}).get("endpoint_rows") != 30
        or endpoint_sdr_binding.get("common_manifest", {}).get("contracted_rows") != 356
        or len(endpoint_sdr_binding.get("common_manifest", {}).get("artifact_pins", [])) != 10
        or len(endpoint_sdr_binding.get("common_manifest", {}).get("object_hashes", {})) != 17
        or endpoint_sdr_binding.get("exact_replay", {}).get("compatibility_links_checked") != 15
        or any(count for key, count in endpoint_sdr_binding.get("exact_replay", {}).items() if key.endswith("defects"))
        or endpoint_sdr_binding.get("claim_flags", {}).get("M3L_COMMON_ENDPOINT_SDR_BOUND") is not True
        or endpoint_sdr_binding.get("claim_flags", {}).get("M3R_TYPED_RESIDUAL_COMPARISON_CONSTRUCTED") is not False
    ):
        errors.append("strict common endpoint-SDR binding")
    residual_comparison = data.get("completion_endpoint_to_residual_comparison", {})
    if (
        residual_comparison != residual_comparison_source
        or residual_comparison.get("result_id") != "STRICT_ENDPOINT_TO_RESIDUAL_SPECTRAL_COMPARISON_V1"
        or residual_comparison.get("comparison", {}).get("source", {}).get("total_dimension") != 4080
        or residual_comparison.get("comparison", {}).get("target", {}).get("dimension") != 470
        or [item.get("dimension") for item in residual_comparison.get("level_blocks", [])] != [10, 40, 82, 136, 202]
        or residual_comparison.get("exact_replay", {}).get("ordered_crosswalk_defects") != 0
        or any(
            residual_comparison.get("exact_replay", {}).get(key) != 0
            for key in (
        ...
        or residual_comparison.get("claim_flags", {}).get("M3R_TYPED_RESIDUAL_COMPARISON_CONSTRUCTED") is not True
        or residual_comparison.get("claim_flags", {}).get("HARMONIC_ANALYSIS_SUPPORT_LOCAL") is not False
        or residual_comparison.get("claim_flags", {}).get("ALL_ENERGY_OR_SMOOTH_COMPLETION_CERTIFIED") is not False
    ):
        errors.append("strict endpoint-to-residual M3R comparison")
    residual_cyclic_obstruction = data.get("completion_residual_cyclic_carrier_obstruction", {})
    if (
        residual_cyclic_obstruction != residual_cyclic_obstruction_source
        or residual_cyclic_obstruction.get("result_id") != "STRICT_RESIDUAL_CYCLIC_CARRIER_OBSTRUCTION_V1"
        or residual_cyclic_obstruction.get("obstruction_replay", {}).get("m3r_residual_coordinates") != 470
        or residual_cyclic_obstruction.get("obstruction_replay", {}).get("m3r_inclusion_degree_counts") != {"0": 470}
        or residual_cyclic_obstruction.get("obstruction_replay", {}).get("pulled_back_odd_pairing_rank") != 0
        or residual_cyclic_obstruction.get("obstruction_replay", {}).get("pulled_back_odd_pairing_nullity") != 470
        or residual_cyclic_obstruction.get("cotangent_preflight", {}).get("total_dimension") != 940
        or residual_cyclic_obstruction.get("cotangent_preflight", {}).get("constructive_exact_rank") != 940
        or residual_cyclic_obstruction.get("claim_flags", {}).get("M3RC_DUAL_COMPARISON_MAPS_CONSTRUCTED") is not False
        or residual_cyclic_obstruction.get("claim_flags", {}).get("M4R_TYPED_RESIDUAL_CYCLICITY_COMPLETE") is not False
    ):
        errors.append("strict residual cyclic-carrier obstruction")
    local_cyclic_pairing = data.get("completion_local_cyclic_pairing", {})
    if (
        local_cyclic_pairing != local_cyclic_pairing_source
        or local_cyclic_pairing.get("result_id") != "STRICT_386_LOCAL_CYCLIC_PAIRING_CLOSURE_V1"
        or local_cyclic_pairing.get("pairing_replay", {}).get("carrier_rows") != 386
        or local_cyclic_pairing.get("pairing_replay", {}).get("exact_rational_rank") != 386
        or local_cyclic_pairing.get("pairing_replay", {}).get("nonzero_ordered_pairing_entries") != 410
        or any(count for key, count in local_cyclic_pairing.get("local_cyclicity_replay", {}).items() if key.endswith("defects"))
        or local_cyclic_pairing.get("claim_flags", {}).get("M4L_LOCAL_GRAPH_CYCLIC_PAIRING_COMPLETE") is not True
        or local_cyclic_pairing.get("claim_flags", {}).get("M4R_TYPED_RESIDUAL_CYCLICITY_COMPLETE") is not False
    ):
        errors.append("strict local cyclic-pairing closure")
    dfinite_cotangent_dual = data.get("completion_dfinite_cotangent_dual_comparison", {})
    if (
        dfinite_cotangent_dual != dfinite_cotangent_dual_source
        or dfinite_cotangent_dual.get("result_id") != "STRICT_DFINITE_COTANGENT_DUAL_COMPARISON_V1"
        or dfinite_cotangent_dual.get("same_source_impossibility", {}).get("original_source_full_dimension") != 4490
        or dfinite_cotangent_dual.get("same_source_impossibility", {}).get("original_source_degree_zero_cohomology_dimension") != 470
        or dfinite_cotangent_dual.get("same_source_impossibility", {}).get("original_source_degree_one_cohomology_dimension") != 0
        or dfinite_cotangent_dual.get("formal_cotangent_completion", {}).get("full_dimension") != 8980
        or dfinite_cotangent_dual.get("formal_cotangent_completion", {}).get("residual_dimension") != 940
        or dfinite_cotangent_dual.get("formal_cotangent_completion", {}).get("all_declared_identity_defects") != 0
        or dfinite_cotangent_dual.get("claim_flags", {}).get("FORMAL_940_COTANGENT_RESIDUAL_COMPARISON_CONSTRUCTED") is not True
        or dfinite_cotangent_dual.get("claim_flags", {}).get("M3RC_ACTION_SUPPORT_IDENTIFICATION_COMPLETE") is not False
    ):
        errors.append("strict formal cotangent-dual projection")
    m3rc_action_support = data.get("completion_m3rc_action_support_dual_identification", {})
    if (
        m3rc_action_support != m3rc_action_support_source
        or m3rc_action_support.get("result_id") != "STRICT_M3RC_ACTION_SUPPORT_DUAL_IDENTIFICATION_V1"
        or m3rc_action_support.get("support_dual_identification", {}).get("compact_source_representatives") != 470
        or m3rc_action_support.get("action_pairing_identification", {}).get("phase_pairing_rank") != 940
        or m3rc_action_support.get("action_pairing_identification", {}).get("pairing_identification_defects") != 0
        or m3rc_action_support.get("claim_flags", {}).get("M3RC_B_REPRESENTED_ACTION_SUPPORT_DUAL_IDENTIFICATION_COMPLETE") is not True
        or m3rc_action_support.get("claim_flags", {}).get("FULL_ALL_ENERGY_CONTINUOUS_DUAL_IDENTIFIED") is not False
    ):
        errors.append("strict M3RC-B action/support projection")
    typed_residual_cyclicity = data.get("completion_typed_residual_cyclicity", {})
    if (
        typed_residual_cyclicity != typed_residual_cyclicity_source
        or typed_residual_cyclicity.get("result_id") != "STRICT_TYPED_RESIDUAL_CYCLICITY_V1"
        or typed_residual_cyclicity.get("exact_cyclic_replay", {}).get("formal_source_dimension") != 8980
        or typed_residual_cyclicity.get("exact_cyclic_replay", {}).get("residual_dimension") != 940
        or typed_residual_cyclicity.get("exact_cyclic_replay", {}).get("residual_pairing_rank") != 940
        or typed_residual_cyclicity.get("exact_cyclic_replay", {}).get("all_identity_defects") != 0
        or typed_residual_cyclicity.get("claim_flags", {}).get("M4R_TYPED_RESIDUAL_CYCLICITY_COMPLETE") is not True
        or typed_residual_cyclicity.get("claim_flags", {}).get("FORMAL_8980_SOURCE_IS_AUTHORITATIVE_ORIGINAL_BV_COMPLEX") is not False
    ):
        errors.append("strict M4R typed residual cyclicity projection")
    gate_v27 = completion.get("strict_gate_v27_reconciliation", {})
    if (
        gate_v27.get("result_id") != gate_v27_source.get("result_id")
        or gate_v27.get("gate_a_status") != "FAIL_CLOSED"
        or gate_v27.get("accepted_top_level_hashes") != 1
        or gate_v27.get("remaining_top_level_hashes") != 6
        or gate_v27.get("local_386_rows_fully_namespaced") != 386
        or gate_v27.get("M1A2_local_semantic_extension_complete") is not True
        or gate_v27.get("M1A3_represented_crosswalk_complete") is not False
        or gate_v27.get("M1A4_ledger_freeze_complete") is not False
    ):
        errors.append("strict Gate V27 projection")
    m1a_local = completion.get("strict_m1a_local_semantic_extension", {})
    if (
        m1a_local.get("result_id") != m1a_local_source.get("result_id")
        or (m1a_local.get("extension_rows"), m1a_local.get("auxiliary_rows_fully_namespaced"), m1a_local.get("mapping_cone_rows_fully_namespaced")) != (356, 36, 320)
        or (m1a_local.get("local_386_rows_fully_namespaced"), m1a_local.get("unresolved_local_rows")) != (386, 0)
        or m1a_local.get("scalar_nonlinear_weyl_weight") != "NOT_APPLICABLE"
        or (m1a_local.get("cotton_component_checks"), m1a_local.get("cotton_component_defects")) != (2560, 0)
        or m1a_local.get("M1A2_complete") is not True
        or m1a_local.get("M1A_complete") is not False
    ):
        errors.append("strict M1A2 local semantic projection")
    m1a_represented = completion.get("strict_m1a_represented_crosswalk", {})
    represented_counts = m1a_represented_source.get("counts", {})
    if (
        m1a_represented.get("result_id") != m1a_represented_source.get("result_id")
        or m1a_represented.get("represented_endpoint_rows") != represented_counts.get("represented_endpoint_coordinates")
        or m1a_represented.get("excluded_test_rows") != represented_counts.get("test_nonminimal_coordinates_excluded")
        or m1a_represented.get("excluded_test_doublets") != represented_counts.get("test_nonminimal_doublets")
        or m1a_represented.get("action_residual_primal_rows") != represented_counts.get("action_residual_primal_coordinates")
        or m1a_represented.get("action_residual_dual_rows") != represented_counts.get("action_residual_dual_coordinates")
        or any(m1a_represented.get(key) != 0 for key in ("q0_cross_partition_defects", "q0_chain_degree_defects", "residual_crosswalk_defects"))
        or m1a_represented.get("M1A3_complete") is not True
        or m1a_represented_source.get("claim_flags", {}).get("M1A4_IMMUTABLE_LEDGER_FREEZE_COMPLETE") is not False
    ):
        errors.append("strict M1A3 represented crosswalk projection")
    m1a_ledger = completion.get("strict_m1a_immutable_typed_ledger", {})
    ledger_counts = m1a_ledger_source.get("counts", {})
    ledger_freeze = m1a_ledger_source.get("diagram_freeze", {})
    if (
        m1a_ledger.get("result_id") != m1a_ledger_source.get("result_id")
        or m1a_ledger.get("authoritative_rows_total") != ledger_counts.get("authoritative_rows_total")
        or m1a_ledger.get("authoritative_carrier_objects") != ledger_counts.get("authoritative_carrier_objects")
        or m1a_ledger.get("untyped_authoritative_rows") != 0
        or m1a_ledger.get("category_identification_defects") != 0
        or m1a_ledger.get("typed_field_dictionary_sha256") != ledger_freeze.get("typed_field_dictionary_sha256")
        or m1a_ledger.get("typed_diagram_sha256") != ledger_freeze.get("sha256")
        or m1a_ledger.get("M1A4_complete") is not True
        or m1a_ledger.get("M1A_complete") is not True
        or m1a_ledger_source.get("claim_flags", {}).get("M1B_REPRESENTED_COMPOSITE_CONTRACTION_COMPLETE") is not False
    ):
        errors.append("strict M1A4 immutable typed-ledger projection")
    gate_v28 = completion.get("strict_gate_v28_reconciliation", {})
    gate_v28_disposition = gate_v28_source.get("gate_disposition", {})
    gate_v28_m1a = gate_v28_source.get("m1a_completion_resolution", {})
    if (
        gate_v28.get("result_id") != gate_v28_source.get("result_id")
        or gate_v28.get("gate_a_status") != "FAIL_CLOSED"
        or gate_v28.get("exports_total") != gate_v28_disposition.get("exports_total")
        or gate_v28.get("exports_receiver_verified_scoped") != gate_v28_disposition.get("same_theory_receiver_verified_scoped")
        or gate_v28.get("freeze_checks_total") != gate_v28_disposition.get("freeze_checks_total")
        or gate_v28.get("freeze_checks_receiver_verified_scoped") != gate_v28_disposition.get("freeze_checks_receiver_verified_scoped")
        or gate_v28.get("accepted_top_level_hashes") != gate_v28_disposition.get("accepted_common_snapshot_hashes")
        or gate_v28.get("authoritative_rows_total") != gate_v28_m1a.get("authoritative_rows_total")
        or gate_v28.get("M1A_complete") is not True
        or gate_v28.get("M1B_represented_composite_contraction_complete") is not False
        or gate_v28.get("M1C_common_manifest_replay_complete") is not False
    ):
        errors.append("strict Gate V28 projection")
    gate_v29 = completion.get("strict_gate_v29_reconciliation", {})
    gate_v29_disposition = gate_v29_source.get("gate_disposition", {})
    if (
        gate_v29.get("result_id") != gate_v29_source.get("result_id")
        or gate_v29.get("gate_a_status") != "FAIL_CLOSED"
        or gate_v29.get("accepted_top_level_hashes") != 1
        or gate_v29.get("remaining_top_level_hashes") != 6
        or gate_v29.get("M1B_primal_complete") is not True
        or gate_v29.get("M1B_action_dual_complete") is not False
        or gate_v29.get("M1B_cyclic_replay_complete") is not False
        or gate_v29.get("M1B_complete") is not False
        or gate_v29_disposition.get("gate_a_status") != "FAIL_CLOSED"
    ):
        errors.append("strict Gate V29 projection")
    m1b_primal = completion.get("strict_m1b_primal_composite_contraction", {})
    m1b_aggregate = m1b_primal_source.get("represented_contraction", {}).get("aggregate", {})
    if (
        m1b_primal.get("result_id") != m1b_primal_source.get("result_id")
        or m1b_primal.get("content_sha256") != m1b_primal_source.get("content_sha256")
        or (m1b_primal.get("represented_endpoint_rows"), m1b_primal.get("primal_residual_rows")) != (4080, 470)
        or (m1b_primal.get("q0_nonzero_entries"), m1b_primal.get("homotopy_nonzero_entries"), m1b_primal.get("inclusion_nonzero_entries"), m1b_primal.get("projection_nonzero_entries")) != (1805, 1805, 470, 470)
        or m1b_primal.get("represented_endpoint_rows") != m1b_aggregate.get("represented_rows")
        or (m1b_primal.get("represented_identity_defects"), m1b_primal.get("formal_composition_defects")) != (0, 0)
        or m1b_primal.get("local_graph_factor_support_local") is not True
        or m1b_primal.get("harmonic_restriction_support_local") is not False
        or m1b_primal.get("raw_component_matrix_constructed") is not False
        or m1b_primal.get("M1B_primal_complete") is not True
        or m1b_primal.get("M1B_complete") is not False
    ):
        errors.append("strict M1B primal composite projection")
    gate_v30 = completion.get("strict_gate_v30_reconciliation", {})
    if (
        gate_v30.get("result_id") != gate_v30_source.get("result_id")
        or gate_v30.get("gate_a_status") != "VERIFIED"
        or (gate_v30.get("exports_receiver_verified"), gate_v30.get("exports_total")) != (20, 20)
        or (gate_v30.get("freeze_checks_receiver_verified"), gate_v30.get("freeze_checks_total")) != (10, 10)
        or (gate_v30.get("accepted_top_level_hashes"), gate_v30.get("remaining_top_level_hashes")) != (7, 0)
        or gate_v30.get("M1B_complete") is not True
        or gate_v30.get("M1C_complete") is not True
        or gate_v30.get("nonlinear_green_compatibility_certified") is not False
        or gate_v30.get("full_complex_hadamard_state_constructed") is not False
        or gate_v30_source.get("gate_disposition", {}).get("gate_a_status") != "VERIFIED"
    ):
        errors.append("strict Gate V30 projection")
    m1b_dual = completion.get("strict_m1b_action_dual_lift", {})
    if (
        m1b_dual.get("result_id") != m1b_dual_source.get("result_id")
        or m1b_dual.get("content_sha256") != m1b_dual_source.get("content_sha256")
        or (m1b_dual.get("compact_source_action_duals"), m1b_dual.get("represented_check_coordinates")) != (470, 4080)
        or (m1b_dual.get("local_pairing_rank"), m1b_dual.get("residual_action_pairing_rank")) != (386, 940)
        or m1b_dual.get("identity_defects") != 0
        or m1b_dual.get("M1B_action_dual_complete") is not True
        or m1b_dual.get("full_algebraic_dual_identified_with_compact_sources") is not False
    ):
        errors.append("strict M1B action-dual projection")
    m1b_cyclic = completion.get("strict_m1b_typed_cyclic_composite", {})
    if (
        m1b_cyclic.get("result_id") != m1b_cyclic_source.get("result_id")
        or m1b_cyclic.get("content_sha256") != m1b_cyclic_source.get("content_sha256")
        or (m1b_cyclic.get("verification_core_coordinates"), m1b_cyclic.get("residual_action_pairing_rank"), m1b_cyclic.get("typed_identities_replayed")) != (8160, 940, 13)
        or m1b_cyclic.get("identity_defects") != 0
        or m1b_cyclic.get("M1B_complete") is not True
        or m1b_cyclic.get("verification_core_is_authoritative_full_bv_source") is not False
    ):
        errors.append("strict M1B cyclic projection")
    m1c = completion.get("strict_m1c_common_snapshot", {})
    if (
        m1c.get("result_id") != m1c_source.get("result_id")
        or m1c.get("content_sha256") != m1c_source.get("content_sha256")
        or m1c.get("snapshot_id") != m1c_source.get("snapshot_id")
        or m1c.get("snapshot_sha256") != m1c_source.get("snapshot_sha256")
        or tuple(m1c.get(key) for key in ("artifact_pins", "exports_bound", "top_level_hashes_bound", "gate_checks_replayed", "supplemental_checks_replayed")) != (16, 20, 7, 10, 3)
        or m1c.get("M1C_complete") is not True
    ):
        errors.append("strict M1C snapshot projection")
    ...
    if result_flags.get("strict_M1B_primal_composite_contraction_exposed") is not True or result_flags.get("strict_M1B_action_dual_lift_exposed") is not True or result_flags.get("strict_M1B_typed_cyclic_replay_exposed") is not True or result_flags.get("strict_M1B_represented_composite_contraction_exposed") is not True:
        errors.append("site M1B primal/full exposure flags")
    ...
    if result_flags.get("strict_m3l_common_endpoint_sdr_binding_exposed") is not True or result_flags.get("strict_m3r_typed_residual_comparison_exposed") is not True:
        errors.append("site M3L/M3R exposure flags")
    if result_flags.get("strict_M3R_represented_dfinite_comparison_exposed") is not True or result_flags.get("strict_M4R_residual_cyclicity_exposed") is not True:
        errors.append("site represented M3R/M4R boundary flags")
    if result_flags.get("strict_current_470_induced_odd_pairing_rank_zero_exposed") is not True or result_flags.get("strict_finite_940_cotangent_preflight_exposed") is not True or result_flags.get("strict_original_dfinite_H1_zero_exposed") is not True or result_flags.get("strict_M3RC_A_formal_cotangent_dual_comparison_exposed") is not True or result_flags.get("strict_formal_8980_to_940_cotangent_SDR_exposed") is not True or result_flags.get("strict_M3RC_B_action_support_identification_exposed") is not True or result_flags.get("strict_M3RC_dual_comparison_maps_exposed") is not True or result_flags.get("strict_all_470_formal_duals_have_compact_source_representatives_exposed") is not True or result_flags.get("strict_action_pairing_equals_canonical_940_cotangent_pairing_exposed") is not True:
        errors.append("site M3RC carrier-obstruction exposure flags")
    if result_flags.get("strict_m4l_local_graph_cyclic_pairing_exposed") is not True or result_flags.get("strict_m4r_typed_residual_cyclicity_exposed") is not True or result_flags.get("strict_M4R_projection_equals_inclusion_sharp_exposed") is not True or result_flags.get("strict_M4R_homotopy_skew_adjoint_exposed") is not True or result_flags.get("strict_M1_common_strict_snapshot_exposed") is not True:
    ...
    if (
        ...
        or result_flags.get("strict_classical_gate_a_passed") is not True
    ):
