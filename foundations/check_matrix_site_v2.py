...
COMPLETION_ATLAS = ROOT / "foundations/results/FOUNDATIONAL_LORENTZIAN_WEYL_BV_COMPLETION_ATLAS_V34.json"
...
COMPLETION_GATE_V16 = ROOT / "quantum-weyl/classical_import/certificates/CLASSICAL_IMPORT_GATE_V16_RECONCILIATION.json"
...
def digest(data: dict[str, Any]) -> str:
    projection = {key: data[key] for key in ("axes", "cells", "evidence", "ladder", "graph", "completion_atlas", "completion_residual_zero_modes", "cross_cell_interfaces", "carrier_interfaces", "numerical_reproducibility_records")}
...
def check(data: dict[str, Any] | None = None) -> tuple[list[str], dict[str, Any]]:
    ...
    if completion.get("result_id") != "FOUNDATIONAL_LORENTZIAN_WEYL_BV_COMPLETION_ATLAS_V34":
    ...
    if route_names[:3] != ["STRICT_CENTERED_H3_H4_H5_REPRESENTATIVE_PAYLOAD", "STRICT_COMMON_FREEZE_SNAPSHOT_AND_FINAL_CYCLIC_CONTRACTION", "STRICT_RESIDUAL_SDR_COMMON_CARRIER"]:
        errors.append("strict V34 residual/centered route frontier")
    ...
    gate_v16 = completion.get("strict_gate_v16_reconciliation", {})
    if (
        gate_v16.get("result_id") != gate_v16_source.get("result_id")
        or gate_v16.get("gate_a_status") != "FAIL_CLOSED"
        or gate_v16.get("accepted_top_level_hashes") != 1
        or gate_v16.get("remaining_top_level_hashes") != 6
        or gate_v16.get("M5_payload_complete") is not True
        or gate_v16.get("zero_mode_hash_common_bound") is not False
        or gate_v16.get("minimal_missing_bundle") != ["M1_COMMON_STRICT_SNAPSHOT", "M3_RESIDUAL_SDR", "M4_FULL_CYCLIC_PAIRING", "M6_CENTERED_REPRESENTATIVES"]
    ):
        errors.append("strict Gate V16 projection")
    if [item.get("route") for item in completion.get("route_selection", [])[:4]] != ["STRICT_CENTERED_H3_H4_H5_REPRESENTATIVE_PAYLOAD", "STRICT_COMMON_FREEZE_SNAPSHOT_AND_FINAL_CYCLIC_CONTRACTION", "STRICT_RESIDUAL_SDR_COMMON_CARRIER", "STRICT_FULL_CYCLIC_PAIRING"]:
        errors.append("strict V34 route frontier")
