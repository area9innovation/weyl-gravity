...
COMPLETION_ATLAS = ROOT / "foundations/results/FOUNDATIONAL_LORENTZIAN_WEYL_BV_COMPLETION_ATLAS_V43.json"
...
COMPLETION_GATE_V25 = ROOT / "quantum-weyl/classical_import/certificates/CLASSICAL_IMPORT_GATE_V25_RECONCILIATION.json"
...
def check(data: dict[str, Any] | None = None) -> tuple[list[str], dict[str, Any]]:
    ...
    if completion.get("result_id") != "FOUNDATIONAL_LORENTZIAN_WEYL_BV_COMPLETION_ATLAS_V43":
    ...
    if route_names[:3] != ["STRICT_COMMON_FREEZE_SNAPSHOT_AND_FINAL_CYCLIC_CONTRACTION", "STRICT_LAMBDA2_GENERAL_SOURCE_COCYCLE_CLOSURE", "STRICT_CANDIDATE_Q2_Q3_GREEN_LAMBDA2_RESPONSE"]:
        errors.append("strict V43 M1/lambda2/q2-q3-Green route frontier")
    if "STRICT_M3RC_ACTION_SUPPORT_DUAL_IDENTIFICATION" in route_names or "STRICT_TYPED_RESIDUAL_CYCLICITY" in route_names:
        errors.append("strict V43 completed M3RC-B/M4R routes retained")
    ...
    gate_v25 = completion.get("strict_gate_v25_reconciliation", {})
    if (
        gate_v25.get("result_id") != gate_v25_source.get("result_id")
        or gate_v25.get("gate_a_status") != "FAIL_CLOSED"
        or gate_v25.get("accepted_top_level_hashes") != 1
        or gate_v25.get("remaining_top_level_hashes") != 6
        or gate_v25.get("M3RC_action_support_dual_identification_complete") is not True
        or gate_v25.get("M4R_typed_residual_cyclicity_complete") is not True
        or gate_v25.get("M1_common_strict_snapshot_complete") is not False
        or gate_v25.get("minimal_missing_bundle") != ["M1_COMMON_STRICT_SNAPSHOT"]
    ):
        errors.append("strict Gate V25 projection")
    if [item.get("route") for item in completion.get("route_selection", [])[:1]] != ["STRICT_COMMON_FREEZE_SNAPSHOT_AND_FINAL_CYCLIC_CONTRACTION"]:
        errors.append("strict V43 route frontier")
