...
COMPLETION_ATLAS = ROOT / "foundations/results/FOUNDATIONAL_LORENTZIAN_WEYL_BV_COMPLETION_ATLAS_V21.json"
...
def check(data: dict[str, Any] | None = None) -> tuple[list[str], dict[str, Any]]:
    ...
    full_d_source, q2_preflight_source, q2_green_source, recursive_tree_source, formal_source, typed_inverse_source, gate_v7_source = load(COMPLETION_FULL_D), load(COMPLETION_Q2_PREFLIGHT), load(COMPLETION_Q2_GREEN), load(COMPLETION_RECURSIVE_TREES), load(COMPLETION_FORMAL_COEFFICIENTS), load(COMPLETION_FIELD_EQUATION_QUOTIENT_INVERSE), load(COMPLETION_GATE_V7)
    errors: list[str] = []
    ...
    if completion.get("result_id") != "FOUNDATIONAL_LORENTZIAN_WEYL_BV_COMPLETION_ATLAS_V21":
    ...
    if completion.get("route_selection", [{}])[0].get("route") != "STRICT_386_AUTHORITATIVE_Q2_IDENTITY":
        errors.append("strict V21 route frontier")
    if completion.get("route_selection", [{}, {}])[1].get("route") != "STRICT_Q2_Q3_SOURCE_COCYCLE_CLOSURE" or any(item.get("route") == "STRICT_TYPED_FIELD_EQUATION_GREEN_INVERSE" for item in completion.get("route_selection", [])):
        errors.append("strict V21 source-closure frontier")
