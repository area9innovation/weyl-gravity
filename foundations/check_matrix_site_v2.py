...
COMPLETION_ATLAS = ROOT / "foundations/results/FOUNDATIONAL_LORENTZIAN_WEYL_BV_COMPLETION_ATLAS_V39.json"
...
COMPLETION_GATE_V21 = ROOT / "quantum-weyl/classical_import/certificates/CLASSICAL_IMPORT_GATE_V21_RECONCILIATION.json"
...
def check(data: dict[str, Any] | None = None) -> tuple[list[str], dict[str, Any]]:
    ...
    if completion.get("result_id") != "FOUNDATIONAL_LORENTZIAN_WEYL_BV_COMPLETION_ATLAS_V39":
    ...
    if route_names[:2] != ["STRICT_TYPED_RESIDUAL_CYCLICITY", "STRICT_COMMON_FREEZE_SNAPSHOT_AND_FINAL_CYCLIC_CONTRACTION"]:
        errors.append("strict V39 residual-cyclicity/freeze route frontier")
    if "STRICT_ENDPOINT_TO_RESIDUAL_SPECTRAL_COMPARISON" in route_names:
        errors.append("strict V39 completed M3R route retained")
    ...
    gate_v21 = completion.get("strict_gate_v21_reconciliation", {})
    if (
        gate_v21.get("result_id") != gate_v21_source.get("result_id")
        or gate_v21.get("gate_a_status") != "FAIL_CLOSED"
        or gate_v21.get("accepted_top_level_hashes") != 1
        or gate_v21.get("remaining_top_level_hashes") != 6
        or gate_v21.get("M3L_common_endpoint_sdr_bound") is not True
        or gate_v21.get("M3R_typed_residual_comparison_constructed") is not True
        or gate_v21.get("M4L_local_graph_cyclic_pairing_complete") is not True
        or gate_v21.get("M4R_typed_residual_cyclicity_complete") is not False
        or gate_v21.get("minimal_missing_bundle") != ["M1_COMMON_STRICT_SNAPSHOT", "M4R_TYPED_RESIDUAL_CYCLICITY"]
    ):
        errors.append("strict Gate V21 projection")
    if [item.get("route") for item in completion.get("route_selection", [])[:2]] != ["STRICT_TYPED_RESIDUAL_CYCLICITY", "STRICT_COMMON_FREEZE_SNAPSHOT_AND_FINAL_CYCLIC_CONTRACTION"]:
        errors.append("strict V39 route frontier")
