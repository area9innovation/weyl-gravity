...
TEXT_KEYS={'title','label','question','meaning','plain_meaning','includes','summary','boundary','migration_rationale','statement','description','object','adds','establishes','does_not_establish','sufficient_base','rationale','scope','note','notes','paragraphs','heading','deck','background','purpose','candidate_upper_bound','establishes_if_formalized','open','separation'}
...
def generate():
    ...
    payload['ladder_index']=ladder_terms.inventory(json.loads((ROOT/'foundations/site/data.json').read_text())['ladder'])
    inputs.update(payload['ladder_index']['input_hashes'])
    ...
    lines+=['','## Strength-ladder terminology','',f"The page-specific index covers displayed stage names, headings, bases, additions, established statements, exclusions and separation notes. It currently lists {payload['ladder_index']['term_count']} concepts/status labels; {payload['ladder_index']['dictionary_links']} link to related dictionary entries. The remainder are definition work, not implicitly explained vocabulary.",'','| Term | Stages | Dictionary coverage |','|---|---|---|']
    for term in payload['ladder_index']['terms']:
        levels=', '.join(dict.fromkeys(m['level'].split('_')[0] for m in term['matches']))
        lines.append(f"| {term['label']} | {levels} | {term['dictionary_id'] or 'pending'} |")
