#!/usr/bin/env python3
"""Schema, determinism and independence verifier for completion atlas V13."""
...
SCHEMA = HERE / "schema/foundational-lorentzian-weyl-bv-completion-atlas-v13.schema.json"
...
builder = module(HERE / "build_lorentzian_weyl_bv_completion_atlas_v13.py", "atlas_v13_builder")
checker = module(HERE / "check_lorentzian_weyl_bv_completion_atlas_v13.py", "atlas_v13_checker")
...
def main() -> int:
    ...
    for token in ("Exact fixed-basis canonical shear", "1321", "A(-Tsharp)", "T(-Asharp)", "PBW", "graph", "Green", "Gate A", "Hadamard", "QME", "Boundaries"):
        if token not in report:
            errors.append("report token " + token)
    print("FOUNDATIONAL_LORENTZIAN_WEYL_BV_COMPLETION_ATLAS_V13_SCHEMA: " + ("PASS" if not errors else "FAIL"))
    for error in errors:
        print("  - " + error)
    return bool(errors)
