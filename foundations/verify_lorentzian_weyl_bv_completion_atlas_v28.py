#!/usr/bin/env python3
"""Verify schema, semantics, exact types and determinism for Atlas V28."""
...
SCHEMA = ROOT / "foundations/schema/foundational-lorentzian-weyl-bv-completion-atlas-v28.schema.json"
BUILDER = ROOT / "foundations/build_lorentzian_weyl_bv_completion_atlas_v28.py"
CHECKER = ROOT / "foundations/check_lorentzian_weyl_bv_completion_atlas_v28.py"
...
def verify(value: dict | None = None, report: str | None = None) -> list[str]:
    ...
    errors.extend("checker: " + item for item in module("atlas_v28_checker", CHECKER).check(value))
    errors.extend("floating-point value in exact new block: " + item for item in floats(value.get("strict_shifted_auxiliary_cubic_inventory", {})))
    for token in (value.get("result_id", ""), "Known required / complete / open families: **7 / 2 / 5**", "vv field / cotangent coefficients: **22 / 16**", "Gate V10 remains **FAIL_CLOSED**"):
        if token not in report:
            errors.append("report token missing: " + token)
    if value == json.loads(RESULT.read_text()):
        result, generated_report = module("atlas_v28_builder", BUILDER).generated()
...
def main() -> int:
    errors = verify()
    print("FOUNDATIONAL_LORENTZIAN_WEYL_BV_COMPLETION_ATLAS_V28_SCHEMA: " + ("PASS" if not errors else "FAIL"))
    for error in errors:
        print("  - " + error)
    return bool(errors)
