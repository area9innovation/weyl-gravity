...
SOURCE = ROOT / "foundations/build_lorentzian_weyl_bv_completion_atlas_v44.py"
CHECKER = ROOT / "foundations/check_lorentzian_weyl_bv_completion_atlas_v44.py"
...
SCHEMA = ROOT / "foundations/schema/foundational-lorentzian-weyl-bv-completion-atlas-v44.schema.json"
...
producer = load(SOURCE, "lorentzian_weyl_bv_completion_atlas_v44_source")
checker = load(CHECKER, "lorentzian_weyl_bv_completion_atlas_v44_checker")


class LorentzianWeylBVCompletionAtlasV44Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_m1_routes_are_first_and_ordered(self):
        self.assertEqual([row["route"] for row in self.value["route_selection"][:3]], [
            "STRICT_M1A_FULL_TYPED_CARRIER_LEDGER", "STRICT_M1B_REPRESENTED_COMPOSITE_CONTRACTION", "STRICT_M1C_COMMON_MANIFEST_REPLAY",
        ])

    def test_partition_mutation_fails(self):
        value = copy.deepcopy(self.value)
        value["strict_m1_common_snapshot_preflight"]["hash_objects_ready"] = 5
