...
SOURCE = ROOT / "foundations/build_lorentzian_weyl_bv_completion_atlas_v38.py"
CHECKER = ROOT / "foundations/check_lorentzian_weyl_bv_completion_atlas_v38.py"
...
producer = load(SOURCE, "lorentzian_weyl_bv_completion_atlas_v38_source")
checker = load(CHECKER, "lorentzian_weyl_bv_completion_atlas_v38_checker")


class LorentzianWeylBvCompletionAtlasV38Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_m4l_route_completed_and_m3r_first(self):
        routes = [row["route"] for row in self.value["route_selection"]]
        self.assertNotIn("STRICT_FULL_CYCLIC_PAIRING", routes)
        self.assertEqual("STRICT_ENDPOINT_TO_RESIDUAL_SPECTRAL_COMPARISON", routes[0])
        self.assertEqual("STRICT_TYPED_RESIDUAL_CYCLICITY", routes[1])
        self.assertTrue(self.value["claim_flags"]["strict_M4L_local_graph_cyclic_pairing_complete"])
