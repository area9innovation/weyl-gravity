...
builder = module(HERE / "build_lorentzian_weyl_bv_completion_atlas_v11.py", "test_atlas_v11_builder")
checker = module(HERE / "check_lorentzian_weyl_bv_completion_atlas_v11.py", "test_atlas_v11_checker")
...
class CompletionAtlasV11Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_q1_coefficient_count_mutation_fails(self):
        value = deepcopy(self.value)
        value["strict_full_q1_component_jet_table"]["nonzero_rational_coefficients"] += 1
        self.assertTrue(checker.check(value))

    def test_sdr_promotion_fails(self):
        value = deepcopy(self.value)
        value["claim_flags"]["strict_386_full_sdr_operator_tables_serialized"] = True
        self.assertTrue(checker.check(value))
