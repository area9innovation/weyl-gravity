...
def verify(*, value: dict[str, Any] | None = None) -> tuple[list[str], list[str]]:
    ...
    for assembly in assemblies:
        ...
        if len(assembly.get("interfaces", [])) != 7 or any(item.get("relation") != "NOT_ASSESSED" or item.get("evidence") for item in assembly.get("interfaces", [])):
            errors.append("fail-closed interfaces " + assembly["id"])
    ...
    checks.append("independent cell selection, coverage, interfaces, and hard gates")
    ...
    for name in ("prototype_assemblies_generated", "selected_cells_content_addressed", "interface_and_coverage_states_separated", "empirical_record_schema_declared"):
