#!/usr/bin/env python3
"""Fail-closed schema, generator and independence verifier for atlas V4."""
...
RESULT = DIRECTORY / "results/FOUNDATIONAL_LORENTZIAN_WEYL_BV_COMPLETION_ATLAS_V4.json"
REPORT = DIRECTORY / "reports/lorentzian-weyl-bv-completion-atlas-v4.md"
SCHEMA = DIRECTORY / "schema/foundational-lorentzian-weyl-bv-completion-atlas-v4.schema.json"
CHECKER = DIRECTORY / "check_lorentzian_weyl_bv_completion_atlas_v4.py"
...
builder = module(DIRECTORY / "build_lorentzian_weyl_bv_completion_atlas_v4.py", "completion_atlas_v4_builder")
checker = module(CHECKER, "completion_atlas_v4_checker")
...
def verify(value: dict | None = None, report: str | None = None) -> tuple[list[str], list[str]]:
    ...
    for token in ("Strict Gate-A progress", "540 -> 0", "Causal convention-stability result", "386=356+30", "381 positive", "common bytes", "PRA", "Updated route selection", "does not establish"):
        if token not in report:
            errors.append("report token " + token)
    return errors, [
        "Draft 2020-12 schema",
        "independent 77-cell audit",
        "V3 preservation",
        "Gate-V5 minimal/full-carrier firewall",
