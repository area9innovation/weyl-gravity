...
def main() -> None:
    ...
    print(
        ...
        "machine-readable four-flag claim DAG now certifies all four terminal "
        "flags. The selected curvature route has exact curved "
        "Weyl--Cotton equations, a formally equivalent symmetric-hyperbolic "
        "constraint adjustment with compatible sources, and the symbolic all-level "
        "E/A/L spectrum. The support-local all-row mapping cylinder and its "
        "off-shell prolonged-current comparison are exact. Curved cyclic BGG "
        "transfer, the trace/Weyl shear, and the 356/30 hybrid retract give a "
        "causal homotopy on all 386 prolonged rows. Its causal difference is a "
        "compact-to-global quasi-isomorphism on the cylinder; cutoff sources "
        "recover all fifteen CKVs and their dual endpoints without duplication, "
        "and the cutoff commutator gives SO(4,2)-equivariant transport up to "
        "chain homotopy. Direct cyclic Green/current transport identifies the "
        "covariant, Cauchy, energy, and residual pairings. Thus final covariant "
        "H4 is spanned by W_+^2 and W_-^2 with Gram matrix I2, without a second "
        "CE computation. The canonical prolonged Green witness and canonical "
        "endpoint causal Green inverse remain scoped false implementation flags "
        "and are not dependencies of the direct tractor route. A direct "
        "same-bundle metric factorization remains ruled out in its certified family. "
