...
def _mutation_guards(record: dict[str, Any]) -> list[str]:
    ...
    mutant["work_packages"]["relational_clock"]["evidence_refs"] = [
        ref
        for ref in mutant["work_packages"]["relational_clock"]["evidence_refs"]
        if ref != "berger_minimal_bv_clock_sdr"
    ]
    rejected("berger_minimal_clock_sdr_erased", mutant)

    mutant = deepcopy(record)
    mutant["work_packages"]["relational_clock"]["evidence_refs"] = [
        ref
        for ref in mutant["work_packages"]["relational_clock"]["evidence_refs"]
        if ref != "berger_retained_minimal_layout"
    ]
    rejected("berger_retained_layout_erased", mutant)

    mutant = deepcopy(record)
    mutant["work_packages"]["relational_clock"]["evidence_refs"] = [
        ref
        for ref in mutant["work_packages"]["relational_clock"]["evidence_refs"]
        if ref != "berger_retained_minimal_operator_preflight"
    ]
    rejected("berger_retained_operator_preflight_erased", mutant)

    mutant = deepcopy(record)
    mutant["work_packages"]["relational_clock"]["evidence_refs"] = [
        ref
        for ref in mutant["work_packages"]["relational_clock"]["evidence_refs"]
        if ref != "berger_retained_minimal_operator"
    ]
    rejected("berger_retained_operator_erased", mutant)
    ...
    mutant["work_packages"]["relational_clock"]["evidence_refs"] = [
        ref
        for ref in mutant["work_packages"]["relational_clock"]["evidence_refs"]
        if ref != "berger_minimal_34_portable_contraction"
    ]
    rejected("berger_minimal_34_portable_contraction_erased", mutant)
    ...
    mutant["work_packages"]["relational_clock"]["evidence_refs"] = [
        ref
        for ref in mutant["work_packages"]["relational_clock"]["evidence_refs"]
        if ref != "berger_rational_fixture_q2_d_block"
    ]
    rejected("berger_rational_fixture_q2_d_block_erased", mutant)

    mutant = deepcopy(record)
    mutant["work_packages"]["relational_clock"]["evidence_refs"] = [
        ref
        for ref in mutant["work_packages"]["relational_clock"]["evidence_refs"]
        if ref != "berger_nonzero_d_weight_finite_block_no_go"
    ]
    rejected("berger_nonzero_d_weight_finite_block_no_go_erased", mutant)
    return failures
...
def main() -> int:
    ...
    if args.guards:
        ...
        if failures:
            ...
            return 1
        print("mutation guards: 24/24 PASS")
