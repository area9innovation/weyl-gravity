...
def build() -> dict:
    ...
    payload = {
        ...
        "gauge_and_causal_audit": {
            ...
            "source_covariance": "the switch may equivalently be written as chi((theta-tau_source)/omega), so the preparation family transforms with the clock rather than selecting an absolute coordinate time",
        ...
        "claim_boundary": "This exact G0 retarded probe-mode theorem upgrades the kinematic 1+z=2 fixture to a genuine retarded Maxwell signal: a compact spacetime source prepares the certified two-phase mode before emission, and the clock-slice field-strength observable is Maxwell-gauge, Weyl, diffeomorphism and raw-D invariant with retarded causal dependence. It treats the periodic S1 clock by an integer winding label, derives the exact reduced bracket and nontrivial lifted-tau evolution, and explains why a family of Dirac observables can evolve relationally although raw D is gauge. It remains a spatially averaged probe-sector theorem and does not exhaust a full harmonic signal sector. The localized rod/memory observer morphism stops fail-closed because the mixed epsilon_R^2*kappa profile, transport and cotangent coefficients are absent from the certified 84-row axial first-jet complex. It does not claim their nonexistence, a localized endpoint algebra, full Maxwell backreaction, an unqualified 84-row theorem, phenomenology, Hadamard data, a QME result, or any quantum statement.",
    }
    verify(payload)
    return payload
