...
- Jet-aware transverse parent/middle gate: an exact bivariate Taylor-jet
  recurrence derives every coordinate covariant curvature jet requested
  through order three and transforms it to the moving orthonormal frame.  It
  independently reproduces the certified first jet and closes the corrected
...
- Transverse formal metric Green variation: the Kantowski--Sachs tangent is
  generated by an exact Einstein family on every fixed compact time slab.
  Differentiating the global four-row metric Green identity gives the finite
  same-sided Duhamel formula
  `Gdot_+/-=-G0_+/- Pdot G0_+/-`, both inverse identities, the differentiated
  chain homotopy and causal support.  The rank-310 transfer remains
  fail-closed: its current SDR variation is a fifth-order Taylor/PBW jet at
  `t=asinh(1)`, not a global smooth coefficient export, and the transverse
