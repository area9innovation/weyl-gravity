...
def entries() -> list[dict[str, Any]]:
    ...
    values.append(_entry(
        ...
        ("OBSTRUCTED", "The frozen-parallel shortcut and a purely algebraic transverse Schur correction are obstructed. The complete first-order endpoint gauge equation has a unique solution. Phi-only repair is obstructed, the homogeneous incidence/L1 continuation is rigid, and the complete 44-dimensional normalized-L0 family has rank 44 versus augmented rank 45. Formal K directions hit the remaining witness, but the authoritative action-derived transverse delta_K is exactly zero, so those directions are inadmissible."),
        ...
        _evidence("Nariai_transverse", "Nariai_incidence", "Nariai_PBW_gate", "Nariai_jet_aware_parent", "Nariai_first_order_schur", "Nariai_Phi_only_obstruction", "Nariai_incidence_L1_rigidity", "Nariai_normalized_L0_obstruction", "Nariai_K_admissibility", "cone"),
        "Every requested curvature jet is derived and the jet-aware BGG first square and parent identity are exact. The unique 59-coefficient endpoint gauge repair does not close the separate 207-coefficient shifted-chain defect. Phi-only repair is obstructed; the homogeneous algebraic-incidence/order-at-most-one-L1 first-square map is invertible; and the normalized-L0 response map has rank 44 but augmented rank 45 with a five-term witness. Exactly 23 of 180 formal order-at-most-one K directions hit that witness, but the action-derived transverse first-BGG variation has zero coefficients, so none is an admissible repair of the fixed metric BV complex. Equation/constraint/cotangent variation, action-derived cyclic identification, complete SDR and causal transfer remain open.",
    ))
    values.append(_entry(
