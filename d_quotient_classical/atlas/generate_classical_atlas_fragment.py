...
def entries() -> list[dict[str, Any]]:
    ...
    values.append(_entry(
        ...
        ("OBSTRUCTED", "The relative obstruction class factors exactly through the five Taub charges in normalized smooth finite-harmonic quotient coordinates. The complete current C_X is portable through coefficient-jet order two, but the endpoint-normalized 406-parameter invariant order-one chain system has rank 398 and augmented rank 399. Its two-row left-null witness blocks that order-one unary incidence before f2 can be tested. The complete 626-dimensional invariant A1 order-two symbol space acts surjectively on this one-dimensional obstruction quotient, so a full order-two solve is now authorized but remains open."),
