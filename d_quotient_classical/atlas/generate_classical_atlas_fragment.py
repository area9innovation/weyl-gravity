...
def entries() -> list[dict[str, Any]]:
    ...
    values.append(_entry(
        ...
        ("OPEN", "The frozen-parallel shortcut is obstructed. The associative coefficient-jet PBW backend, corrected splittings, parent middle, shifted chain, factorized Hom adjoint, compressed Schur, upper relative-saddle row, scaled endpoint target and action-derived Bach-Hessian variation now close exactly. The complete rank-310 SDR and causal transfer remain open; the earlier Phi/L0/K rank screens remain backend regression data, not operator no-go theorems."),
