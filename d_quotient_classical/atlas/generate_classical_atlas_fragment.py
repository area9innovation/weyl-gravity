...
def entries() -> list[dict[str, Any]]:
    ...
    values.append(_entry(
        ...
        _evidence("complex_compensator_minimal_action_no_go", "complex_compensator_active_clock_px2_no_go", "complex_compensator_active_clock_px2_freeze_audit"),
        "The producer's exact cylinder-plus-Berger Euler matrix has rank five and kernel t(81/20,27/3290,-324/1645,486/1645,18/25,1). A method-distinct freeze audit reconstructs the declared invariant basis and Berger tensors, clears denominators and derives the same primitive kernel from all six integer maximal cofactors. For every nonzero t, the coupled homogeneous velocity spectrum contains +3 and -3. Independently, cylinder standard-sign clock health requires t<0, while the Berger principal tensor has P_X=-81t/200 and P_X+2XP_XX=-531t/200, so Berger standard-sign hyperbolicity requires t>0 and has sound speed squared 9/59. The common good locus is EMPTY, the scoped quadratic-active-clock no-go is theorem-frozen, and no Candidate C_active is selected. This is a scoped quadratic-P(X), two-background result, not a universal k-essence or compensator no-go. It constructs no complete causal parent and establishes no Hadamard, anomaly/QME, particle, scattering, positivity or unitarity result.",
    ))
    values.append(_entry(
