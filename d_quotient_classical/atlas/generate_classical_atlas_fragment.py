...
def entries() -> list[dict[str, Any]]:
    ...
    values.append(_entry(
        ...
        ("OPEN", "The frozen-parallel shortcut is obstructed. The associative coefficient-jet PBW backend, corrected splittings, parent middle, shifted chain, factorized Hom adjoint, compressed Schur, upper relative-saddle row, action-derived Bach-Hessian variation and complete ten-block rank-310 cyclic SDR first variation close exactly. The global four-row metric Green homotopy now has an exact same-sided formal first variation. Rank-310 causal transfer remains open because its differentiated SDR coefficients are certified only as a one-point Taylor/PBW jet."),
