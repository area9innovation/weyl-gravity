...
## Third-order Kuranishi input obstruction

The first higher operation has been fixed formally. With
`Phi=Phi_bar+epsilon*u+epsilon^2*v+epsilon^3*w+...`, the cubic equation is
...
Changing the chosen second-order correction by a homogeneous solution
`v -> v+z` changes the representative by `l2(u,z)`. The intrinsic candidate is
therefore `[K3] in O/im(l2(u,-))`. On the real balanced two-amplitude slice,
`l2(u,-)` has rank one, so the formal quotient has dimension four. Gauge
representative independence is not promoted: it additionally needs the second
jet of the bundle-covariant algebroid action and the arity-three Noether
identity.

The smallest complete third-order closure is now exact. Its output parity is
axial, its angular rows are `ell=2,4,6`, and its frequency set consists of the
sixteen signed odd-length lattice points generated by `omega_minus` and
`omega_extra`. Exact shell comparison finds only four coincidences: the
original `ell=2` q-minus frequencies `(+-1,0)` and p-extra frequencies
`(0,+-1)`. No new shell type appears kinematically.

The cubic class itself is not yet defined by the available action payload. The
complete second-order certificate contains `D2E` on two first-order axial
inputs and a chosen correction `v`, but it does not serialize
`D3C[u,u,u]`, the mixed `D2C[u,v]` tensor on the `ell=0,2,4` correction
channels, their stabilizer/resonant adjoint projections, or the arity-three
gauge/Noether identity. Therefore neither the balanced cubic obstruction nor
the sufficiency of the five quadratic Taub charges through third order is
decided.
