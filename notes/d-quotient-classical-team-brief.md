...
## Paper 9 legacy source-binding gate (2026-07-21)

The ten-claim classical table selected the evidence-preserving `REPIN`
disposition.  Its scientific rows and scopes are unchanged; only the main and
supplement TeX hashes moved to the publication-current sources.  The table now
also pins the `DRAFT_ALLOWED` 22-claim superset, its closeout report and its
receipt, with explicit mutations rejecting stale hashes, dropped claims,
scope widening and silent certificate deletion.  The publication superset
must be regenerated by its owner to import the repinned table hash; no Paper 9
file was edited by the classical lease.
