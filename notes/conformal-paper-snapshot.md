...
This is the fast required rail: every non-slow positive certificate plus all
fail-closed overclaim guards.  Pull requests and ordinary pushes run this
rail.  For a publication snapshot or complete independent reproduction use:

```bash
python3 symbolic/verify_conformal_paper_free.py --reproduce --timeout 1800
```

The exhaustive rail runs every positive certificate and every guard.  The
weekly/manual CI job also checks generated-certificate freshness and uploads
the receipts.  The durable terminal evidence is indexed by the
[covariant H4 proof ledger](../covariant_completion/generated/covariant_H4_proof_ledger.md).
The full command list, artifact policy, scoped legacy routes, and current
out-of-scope boundary are in the
[publication and reproduction guide](conformal-publication-reproduction.md).

Focused compatibility switches remain available for local development:
...
completed H^4=span{W+^2,W-^2}, G=I2
             |
             v
curved auxiliary operator + local SDR + d+Q current comparison
             |
             v
Weyl--Cotton prolongation + curved cyclic adjoint-tractor retract
             |
             v
386-row retarded/advanced causal homotopies
             |
             v
compact-to-global causal quasi-isomorphism on R x S3
             |
             +--> 15 CKVs + 15 dual endpoints, no duplication
             +--> SO(4,2) transport up to compact chain homotopy
             +--> direct Green/current pairing transport
             |
             v
H^4_cov=span{W+^2,W-^2}, G_cov=I2
```
...
The direct curved adjoint-tractor route now closes the missing bridge.  Its
all-row retarded/advanced homotopies give the causal quasi-isomorphism, recover
the fifteen CKVs and dual endpoints without duplication, transfer the
`SO(4,2)` action up to compact chain homotopy, and identify the causal/current
pairing.  Thus `final_covariant_H4=true` with `H4_cov=C2` and `G_cov=I2`.
The canonical endpoint inverse and monolithic prolonged witness remain false
only as scoped legacy implementation flags.

### Historical causal-witness development record

The following chronology is retained to explain the scoped no-go results and
the design path that preceded the direct tractor closure.  Statements below
that a causal or transport flag "remained false" describe their historical
checkpoint, not the current dependency DAG.

At that checkpoint the BV-canonical graph SDR, autonomous `26 -> 40 -> 14`
curvature complex with cotangent adjoint, and analytic block witness were
exact.  The state/equation chain map was exact:
...
The actual curved `p_E/p_I` maps proved `N A_eq=B_id C_aux`, the complete
degree/sign-resolved odd BV mapping cylinder, nilpotent prolonged operator,
and support-local all-row SDR.  The generic prenormal
...
| Curved current workstream | Exact off-shell and causal pairing comparison | Both curved potentials and their `d+Q` improvement are certified, together with the Cauchy and `E/A/L` regressions; the direct cyclic transport identifies the Green/current pairing on cohomology |
| Covariant final-claim dependency DAG | Exact closed meta-certificate | All terminal requirements are true and content-bound; `final_covariant_H4=true`, with the existing residual `C2` and `I2` transported rather than recomputed |
...
| Curvature-prolonged causal BV bridge | Exact direct tractor causal theorem | The curved Weyl--Cotton system, all-level spectrum, all-row local prolongation, cyclic adjoint-tractor retract, and 386-row causal homotopies are exact. Their causal difference is a compact-to-global quasi-isomorphism; all fifteen endpoint pairs are recovered without duplication, `[kappa,rho]=[Q,[chi,rho]]` gives homotopy-equivariant transport, and the current pairing agrees with the causal pairing. The canonical endpoint inverse and monolithic prolonged witness remain scoped legacy false flags, not blockers. |
