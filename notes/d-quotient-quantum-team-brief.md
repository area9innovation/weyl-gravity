...
The gravity--Maxwell \(q_2\) remains a classical input, not a quantum vertex.
The committed repair doubles all Maxwell-output components and adds the
BV-canonical coboundary \([q_1,F_2]\) for
\(F_2(c_M)=c_M-2\iota_cA\), including its complete cotangent lift.  The
quantum-side content-addressed consumer independently reconstructs 1,890 full
and 1,474 retained coefficients.  Full and retained \(q_1q_2\), full and
retained cyclicity, and all transfer missing/extra/changed counts vanish
...
That AFN0 total-complex step is now complete. The universal total-form
comparison reduces covariant Diff and mixed carriers to the certified Weyl
top representatives and their generated descendants. The non-covariant
pure-gravity sector is controlled by degree-three invariant polynomials of the
...
is fixed, but it is not the complete renormalized `Q1`. A separate exact
Paneitz/Riegert solve now supplies one anomaly-induced Euclidean `Gamma1`
representative. In the functional basis
`(<Ecal4,G4 C2>,<Ecal4,G4 Ecal4>,integral R2)` its coefficients are
`(199/120,-87/160,29/120)`; the last coordinate converts
`Ecal4=E4-(2/3)BoxR` back to the certified `BoxR=0` scheme. The result is
conditional on an invertible boundary problem or compatible source sector,
and retains Paneitz kernel/global data as open. The allowed extended
counterterms still have an exact rank-two bulk response on flat TT and
conformal fixtures, generated by `C(g_hat)^2` and `R(g_hat)^2`. The
universal nonzero-momentum flat-TT logarithmic form factor is now also exact:

\[
 F_C(p^2;\mu)=-\frac{199}{60}\log\frac{p^2}{\mu^2}+z_C(\mu),
 \qquad \mu\partial_\mu F_C=\frac{199}{30}.
\]
...
all ten pole-three rows now also possess exact relative-IBP reductions to the
scalar triangle and two independent first derivatives. Four explicit
...
mixed rows and complete physical form-factor assembly. That gate is now
split exactly. Each contact numerator is quadratic in its Feynman parameter,
so its two certified endpoints, midpoint and Wick-pair coefficient determine
the full minimally-subtracted finite contact row. All three contacts are now
...
The minimally-subtracted finite contact rows are exact as well. The remaining
physical gate is the renormalized tensor-triangle bulk, explicit finite-
counterterm normalization and complete form-factor assembly. The isolated
...
its statement rather than inventing an out-of-schema lifecycle value.
...
The first-order operator boundary is certified separately in
[`one-loop-slavnov-q1-disposition.md`](../quantum-weyl/reports/one-loop-slavnov-q1-disposition.md).
The anomaly-induced functional representative and its zero-mode boundary are
certified in
[`anomaly-induced-nonlocal-gamma1.md`](../quantum-weyl/reports/anomaly-induced-nonlocal-gamma1.md).
...
BERGER_TEMPORAL_CUTOFF_COMPANION_GREEN_FAMILY [CERTIFIED]
  ...
  -> GLOBAL_GHOST/IDENTITY_HADAMARD + EXACT CCR [CERTIFIED]
  -> OMEGA26=W26 H26 EXACT-CCR CANDIDATE [CERTIFIED]
  -> smooth q26-equivariant Feynman/Ward completion
...
with exact CCR. The three ghost and three identity rows now also have a
global exact-CCR Hadamard pair, constructed from their two normally
hyperbolic factors and formal adjoint. With all endpoint carriers available,
the cyclic witness gives
\(\Omega_{26}^{+}=W_{26}H_{26}^{+}\), and its exact Ward defect is the smooth
kernel \(W_{26}[H_{26}^{+},q_{26}]\). The remaining direct task is a
\(q_{26}\)-equivariant global Feynman selection or a smooth correction on a
declared past/future-compact or time-slice bikernel class. The existing
compact-source causal homotopy does not by itself authorize that extension.
The conditional 26-to-54 lift is already certified and must not be rebuilt.
The BRST Ward identity and physical/Krein positivity policy remain
independent gates.
...
The completed six-row pair and exact retained Ward reduction are recorded in
[`berger-ghost-identity-global-hadamard-pair.md`](../quantum-weyl/reports/berger-ghost-identity-global-hadamard-pair.md)
and
[`berger-retained26-hadamard-ward-reduction.md`](../quantum-weyl/reports/berger-retained26-hadamard-ward-reduction.md).
...
## Conditional all-order formal local QME stability (2026-07-20)

The formal \(\tau\)-adic compensator theory now has a complete
algebraic-renormalization induction. Its antifield-complete anomaly module
vanishes,
...
and its stable dimension-four counterterm module is freely generated by
...
modulo the horizontally exact \(\Box R(\widehat g)\) direction and
\(s_{\rm ext}\)-exact canonical transformations. The dressed
\(R(\widehat g)^2\) direction is indispensable.

Counterterm deformations raise the complete formal coupling filtration, so
the Koszul--Tate contraction persists through
...
Consequently, if a quantum action principle makes the first order-\(n\)
breaking local in the declared algebra and supplies the Wess--Zumino
consistency equation, the breaking is exact and removable. Coupling
redefinitions and canonical transformations keep the repair inside the same
stable module, closing the induction at every finite formal order.

The result is deliberately conditional. No all-order regulator or
subtraction scheme proving that quantum action principle on the completed
\(\tau\)-adic domain has been constructed. That analytic realization is now
a separate routed request. The strict theory remains obstructed at one loop,
and no convergence, global-anomaly, Lorentzian-QME, residual, state or
particle claim follows.

This result is `LOCAL-ALGEBRAIC`.
