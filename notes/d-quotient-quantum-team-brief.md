...
is fixed, but it is not the complete renormalized `Q1`. The allowed extended
counterterms have an exact rank-two bulk response on flat TT and conformal
fixtures, generated by `C(g_hat)^2` and `R(g_hat)^2`. The finite nonlocal
effective action, renormalized BV Laplacian or time-ordered product, and
finite normalization conditions remain absent. Complete `Q1` is therefore
fail-closed as `NO_CERTIFIED_OPERATOR`, independently of the still-missing
