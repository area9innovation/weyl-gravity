...
Observer:    implement detector-coefficient and nested causal-convolution interval backends, then the tail-aware four-stream loop before restricting records to Z2^C
...
The first executable successor now aggregates one supplied shell with exact
rational intervals.  It requires both feedback channels and every passive
column, applies `g_b g_c^2` and `(two_j+1)/Vol_Berger`, and detects deletion of
those factors.  The readiness audit promotes only this shell evaluator.
Detector-coefficient intervals, nested causal time convolutions and the
tail-aware four-stream stop loop remain `OBSTRUCTED`, so external numerical
inputs remain deferred under the strict specialization schema.
...
five-carrier quotient as rational simplex integrands. Exact Hodge-resolvent
...
carrier data, simplex-integrated
repository functions and coefficients, and the parity-odd derivative
...
| `BERGER_RECOIL_SCALAR_STREAM_ACTIVATION_GATE` (fired; shell aggregation certified, complete stream obstructed) | implement detector coefficient and nested causal-convolution interval backends; defer external parameters |
...
| `BERGER_RECOIL_STREAM_EXECUTABLE_READINESS_AUDIT` (fired; shell evaluator certified, three capabilities obstructed) | implement the detector coefficient provider, nested causal convolution and tail-aware four-stream stop loop; only then activate the exact numerical input schema |
