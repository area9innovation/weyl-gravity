# Universe-building roadmap and team activation gates

## Purpose

The programme now has enough exact infrastructure to organize its next work in
terms of recognizable physical phenomena: clocks and redshift, gravitational
waves, particles, black holes, galaxies, and cosmology.  This roadmap does not
promote any of those phenomena before their mathematical gates pass.  It says
which present calculation unlocks which later question, and when team capacity
should move.

The shared scientific question remains:

> Is cylinder-time translation `D` genuinely gauge after clocks,
> interactions, quantization, or boundaries are added?
...
| Two-phase counterflow Berger successor | a changed action with two phases and one auxiliary diagonal `U(1)` has an exact 70-component cyclic BV parent on the selected Berger point; the diagonal gauge sector is contractible, the original dressed-trace class is eliminated, and support-local retarded/advanced chain homotopies are certified; the exact fixed-action homogeneous stationary classification makes the positive point the isolated singleton `(q,x,C)=(9/40,1,9/16)`, with a positive reduced homogeneous trace block and no same-action path to the round cylinder; the derived fixed-`Q_rel` reduction is exactly acyclic in the relative-clock sector, so making `D` null removes the clock; the unrestricted homogeneous clock is an exact positive action--angle system: its zero Jordan chain is the tangent to charge-dependent frequency, fixed-charge orbital and shifted-frequency modulated stability pass, and no negative or exponential mode occurs, although absolute phase stability fails and the full coupled constraints isolate the selected charge; the first all-Hodge audit is exactly undefined because the 70-row parent lacks same-background Berger scalar/vector/tensor/exceptional inclusions, projections and physical block maps; the selected background fails conventional same-source Einstein--matter incidence, and the exact five-dimensional stabilizer cannot yet be promoted to a rowwise residual receiver because the four spatial lifts and moment maps are absent | split scalar, vector/tensor and exceptional-stabilizer carrier exports followed by the full nonhomogeneous pairing/gradient/characteristic assembly; a nonzero receiver class and relational readout; structural stability under action deformation; action-derived `q2`; a different common background for any Einstein bridge; action-specific anomaly/Hadamard data, particles, or unitarity |
| Compact Einstein--Maxwell product | complete standard harmonic Einstein--Maxwell inclusion with nondegenerate nonsymplectic Weyl--Maxwell pullback; direct axial and polar extra-branch Lee--Wald currents; maximal parity-complete pre-residual solution exact sequence with generic dimensions `4 -> 8 -> 4` and zero radicals; a natural ungauged equation/Noether map whose fixed-identity strict cyclic BV lift is generically obstructed; pure-extra Taub no-go, classified mixed moment-map cones, and a two-parameter second-order-extendible face | corrected nonidentity or up-to-homotopy cyclic maps, the common moment-map-zero residual carrier, quadratic-source disposition on every mixed-cone stratum, nonlinear causal closure, or asymptotic radiation |
...
all-Hodge audit has reached an exact carrier shortfall: the 70-row parent does
not export same-background Berger Peter--Weyl inclusions or projections, so
even its first scalar physical block is undefined.  The repair is now split
into scalar, vector/tensor, exceptional-stabilizer and final assembly tasks.
This is missing input, not a negative spectrum.  Nonlinear,
...
an action-derived Euclidean gauge-fixed Lagrangian integration slice and full
Hessian, including determinant/Berezinian row map, measure, domains, zero-mode
projectors, contours and regulator.  The existing 54-row classical unary
complex is not a loop operator.  Thus the zero quotient proves only that a
...
`H1^3` carrier span is now exact: the rank-49 tangent-plus-scalar-master
system grows through the `M14` singlet and a standard-`S3` pair to rank 52,
and all eleven physical rows lie in it by five exact orbit-representative
ranks plus a generated `S3` crosswalk. The three new renormalized master
values and scale derivatives are now exact in the common Mellin scheme. The
sixty-six reduced rational physical master-coordinate functions are now exact.
All eleven physical relative-IBP boundary fluxes and the five-carrier
physical Mellin-MS assembly are now exact. Adding the eleven integrated
ghost-`n=3` channels preserves the coefficientwise `I28` relation and gives
a coefficient-bearing partial-BV representative. Curved-Endo ghost
...
causal support and adjoint reversal. The cutoff Pauli--Jordan kernel now has a
Schwartz kernel, two-sided bisolution identity, no one-sided zero covectors and
a factorwise null wavefront bound; its compact-slab time-slice source map is
regular. The quantum consumer must next exclude the two same-orientation
wavefront sectors and prove the endpoint response-map cone action. The metric
companion now has a canonical indefinite RFHGHO dilation, and the past/future
agreement regions supply two regular Cauchy morphism legs; this is not yet a
full graded BV realization or a state on the undoubled carrier. A global seed
...
```text
Einstein:    the direct polar current is complete and the natural ungauged equation/Noether map exists, but the fixed-identity cyclic BV lift is obstructed; assemble the maximal parity-complete residual exact sequence, then classify corrected maps or cyclic homotopies and the common moment-map-zero carrier
Nonlinear:   continue the action-defined q2 on the full/unrestricted parent, certify that the fixed-charge clock projection is zero, and test Q_rel preservation without nonlocal projection; do not activate q3 as a healthy-clock result
Classical:   the all-Hodge census stopped at missing Berger iota/pi maps before the scalar block; build scalar, vector/tensor and exceptional-stabilizer exports, then assemble the complete health theorem
Quantum:     continue the action-specific local anomaly and full-parent Hadamard preflights, but type the fixed-charge quotient as having no relative-clock state; the older positive-Berger K-Cartan insertion remains nondefined until its Euclidean integration slice and Q1 land
Observer:    certify fixed-charge relative-clock readout as nondefined and construct a relational observable only on the unrestricted charged-D carrier, keeping raw D, R_rel and K distinct
...
order, gauge survival, and exact profile/Green/boundary inputs.  Its
restriction `O_detector|Z2^C` is `OPEN` for bounded or quasiperiodic,
...
polarization gate is now closed: exact detector prefactors and the recurrence
produce 86,736 detector-component entries and 520,416 reconstructible
...
the spatial tail.  The normalized clock moments and all nine additional
external-clock scalar shards through `p=28` are now validated without
rewriting the six lower-order shards.  Detector-prefactored polarization for
the nine new powers is now certified: 780,624 adaptive intervals and all 330
direct `p=12` low-mode comparisons pass.  All even-power inputs through
`p=28` are complete.  Closed tridiagonal `Delta1` blocks and codifferential
rows match the dense de Rham engine through `two_j=8`, and the complete
order-14 application is now streamed through `two_j=138`: 48,372 populated
blocks yield 2,147,700 spatial and 717,255 temporal coefficient intervals.
The geometric ratios contract, but exact extreme-block witnesses prove that
the truncation errors remain large, so temporal Green promotion is
`OBSTRUCTED`.  Exact angle addition now removes the large-`T` Taylor
...
retains the same width sentinels.  The noncentral Jacobi factorization is now
...
temporal approximation.  The first intermediate extension is now certified:
the adjacent even/odd `two_j=512,513`, `r=128` widths are approximately
`0.07072` and `0.07075` on a `64 x 64` grid, while the halved-resolution even
mutation remains about `0.14099`.  The declared-fraction successor now
...
on these declared two-scale rows, not another uniform-grid widening.  The
recurrence-closure successor now derives the exact scalar union required by 18
selected form-`two_j=1024` detector-component entries: 12 rows on scalar
...
maximum below `0.099`.  Those intervals are uniform over the full normalized
clock support.  The selected clock-power successor now propagates all 18
entries through the 15 even powers `p=0,2,...,28`: all 270 complex intervals
are content-addressed, `p=0` is reproduced exactly, every width remains below
`0.1`, and no clock/profile independence is assumed.  The active profile gate
is sharpened by the exact charge-block closure audit: the 18 selected real
entries seed 18 distinct `q=m+s` Maxwell blocks, with 27 structural zeros and
33 additional on-support real companions.  Direct temporal promotion is
`OBSTRUCTED`.  The companions require 18 scalar recurrence rows; 12 are
already certified and the six missing rows are `(1023;129,257,385)` and
`(1025;130,258,386)`.  The active gate is their evaluation and propagation of
the 33 companions through `p=28`, followed by exact temporal functional
calculus and a controlled spatial tail—not a complete-form or Green-image
claim.  The six-row scalar successor now closes the first part: every new
width is below `0.1` (maximum below `0.099`), with radial-only `128 x 64`
refinement at indices `385,386`.  All 18 scalar inputs are present.  The active
form-companion successor now completes the finite charge-block input gate:
33 on-support real-form companions use 84 exact recurrence-term applications
and give 495 complex intervals through `p=28`, all below width `0.1`.  With
the original 18 selected entries and 27 structural zeros, they export 270
...
spectral projectors, checked against the certified lower band.  That successor
is now certified: 27 direct eigenvalue transforms and nine exact projector
families act on all 18 completed inputs while preserving the 18 shared scalar
variables through the contraction.  Clock-transform widths are below `0.004`,
selected spatial widths below `0.02`, and amplified temporal coderivative
...
full-image promotion.  The fixed-vector direct Green-tail successor now uses
two boundary-flat clock integrations by parts to certify
`|T(lambda)|<=2304 C/lambda`.  Its frozen-profile tail is still about `124`
above retained `two_j=1024`, and retained `two_j=3421` is the first integer
cutoff making that particular bound smaller than one for both polarizations.
The physical profile moves with the clock-dependent rods and Gram factor, so
the active input is its clock-derivative/commutator bound; the frozen-vector
target does not by itself justify widening or full-image promotion.
The moving-profile successor now differentiates the physical profile at fixed
Berger spatial point through second amplitude order.  Its operator-valued
clock estimate raises the current bound to about `196` and fixes
`two_j=3835` as the first sufficient sub-unit cutoff for both polarizations.
Selected rows or the bound alone do not constitute a full Green image.  The
capacity audit replaces the provisional complete-projection materialization:
a legacy rail through
`two_j=3835` would require `44,140,852` supported entries and `662,112,780`
clock-power intervals, while the desired outputs are fixed scalars.  The
active route is therefore a response-specific shell stream with scalar tail
`B_ab E_N`.  Certifying the massive/recoil-chain dual bounds `B_ab` and the
complete modewise scalar integrand precedes any numerical stopping claim; the
Maxwell-tail-to-recoil map remains `NO_CERTIFIED_MAP` until then.
The recoil-chain norm gate now sharpens the first input: on each nonvanishing
causally ordered chain, differentiating the exact compact switch produces an
`i_grad(h_b)dA` term, while the longitudinal massive inverse is `1/m_b^2`
and supplies no high-mode smoothing.  Thus the present Maxwell `L2` tail
cannot by itself define `B_ab`.  The selected factorwise route must first
certify a correlated Maxwell energy/graph-norm tail and a finite-time massive
energy constant; a direct exact block cancellation remains an open
alternative.  No unboundedness or numerical-recoil claim is inferred.
The Maxwell energy successor now closes the first branch of that alternative:
the three four-dimensional field-strength pieces each obey the exact tail
`2304 S_a Lambda_N^(-3/2)`.  Their safe component sum is about `3.01e5`
above retained `two_j=1024` and first falls below one at retained
`two_j=68743`.  Dense completion there would require more than 14.17 billion
...
are now machine-readable for all twenty-five Maxwell/massive blocks through
`two_j=4`: Maxwell degrees zero and one plus massive de Rham degrees zero,
one and two, with symbolic positive mass retained.  The added massive scalar
carrier is required by the physical
`m^-2 d G_(P1+m^2) delta` one-form correction.  A finite-mode successor now
evaluates these algebraic matrices by outward rational intervals over a
caller-declared finite slab and strictly positive rational massive
mass-squared domain, exports all six coefficient matrices and a uniform
sine-series tail, and fails closed when the tail majorant does not contract.
This is runtime parameterization, not a physical mass specialization.
...
successor now distinguishes that twelve-component spacetime jet from the
six-component canonical emitter carrier.  For `K=dt wedge alpha+beta` it takes
`(q,p)=(beta,partial_t beta-dSigma alpha)`.  Eliminating `alpha` gives
`A=I+m^-2 dSigma deltaSigma` and `L=deltaSigma dSigma+m^2`, so the full
positive-energy dual is `tilde_u=(-A p,Lq)`.  Exact Hodge-projector audits
...
first leading retarded Maxwell channel, not by itself a detector record or
feedback recoil coefficient.  Its whole-support hull leaves nonvanishing and
the detector contraction open until the partitioned successor below.
The partitioned successor removes the zero-containing whole-support switch
hull before testing the record.  It accumulates 32 positive switch-cell
moments and proves that the `two_j=0,k=0` advanced Cauchy covectors for D0 and
D1 each have a coordinate rectangle excluding zero uniformly for
`m_a^2 in [1,2]`.  Strict rational coefficient-block energy lower bounds,
the exact positive `1/Vol_Berger` Peter--Weyl factor and Green adjunction then
give nonzero diagonal records; with the causal zero `M_01=0`,
the detector-selected leading matrix has rank two for nonzero couplings on
that validation parameter domain.  This closes the selected leading `d/Q_a`
contraction without forward detector-window reintegration.  Arbitrary
positive/physical mass domains, extension beyond `two_j=4` and the tail-aware
four-stream stop loop remain `OBSTRUCTED`; the validation interval is not a
physical mass declaration.
The first absolute-`g3` feedback successor evaluates the detector-matched
coefficient blocks `I_000[0,0]` and `I_111[0,0]` on that same validation
domain.  Green adjunction reduces each detector contraction to the Lorentzian
two-form pairing of the advanced physical emitter field with the switched
derivative of the leading retarded Maxwell field.  The implementation retains
the full physical massive correction, including the newly exported scalar
Green block, and detects an all-plus pairing-sign mutation.  Both rigorous
whole-support interval enclosures contain zero, so no sign, nonvanishing or
recoil-corrected rank claim follows.  The cellwise causal successor partitions
...
validation mass domain.  The first-omitted-shell successor now exports both
direct detector polynomials, the D1/h0 remainder and all five Maxwell/massive
kernel blocks at `two_j=5`.  Exact source-manifest hashes crosswalk it only to
the direct `two_j<=4` carrier; the separate hashed exact-`T` stream through
`two_j=138` remains `NO_CERTIFIED_MAP`.  Its partitioned successor binds all
eight channels and all six passive columns at `two_j=5`, giving 48 rigorous
blocks: 24 exact support zeros and 24 causally allowed zero-containing
enclosures on the validation mass domain.  All four allowed `k=0` paths
contract in both components from two to four cells.  The direct provider now
has a content-addressed generic finite-shell successor with a
...
channel sums.  The reality-folded successor now certifies all 56 `two_j=6`
channel-column blocks from 32 direct representatives and 24 exact partners,
and exports eight real channel sums.  The generic successor now builds,
binds, directly evaluates only the independent columns, reality-folds every
partner, aggregates all four detector/source entries and invokes the stop
rule after each contiguous shell.  Its actual `two_j=6` carrier-building
replay matches the prior 56-block certificate exactly.  The internal stream
is ready; exact physical input declaration, Peter--Weyl accumulation,
quotient descent and second-order-cone restriction remain open.
...
the coefficientwise affine-`K_Berger` observer morphism.  This is not finite-
`r`, fixed-background linear-`K`, full-`q4`, or all-orders stability.
...
anomaly-induced Euclidean effective action now has one conditional exact
Paneitz/Riegert representative with coefficients
`(199/120,-87/160,29/120)`; its local `R^2` term restores the certified
`BoxR=0` convention. On the nonzero-momentum flat Euclidean TT carrier the
universal logarithmic form factor is also fixed:
`F_C(p^2;mu)=-(199/60)log(p^2/mu^2)+z_C(mu)`, with scale response `199/30`
and a scheme-independent momentum difference. Its spectral covariantization
`-(199/60)<C,log(Delta_C/mu^2)C>` is universal through curvature order two;
the resolvent filtration places admissible operator-choice dependence at
curvature order three. Its normalized Fradkin--Vilkovisky scalar-flat
evaluation now supplies an exact Weyl-orbit completion of this selected
carrier. The zero-derivative algebraic cubic Weyl sector is now exhaustive,
with one even and one odd carrier. The parity-even third-curvature manifest is
also complete: scalar-flat transversality enhances `I29` from source-generic
`C3` to effective `S3`, so five carrier-labelled functions span eleven raw
effective label channels and one symmetric four-dimensional relation leaves ten. The five
...
The scalar-flat normalization seam is now exact:
`K_munu=Ric_munu+O(curvature^2)`, with the first cubic replacement error at
fourth curvature order and an independent flat-TT sign fixture. The labelled-
Ricci kernel is now projected exactly onto the ten-dimensional scalar-flat
five-carrier quotient as rational simplex integrands. At the symmetric
nonexceptional point all eleven coordinates are integrated exactly as
rational combinations of `4 Cl2(pi/3)/sqrt(3)`. Generically, ten of eleven
numerators cancel one `Delta`; every raw row except `I10` then vanishes on all
open simplex edges, with positive vertex margins and a pointwise `I28`
relation. All ten pole-three rows reduce exactly to the scalar triangle and
two first derivatives. Normalized rank-stable dual witnesses prove that
punctured-corner flux is unavoidable. The exact `S3`-covariant scalar-triangle
differential system reduces those derivatives to `J` plus two bubble-log
ratios; the only nonzero corner of every canonical primitive has equal angular
weights and integrates rationally. All ten generic pole-three functions are
therefore complete and regress exactly to the symmetric-point certificate.
The pole-four `I29` row is also complete by a full 55-row exact relative-IBP
identity with ranks `46/49/49`, rational corner flux and no new
transcendental master. Thus all eleven generic ghost `n=3` functions are
complete. Exact Hodge-resolvent
integration reduces the curved `n=1/n=2` architecture to five minimal
vector/scalar carriers with fixed rational coefficients. The pure-vector
physical sum is now evaluated exactly from CPT rows 1, 3 and 14 and projected
to the scalar-flat carrier quotient. The other three carriers contain the
anisotropic principal-symbol insertion `D_W=delta W d`; imported minimal-`P`
kernels do not evaluate them separately. The exact Schur factorization now
resums all three towers into
`S_L(W)=(2/3)I+(1/3)delta(F+W)^-1 d`, with cubic weights
`(-1/3,1/9,-1/81)`. The correction lies in `S_3`, so its canonical `det_3`
tail is defined. The exact local rows are `Wres(K)`, `Wres(K^2)`, and
`Wres(log S_L)`. The declared order-two weighted trace now fixes the Schur
pole and scale row. The round-`S4` reference finite `R(K)` and `R(K^2)` rows
are exact digamma/trigamma values, and exact rational bounds also fix its
canonical `det_3` tail and weighted modified determinant; a finite-rank smoothing witness proves
that their generic versions require a full primed Green/spectral carrier. The
commuting round-`S4` Einstein-ratio defect is exactly `5/3`; for the distinct
generic weight-raised convention the noncommuting BCH trace is exact through
residue order and gives `-(1/4)Wres(K^2)`, specializing to `-1/3`. The global
carrier data, generic repository functions and coefficients pending the
physical fourth-order Hessian, and the parity-odd derivative
manifest remain open. The raw
...
renormalized BV Laplacian or time-ordered product remain absent, so complete
`Gamma1` and `Q1` are still fail-closed. The Berger `tau` ghost is not the WZ
compensator, so its otherwise exact `34 -> 26` SDR cannot be used as the
missing extended contraction.
...
2. The apparatus `q2,q3` action jets, cyclic completion, unshifted identities,
   first shifted arity-two identity, and Ward-determined affine arity-three
   contraction are now certified.  Full `q4` and the optional co-rotating rod
   replacement remain separate gates.
3. Define the first observer morphism from global observables to the two
   ...
   redshift.
   This coefficientwise affine-`K` family morphism is now certified through
   ...
   recoil Green term and coefficientwise causal chain identity are now
   certified through `g^2`.  Green adjunction and a constraint-compatible
   massive polarization now give two actual localized Cauchy preparations with
   a triangular rank-two leading matrix.  Recoil parity makes the absolute
   `g^2` detector term zero and fixes the first feedback operator at absolute
   ...
   `two_j=4` uniformly over the emitter switches.  The switched massive image,
   physical full-form jet, unrestricted canonical spatial trace and full
   coupling-stripped positive-energy preparation coefficients are now callable
   on the same finite rail.  The older co-closed restriction is certified as a
   zero observer-source rail.  Free emitter evolution, its conserved switched
   source and the first retarded Maxwell channel are now certified on the
   finite rail.  A 32-cell Green-adjoint successor proves selected leading
   rank two for `m_a^2 in [1,2]`; the first two detector-matched absolute-`g3`
   coefficient blocks are also evaluated, but their whole-support enclosures
   contain zero and do not promote recoil-corrected rank.  Parseval proves that
   ...
   charge-block images are now certified through form `two_j=138`.  Exact
   ...
   `two_j=0,k=0` blocks are now evaluated.  The direct provider now includes
   the first omitted shell `two_j=5`, and its partitioned successor evaluates
   ...
   conjugate-column theorem, and the complete `two_j=6` validation shell is
   certified from 32 direct and 24 exact reality-derived blocks.  The generic
   reality-folded adapter now advances contiguous shells, evaluates only the
   independent columns, derives exact partners, aggregates all four entries
   and invokes the stop rule after every shell.  Its v2 numerical input
   schema and callable translator now cover every declaration-controlled
   runtime argument and all four stopping goals; legacy v1 is explicitly
   `OBSTRUCTED_SCHEMA_RUNTIME_MISMATCH`.  Await a provenance-complete
   `EXPLICIT_EXTERNAL_VALUES` declaration without inventing values.  The
   independent emitter stress and reciprocal clock-switch `q2` jet is
   exported; its complete covariant 108-row `q1 q2` identity is certified.
   The scalar PBW promotion remains exactly `NO_CERTIFIED_MAP`, but its first
   activation prerequisite is now certified: an ordered 108-row
   component/cotangent carrier, rank-108 odd pairing, exact detector/switch
   bindings and executable arbitrary-finite differential coefficient-jet
   normal form over `Q(sqrt(10))`.  A corrective exact audit has repaired its
   ...
   background quotient is `CERTIFIED`.  The scalar emitter successor now
   serializes all six massive-two-form unary ranges as 132 distinct matrix
   ...
   audited.  Their complete composition exposed and repaired one emitter
   interface error: form-valued Euler equations require `+eta_1` on Maxwell
   ...
   contraction remove the former spatial witness.  The historical
   `epsilon_R_squared` replay exposed 374 quotient defects on 54 positions and
   the exact output-27/input-4 Weyl coefficient `-49/20`.  Its successor now
   exports the missing object as a generated 95-key nonlinear clock
   correction: 21 radial rod-action Hessian keys, 9 Weyl-clock cotangent keys
   and 65 temporal-clock cotangent keys.  Each block and the completed unary
   are exactly odd-cyclic.  The old Weyl witness and every free tau/Theta
   input-column defect cancel; the remaining 261-key free residual evaluates
   on 699 modes and reduces identically to zero in the certified
   same-background quotient.  The mixed `epsilon_R_squared*kappa` square
   remains freely zero.  This closes the nonlinear team's unary activation
   ...
   components.  Its derivative-dependent BV cotangent lift is now exact:
   coefficientwise formal adjunction in the noncommuting Berger PBW algebra
   yields 93 `F2` plus 135 `F3` cotangent entries, and the degree-two and
   degree-three canonical one-form inverse defects vanish.  Three
   nonholonomic `sqrt(10)` entries are necessary.  The combined chart is now
   generated from one geometric formula: 55 `F2` plus 174 `F3` field entries
   and 132 `F2` plus 268 `F3` cotangent entries, with exact radial and temporal
   restrictions and all 69 mixed terms.  Its combined signed one-form replay
   passes and closes the coordinate-map nonuniqueness.  The active gate is
   ...
   Hessian regression, third-variation and cyclic-transpose checks.  The
   profile-free two-channel memory transport adds 192 exact rational PBW keys
   from the verified first clock-flow jet.  The normalized two-detector
   ...
   The normalized two-detector readout adds 338,364 operator keys and
   1,085,112 exact profile-jet monomials in 23 hashed row chunks, retaining
   f'', rho_IJ and the full 26-coordinate normalized-Jacobian Hessian with
   ...
   The corrected complete assembly now imports the separate 5,812,130-term
   gravity-clock payload and 59,598-term typed Maxwell overlay, then adds all
   ...
   raw temporal common-action re-export is now certified and closes
   negatively on the present component-preserving carrier.  The typed
   Maxwell, physical emitter Hessian and temporal emitter-Diff variations
   induce the normalization triangle
   `s_Maxwell=2 s_tau`, `s_Maxwell=s_emitter`,
   `s_emitter=s_tau`.  Its determinant is `-1`.  The invariant successor
   `BERGER_108_ROW_COMMON_ACTION_COMPATIBILITY_THEOREM` proves the general
   criterion `a=b c` and the field-rescaling/presentation invariant holonomy
   `H=a/(b c)`, whose frozen value is two.  It exhausts the three one-edge
...
The finite multi-`ell` constant-twist gate is also `CERTIFIED`: quadratic
bilinearity reduces it to the finite sum of the fixed-`ell` `A`--wave
corrections, while the existing finite-wave theorem controls every wave--wave
cross term.  The complete standard-global stratified cone therefore extends
to arbitrary finite generic `ell>=2` sums at `k=0`.  Infinite harmonic
...
cyclic unary ambient carrier.

`EINSTEIN_WEYL_RELATIVE_REDUCED_TAUB_FACTORIZATION_V1` closes the finite
smooth-secular obstruction-module gate on `Sym^2 H^0(q1_EM)`.  The complete
target cokernel theorem makes `chi_X([S])=(1/2)<zeta_X,S>` an isomorphism onto
the five charge coordinates, and the global current replay gives
...
Within the same tuned axisymmetric carrier, the full coefficient cone is now
closed.  The resonance ideal has two mixed planes with common sign
`a_sigma=+/-sqrt(3)*p_sigma` and two one-sided travelling planes.  Compact
momentum balance excludes the one-sided planes away from the origin.  Writing
`r=omega_minus/omega_plus`, the two mixed components survive precisely on

```text
(1-r)/(1+r) <= |p_+|^2/|p_-|^2 <= (1+r)/(1-r),
```

with q-plus occupations fixed by `H=P_x=0` up to phase and multiplicity
factorization.  This is bounded necessity and sufficiency for the declared
...
Candidates 17/20 have a different incidence pattern: their two algebraic
...
pairing, and the relevant QME/Ward identities have all been certified.

## Physical rails and their gates

### 1. Clocks, time dilation, and redshift

The healthy Berger clock makes this the first accessible observer-level rail.
Its next target is not merely a time-dependent field.  It is a relational,
gauge-invariant comparison of two clock or signal readings.

Activation gates:

1. authoritative `q2` domain;
2. retained causal Green homotopy;
3. interacting Cartan result on the observable's domain;
4. construction of a physical metric/clock observable;
5. a null or characteristic signal comparison giving a frequency ratio.

A passed redshift fixture would show that quotienting total (D) does not
...
directions, and the standard radiative modes now have a positive covariantly
matched pairing.  To complete their physical interpretation:

1. close the global `ell=0` and axial-twist symplectic sectors;
2. compute the Weyl--Maxwell pullback;
3. separate and norm any extra fourth-order branches;
4. run quadratic extension/Taub tests;
...
This is the central junction rather than a separate destination:

1. complete `q2`;
2. certify the arity-two interacting Cartan identity or obstruction;
3. test the resonant `+lambda,-lambda -> 0` channel;
4. compute the first nontrivial transferred physical vertex;
5. determine whether a negative physical direction is created.

A no-go at any one of these stages is a publishable boundary of the model.  A
pass activates observer and wave calculations on the certified domain only.

### 4. Particles and gravitons

Particle language requires more than a classical normal-mode solution.  The
minimum activation sequence is:

1. Lorentzian Green/Hadamard framework;
2. renormalized observable algebra and restored QME, or a precise anomaly;
3. physical BRST/BV cohomology and pairing;
4. a state or asymptotic structure that supplies positive-frequency data;
5. interaction amplitudes or detector response.

The existing centered classes `[W_+^2]` and `[W_-^2]` remain deformation or
vertex classes; they must not be renamed one-particle gravitons.

### 5. Boundaries, scattering, and black holes
...
2. boundary-preserving generators and differentiable charges;
3. horizon and infinity flux balance;
4. classification of Einstein and extra Weyl branches;
5. causal boundary conditions and physical symplectic signs;
6. only then, perturbations, stability, thermodynamics, or Hawking questions.

An exact black-hole metric would establish existence, not physical selection,
stability, entropy, or radiation.

### 6. Galaxies and the dark-matter question

Mannheim-type phenomenology becomes relevant after the static weak-field and
boundary sectors are under control.

Activation gates:

1. identify the physical metric seen by clocks and matter;
2. derive the static weak-field potential from certified field equations and
   admissible boundary data;
3. determine which integration constants are charges, environmental boundary
   data, or gauge;
4. derive lensing and orbital observables from the same solution;
5. fit galaxies only after those constants and priors have physical meaning;
6. compare with dark-matter predictions and cluster/cosmological constraints.

This rail can test whether modified conformal dynamics explains some effects
attributed to dark matter.  It must not announce that dark matter has been
removed on the basis of rotation curves alone.

### 7. Cosmology and dark energy

The cosmological rail needs homogeneous solutions beyond the present clock
fixture and their perturbations.

Activation gates:

1. classify homogeneous/isotropic solutions with matter and clock sectors;
2. compute the physical expansion and redshift observables;
3. establish stability and causal perturbations;
4. derive distance--redshift and growth equations;
5. determine whether acceleration is dynamical, boundary-selected, or a
   parameter choice;
...
The next coefficient-bearing analytic gate is a generic primed Green or
spectral carrier lifting the exact round-`S4` finite rows, together with
evaluation of the mixed rows. The same-gauge `H1` rows,
their exact formal-adjoint rank-nine momentum vertex, full alpha numerator,
28-point interpolation and two unseen projection fixtures are already
certified. Their isolated symmetric-point integral is logarithmically
`M14`-corner-obstructed, with exact rank jump `49 -> 50`, normalized dual
witness and total coefficient `1/2`. The projected algebraic `H2` block is now
imported with 18 source rows, nine effective scalar-flat rows, an exact
gauge-ordering commutator crosswalk, and round split `24-16=8`. Its exact
equal-box TT polarization gives raw `H1^3`, mixed-endpoint and combined
coefficients `-1975/72`, `2704/27`, and `15707/216`. Universal algebraic
`H2` cancellation is therefore refuted on this fixture. A resolved common
Mellin minimal subtraction promotes the combined residue to the exact fixture
...
finite `C2` normalization remains open. All eleven integrated scalar-flat
ghost-`n=3` functions are now added to that same carrier representative with
the coefficientwise `I28` relation preserved. The pure-vector ghost
...
REPOSITORY_RELEASED artifact is committed, linked to its receipts, and public in the open repository
```
...
| Paper 7, residual cohomology and pairing | `ARTIFACT_READY` | final open-repository claim/link audit, then `REPOSITORY_RELEASED` |
...
| Paper 12, pure-Weyl one-loop BV anomaly/QME | `DRAFT_ALLOWED`; theorem-spine manuscript, computational supplement, both compiled PDFs, generated exact tables, and fail-closed claim map assembled for the strict obstruction, tau-adic extended one-loop local Euclidean restoration, exact rank-two `Q1` underdetermination, anomaly-induced and FV effective-action representatives, exact `BoxR` scheme conversion, complete algebraic and parity-even third-curvature carrier manifests, five universal CPT kernels, the generic ghost Endo--Duhamel/Hodge reductions, exact pure-vector n=1+n=2 CPT slice, exact all-order longitudinal Schur resummation, sharp `S_3` classification, exact `Wres(K)`, `Wres(K^2)`, and `Wres(log S_L)` rows, an exact declared order-two weighted-trace pole/scale conversion, complete round-`S4` finite Schur `K/K2`, canonical `det_3`, weighted modified-determinant rows and Einstein-ratio defect `5/3`, plus the generic weight-raised local defect `-(1/4)Wres(K^2)` and round specialization `-1/3`, exact n=3 nonzero-momentum triangle and ten-dimensional scalar-flat projection, exact generic barycentric denominator/boundary factorization, exact relative-IBP reduction of all ten pole-three rows with normalized corner non-membership witnesses, the exact scalar-triangle differential system and complete generic J-plus-two-log functions for those ten rows, a full exact 55-row pole-four `I29` reduction to the same master basis, all eleven generic ghost functions with exact symmetric-point regressions, an exact formally self-adjoint physical `H1` momentum vertex whose full three-linear alpha numerator is projected onto the five scalar-flat carriers with 28 training and two unseen fixtures, the projected algebraic physical `H2` block with exact gauge-ordering and round-operator crosschecks, and an exact raw equal-box mixed `H1-H2` fixture with nonzero combined logarithmic coefficient `15707/216`; focused major revision and machine/literature traceability audit completed with the determinant-to-Slavnov bridge, directly bound pure-Diff/mixed quotient, regular-Bach spectral sequence, explicit quartet homotopy and Euler primitive, matter vectors, narrowed effective-action claims, and a content-addressed referee-response ledger | final human author signoff before theorem freeze; the next coefficient theorem requires a generic primed Green/spectral carrier and a local covariant subtraction/distribution extension followed by renormalized mixed physical rows disposing the `M14` class before matching the repository third-curvature functions and coefficients, plus the parity-odd derivative manifest, finite `C2`/absolute dressed `R(g_hat)^2` normalization, renormalized products, a same-background compensator contraction, and quantum `D`-defect disposition; Lorentzian/state claims remain separate |
...
certificate gates.  Open-repository release never waits merely for a broader
result that the paper explicitly does not claim.  This portfolio does not use
journal submission or venue acceptance as a lifecycle gate.

## Reshuffle policy

Do not reshuffle whole teams now.  Use explicit activation triggers:

| Trigger | Capacity move |
|---|---|
...
| `BERGER_84_ROW_APPARATUS_Q2_Q3_K_GATE` (fired with affine-$K$ obstruction) | observer closes `q4(K0,-,-,-)` or certifies a time-translation-closed rod replacement before replaying the observer morphism; formal unary rank two is retained but not quotient-descended |
...
| `BERGER_108_ROW_POLARIZATION_EMITTER_CAUSAL_CHAIN_HOMOTOPY` (fired) | choose two localized free-emitter Cauchy preparations, compute `M_ab^(K)`, and prove or obstruct rank two before evaluating the detector recoil term |
| `BERGER_DYNAMICAL_EMITTER_CAUCHY_RANK_TWO` (fired) | evaluate the `g^2` detector recoil correction for the fixed preparations, then export emitter stress and clock-switch backreaction |
| `BERGER_DYNAMICAL_EMITTER_RECOIL_ORDER_AND_INPUT_GATE` (fired) | export serialized compact `u_0,u_1,h_0,h_1` and evaluate their massive Green images and absolute-`g^3` detector coefficient; the independent stress/clock jet may proceed in parallel |
| `BERGER_EXACT_NORMALIZED_EMITTER_SWITCH_PROFILES` (fired) | construct `u_0,u_1` against the actual advanced detector covectors, then evaluate their massive Green images and recoil coefficient; arbitrary bumps are not adequate witnesses |
...
| `BERGER_CLOCK_MICROPHASE_TAIL_ENVELOPE` (fired for fixed spatial profiles; moving profile open) | bound the clock derivatives of the actual rod/Gram-dependent detector profile before using the frozen-profile `two_j=3421` target; do not identify the moving source with a fixed vector |
| `BERGER_MOVING_PROFILE_CLOCK_DERIVATIVE_TAIL` (fired; physical tail bound certified, projection open) | construct a complete retained projection through `two_j=3835`, then compose the massive-two-form image; do not treat selected rows or a sub-unit omitted-tail bound as a full Green image |
...
| `BERGER_RECOIL_SCALAR_STREAM_ACTIVATION_GATE` (fired; internal executable stream and v2 translator ready, external numerical specialization deferred) | await a provenance-complete `EXPLICIT_EXTERNAL_VALUES` declaration; do not invent physical values or mark the four streams active before a separate activation certificate |
...
| `BERGER_RECOIL_FINITE_NESTED_TIME_CONVOLUTION` (fired for supplied polynomial interval enclosures) | bind the finite Berger kernel intervals, switches, detector coefficients and form contractions; do not identify the exact beta-integral fixture with a physical channel |
| `BERGER_RECOIL_EXACT_MODE_KERNEL_PAYLOAD` (fired for twenty-five physical blocks through `two_j=4`, including the massive scalar correction carrier) | pass the algebraic coefficients to the finite interval enclosure; do not invent masses or identify factored series data with a Green image |
...
| `BERGER_RECOIL_TWO_J5_ALL_CHANNEL_COLUMN_BINDING` (fired; 48 channel-column blocks evaluated, 24 exact support zeros and 24 allowed zero-containing enclosures) | consume the certified `two_j=6` reality-folded successor and retain fail-closed zero containment |
| `BERGER_RECOIL_DIRECT_SHELL_AND_TAIL_STOP_GATE` (fired; generic direct finite-shell carrier, contiguous `two_j=6` sentinel and four stop goals certified) | consume the certified reality-folded `two_j=6` binding, keep the hashed exact-`T` rail `NO_CERTIFIED_MAP` and do not promote the validation stop fixture |
| `BERGER_RECOIL_REAL_SHELL_EXTRACTION` (fired; exact SU(2) conjugate-column theorem and eight real `two_j=5` channel sums certified) | consume the certified `two_j=6` reality-folded binding; do not discard imaginary rectangles, choose physical parameters or infer recoil-corrected rank |
| `BERGER_RECOIL_TWO_J6_REALITY_FOLDED_BINDING` (fired; all 56 blocks certified from 32 direct and 24 exact reality-derived blocks; eight real channel sums exported) | consume the certified generic stream adapter and retain fail-closed zero containment; do not infer nonzero recoil or recoil-corrected rank from the validation shell |
| `BERGER_RECOIL_REALITY_FOLDED_SHELL_STREAM_ADAPTER` (fired; contiguous-shell build/bind/evaluate/fold/four-entry-stop loop certified with exact `two_j=6` replay) | consume the certified v2 numerical contract; retain `NO_CERTIFIED_MAP` to hashed exact-`T` and do not promote the unit-normalization replay to physical recoil |
| `BERGER_RECOIL_NUMERICAL_INPUT_CONTRACT_V2` (fired; exact schema and callable translator certified for mass or mass-squared domains and all four runtime goals; legacy v1 obstructed) | await a provenance-complete `EXPLICIT_EXTERNAL_VALUES` declaration and verify every source hash before a separate physical-stream activation; never treat the validation fixture as physical data |
| `BERGER_RECOIL_STREAM_EXECUTABLE_READINESS_AUDIT` (fired; internal executable stream and v2 translator ready, external values undeclared) | activate physical values only with explicit provenance for masses, couplings, inverse volume, shell/tail schedule, precision and stop goal |
...
| `BERGER_EXACT_MAXWELL_CHARGE_BLOCK_FORMULAS` (fired) | stream the order-14 application using the exact at-most-three-dimensional blocks; do not reconstruct dense form Laplacians |
...
| `BERGER_SELECTED_CLOCK_POWER_POLARIZED_FORM_RAIL` (fired) | apply the exact temporal functional calculus to the 18 selected rows and derive a controlled spatial tail; do not infer complete form coverage, a Green image, response rank or recoil from the 270 input intervals |
...
| `BERGER_NONLINEAR_CLOCK_TEMPORAL_COTANGENT_F2_F3` (fired; 93 F2 plus 135 F3 cotangent entries and exact PBW canonical one-form inverse) | compose the separately canonical radial and temporal submaps, export every mixed radial-temporal F2/F3 term and replay the combined signed one-form identity; keep scalar interactions, observer reduction, cone response and Bridge 3 fail-closed until that composition passes |
| `BERGER_NONLINEAR_CLOCK_COMBINED_CANONICAL_MAP_F2_F3` (fired; exact restrictions, 69 mixed terms, 229 field and 400 cotangent entries, combined canonical one-form pass) | regenerate scalar 108-row apparatus/emitter `q2,q3` through the certified chart and replay `q1q2` plus `q2q2+q1q3`; keep `K_Berger`, observer morphism, cone response, nonlinear rank and Bridge 3 fail-closed until those interaction gates pass |
...
| `COMPACT_WAVE_NONLINEAR_GATE_DECIDED` | Einstein may shift substantial capacity to asymptotic radiation and black holes |
| `LORENTZIAN_BOUNDARY_PHASE_SPACE_CERTIFIED` | activate scattering, black-hole charge, and static galaxy-potential consumers |
| `LOCAL_QME_RESTORED` plus a certified Lorentzian state framework | activate particle, graviton, detector, and Hawking calculations |
| `CLOCKED_COSMOLOGY_STABLE` | activate dark-energy phenomenology and cosmological matter tests |

Reshuffle earlier only if a team is genuinely blocked on an external input and
has exhausted independent certificate, verifier, and abstraction work.  A
hard negative result also triggers reassignment: preserve and publish the
no-go, then move the team to the nearest rail that does not assume the failed
gate.

## Common claim discipline

Every result retains its dependency tag:
...
Every quantum result also retains its lifecycle state:
...
The programme is building a sequence of certified laboratories, not silently
adding assumptions until a preferred answer appears.  A sector restriction is
