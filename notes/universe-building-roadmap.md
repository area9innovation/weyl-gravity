...
Quantum:     fix the curved Weyl-invariant Gamma1 completion and finite C2/R2 normalization, then import an extended classical contraction before residual Q1 transfer
...
and a scheme-independent momentum difference. The additive finite `C2`
constant, finite `R2` normalization, curved Weyl-invariant completion,
Paneitz kernel/global data, and renormalized BV Laplacian or time-ordered
product remain absent, so complete `Gamma1` and `Q1` are still fail-closed.
...
| Paper 12, pure-Weyl one-loop BV anomaly/QME | `DRAFT_ALLOWED`; theorem-spine manuscript, computational supplement, both compiled PDFs, generated exact tables, and fail-closed claim map assembled for the strict obstruction, tau-adic extended one-loop local Euclidean restoration, exact rank-two `Q1` underdetermination, the conditional anomaly-induced Paneitz/Riegert `Gamma1` representative, and the universal nonzero-momentum flat-TT logarithmic form factor | complete specialist prose/citation and clean release review; the next theorem requires the curved Weyl-invariant `Gamma1` completion and finite `C2/R2` normalization, an extended classical contraction, and quantum `D`-defect disposition; Lorentzian/state claims remain separate |
