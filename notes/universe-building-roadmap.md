...
`SO(4,2)` receiver therefore cannot be inherited.  Programme effort now
consumes this parent before reopening another action search.  Classical first
audits the fixed-charge physical pairing, characteristic/Jordan structure and
the separate `Q_diag`, `Q_rel`, `D` and `K=D-Omega R_rel` charges.  Nonlinear,
Observer and Quantum import the same parent for the first `q2`, relational
readout and action-specific anomaly/Hadamard preflights.  Bridge completes its
current compact-Cauchy AMM gate before testing the counterflow stress in the
Einstein-source condition.  A later Classical task derives the actual Berger
stabilizer and residual BFV/Koszul receiver; absence of a round-cylinder
generator is not a quotient theorem.
...
```text
Einstein:    finish the active compact-Cauchy Arms--Marsden--Moncrief semi-Fredholm slice theorem, then import the counterflow parent and test the exact Einstein/additional-Weyl/relative-clock sequence, the compensated source condition Q(T)=0 and fixed-versus-variable Q_rel obstruction; diagonal gauge neutrality is not source closure
...
Classical:   consume the 70-component counterflow parent without replay: first compute fixed-Q_rel physical cohomology, descended pairing/inertia, characteristic multiplicities/Jordan chains and Q_diag/Q_rel/D/K charges; then classify the exact healthy background component and round-cylinder wall, and only afterwards build the true Berger stabilizer/BFV receiver
