...
Quantum:     keep reduced vacuum-cylinder Bridge 4 scoped to its Krein carrier; compute the C2 cubic completion, independent R2 form factor and finite C2/R2 normalization, then import a same-background compensator contraction before residual Q1 transfer
...
curvature order three. The additive finite `C2` constant, finite `R2`
normalization, independent `R2` form factor, cubic-and-higher `C2` completion, Paneitz kernel/global data, and
...
| Paper 12, pure-Weyl one-loop BV anomaly/QME | `DRAFT_ALLOWED`; theorem-spine manuscript, computational supplement, both compiled PDFs, generated exact tables, and fail-closed claim map assembled for the strict obstruction, tau-adic extended one-loop local Euclidean restoration, exact rank-two `Q1` underdetermination, the conditional anomaly-induced Paneitz/Riegert `Gamma1` representative, and the covariant curvature-squared `C2` logarithmic form factor | complete specialist prose/citation and clean release review; the next theorem requires the independent `R2` form factor, cubic-and-higher `C2` completion and finite `C2/R2` normalization, a same-background compensator contraction, and quantum `D`-defect disposition; Lorentzian/state claims remain separate |
