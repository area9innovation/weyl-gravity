...
```text
Einstein:    the complete finite coefficientwise bounded obstruction map is now the direct join {mu,P,R}; its pure-extra face is the origin, its smooth cone is mu=0, and the maximal solved bounded mixed carrier is all standard globals plus arbitrary finite generic ell>=2,k=0 waves; next classify exceptional ell=1 wave cross columns with generic/global arbitrary-k inputs, then multiple |k| fibres without merging phases or circumference backgrounds
...
Classical:   the frozen normalized Berger retained-26 -> 104-row formal Cauchy graph is now obstructed at every finite PBW order (157 square and 207 evolution defects; any carrier extension needs at least 5 degree-zero plus 1 degree-one rows, necessity only); change the normalized companion/A104 or build a larger carrier, while separately attaching the certified 188-row physical/current q2 interface to the 78-row relative mapping cofiber and keeping causal Green data and the eighteen candidate-13 spectral receivers fail-closed
...
Observer:    the certified 444-coordinate Ward carrier is not Berger-representation closed; its minimal closure is 900-dimensional, and the unique reflection-odd transverse epsilon Hessian enlarges the action image from rank four to six and cancels all eight displayed A0--K12 pseudoscalar coordinates, but the complete typed-Maxwell source still raises the rank to seven; preserve the scoped C-G4 record algebra, leading free-emitter rank two, source-free G0 redshift, formal absolute-g3 recoil operator and stress/clock q2 ledger without nonlinear promotion, then realize or obstruct the required weight-zero line in the 108-dimensional profile-first-jet sector before any interacting gauge descent, detector replay or Z2^C restriction; physical Bridge 3 still requires a same-background branch dictionary
