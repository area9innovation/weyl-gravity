...
tail is defined, and the exact critical row is
`Wres(K^2)=(4 pi)^-2 integral (R^2+2 Ric^2)/27`. The regulated `R(K)`, finite
`R(K^2)`, `Wres(K)`, and possible local zeta multiplicative term remain
unevaluated. Those data, simplex-integrated
...
The next coefficient-bearing analytic gate is computation of regulated
`R(K)`, finite `R(K^2)`, `Wres(K)`, any local zeta term, and the physical
fourth-order Hessian kernel. The canonical `det_3` tail and `Wres(K^2)` are
already fixed. The exact Endo--Duhamel,
