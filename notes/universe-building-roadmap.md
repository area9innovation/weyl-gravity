...
Observer:    await and independently verify an explicit provenance-complete physical declaration under the certified v2 recoil contract; in parallel activate the obstructed 108-row PBW promotion only after a differential coefficient-jet/component contract exists, then restrict records to Z2^C
...
3. Define the first observer morphism from global observables to the two
   ...
   The scalar PBW promotion is now exactly `NO_CERTIFIED_MAP`: the current
   dependency closure omits a canonical differential coefficient-jet grammar,
   scalar 108-row unary, and apparatus/emitter component expander, with profile
   and switch rescalings giving explicit factor-eight and factor-two
   non-uniqueness witnesses.  Import the later exact profiles and switches into
   that declared component contract before a PBW replay or solved backreacted
   branch.
   A common source
...
| `BERGER_108_ROW_EMITTER_Q1_Q2_MASTER_IDENTITY` (fired; PBW promotion obstruction certified) | declare the 108-row scalar component/cotangent crosswalk and differential coefficient-jet grammar, import the later exact profiles/switches, and export complete scalar q1/q2 payloads; do not infer coefficients from covariant row coverage |
