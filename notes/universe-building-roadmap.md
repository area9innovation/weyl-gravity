...
vector/scalar carriers with fixed rational coefficients. Those five carriers,
...
projection of the five exact minimal ghost `n=1/n=2` resolvent carriers, plus
...
| Paper 12, pure-Weyl one-loop BV anomaly/QME | `DRAFT_ALLOWED`; theorem-spine manuscript, computational supplement, both compiled PDFs, generated exact tables, and fail-closed claim map assembled for the strict obstruction, tau-adic extended one-loop local Euclidean restoration, exact rank-two `Q1` underdetermination, the conditional anomaly-induced Paneitz/Riegert `Gamma1` representative, the covariant curvature-squared `C2` logarithmic form factor and its exact FV scalar-flat Weyl-orbit completion, the exact FV anomaly action and Ricci-sector dependence theorem, the exact raw-to-`BoxR=0` strict-metric `R2` scheme conversion, the exhaustive one-even/one-odd zero-derivative algebraic `C3` basis, the parity-even five-carrier third-curvature manifest with exact scalar-flat `I29` symmetry enhancement and `11 -> 10` effective label quotient, five exact universal CPT source kernels, the generic ghost direct-minimal-CPT architecture obstruction, its exact Endo–Duhamel reduction, exact Hodge-resolvent reduction of the n=1/n=2 architecture to five minimal carriers, exact n=3 generic nonzero-momentum triangle, scalar-flat `K`--Ricci cubic normalization, and exact rational projection onto the ten-dimensional scalar-flat five-carrier quotient; focused major revision completed with the determinant-to-Slavnov bridge, regular-Bach spectral sequence, explicit quartet homotopy and Euler primitive, matter vectors, narrowed effective-action claims, and a content-addressed referee-response ledger | final human and literature review before theorem freeze; the next coefficient theorem requires evaluation of the five minimal n=1/n=2 resolvent carriers and the physical fourth-order Hessian kernel before simplex integration and matching of the repository third-curvature functions and coefficients, plus the parity-odd derivative manifest, finite `C2`/absolute dressed `R(g_hat)^2` normalization, renormalized products, a same-background compensator contraction, and quantum `D`-defect disposition; Lorentzian/state claims remain separate |
