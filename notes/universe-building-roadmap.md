...
Quantum:     keep reduced vacuum-cylinder Bridge 4 scoped to its Krein carrier; complete curved-Endo ghost n=1/n=2 and lift the exact round-S4 finite Schur benchmark with a generic primed Green/spectral carrier, then add remaining BV rows to the exact physical-plus-ghost-n3 carrier representative; separately classify the odd derivative sector, fix finite C2/absolute dressed R(g_hat)^2 normalization and renormalized products, then import a same-background compensator contraction before residual Q1 transfer
Observer:    await and independently verify an explicit provenance-complete physical declaration under the certified v2 recoil contract; in parallel repair the exact epsilon_R_squared scalar q1-squared witness in the shifted-gravity/rod-memory composition, replay it, and only then export scalar q2, replay q1q2 and restrict records to Z2^C
...
the coefficientwise `I28` relation preserved. Curved-Endo ghost `n=1/n=2`,
generic finite Schur and remaining BV rows, and therefore the complete
repository form factors remain open. The selected generic weight-raised
