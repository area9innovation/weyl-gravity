...
pole and scale row. The reference-scale finite `R(K)` and `R(K^2)` rows and
possible local zeta multiplicative term remain unevaluated. Those data, simplex-integrated
...
The next coefficient-bearing analytic gate is computation of the
reference-scale finite `R(K)` and `R(K^2)` rows, any local zeta term, and the
physical fourth-order Hessian kernel. The canonical
