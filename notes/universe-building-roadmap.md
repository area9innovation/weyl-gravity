...
3. Define the first observer morphism from global observables to the two
   ...
   normal form over `Q(sqrt(10))`.  Export the complete scalar 108-row `q1`
   PBW matrix next, then the action-derived scalar `q2` tensor and independent
   replay before a solved backreacted branch.
   A common source
...
| `BERGER_108_ROW_COMPONENT_JET_CONTRACT` (fired) | export the complete scalar 108-row q1 PBW matrix in the certified arbitrary-finite coefficient-jet normal form, then derive scalar q2 and replay q1q2; do not infer either tensor from covariant block ranges or row coverage |
