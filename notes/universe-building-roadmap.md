...
`Wres(log S_L)`. The finite regulated `R(K)` and `R(K^2)` rows,
reference-specific scale conversion, and possible local zeta multiplicative
term remain unevaluated. Those data, simplex-integrated
...
The next coefficient-bearing analytic gate is computation of the finite
regulated `R(K)` and `R(K^2)` rows, reference-specific scale conversion, any
local zeta term, and the physical fourth-order Hessian kernel. The canonical
