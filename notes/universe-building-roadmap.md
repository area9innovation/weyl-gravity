...
`BoxR=0` convention. The Weyl-invariant finite remainder, Paneitz
kernel/global data, renormalized BV Laplacian or time-ordered product, and
finite normalization conditions remain absent, so complete `Gamma1` and
`Q1` are still fail-closed.
...
| Paper 12, pure-Weyl one-loop BV anomaly/QME | `DRAFT_ALLOWED`; theorem-spine manuscript, computational supplement, both compiled PDFs, generated exact tables, and fail-closed claim map assembled for the strict obstruction, tau-adic extended one-loop local Euclidean restoration, exact rank-two `Q1` underdetermination, and the conditional anomaly-induced Paneitz/Riegert `Gamma1` representative | complete specialist prose/citation and clean release review; the next theorem requires the Weyl-invariant finite `Gamma1` remainder and normalization, an extended classical contraction, and quantum `D`-defect disposition; Lorentzian/state claims remain separate |
