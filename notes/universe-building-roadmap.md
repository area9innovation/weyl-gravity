...
| Two-phase counterflow Berger successor | a changed action with two phases and one auxiliary diagonal `U(1)` has an exact 70-component cyclic BV parent on the selected Berger point; the diagonal gauge sector is contractible, the original dressed-trace class is eliminated, and support-local retarded/advanced chain homotopies are certified; the exact fixed-action homogeneous stationary classification makes the positive point the isolated singleton `(q,x,C)=(9/40,1,9/16)`, with a positive reduced homogeneous trace block and no same-action path to the round cylinder; the subsequent derived fixed-`Q_rel` reduction is exactly acyclic in the relative-clock sector, so making `D` null there removes the complete clock Darboux pair | health of the distinct unrestricted variable-`Q_rel` theory in which the clock pair survives but `D` is charged, its full nonhomogeneous pairing/characteristic census, a phase-space-correct residual receiver, structural stability under action deformation, action-derived `q2`, relational readout, Einstein-source closure, action-specific anomaly/Hadamard data, particles, or unitarity |
...
clock.  The distinct unrestricted branch retains the positive clock Darboux
pair while raw `D` remains charged and `K=D-Omega R_rel` is the helical
stabilizer.  Classical is now proving this charge-clock complementarity and
auditing full nonhomogeneous health on the unrestricted branch.  Nonlinear,
...
```text
Einstein:    import the fixed-charge clock obstruction and classify Einstein/source closure separately on the clockless D-null quotient and the unrestricted charged-D clock carrier; diagonal gauge neutrality is not source closure
...
Classical:   prove the charge-clock complementarity theorem and complete the all-Hodge pairing/characteristic audit on the unrestricted variable-Q_rel carrier; only then construct the residual receiver for the selected physical phase space
