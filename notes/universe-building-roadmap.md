...
```text
Einstein:    the parity-complete currents and pre-residual exact sequence are complete; a canonical target-orthogonal split exists, but invariant inertia obstructs every declared strict cyclic split; construct the common constraint-algebroid/Kuranishi moment-map-zero carrier before asking about non-strict homotopy or final descent
...
Classical:   the round-style scalar Hodge block is exactly not closed for k != 0; build the complete SU(2)_L x U(1)_R isotypical closure under q70, then resolve k=0/low-j exceptions and assemble health
...
Observer:    sharp, finite-resolution, reciprocity, composition and loop-holonomy laws are certified conditionally; freeze the receiver/fibre-crosswalk contract while awaiting a nonradical carrier from the isotypical Classical assembly before calling any value redshift
