...
9. **Higher orders of the Bogoliubov map**: (C4)–(C5) hold only up to $\mathcal{O}(\lambda)$; the interacting corrections to $R_t$'s action on oscillators are not given in the Letter.  The repository has now expanded Eq. (16) and computed the complete resonant two-annihilator order-$\lambda$ coefficient after the label repair (`REVERSE_PHYSICS_BT_RT_JORDAN_KERNEL_V1`).  Its apparent secular terms cancel on the BT carrier, but its fixed-splitting Krein Gram has cubic endpoint poles.  `REVERSE_PHYSICS_BT_ENDPOINT_EXTENSION_AMBIGUITY_V1` further proves that reflection-even scaling-degree-three extensions have three independent endpoint constants: triple-plus and cutoff finite-part prescriptions already disagree on the inclusive constant test, and `1/48` can be fitted by an allowed delta term but is not predicted.  The fixed-vacuum oscillatory certificate `REVERSE_PHYSICS_BT_OSCILLATORY_RADICAL_NO_MATCHING_V1` remains correct on its declared oscillator grading: there $a_1^\dagger\mapsto b_\Upsilon^\dagger$ has charge $-1$, $Q_t$ has charge $-2$, and the certified negative radical is trace-null.  Its use as an exclusion theorem does not transfer to the covariant broken-vacuum carrier described below.  The apparent coisometric range gap is nevertheless absent perturbatively: `REVERSE_PHYSICS_BT_PERTURBATIVE_COISOMETRY_RIGIDITY_V1` uses the published cross-CCR to prove $R_t^\dagger R_t=1$ at free order, and $\Pi^2=\Pi$ then kills every formal correction to $\Pi$.  Finally, `REVERSE_PHYSICS_BT_CANONICAL_ENDPOINT_AMBIGUITY_V1` supplies an exact four-channel countermodel to uniqueness: skew canonical transport, CCR preservation, projector idempotence, trace preservation, and reflection symmetry all hold for a three-parameter endpoint family.  The target $1/48$ is algebraically compatible but is not selected.  This does not prove that every family member is realized by BT; it proves that the published canonical identities alone cannot determine the missing continuum transport.
   ...
   `REVERSE_PHYSICS_BT_ABEL_NAIMARK_ASYMPTOTIC_DILATION_V1` supplies the first time-to-resolution mechanism.  Abel integration of the certified formal soft Hamiltonian $H_{\rm as}^{(1)}(t)=d(e^{-idt}D+e^{idt}D^\sharp)$ gives $A_\epsilon(d)=-id/(\epsilon+id)$ and $|A_\epsilon(d)|^2=d^2/(\epsilon^2+d^2)$.  On $d=\alpha r$, $y=-\log r$, this is exactly $q_R(y)=(1+e^{2(y-R)})^{-1}$ with $R=\log(\alpha/\epsilon)$, so multiplying inverse Abel time by $c$ translates detector resolution by $\log c$.  The raw coherent columns still fail strongly: their fixed-ratio distance tends to $\log(c)(c-1)/(c+1)$, which is positive and differs from the detector response $\log c$.  The canonical purification on $L^2(\mathbb R_s\times\mathbb R_y)\otimes\mathbb C^3$ uses $p_s(y)=\tfrac12\operatorname{sech}^2(y-s)$ and $\Xi_{R,a}=\mathbf1_{[R,R+a]}\sqrt{p_s/a}$; it has unit norm, makes disjoint resolution intervals orthogonal, and has marginal $(q_{R+a}-q_R)/a$.  Coupling its three channels with the independent physical density $1/48$ gives $+a/16$ real and $-a/16$ hard response.  The auxiliary $s$ label is resolution/noise, not spacetime.  The formal $R_t$ operator and physical S-matrix operator remain distinct: amplitude-level collinear factorization must still compare their Krein species and phase after zero-mode completion.  The result is `LOCAL-ALGEBRAIC` and `REDUCED-MODE`; complete sectors, a full Møller operator, finite NLO, and Eq. (19) remain open.

   `REVERSE_PHYSICS_BT_PHYSICAL_COLLINEAR_OPERATOR_FACTORIZATION_V1` completes that amplitude-level comparison on the declared external mass-jet cylinder.  The complete five-point tree has relevant spectator components $L=-(a_0-a_1)^2/(4\tau)$ and $Q=[2\tau(a_0+a_1)-(a_0-a_1)^2]/(4\tau^2)$, while the recombined four-point parent tree has both components equal to $1/2$.  In the one-leg delta-prime cross basis the physical map is therefore $T=\operatorname{diag}(2Q,2L)$, with $-T^\sharp T=\rho I_2$ and $\rho=(a_0-a_1)^2[2\tau(a_0+a_1)-(a_0-a_1)^2]/(4\tau^3)>0$ above unequal-mass threshold.  Its common tree phase cancels and its exact normalization reproduces $1/48$ per pair.  The completed public same-sign quadratic $R_t$ map instead has raised Gram $\left(\begin{smallmatrix}0&2\\0&0\end{smallmatrix}\right)$: nonzero, rank one, and nilpotent.  Since rank and Jordan type survive metric-compatible basis changes, phases, nonzero scalar factors, zero-mode similarities, and Abel multipliers, the public $D$ is not the physical splitting operator on this cylinder.  The physical process needs both null halves of the dipole jet; the public map supplies only one Gram direction.  This is `LOCAL-ALGEBRAIC` and `REDUCED-MODE`, not a full Møller/S construction, complete NLO probability, or all-order Eq. (19).

   `REVERSE_PHYSICS_BT_RIGGED_RESOLUTION_JORDAN_MOLLER_V1` integrates that physical map over the exact pair-threshold measure.  For each fixed mass ratio $0<r<1$, $(V_rh)(u)=T(r,u)h$ is a finite direct-integral column with $V_r^\sharp V_r=I(r)I_2$ and an exact finite-regulator pseudo-unitary block.  Here $I(r)=-2H(r)/3=5/24+r[(1/4)\log r+1/12]+O(r^2\log r)$.  The massless Gram limit is finite, but $I'(0+)=-\infty$.  Any strongly differentiable pair of columns on a fixed Krein carrier with bounded fundamental symmetry would have a differentiable Gram, so an ordinary $C^1$ Møller column cannot implement the BT delta-prime derivative.  The divided germ $D(r)=[I(r)-I(0)]/r$ has the exact scale cocycle $D(cr)-D(r)\to\log(c)/4$, or $\log(c)/48$ per physical pair.  With $R=-\log r$, this affine law is linearized by the nilpotent resolution generator $\left(\begin{smallmatrix}0&-1/4\\0&0\end{smallmatrix}\right)$ on $\operatorname{span}\{1,s\}\subset\mathcal S'(\mathbb R_s)$.  Neither affine generator is an $L^2$ vector.  Thus the Abel--Naimark carrier supplies a minimal rigged relative-resolution cocycle, not an endpoint state.  A generalized-Born functional on the rigging and its physical Hamiltonian affiliation are now the constructive gate; full Møller/S, finite NLO, and Eq. (19) remain open.

   `REVERSE_PHYSICS_BT_RESOLUTION_LOCAL_COHERENT_BORN_PROCESS_V1` constructs that generalized-Born object on the resolution-local CCR net.  Absolute translation-invariant normalization on the whole line is incompatible with nonzero local cell weight, but the relative weight $\tau_{\rm rel}(f,g)=\int(f-g)ds$ is positive on ordered integrable differences and gives $a/48$ per pair for a resolution interval of length $a$.  The physical Gram is $(1/48)I_2$, so its minimal Kolmogorov factor $k_s(y)=\sqrt{p_s(y)/48}I_2$ retains both jet species with normalized trace $\operatorname{Tr}_2/2$.  Summing three pair channels gives a finite-interval amplitude of norm squared $a/16$.  Its Weyl displacement defines compatible locally normal coherent states and a resolution-local Møller automorphism.  The exact count law is $P_n(a)=e^{-a/16}(a/16)^n/n!$: hard survival is $e^{-a/16}$, each pair has mean $a/48$, and all probabilities sum to one.  Disjoint intervals factor and joint $(s,y)$ translations act covariantly.  Globally the norm grows as $L/16$, so there is no Fock vector or inner Weyl implementer; the state is non-Fock at infinity.  This constructs a positive normalized leading-log process under the explicit coherent stationary-independent-increment completion.  It does not prove that nonlinear BT amplitudes are coherent: the next discriminator is the six-point strongly ordered double-collinear jet.  Resolution locality is not spacetime locality, and the complete physical S-matrix, finite NLO probability, and Eq. (19) remain open.
   `REVERSE_PHYSICS_BT_SIX_POINT_STRONGLY_ORDERED_TREE_V1` performs that discriminator and rejects the coherent independent-increment dynamics.  A low-memory subset recursion counts and sums all 220 six-point trees: 10 $V_4^2$, 105 $V_3^2V_4$, and 105 $V_3^4$, with relative signs $+,-,+$.  It retains $s_{01}=\delta\epsilon\tau_1$ and $s_{012}=\delta\tau_2$ together with all six external mass jets before taking $\delta\to0$ and then the nested hierarchy $\epsilon\to0$.  The three-hard-spectator square coefficient is identical at three unrelated hard fixtures.  The hierarchy limit is nonuniform, so the two Källén thresholds are reduced at finite $\epsilon$; the physical-invariant-cutoff finite part has inner $r\log r$ coefficient $5$, while the outer moments each have unit $r\log r$ coefficient.  The mixed raw cocycle is $15/32$.  With the exact $2\to4$ phase normalization, six external delta-prime sign, twelve labeled nested histories, and ordered simplex, $P_2(a)=5a^2/512$.  The coherent prediction is $a^2/512$, so the ordered resolution-sector second factorial cumulant is $a^2/64$: the tree process is super-Poisson by a factor of five.  The relative $1/48$ one-emission weight and rank-two GNS factor survive, but the Gaussian coherent state is not dynamically selected.  Two moments do not determine a unique non-Gaussian state; the next discriminator is the seven-point triple-strongly-ordered tree jet.  The full non-strongly-ordered six-body probability, finite terms, spacetime Møller/S operator, beyond-tree positivity, universal hard-angle independence beyond the three fixtures, and Eq. (19) remain open.

   `REVERSE_PHYSICS_BT_SEVEN_POINT_COX_SELECTION_V1` performs that next discriminator.  A low-memory exact recursion counts and sums all 2,485 seven-point trees: 945 $V_3^5$, 1,260 $V_3^3V_4$, and 280 $V_3V_4^2$.  The complete amplitude begins at $\delta^2$ and yields the same strong-order square kernel at two unrelated producer fixtures; an independent explicit-tree rail checks a third fixture.  Three fixed-physical-invariant Källén reductions give raw cocycle $-81/128$ and, after the seven external delta-prime sign, phase normalization, 60 labeled histories, and ordered three-simplex, $P_3(a)=9a^3/8192$.  Thus $M_3=27a^3/4096$ and $\kappa_3=7a^3/2048$.  This excludes the gamma--Cox law fixed by the first two moments, whose prediction is $15a^3/8192$, but it does not exclude scalar Cox positivity: $m_1m_3-m_2^2=1/32768>0$.  The unique at-most-two-atom intensity law matching the first three moments has $x_\pm=(11\pm\sqrt{113})/64$ and $p_\mp=(\sqrt{113}\pm7)/(2\sqrt{113})$.  Its controlled mixture of local Weyl displacements is positive, normalized, locally normal, inclusion compatible, and translation covariant.  This is a minimal-support state completion through three ordered moments, not a unique positive process, dynamical zero mode, full probability, spacetime Møller/S operator, Eq. (19), or metric/BRST result.  A channel-resolved seven-point projector and the complete eight-point ordered tree are the next discriminators.

   `REVERSE_PHYSICS_BT_CHANNEL_RESOLVED_BRANCHING_INSTRUMENT_V1` lifts those total moments to the labeled strongly ordered channels.  The histories are rooted combs on $k+2$ outgoing leaves, so their dimensions are $(k+2)!/2$: $3$, $12$, and $60$ through three emissions.  Canonical insertion of one new leaf gives $3$, $4$, and $5$ children per parent.  The per-history factorial Grams $1/48$, $5/3072$, and $9/81920$ therefore factor into positive per-extension rate squares $q_0=1/48$, $q_1=5/64$, and $q_2=27/400$, with total exit rates $1/16$, $5/16$, and $27/80$.  Jump operators $L_{h\to c}=\sqrt{q_k}|c\rangle\langle h|\otimes I_2$ define an exact finite GKSL instrument on the 152-dimensional history/species carrier.  Its probabilities are positive and normalized for every finite resolution length, and their leading one-, two-, and three-emission terms reproduce the tree coefficients.  At construction time only the first jump was amplitude-affiliated; the higher identity-species lift and level-three absorbing closure were positive constructions rather than derived species tensors or phases.

   `REVERSE_PHYSICS_BT_SEVEN_POINT_PROFILE_QUOTIENT_AFFILIATION_V1`, together with its six-point parent/profile predecessor, resolves the complete available pre-trace chain.  The six-point two-species restriction has a complex eigenpair, but the minimal grading-faithful constant/linear-parent times singleton/pair-profile carrier has a nondegenerate collapse-invisible kernel and a two-dimensional Krein-orthogonal image that reconstructs the amplitude pointwise, affiliating $q_1=5/64$.  At seven points the exact 2,485-tree amplitude retains seven spectator masks and reduces to equal singleton and complementary-pair profiles.  Their unique recombined-parent coefficients satisfy $u<0<v$ on the full nested threshold domain.  The seven-external-leg delta-prime sign gives physical quotient eigenvalue $-2uv>0$ and the signed pointwise identity affiliates $q_2=(9/81920)/(5/3072)=27/400$.  An independent explicit-tree rail matches all seven components at separate exact kinematics.  Thus all three available jumps are amplitude-affiliated, while the absorbing closure, a fourth jump, complete probability, spacetime Møller/S operator, Eq. (19), and metric/BRST lift remain open.  The result is `LOCAL-ALGEBRAIC` and `REDUCED-MODE`, not a new spacetime dimension or a Lorentzian theorem.

   `REVERSE_PHYSICS_BT_THREE_JUMP_KREIN_MOLLER_JET_V1` closes the finite reverse-block gate.  For the exact rooted-comb incidences $B_k^TB_k=(k+3)I$ and affiliated rates $(1/48,5/64,27/400)$, the unique positive equal-edge constant-generator weights are $\alpha_k^2=(k+1)q_k$, hence $(\sqrt3/12,\sqrt{10}/8,9/20)$.  The 152-dimensional two-species generator $K=F-F^\dagger$ is exactly skew, has rank 52 and a 100-dimensional nondegenerate dark kernel, and contains a nonzero reverse block from three to two emissions.  With $U_x=e^{xK}$ and $x^2=a$, its incoming Taylor column reproduces $P_1=a/16$, $P_2=5a^2/512$, and $P_3=9a^3/8192$ at their first allowed orders; any future fourth block is too far away to change this jet through $x^3$.  The uniform radial block has characteristic polynomial $z^4+(17/10)z^2+81/1280$ and positive frequency squares, so the finite witness is stable.  The same calculation exactly obstructs an ordinary additive-resolution generator: the physical transition probability is $a/16+O(a^2)$, while a bounded strongly differentiable $V(a)=I+aG+o(a)$ has off-diagonal probability $O(a^2)$, and the difference quotient of $U_{\sqrt a}$ diverges.  Thus the physical finite Møller coupling jet is constructed but an additive-resolution Møller cocycle requires a quantum-stochastic/noise or unbounded rigged continuation.  No fourth jump, all-order BT Hamiltonian, complete probability, Eq. (19), gravity/BRST lift, new spacetime dimension, or `LORENTZIAN-CAUSAL` result follows.

   `REVERSE_PHYSICS_BT_QUANTUM_STOCHASTIC_MOLLER_DILATION_V1` constructs the required finite additive-resolution continuation.  The system remains the 152-dimensional positive quotient, while the environment is Boson Fock space over $L^2(\mathbb R_+,da)\otimes\mathbb C^{75}$, with one noise mark for each rooted-comb insertion edge and $J_e=\sqrt{q_k}|c\rangle\langle h|\otimes I_2$.  The drift $D=\frac12\sum_eJ_e^\dagger J_e$ has level eigenvalues $(1/32,5/32,27/160,0)$.  Both Hudson--Parthasarathy structure identities hold exactly, so creation together with reverse annihilation defines a strongly continuous unitary cocycle satisfying the shifted additive law.  Vacuum reduction reproduces the pinned GKSL generator hash and exact normalized population resolvents.  The ordered $k$-noise simplex has leading amplitude $a^{k/2}\sqrt{\prod_{j<k}q_j/k!}$; with $x=\sqrt a$ and $\alpha_j^2=(j+1)q_j$, this is precisely $x^k\prod_{j<k}\alpha_j/k!$, recovering all three certified Møller-jet coefficients.  The vectorized edge Kraus Gram is positive diagonal of rank 75, proving minimal multiplicity for the pinned channel-resolved completely positive map.  This breaks the ordinary-generator barrier by quantum white-noise scaling, not by strong differentiability.  The auxiliary resolution coordinate is not a new physical or spacetime dimension.  Level-three absorption remains a truncation artifact; the fourth jump, inductive-limit carrier, physical continuum trace, spacetime Møller/LSZ/S operator, all-order BT Hamiltonian, Eq. (19), metric/BRST lift, and every `LORENTZIAN-CAUSAL` claim remain open.

   `REVERSE_PHYSICS_BT_ABEL_FOCK_PHYSICAL_INTERTWINER_V1` affiliates the first stochastic jump with the complete normalized five-point collinear direct-integral shape.  The raw columns cannot be resolution translates because their exact Gram $I(r)$ is nonconstant; explicitly $I(1/16)-I(1/4)=11\log2/80-57/2048>419/10240$.  Normalizing their positive polar ranges gives $E_R^\sharp E_R=I_2$ and canonical transports $C_b(R)=E_{R+b}E_R^\sharp$.  Combining these with $p_s(y)=\operatorname{sech}^2(y-s)/2$ yields the isometry $(Af)_i(s,y)=\sqrt{p_s(y)}E_yf_i(s)$ and exact resolution-shift intertwining.  With $q_0=1/48$, its interval norms are $a/48$ per pair and $a/16$ in total.  The correct target is the correlated system--noise sector: one pair-noise mark is one-dimensional, while the physical species Gram has rank two, so a channel-faithful noise-only map is exactly impossible and the unchanged system species is essential.  This constructs physical continuum affiliation for edge marks $0,1,2$ only.  The other 72 marks remain amplitude-affiliated finite quotients without measurable nested continuum columns.  Consequently the next physical gate is the six-point nested direct-integral column for the 12 second-level edges, not merely an eight-point scalar fourth rate.  The full 75-mark operator, complete probability, spacetime Møller/LSZ/S map, Eq. (19), gravity/BRST lift, and `LORENTZIAN-CAUSAL` claims remain open.

   `REVERSE_PHYSICS_BT_SIX_POINT_NESTED_CONTINUUM_INTERTWINER_V1` constructs that second physical layer.  For the inner daughter ratio $r$, invariant $w$, Källén root $\Delta_r$, and positive six-point quotient parameter $q=[2w(1+r)-(1-r)^2]/(2w^2)$, the conditional Gram is $\lambda_6=a_2q$.  The normalized physical increment $d\sigma_r=\lambda_6d\mu_r/[a_2(1+r)]=q\Delta_rdw/[(1+r)w]$ is positive, daughter-exchange invariant, zero at threshold, and asymptotic to $d\log w$.  An exact rationalized primitive proves that it maps the threshold ray bijectively to $\mathbb R_+$.  The normalized quotient range $E_6=N_+/\sqrt{6qv}$ therefore gives the unitary change of variables $(B_rf)(w)=\sqrt{\lambda_6/[a_2(1+r)]}E_6f(\sigma_r(w))$.  Composing this with the first Abel column yields the ordered two-noise isometry for all twelve second-level HP histories.  The exact rate product is $q_0q_1=(1/48)(5/64)=5/3072$; the ordered simplex and twelve histories reproduce $5a^2/512$.  Finite-hierarchy domains exhaust a common compactly supported dense core without taking the already obstructed strong massless endpoint derivative.  Thus marks $0$ through $14$ have physical continuum affiliation.  The remaining sixty seven-point marks, full 75-mark operator, fourth jump, complete probability, spacetime Møller/LSZ/S map, public $R_t$ identification, Eq. (19), gravity/BRST lift, and `LORENTZIAN-CAUSAL` claims remain open.

   `REVERSE_PHYSICS_BT_SEVEN_POINT_NESTED_CONTINUUM_INTERTWINER_V1` closes the continuum affiliation of the available finite hierarchy.  With $\alpha=A/\tau_1^2=2(1-q_{\rm inner})$, $s=a_3/\tau_3$, and $w=\tau_2/a_2$, the signed quotient eigenvalue is $\lambda_7=\tau_3\alpha H(w)/[4(1+s)]$, where $H(w)=H_0+(6-2\alpha)/w+\alpha/w^2$ and $H_0=2+\alpha s(2-s)$.  On $1\leq\alpha<2$, $0<s<1$, and $w>1$, every coefficient is positive.  Dividing by the exact asymptotic scale and multiplying by the massless-hierarchy middle Källén measure gives $d\chi=[H(w)/H_0](w-1)dw/w$.  Its elementary primitive is strictly increasing from zero and asymptotic to $w$, hence maps the threshold ray onto $\mathbb R_+$.  The normalized signed range $E_7=N_+/\sqrt{-6uv}$ yields the third conditional direct-integral unitary, and composition with the two-emission column affiliates marks $15$ through $74$.  The rate chain $(1/48)(5/64)(27/400)=9/81920$, the ordered simplex, and sixty histories reproduce $9/8192$.  Thus all 75 marks already present in the finite HP instrument have physical continuum columns.  This does not construct a fourth jump, an all-order inductive intertwiner, complete probability, a spacetime Møller/LSZ/S map, public $R_t$ identification, Eq. (19), a gravity/BRST lift, a new dimension, or a `LORENTZIAN-CAUSAL` result.

   `REVERSE_PHYSICS_BT_FINITE_PHYSICAL_MOLLER_COLUMN_V1` composes those ranges with the finite stochastic dynamics.  For vacuum inclusion $I_\Omega$, the 75-channel HP unitary $U_a$, and $\mathcal A_{\le3}=I_{\rm vac}\oplus\mathcal A_1\oplus\mathcal A_2\oplus\mathcal A_3$, the column $\mathcal M_a=\mathcal A_{\le3}U_aI_\Omega$ satisfies $\mathcal M_a^*\mathcal M_a=I_2$.  Its exact ordered-history kernels have amplitude drifts $(1/32,5/32,27/160,0)$; their $1,3,12,60$ sector norms are nonnegative, sum to one for every $a\ge0$, and retain leading coefficients $1/16,5/512,9/8192$.  The hard and real linear responses are therefore $-1/16$ and $+1/16$ on the same pinned physical column, giving $-3/512+3/512=0$ after the Born factor without adding the zero public $R_t$ response to the scattering ledger.  The public rank-one leg can occur as a compression of the physical first-emission map only with a forced two-dimensional cross-Krein complement: in the common cross basis, $C^TJC=-\rho J-\operatorname{diag}(0,2)=\left(\begin{smallmatrix}0&-\rho\\-\rho&-2\end{smallmatrix}\right)$, realized by $C=\left(\begin{smallmatrix}-\rho&-1\\0&1\end{smallmatrix}\right)$.  Its determinant is $-\rho^2$ and its raised trace is $-2\rho$, so neither a positive-noise fibre nor a trace-null Eq. (19) remainder alone can repair the rank defect.  This fixes the minimal missing fibre and constructs the finite physical vacuum column; it does not derive the complement from BT dynamics, construct arbitrary incoming-noise or two-sided spacetime scattering, add a fourth jump, prove complete BT probability, or prove Eq. (19) all orders.

   `REVERSE_PHYSICS_BT_ENDPOINT_COMPLEMENT_MATCHING_V1` tests whether the already classified endpoint freedom can supply that missing fibre without new dynamics.  On the fixed two-profile lift $v=(1,z(1-z))$, the three products are exactly the independent probes $1,z(1-z),z^2(1-z)^2$.  Relative to the symmetric triple-plus reference, whose actions on those probes are $(0,0,3/2)$, the endpoint jets have matrix $\left(\begin{smallmatrix}2&0&0\\0&-2&-4\\0&0&4\end{smallmatrix}\right)$.  The induced Gram is therefore $K(c)=\left(\begin{smallmatrix}2c_0&-2c_1-4c_2\\-2c_1-4c_2&3/2+4c_2\end{smallmatrix}\right)$.  Matching the forced $\left(\begin{smallmatrix}0&-\rho\\-\rho&-2\end{smallmatrix}\right)$ has the unique pointwise solution $c_0=0$, $c_1=7/4+\rho/2$, $c_2=-7/8$.  The three exact physical fixtures have distinct $\rho$, hence require distinct $c_1$; no single fixed reflection-even extension matches the physical continuum on this lift.  A different fixed affine reference shifts the intercept but leaves $dc_1/d\rho=1/2$.  Thus a pointwise match remains possible only with spectator-dependent endpoint data or an equivalent genuinely dynamical profile, neither of which is derived by the public order-$\lambda$ map.  This is a scoped fixed-profile obstruction, not a no-go for every local extension, a proof of Eq. (19), or a physical-theory completion.

   `REVERSE_PHYSICS_BT_EIGHT_POINT_HARD_PROFILE_OBSTRUCTION_V1` performs the complete fourth-jump preflight and shows that the bare scalar hierarchy used at six and seven points does not continue unchanged.  The exact recursion sums all $34{,}300$ eight-point cubic/quartic trees.  At identical soft data, two fixtures differing only in one hard adjacent invariant, $33\mapsto34$, both start at common mass order two, contain all eight spectator masks, and have ordered hierarchy valuation $(0,0,-1)$.  Their leading residues nevertheless differ by exactly $257/1568$.  A method-distinct rational-series verifier using the invariant triangle cubic vertex reproduces both complete finite-hierarchy projected values.  Thus the provisional hard-independent coefficient $629$ and the associated exclusion of the two-atom Cox completion are withdrawn: the threshold-integrated fourth moment has not yet been computed.  The next gate is to retain the outer hierarchy through its fixed-invariant Källén reduction and resolve the resulting hard-profile quotient before any scalar fourth jump is normalized.  This is `LOCAL-ALGEBRAIC`, `REDUCED-MODE`; it is not a complete $2\to6$ probability, Eq. (19), a gravity/BRST lift, or a `LORENTZIAN-CAUSAL` result.

   `REVERSE_PHYSICS_BT_EIGHT_POINT_OUTER_THRESHOLD_PROFILE_V1` performs that outer reduction and proves that it does not restore scalar universality.  Retaining $u=\tau_4$, the two complete $34{,}300$-tree profiles have leading difference $771/(1568e_3u)$.  The outer phase-space measure maps the Laurent rows $1,u^{-1},u^{-2},u^{-3}$ to the fixed-invariant moments $J_1,J_2,J_3,J_4$; an independent pole-derivative calculation gives unit $r\log r$ coefficient for all four.  The difference is therefore a pure $J_2$ direction and survives with threshold coefficient $771/(1568e_3)$.  Its evaluation at the predecessor's $u=3$ is exactly $257/(1568e_3)$, explaining the earlier residue, while multiplication by $r=e_3\tau_3/a_4=5e_3$ gives a physical outer-log difference $3855/1568$.  The correct outer integration amplifies rather than cancels the two-fixture distinction.  The remaining $\tau_3,\tau_2,\tau_1$ reductions, the $e_3$ scale Jacobian, and any collapse/profile quotient are still required before a fourth factorial moment or Cox test is authorized.  This remains `LOCAL-ALGEBRAIC`, `REDUCED-MODE`, not Eq. (19), complete scattering, gravity, or a causal theorem.

   `REVERSE_PHYSICS_BT_EIGHT_POINT_MIDDLE_THRESHOLD_PROFILE_V1` restores $\tau_3$ symbolically and closes the next reduction.  After all three ordered hierarchy valuations, the exact two-fixture difference is $3(27213\tau_3^2-13462\tau_3+29485)/(156800\tau_3^2\tau_4)$.  It reproduces both the separately computed $\tau_3=4$ value $246627/501760$ and the outer certificate's $\tau_3=5$ value $771/1568$.  The outer $J_2$ functional removes $\tau_4^{-1}$; with $a_3=2$, the remaining $1,\tau_3^{-1},\tau_3^{-2}$ rows map to $a_3J_1,J_2,a_3^{-1}J_3$.  Their unit nonanalytic coefficients leave the strictly nonzero middle difference $334239/313600$.  Thus neither of the first two physical threshold reductions restores a scalar fourth jump.  The final $\tau_2$ reduction and an inner $\tau_1$ calculation retaining independent $a_0,a_1$ are still mandatory; no fourth moment, Cox exclusion, complete probability, Eq. (19), or gravitational result follows yet.

   `REVERSE_PHYSICS_BT_EIGHT_POINT_TAU2_THRESHOLD_PROFILE_V1` evaluates the first two unit-$J_n$ coefficient functionals at $\tau_4=a_4=1$ and $\tau_3=a_3=2$, retains $\tau_2$ symbolically through the complete $34{,}300$-tree recursion, and finds the exact hard-fixture difference $3(2090\tau_2^2+1146\tau_2+981)/(12800\tau_2^2)$.  At $\tau_2=7$, the middle measure scale $a_3=2$ exactly replays $334239/313600$.  The remaining constant, $\tau_2^{-1}$ and $\tau_2^{-2}$ rows map at $a_2=3$ to $a_2J_1,J_2,a_2^{-1}J_3$ and leave $23229/6400\ne0$.  The hard distinction therefore survives three of four threshold functionals.  This coefficient tests collapse; it is not the normalized fourth moment because the nested daughter-mass-ratio factors and the independent-$a_0,a_1$ inner $\tau_1$ residue remain to be assembled.  The Cox decision, complete probability, Eq. (19), gravity/BRST lift, and every `LORENTZIAN-CAUSAL` claim remain open.

   `REVERSE_PHYSICS_BT_EIGHT_POINT_INNER_THRESHOLD_OBSTRUCTION_V1` closes the scalar threshold chain.  Homogeneity fixes only the redundant scale $a_0=1$ while retaining $r=a_1/a_0$ and $u=\tau_1/a_0$.  The two complete independent-mass profiles differ by $3[9r^2-18r(u+1)+34u^2-18u+9]/(128u^2)$ and replay $23229/6400$ at $(r,u)=(4,10)$ after the first-three scale $a_2a_3=6$.  The invariant-cutoff rationalization $u=1+m^2+m(z+z^{-1})$, $r=m^2$, gives individual $r\log r$ coefficients $-6699/128$ and $-7149/128$; the method-distinct residue of their difference is $225/64\ne0$.  Thus none of the four physical coefficient functionals restores a hard-independent scalar fourth jump on the declared fixtures.  A channel/profile-valued successor or an additional dynamical trace relation is required if the complete tree data are retained.  This is a scoped reduced-mode obstruction, not a normalized fourth probability, Cox decision, all-order Eq. (19), gravity/BRST lift, or `LORENTZIAN-CAUSAL` result.
