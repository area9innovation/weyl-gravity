PULLED by agent quantum-qme-1 — stream quantum-qme — lease exclusive (acquired 2026-07-20T20:10:29Z, expires 2026-07-21T00:10:29Z)

WORK PACKAGE sf:program/work/quantum-paper12-active-clock-background-stability-headline  (state: ACTIVE, owner: quantum-qme)

## Objective
Update Paper 12 so the quadratic active-clock theorem is stated as an exact open-neighbourhood no-go with a tested q=1/4 bifurcation, not a frozen-fixture failure.

## Stop condition (what DONE means)
Import COMPENSATOR_ACTIVE_CLOCK_BACKGROUND_STABILITY_V1 by certificate hash and scientific commit.  Update the theorem statement, abstract/status ledger and claim map to state the exact rational neighbourhood, the one-ray stationary locus, structurally stable split gravity--auxiliary inertia and raw-D failure, and the first q=1/4 clock-sign bifurcation with witnesses on both sides.  State that crossing the bifurcation repairs only the clock-sign conflict and leaves the other terminal obstructions.  Preserve the scope: couplings vary with backgrounds; no fixed action across the neighbourhood, general k-essence, causal parent, anomaly/QME, particle or unitarity claim.  Run the paper claim verifier, filename policy and PDF build.

## Current gate
Paper 12 background-stability and bifurcation claim synchronization

## Forbidden
no universal P(X) no-go; no fixed-action neighbourhood theorem; no bifurcation called a viable phase; no classical no-go conflated with failure of local Wess--Zumino anomaly trivialization; no paper promotion without exact evidence edge

## Activates (downstream this unlocks)
- a current Paper 12 compensator theorem statement and result-to-paper evidence edge

## Dependencies (verify state before starting)
- sf:program/work/classical-compensator-active-clock-background-stability — DONE (met)

## Additional item fields (schema superset — passed through)
- `allowed_paths`: ["paper/12-pure-weyl-one-loop-bv-anomaly.tex","paper/12-pure-weyl-one-loop-bv-anomaly-claim-map.json","paper/build/","reports/","planning/paper-coverage/","notes/d-quotient-quantum-team-brief.md"]
- `resource_hint`: "QUANTUM/PUBLICATION SYNC.  The new theorem strengthens scope and must not be left as a pointwise no-go."
- `notes`: ["This is a manuscript synchronization task, not a new quantum calculation."]
- `input_commit`: "b0ee2bea2"
- `stream`: "quantum-qme"
