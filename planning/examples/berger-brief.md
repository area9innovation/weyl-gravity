...
all two_j=6 feedback channel sums are certified with an independent rail (Peter--Weyl accumulation over the reality fold), OR the first obstruction is ledgered; physical masses, quotient descent, and the second-order-cone restriction remain declared-open.

## Forbidden
activating the Bridge 3 physical-branch interpretation from the unsplit apparatus calculation; inferring any numerical-recoil, nonvanishing, or sign claim from a zero-containing interval enclosure (roadmap: 'both 8-cell intervals still contain zero').

## Dependencies (verify state before starting)
- the certified SU(2) conjugate-column real-shell theorem (two_j=5 channel sums, met) — free-text (verify manually)
- the certified fail-closed four-stream tail-stop controller (met) — free-text (verify manually)
