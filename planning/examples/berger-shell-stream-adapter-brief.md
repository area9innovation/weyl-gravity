...
a reusable callable accepts a contiguous declared shell sequence, evaluates only independent SU(2) columns, derives every partner exactly, aggregates all four detector/source shell intervals, and invokes the fail-closed stop rule after each shell; an actual two_j=6 replay agrees with BERGER_RECOIL_TWO_J6_REALITY_FOLDED_BINDING and an independent verifier rejects missing/duplicate shells, incomplete reality representatives, and absent four-stream tail data, OR the first exact obstruction is ledgered.

## Forbidden
choosing or inventing physical masses, couplings or a stopping goal; identifying the hashed exact-T carrier with the direct carrier by matching mode names; inferring nonzero recoil, sign, recoil-corrected rank, quotient descent, tangent-cone survival, Bridge 3 activation or a quantum claim from validation-domain or zero-containing intervals.

## Dependencies (verify state before starting)
- sf:program/work/berger — DONE (met)
- BERGER_RECOIL_TWO_J6_REALITY_FOLDED_BINDING — free-text (verify manually)
- BERGER_RECOIL_DIRECT_SHELL_AND_TAIL_STOP_GATE — free-text (verify manually)
- BERGER_RECOIL_FINITE_SHELL_INTERVAL_AGGREGATOR — free-text (verify manually)
