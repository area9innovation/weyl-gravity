...
the certified schema and translator map every accepted declaration exactly to run_reality_folded_shell_stream, an independent verifier proves agreement for all four goal variants, and mutations reject the old goal-name mismatch, missing inverse volume, missing/duplicate shells, absent tail entries, nonpositive masses or volume, zero couplings, and undeclared provenance; physical stream activation remains false until a separate explicit value declaration exists, OR the first exact obstruction is ledgered

## Forbidden
choosing, guessing or recommending physical masses, couplings, inverse volume, shell cutoff or stopping goal; treating a validation fixture as physical data; identifying the hashed exact-T carrier by matching labels; activating four physical streams, recoil-corrected rank, tangent-cone survival, Bridge 3 or a quantum claim without their separate gates

## Dependencies (verify state before starting)
- sf:program/work/berger-shell-stream-adapter — DONE (met)
- BERGER_RECOIL_REALITY_FOLDED_SHELL_STREAM_ADAPTER — free-text (verify manually)
- BERGER_RECOIL_STREAM_EXECUTABLE_READINESS_AUDIT — free-text (verify manually)
- BERGER_RECOIL_SCALAR_STREAM_ACTIVATION_GATE — free-text (verify manually)
